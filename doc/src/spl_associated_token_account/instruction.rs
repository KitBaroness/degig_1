...
</pre></div><pre class="rust"><code><span class="doccomment">//! Program instructions

</span><span class="kw">use </span>{
    <span class="kw">crate</span>::{get_associated_token_address_with_program_id, id},
...
<span class="kw">fn </span>build_associated_token_account_instruction(
    ...
    <span class="comment">// safety check, assert if not a creation instruction
    </span><span class="macro">assert_matches!</span>(
        instruction,
    ...
    Instruction {
        ...
        data: borsh::to_vec(<span class="kw-2">&amp;</span>instruction).unwrap(),
    }
}
...
<span class="doccomment">/// Creates a `RecoverNested` instruction
    ...
    <span class="kw">let </span>instruction_data = AssociatedTokenAccountInstruction::RecoverNested;
    ...
    Instruction {
        ...
        data: borsh::to_vec(<span class="kw-2">&amp;</span>instruction_data).unwrap(),
    }
}
