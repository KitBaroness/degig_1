...
<span class="macro">pin_project!</span> {
    <span class="doccomment">/// Future that resolves into a [`Collected`].
    </span><span class="kw">pub struct </span>Collect&lt;T&gt;
    <span class="kw">where
        </span>T: Body,
    {
        <span class="attr">#[pin]
        </span>body: T,
        collected: <span class="prelude-ty">Option</span>&lt;Collected&lt;T::Data&gt;&gt;,
        is_data_done: bool,
    }
}

<span class="kw">impl</span>&lt;T: Body&gt; Collect&lt;T&gt; {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new(body: T) -&gt; <span class="self">Self </span>{
        <span class="self">Self </span>{
            body,
            collected: <span class="prelude-val">Some</span>(Collected::default()),
            is_data_done: <span class="bool-val">false</span>,
        }
    }
}

<span class="kw">impl</span>&lt;T: Body&gt; Future <span class="kw">for </span>Collect&lt;T&gt; {
    <span class="kw">type </span>Output = <span class="prelude-ty">Result</span>&lt;Collected&lt;T::Data&gt;, T::Error&gt;;
    ...
    <span class="kw">fn </span>poll(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;<span class="self">Self</span>::Output&gt; {
        ...
        <span class="kw">loop </span>{
            <span class="kw">if </span>!<span class="kw-2">*</span>me.is_data_done {
                <span class="kw">match </span>me.body.as_mut().poll_data(cx) {
                    ...
                    Poll::Ready(<span class="prelude-val">None</span>) =&gt; {
                        <span class="kw-2">*</span>me.is_data_done = <span class="bool-val">true</span>;
                    }
                    Poll::Pending =&gt; <span class="kw">return </span>Poll::Pending,
                }
            } <span class="kw">else </span>{
                <span class="kw">match </span>me.body.as_mut().poll_trailers(cx) {
                    Poll::Ready(<span class="prelude-val">Ok</span>(<span class="prelude-val">Some</span>(trailers))) =&gt; {
                    ...
                    Poll::Ready(<span class="prelude-val">Ok</span>(<span class="prelude-val">None</span>)) =&gt; <span class="kw">break</span>,
                    Poll::Pending =&gt; <span class="kw">return </span>Poll::Pending,
                }
            }
        }

        Poll::Ready(<span class="prelude-val">Ok</span>(me.collected.take().expect(<span class="string">"polled after complete"</span>)))
    }
}

<span class="doccomment">/// A collected body produced by [`Body::collect`] which collects all the DATA frames
/// and trailers.
</span><span class="attr">#[derive(Debug)]
</span><span class="kw">pub struct </span>Collected&lt;B&gt; {
    bufs: BufList&lt;B&gt;,
    trailers: <span class="prelude-ty">Option</span>&lt;HeaderMap&gt;,
}

<span class="kw">impl</span>&lt;B: Buf&gt; Collected&lt;B&gt; {
    <span class="doccomment">/// If there is a trailers frame buffered, returns a reference to it.
    ///
    /// Returns `None` if the body contained no trailers.
    </span><span class="kw">pub fn </span>trailers(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>HeaderMap&gt; {
        <span class="self">self</span>.trailers.as_ref()
    }

    <span class="doccomment">/// Aggregate this buffered into a [`Buf`].
    </span><span class="kw">pub fn </span>aggregate(<span class="self">self</span>) -&gt; <span class="kw">impl </span>Buf {
        <span class="self">self</span>.bufs
    }

    <span class="doccomment">/// Convert this body into a [`Bytes`].
    </span><span class="kw">pub fn </span>to_bytes(<span class="kw-2">mut </span><span class="self">self</span>) -&gt; Bytes {
        <span class="self">self</span>.bufs.copy_to_bytes(<span class="self">self</span>.bufs.remaining())
    }

    <span class="kw">fn </span>push_data(<span class="kw-2">&amp;mut </span><span class="self">self</span>, data: B) {
        <span class="comment">// Only push this frame if it has some data in it, to avoid crashing on
        // `BufList::push`.
        </span><span class="kw">if </span>data.has_remaining() {
            <span class="self">self</span>.bufs.push(data);
        }
    }

    <span class="kw">fn </span>push_trailers(<span class="kw-2">&amp;mut </span><span class="self">self</span>, trailers: HeaderMap) {
        <span class="kw">if let </span><span class="prelude-val">Some</span>(current) = <span class="kw-2">&amp;mut </span><span class="self">self</span>.trailers {
            current.extend(trailers);
        } <span class="kw">else </span>{
            <span class="self">self</span>.trailers = <span class="prelude-val">Some</span>(trailers);
        }
    }
}

<span class="kw">impl</span>&lt;B&gt; Default <span class="kw">for </span>Collected&lt;B&gt; {
...
<span class="kw">impl</span>&lt;B&gt; Unpin <span class="kw">for </span>Collected&lt;B&gt; {}

<span class="attr">#[derive(Debug)]
</span><span class="kw">struct </span>BufList&lt;T&gt; {
    bufs: VecDeque&lt;T&gt;,
}
...
<span class="kw">impl</span>&lt;T: Buf&gt; Buf <span class="kw">for </span>BufList&lt;T&gt; {
    ...
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>chunks_vectored&lt;<span class="lifetime">'t</span>&gt;(<span class="kw-2">&amp;</span><span class="lifetime">'t </span><span class="self">self</span>, dst: <span class="kw-2">&amp;mut </span>[std::io::IoSlice&lt;<span class="lifetime">'t</span>&gt;]) -&gt; usize {
...
<span class="kw">impl</span>&lt;T&gt; Default <span class="kw">for </span>BufList&lt;T&gt; {
    <span class="kw">fn </span>default() -&gt; <span class="self">Self </span>{
        BufList {
            bufs: VecDeque::new(),
        }
