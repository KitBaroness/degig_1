...
</pre></div><pre class="rust"><code><span class="doccomment">//! Instructions and constructors for the system program.
//!
...
<span class="attr">#[derive(Error, Debug, Serialize, Clone, PartialEq, Eq, FromPrimitive, ToPrimitive)]
    ...
    <span class="attr">#[error(<span class="string">"an account with the same address already exists"</span>)]
    </span>AccountAlreadyInUse,
    <span class="attr">#[error(<span class="string">"account does not have enough SOL to perform the operation"</span>)]
    </span>ResultWithNegativeLamports,
    <span class="attr">#[error(<span class="string">"cannot assign account to this program id"</span>)]
    </span>InvalidProgramId,
    <span class="attr">#[error(<span class="string">"cannot allocate account data of this length"</span>)]
    </span>InvalidAccountDataLength,
    <span class="attr">#[error(<span class="string">"length of requested seed is too long"</span>)]
    </span>MaxSeedLengthExceeded,
    <span class="attr">#[error(<span class="string">"provided address does not match addressed derived from seed"</span>)]
    </span>AddressWithSeedMismatch,
    <span class="attr">#[error(<span class="string">"advancing stored nonce requires a populated RecentBlockhashes sysvar"</span>)]
    </span>NonceNoRecentBlockhashes,
    <span class="attr">#[error(<span class="string">"stored nonce is still in recent_blockhashes"</span>)]
    </span>NonceBlockhashNotExpired,
    <span class="attr">#[error(<span class="string">"specified nonce does not match stored nonce"</span>)]
...
<span class="comment">// SBF program entrypoint assumes that the max account data length
...
<span class="doccomment">/// An instruction to the system program.
</span><span class="attr">#[frozen_abi(digest = <span class="string">"5e22s2kFu9Do77hdcCyxyhuKHD8ThAB6Q6dNaLTCjL5M"</span>)]
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, AbiExample, AbiEnumVisitor)]
</span><span class="kw">pub enum </span>SystemInstruction {
    <span class="doccomment">/// Create a new account
    ///
    /// # Account references
    ///   0. `[WRITE, SIGNER]` Funding account
    ///   1. `[WRITE, SIGNER]` New account
    </span>CreateAccount {
        <span class="doccomment">/// Number of lamports to transfer to the new account
        </span>lamports: u64,

        <span class="doccomment">/// Number of bytes of memory to allocate
        </span>space: u64,

        <span class="doccomment">/// Address of program that will own the new account
        </span>owner: Pubkey,
    },

    <span class="doccomment">/// Assign account to a program
    ///
    /// # Account references
    ///   0. `[WRITE, SIGNER]` Assigned account public key
    </span>Assign {
        <span class="doccomment">/// Owner program account
        </span>owner: Pubkey,
    },

    <span class="doccomment">/// Transfer lamports
    ///
    /// # Account references
    ///   0. `[WRITE, SIGNER]` Funding account
    ///   1. `[WRITE]` Recipient account
    </span>Transfer { lamports: u64 },

    <span class="doccomment">/// Create a new account at an address derived from a base pubkey and a seed
    ///
    /// # Account references
    ///   0. `[WRITE, SIGNER]` Funding account
    ...
    ///                          and provided as account 0
    </span>CreateAccountWithSeed {
        <span class="doccomment">/// Base public key
        </span>base: Pubkey,

        <span class="doccomment">/// String of ASCII chars, no longer than `Pubkey::MAX_SEED_LEN`
        </span>seed: String,
        ...
        <span class="doccomment">/// Owner program account address
        </span>owner: Pubkey,
    },

    <span class="doccomment">/// Consumes a stored nonce, replacing it with a successor
    ///
    /// # Account references
    ///   0. `[WRITE]` Nonce account
    ///   1. `[]` RecentBlockhashes sysvar
    ///   2. `[SIGNER]` Nonce authority
    </span>AdvanceNonceAccount,

    <span class="doccomment">/// Withdraw funds from a nonce account
    ///
    /// # Account references
    ///   0. `[WRITE]` Nonce account
    ///   1. `[WRITE]` Recipient account
    ...
    </span>WithdrawNonceAccount(u64),

    <span class="doccomment">/// Drive state of Uninitialized nonce account to Initialized, setting the nonce value
    ///
    /// # Account references
    ///   0. `[WRITE]` Nonce account
    ///   1. `[]` RecentBlockhashes sysvar
    ...
    </span>InitializeNonceAccount(Pubkey),

    <span class="doccomment">/// Change the entity authorized to execute nonce instructions on the account
    ///
    /// # Account references
    ///   0. `[WRITE]` Nonce account
    ...
    </span>AuthorizeNonceAccount(Pubkey),

    <span class="doccomment">/// Allocate space in a (possibly new) account without funding
    ///
    /// # Account references
    ...
    <span class="doccomment">/// Allocate space for and assign an account at an address
    ///    derived from a base public key and a seed
    ///
    /// # Account references
    ///   0. `[WRITE]` Allocated account
    ///   1. `[SIGNER]` Base account
    </span>AllocateWithSeed {
        <span class="doccomment">/// Base public key
        </span>base: Pubkey,
    ...
    <span class="doccomment">/// Assign account to a program based on a seed
    ///
    /// # Account references
    ...
    <span class="doccomment">/// Transfer lamports from a derived address
    ///
    /// # Account references
    ///   0. `[WRITE]` Funding account
    ///   1. `[SIGNER]` Base for funding account
    ///   2. `[WRITE]` Recipient account
    </span>TransferWithSeed {
        <span class="doccomment">/// Amount to transfer
        </span>lamports: u64,

        <span class="doccomment">/// Seed to use to derive the funding account address
        </span>from_seed: String,

        <span class="doccomment">/// Owner to use to derive the funding account address
        </span>from_owner: Pubkey,
    },

    <span class="doccomment">/// One-time idempotent upgrade of legacy nonce versions in order to bump
    /// them out of chain blockhash domain.
    ///
    /// # Account references
    ///   0. `[WRITE]` Nonce account
    </span>UpgradeNonceAccount,
}

<span class="doccomment">/// Create an account.
///
...
/// ```
    ...
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[
        AccountMeta::new(<span class="kw-2">*</span>from_pubkey, <span class="bool-val">true</span>),
        AccountMeta::new(<span class="kw-2">*</span>to_pubkey, <span class="bool-val">true</span>),
    ];
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::CreateAccount {
            lamports,
            space,
...
<span class="comment">// we accept `to` as a parameter so that callers do their own error handling when
//   calling create_with_seed()
</span><span class="kw">pub fn </span>create_account_with_seed(
    from_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    to_pubkey: <span class="kw-2">&amp;</span>Pubkey, <span class="comment">// must match create_with_seed(base, seed, owner)
    </span>base: <span class="kw-2">&amp;</span>Pubkey,
    ...
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[
        ...
        AccountMeta::new_readonly(<span class="kw-2">*</span>base, <span class="bool-val">true</span>),
    ];

    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::CreateAccountWithSeed {
...
<span class="doccomment">/// Assign ownership of an account from the system program.
///
/// This function produces an [`Instruction`] which must be submitted in a
/// [`Transaction`] or [invoked] to take effect, containing a serialized
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>assign(pubkey: <span class="kw-2">&amp;</span>Pubkey, owner: <span class="kw-2">&amp;</span>Pubkey) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[AccountMeta::new(<span class="kw-2">*</span>pubkey, <span class="bool-val">true</span>)];
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::Assign { owner: <span class="kw-2">*</span>owner },
        account_metas,
    )
}

<span class="kw">pub fn </span>assign_with_seed(
    address: <span class="kw-2">&amp;</span>Pubkey, <span class="comment">// must match create_with_seed(base, seed, owner)
    </span>base: <span class="kw-2">&amp;</span>Pubkey,
    seed: <span class="kw-2">&amp;</span>str,
    owner: <span class="kw-2">&amp;</span>Pubkey,
) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[
        AccountMeta::new(<span class="kw-2">*</span>address, <span class="bool-val">false</span>),
        AccountMeta::new_readonly(<span class="kw-2">*</span>base, <span class="bool-val">true</span>),
    ];
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::AssignWithSeed {
            base: <span class="kw-2">*</span>base,
            seed: seed.to_string(),
            owner: <span class="kw-2">*</span>owner,
        },
...
<span class="doccomment">/// Transfer lamports from an account owned by the system program.
///
/// This function produces an [`Instruction`] which must be submitted in a
/// [`Transaction`] or [invoked] to take effect, containing a serialized
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>transfer(from_pubkey: <span class="kw-2">&amp;</span>Pubkey, to_pubkey: <span class="kw-2">&amp;</span>Pubkey, lamports: u64) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[
        AccountMeta::new(<span class="kw-2">*</span>from_pubkey, <span class="bool-val">true</span>),
        AccountMeta::new(<span class="kw-2">*</span>to_pubkey, <span class="bool-val">false</span>),
    ];
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::Transfer { lamports },
        account_metas,
    )
}

<span class="kw">pub fn </span>transfer_with_seed(
    from_pubkey: <span class="kw-2">&amp;</span>Pubkey, <span class="comment">// must match create_with_seed(base, seed, owner)
    </span>from_base: <span class="kw-2">&amp;</span>Pubkey,
    from_seed: String,
    from_owner: <span class="kw-2">&amp;</span>Pubkey,
    to_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    lamports: u64,
) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[
        AccountMeta::new(<span class="kw-2">*</span>from_pubkey, <span class="bool-val">false</span>),
        AccountMeta::new_readonly(<span class="kw-2">*</span>from_base, <span class="bool-val">true</span>),
        AccountMeta::new(<span class="kw-2">*</span>to_pubkey, <span class="bool-val">false</span>),
    ];
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::TransferWithSeed {
            lamports,
            from_seed,
            from_owner: <span class="kw-2">*</span>from_owner,
        },
        account_metas,
    )
}

<span class="doccomment">/// Allocate space for an account.
///
/// This function produces an [`Instruction`] which must be submitted in a
/// [`Transaction`] or [invoked] to take effect, containing a serialized
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>allocate(pubkey: <span class="kw-2">&amp;</span>Pubkey, space: u64) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[AccountMeta::new(<span class="kw-2">*</span>pubkey, <span class="bool-val">true</span>)];
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::Allocate { space },
        account_metas,
    )
}

<span class="kw">pub fn </span>allocate_with_seed(
    address: <span class="kw-2">&amp;</span>Pubkey, <span class="comment">// must match create_with_seed(base, seed, owner)
    </span>base: <span class="kw-2">&amp;</span>Pubkey,
    seed: <span class="kw-2">&amp;</span>str,
    space: u64,
    ...
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::AllocateWithSeed {
            base: <span class="kw-2">*</span>base,
            seed: seed.to_string(),
            space,
            owner: <span class="kw-2">*</span>owner,
...
<span class="doccomment">/// Transfer lamports from an account owned by the system program to multiple accounts.
///
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>transfer_many(from_pubkey: <span class="kw-2">&amp;</span>Pubkey, to_lamports: <span class="kw-2">&amp;</span>[(Pubkey, u64)]) -&gt; Vec&lt;Instruction&gt; {
    to_lamports
        .iter()
        .map(|(to_pubkey, lamports)| transfer(from_pubkey, to_pubkey, <span class="kw-2">*</span>lamports))
        .collect()
}

<span class="kw">pub fn </span>create_nonce_account_with_seed(
    from_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    nonce_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    ...
    <span class="macro">vec!</span>[
        create_account_with_seed(
            ...
            seed,
            lamports,
            nonce::State::size() <span class="kw">as </span>u64,
            <span class="kw-2">&amp;</span>system_program::id(),
        ),
        Instruction::new_with_bincode(
            system_program::id(),
            <span class="kw-2">&amp;</span>SystemInstruction::InitializeNonceAccount(<span class="kw-2">*</span>authority),
            <span class="macro">vec!</span>[
                AccountMeta::new(<span class="kw-2">*</span>nonce_pubkey, <span class="bool-val">false</span>),
                <span class="attr">#[allow(deprecated)]
                </span>AccountMeta::new_readonly(recent_blockhashes::id(), <span class="bool-val">false</span>),
...
<span class="doccomment">/// Create an account containing a durable transaction nonce.
///
/// This function produces a vector of [`Instruction`]s which must be submitted
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>create_nonce_account(
    from_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    nonce_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    authority: <span class="kw-2">&amp;</span>Pubkey,
    lamports: u64,
...
<span class="doccomment">/// Advance the value of a durable transaction nonce.
///
/// This function produces an [`Instruction`] which must be submitted in a
/// [`Transaction`] or [invoked] to take effect, containing a serialized
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>advance_nonce_account(nonce_pubkey: <span class="kw-2">&amp;</span>Pubkey, authorized_pubkey: <span class="kw-2">&amp;</span>Pubkey) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[
        AccountMeta::new(<span class="kw-2">*</span>nonce_pubkey, <span class="bool-val">false</span>),
        <span class="attr">#[allow(deprecated)]
        </span>AccountMeta::new_readonly(recent_blockhashes::id(), <span class="bool-val">false</span>),
    ...
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::AdvanceNonceAccount,
        account_metas,
    )
}

<span class="doccomment">/// Withdraw lamports from a durable transaction nonce account.
///
/// This function produces an [`Instruction`] which must be submitted in a
/// [`Transaction`] or [invoked] to take effect, containing a serialized
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>withdraw_nonce_account(
    nonce_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    ...
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::WithdrawNonceAccount(lamports),
        account_metas,
    )
}

<span class="doccomment">/// Change the authority of a durable transaction nonce account.
///
/// This function produces an [`Instruction`] which must be submitted in a
/// [`Transaction`] or [invoked] to take effect, containing a serialized
...
/// # Ok::&lt;(), anyhow::Error&gt;(())
/// ```
</span><span class="kw">pub fn </span>authorize_nonce_account(
    nonce_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    authorized_pubkey: <span class="kw-2">&amp;</span>Pubkey,
    new_authority: <span class="kw-2">&amp;</span>Pubkey,
) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[
        AccountMeta::new(<span class="kw-2">*</span>nonce_pubkey, <span class="bool-val">false</span>),
        AccountMeta::new_readonly(<span class="kw-2">*</span>authorized_pubkey, <span class="bool-val">true</span>),
    ...
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::AuthorizeNonceAccount(<span class="kw-2">*</span>new_authority),
        account_metas,
    )
}
...
<span class="doccomment">/// One-time idempotent upgrade of legacy nonce versions in order to bump
/// them out of chain blockhash domain.
</span><span class="kw">pub fn </span>upgrade_nonce_account(nonce_pubkey: Pubkey) -&gt; Instruction {
    <span class="kw">let </span>account_metas = <span class="macro">vec!</span>[AccountMeta::new(nonce_pubkey, <span class="comment">/*is_signer:*/ </span><span class="bool-val">false</span>)];
    Instruction::new_with_bincode(
        system_program::id(),
        <span class="kw-2">&amp;</span>SystemInstruction::UpgradeNonceAccount,
        account_metas,
    )
}
...
<span class="attr">#[cfg(test)]
    ...
    <span class="kw">use </span>{<span class="kw">super</span>::<span class="kw-2">*</span>, <span class="kw">crate</span>::instruction::Instruction};

    <span class="kw">fn </span>get_keys(instruction: <span class="kw-2">&amp;</span>Instruction) -&gt; Vec&lt;Pubkey&gt; {
        instruction.accounts.iter().map(|x| x.pubkey).collect()
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_move_many() {
        <span class="kw">let </span>alice_pubkey = Pubkey::new_unique();
        <span class="kw">let </span>bob_pubkey = Pubkey::new_unique();
        <span class="kw">let </span>carol_pubkey = Pubkey::new_unique();
        <span class="kw">let </span>to_lamports = <span class="macro">vec!</span>[(bob_pubkey, <span class="number">1</span>), (carol_pubkey, <span class="number">2</span>)];

        <span class="kw">let </span>instructions = transfer_many(<span class="kw-2">&amp;</span>alice_pubkey, <span class="kw-2">&amp;</span>to_lamports);
        <span class="macro">assert_eq!</span>(instructions.len(), <span class="number">2</span>);
        <span class="macro">assert_eq!</span>(get_keys(<span class="kw-2">&amp;</span>instructions[<span class="number">0</span>]), <span class="macro">vec!</span>[alice_pubkey, bob_pubkey]);
        <span class="macro">assert_eq!</span>(get_keys(<span class="kw-2">&amp;</span>instructions[<span class="number">1</span>]), <span class="macro">vec!</span>[alice_pubkey, carol_pubkey]);
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_create_nonce_account() {
        <span class="kw">let </span>from_pubkey = Pubkey::new_unique();
        <span class="kw">let </span>nonce_pubkey = Pubkey::new_unique();
        <span class="kw">let </span>authorized = nonce_pubkey;
        <span class="kw">let </span>ixs = create_nonce_account(<span class="kw-2">&amp;</span>from_pubkey, <span class="kw-2">&amp;</span>nonce_pubkey, <span class="kw-2">&amp;</span>authorized, <span class="number">42</span>);
        <span class="macro">assert_eq!</span>(ixs.len(), <span class="number">2</span>);
        <span class="kw">let </span>ix = <span class="kw-2">&amp;</span>ixs[<span class="number">0</span>];
        <span class="macro">assert_eq!</span>(ix.program_id, system_program::id());
        <span class="kw">let </span>pubkeys: Vec&lt;<span class="kw">_</span>&gt; = ix.accounts.iter().map(|am| am.pubkey).collect();
        <span class="macro">assert!</span>(pubkeys.contains(<span class="kw-2">&amp;</span>from_pubkey));
        <span class="macro">assert!</span>(pubkeys.contains(<span class="kw-2">&amp;</span>nonce_pubkey));
    }
}
