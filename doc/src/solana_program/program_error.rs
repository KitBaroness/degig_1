...
</pre></div><pre class="rust"><code><span class="doccomment">//! The [`ProgramError`] type and related definitions.

</span><span class="attr">#![allow(clippy::arithmetic_side_effects)]
</span><span class="kw">use </span>{
    <span class="kw">crate</span>::{decode_error::DecodeError, instruction::InstructionError, msg, pubkey::PubkeyError},
    borsh::io::Error <span class="kw">as </span>BorshIoError,
...
<span class="doccomment">/// Reasons the program may fail
</span><span class="attr">#[derive(Clone, Debug, Deserialize, Eq, Error, PartialEq, Serialize)]
    ...
    <span class="attr">#[error(<span class="string">"The arguments provided to a program instruction were invalid"</span>)]
    </span>InvalidArgument,
    <span class="attr">#[error(<span class="string">"An instruction's data contents was invalid"</span>)]
    </span>InvalidInstructionData,
    <span class="attr">#[error(<span class="string">"An account's data contents was invalid"</span>)]
    </span>InvalidAccountData,
    <span class="attr">#[error(<span class="string">"An account's data was too small"</span>)]
    </span>AccountDataTooSmall,
    <span class="attr">#[error(<span class="string">"An account's balance was too small to complete the instruction"</span>)]
    </span>InsufficientFunds,
    <span class="attr">#[error(<span class="string">"The account did not have the expected program id"</span>)]
    </span>IncorrectProgramId,
    <span class="attr">#[error(<span class="string">"A signature was required but not found"</span>)]
    </span>MissingRequiredSignature,
    <span class="attr">#[error(<span class="string">"An initialize instruction was sent to an account that has already been initialized"</span>)]
    </span>AccountAlreadyInitialized,
    <span class="attr">#[error(<span class="string">"An attempt to operate on an account that hasn't been initialized"</span>)]
    </span>UninitializedAccount,
    <span class="attr">#[error(<span class="string">"The instruction expected additional account keys"</span>)]
    </span>NotEnoughAccountKeys,
    <span class="attr">#[error(<span class="string">"Failed to borrow a reference to account data, already borrowed"</span>)]
    </span>AccountBorrowFailed,
    <span class="attr">#[error(<span class="string">"Length of the seed is too long for address generation"</span>)]
    ...
    <span class="attr">#[error(<span class="string">"An account does not have enough lamports to be rent-exempt"</span>)]
    </span>AccountNotRentExempt,
    <span class="attr">#[error(<span class="string">"Unsupported sysvar"</span>)]
    </span>UnsupportedSysvar,
    <span class="attr">#[error(<span class="string">"Provided owner is not allowed"</span>)]
    </span>IllegalOwner,
    <span class="attr">#[error(<span class="string">"Accounts data allocations exceeded the maximum allowed per transaction"</span>)]
    </span>MaxAccountsDataAllocationsExceeded,
    <span class="attr">#[error(<span class="string">"Account data reallocation was invalid"</span>)]
    </span>InvalidRealloc,
    <span class="attr">#[error(<span class="string">"Instruction trace length exceeded the maximum allowed per transaction"</span>)]
    </span>MaxInstructionTraceLengthExceeded,
    <span class="attr">#[error(<span class="string">"Builtin programs must consume compute units"</span>)]
    </span>BuiltinProgramsMustConsumeComputeUnits,
    <span class="attr">#[error(<span class="string">"Invalid account owner"</span>)]
    </span>InvalidAccountOwner,
    <span class="attr">#[error(<span class="string">"Program arithmetic overflowed"</span>)]
...
<span class="doccomment">/// Builtin return values occupy the upper 32 bits
</span><span class="kw">const </span>BUILTIN_BIT_SHIFT: usize = <span class="number">32</span>;
<span class="macro">macro_rules!</span> to_builtin {
    (<span class="macro-nonterminal">$error</span>:expr) =&gt; {
        (<span class="macro-nonterminal">$error </span><span class="kw">as </span>u64) &lt;&lt; BUILTIN_BIT_SHIFT
    };
}

<span class="kw">pub const </span>CUSTOM_ZERO: u64 = <span class="macro">to_builtin!</span>(<span class="number">1</span>);
<span class="kw">pub const </span>INVALID_ARGUMENT: u64 = <span class="macro">to_builtin!</span>(<span class="number">2</span>);
<span class="kw">pub const </span>INVALID_INSTRUCTION_DATA: u64 = <span class="macro">to_builtin!</span>(<span class="number">3</span>);
<span class="kw">pub const </span>INVALID_ACCOUNT_DATA: u64 = <span class="macro">to_builtin!</span>(<span class="number">4</span>);
<span class="kw">pub const </span>ACCOUNT_DATA_TOO_SMALL: u64 = <span class="macro">to_builtin!</span>(<span class="number">5</span>);
<span class="kw">pub const </span>INSUFFICIENT_FUNDS: u64 = <span class="macro">to_builtin!</span>(<span class="number">6</span>);
<span class="kw">pub const </span>INCORRECT_PROGRAM_ID: u64 = <span class="macro">to_builtin!</span>(<span class="number">7</span>);
<span class="kw">pub const </span>MISSING_REQUIRED_SIGNATURES: u64 = <span class="macro">to_builtin!</span>(<span class="number">8</span>);
<span class="kw">pub const </span>ACCOUNT_ALREADY_INITIALIZED: u64 = <span class="macro">to_builtin!</span>(<span class="number">9</span>);
<span class="kw">pub const </span>UNINITIALIZED_ACCOUNT: u64 = <span class="macro">to_builtin!</span>(<span class="number">10</span>);
<span class="kw">pub const </span>NOT_ENOUGH_ACCOUNT_KEYS: u64 = <span class="macro">to_builtin!</span>(<span class="number">11</span>);
<span class="kw">pub const </span>ACCOUNT_BORROW_FAILED: u64 = <span class="macro">to_builtin!</span>(<span class="number">12</span>);
<span class="kw">pub const </span>MAX_SEED_LENGTH_EXCEEDED: u64 = <span class="macro">to_builtin!</span>(<span class="number">13</span>);
<span class="kw">pub const </span>INVALID_SEEDS: u64 = <span class="macro">to_builtin!</span>(<span class="number">14</span>);
<span class="kw">pub const </span>BORSH_IO_ERROR: u64 = <span class="macro">to_builtin!</span>(<span class="number">15</span>);
<span class="kw">pub const </span>ACCOUNT_NOT_RENT_EXEMPT: u64 = <span class="macro">to_builtin!</span>(<span class="number">16</span>);
<span class="kw">pub const </span>UNSUPPORTED_SYSVAR: u64 = <span class="macro">to_builtin!</span>(<span class="number">17</span>);
<span class="kw">pub const </span>ILLEGAL_OWNER: u64 = <span class="macro">to_builtin!</span>(<span class="number">18</span>);
<span class="kw">pub const </span>MAX_ACCOUNTS_DATA_ALLOCATIONS_EXCEEDED: u64 = <span class="macro">to_builtin!</span>(<span class="number">19</span>);
<span class="kw">pub const </span>INVALID_ACCOUNT_DATA_REALLOC: u64 = <span class="macro">to_builtin!</span>(<span class="number">20</span>);
<span class="kw">pub const </span>MAX_INSTRUCTION_TRACE_LENGTH_EXCEEDED: u64 = <span class="macro">to_builtin!</span>(<span class="number">21</span>);
<span class="kw">pub const </span>BUILTIN_PROGRAMS_MUST_CONSUME_COMPUTE_UNITS: u64 = <span class="macro">to_builtin!</span>(<span class="number">22</span>);
<span class="kw">pub const </span>INVALID_ACCOUNT_OWNER: u64 = <span class="macro">to_builtin!</span>(<span class="number">23</span>);
<span class="kw">pub const </span>ARITHMETIC_OVERFLOW: u64 = <span class="macro">to_builtin!</span>(<span class="number">24</span>);
<span class="comment">// Warning: Any new program errors added here must also be:
...
</span><span class="kw">impl </span>From&lt;ProgramError&gt; <span class="kw">for </span>u64 {
...
<span class="kw">impl</span>&lt;T&gt; From&lt;T&gt; <span class="kw">for </span>InstructionError
<span class="kw">where
    </span>T: ToPrimitive,
{
    <span class="kw">fn </span>from(error: T) -&gt; <span class="self">Self </span>{
        <span class="kw">let </span>error = error.to_u64().unwrap_or(<span class="number">0xbad_c0de</span>);
        <span class="kw">match </span>error {
            CUSTOM_ZERO =&gt; <span class="self">Self</span>::Custom(<span class="number">0</span>),
            INVALID_ARGUMENT =&gt; <span class="self">Self</span>::InvalidArgument,
            INVALID_INSTRUCTION_DATA =&gt; <span class="self">Self</span>::InvalidInstructionData,
            ...
            <span class="kw">_ </span>=&gt; {
                <span class="comment">// A valid custom error has no bits set in the upper 32
                </span><span class="kw">if </span>error &gt;&gt; BUILTIN_BIT_SHIFT == <span class="number">0 </span>{
                    <span class="self">Self</span>::Custom(error <span class="kw">as </span>u32)
                } <span class="kw">else </span>{
                    <span class="self">Self</span>::InvalidError
                }
            }
        }
    }
...
<span class="kw">impl </span>From&lt;BorshIoError&gt; <span class="kw">for </span>ProgramError {
