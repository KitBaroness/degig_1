...
</pre></div><pre class="rust"><code><span class="doccomment">/// A trait for adding some helper routines to pointers.
...
<span class="doccomment">/// A trait for adding some helper routines to raw bytes.
</span><span class="attr">#[cfg(test)]
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">trait </span>Byte {
    <span class="doccomment">/// Converts this byte to a `char` if it's ASCII. Otherwise panics.
    </span><span class="kw">fn </span>to_char(<span class="self">self</span>) -&gt; char;
}

<span class="attr">#[cfg(test)]
</span><span class="kw">impl </span>Byte <span class="kw">for </span>u8 {
    <span class="kw">fn </span>to_char(<span class="self">self</span>) -&gt; char {
        <span class="macro">assert!</span>(<span class="self">self</span>.is_ascii());
        char::from(<span class="self">self</span>)
    }
}
