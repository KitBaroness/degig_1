...
<span class="doccomment">/// A forward substring searcher that uses the Two-Way algorithm.
</span><span class="attr">#[derive(Clone, Copy, Debug)]
</span><span class="kw">pub struct </span>Finder(TwoWay);

<span class="doccomment">/// A reverse substring searcher that uses the Two-Way algorithm.
</span><span class="attr">#[derive(Clone, Copy, Debug)]
</span><span class="kw">pub struct </span>FinderRev(TwoWay);

<span class="doccomment">/// An implementation of the TwoWay substring search algorithm.
///
...
/// consistent forward or reverse APIs.
    ...
    <span class="doccomment">/// A small bitset used as a quick prefilter (in addition to any prefilter
...
<span class="kw">impl </span>Finder {
    <span class="doccomment">/// Create a searcher that finds occurrences of the given `needle`.
    ///
    ...
    <span class="doccomment">/// Returns the first occurrence of `needle` in the given `haystack`, or
    ...
    /// including at `haystack.len()`.
        ...
        <span class="self">self</span>.find_with_prefilter(<span class="prelude-val">None</span>, haystack, needle)
    }

    <span class="doccomment">/// This is like [`Finder::find`], but it accepts a prefilter for
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>find_with_prefilter(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        pre: <span class="prelude-ty">Option</span>&lt;Pre&lt;<span class="lifetime">'_</span>&gt;&gt;,
    ...
    // is made in the parent meta searcher.
        ...
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        <span class="kw-2">mut </span>pre: <span class="prelude-ty">Option</span>&lt;Pre&lt;<span class="lifetime">'_</span>&gt;&gt;,
        haystack: <span class="kw-2">&amp;</span>[u8],
        needle: <span class="kw-2">&amp;</span>[u8],
        period: usize,
    ) -&gt; <span class="prelude-ty">Option</span>&lt;usize&gt; {
        ...
        <span class="kw">let </span>last_byte_pos = <span class="kw">match </span>needle.len().checked_sub(<span class="number">1</span>) {
            <span class="prelude-val">None </span>=&gt; <span class="kw">return </span><span class="prelude-val">Some</span>(pos),
            <span class="prelude-val">Some</span>(last_byte) =&gt; last_byte,
        };
        <span class="kw">while </span>pos + needle.len() &lt;= haystack.len() {
            ...
            <span class="kw">if let </span><span class="prelude-val">Some</span>(pre) = pre.as_mut() {
                <span class="kw">if </span>pre.is_effective() {
                    pos += pre.find(<span class="kw-2">&amp;</span>haystack[pos..])<span class="question-mark">?</span>;
            ...
            <span class="kw">if </span>!<span class="self">self</span>.<span class="number">0</span>.byteset.contains(haystack[pos + last_byte_pos]) {
                pos += needle.len();
                shift = <span class="number">0</span>;
...
<span class="kw">impl </span>FinderRev {
    ...
    /// including at `haystack.len()`.
        ...
        <span class="kw">let </span>shift = Shift::reverse(needle, period_lower_bound, critical_pos);
        FinderRev(TwoWay { byteset, critical_pos, shift })
    }

    <span class="doccomment">/// Returns the last occurrence of `needle` in the given `haystack`, or
    /// `None` if no such occurrence could be found.
    ///
    /// The `needle` given must be the same as the `needle` provided to
    /// [`FinderRev::new`].
    ...
    <span class="attr">#[inline(always)]
        ...
        <span class="kw">while </span>pos &gt;= nlen {
            ...
            <span class="kw">if </span>i &gt; <span class="number">0 </span>|| first_byte != haystack[pos - nlen] {
...
<span class="kw">impl </span>Shift {
    ...
    /// The lower bound on the period is then the period of the chosen suffix.
        ...
        <span class="kw">if </span>!is_suffix(<span class="kw-2">&amp;</span>v[..period_lower_bound], u) {
            <span class="kw">return </span>Shift::Large { shift: large };
        }
    ...
    /// The lower bound on the period is then the period of the chosen suffix.
        ...
        <span class="kw">if </span>!is_prefix(<span class="kw-2">&amp;</span>v[v.len() - period_lower_bound..], u) {
            <span class="kw">return </span>Shift::Large { shift: large };
        }
        Shift::Small { period: period_lower_bound }
    }
...
<span class="kw">impl </span>Suffix {
    ...
    <span class="kw">fn </span>reverse(needle: <span class="kw-2">&amp;</span>[u8], kind: SuffixKind) -&gt; Suffix {
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>candidate_start = <span class="kw">match </span>needle.len().checked_sub(<span class="number">1</span>) {
            <span class="prelude-val">None </span>=&gt; <span class="kw">return </span>suffix,
            <span class="prelude-val">Some</span>(candidate_start) =&gt; candidate_start,
        };
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[test]
        ...
        <span class="macro">macro_rules!</span> assert_suffix_min {
            (<span class="macro-nonterminal">$given</span>:expr, <span class="macro-nonterminal">$expected</span>:expr, <span class="macro-nonterminal">$period</span>:expr) =&gt; {
                ...
                <span class="kw">let </span>got_suffix = core::str::from_utf8(got_suffix).unwrap();
    ...
    </span><span class="macro">quickcheck::quickcheck!</span> {
    ...
    // j &gt;= shift', which matches the corresponding guard in the forward impl.
        ...
        <span class="kw">let </span>rfind = |h, n| FinderRev::new(n).rfind(h, n);
