...
</pre></div><pre class="rust"><code><span class="doccomment">//! Code to convert the Rust-styled field/variant (e.g. `my_field`, `MyType`) to the
//! case of the source (e.g. `my-field`, `MY_FIELD`).

</span><span class="kw">use </span><span class="self">self</span>::RenameRule::<span class="kw-2">*</span>;
...
<span class="kw">static </span>RENAME_RULES: <span class="kw-2">&amp;</span>[(<span class="kw-2">&amp;</span>str, RenameRule)] = <span class="kw-2">&amp;</span>[
    (<span class="string">"lowercase"</span>, LowerCase),
    (<span class="string">"UPPERCASE"</span>, UpperCase),
    (<span class="string">"PascalCase"</span>, PascalCase),
    (<span class="string">"camelCase"</span>, CamelCase),
    (<span class="string">"snake_case"</span>, SnakeCase),
    (<span class="string">"SCREAMING_SNAKE_CASE"</span>, ScreamingSnakeCase),
    (<span class="string">"kebab-case"</span>, KebabCase),
    (<span class="string">"SCREAMING-KEBAB-CASE"</span>, ScreamingKebabCase),
];

<span class="kw">impl </span>RenameRule {
    <span class="kw">pub fn </span>from_str(rename_all_str: <span class="kw-2">&amp;</span>str) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, ParseError&gt; {
        <span class="kw">for </span>(name, rule) <span class="kw">in </span>RENAME_RULES {
            <span class="kw">if </span>rename_all_str == <span class="kw-2">*</span>name {
                <span class="kw">return </span><span class="prelude-val">Ok</span>(<span class="kw-2">*</span>rule);
            }
        }
        <span class="prelude-val">Err</span>(ParseError {
            unknown: rename_all_str,
        })
    }

    <span class="doccomment">/// Apply a renaming rule to an enum variant, returning the version expected in the source.
    </span><span class="kw">pub fn </span>apply_to_variant(<span class="self">self</span>, variant: <span class="kw-2">&amp;</span>str) -&gt; String {
    ...
    </span><span class="kw">pub fn </span>apply_to_field(<span class="self">self</span>, field: <span class="kw-2">&amp;</span>str) -&gt; String {
        <span class="kw">match </span><span class="self">self </span>{
    ...
    <span class="doccomment">/// Returns the `RenameRule` if it is not `None`, `rule_b` otherwise.
    </span><span class="kw">pub fn </span>or(<span class="self">self</span>, rule_b: <span class="self">Self</span>) -&gt; <span class="self">Self </span>{
        <span class="kw">match </span><span class="self">self </span>{
            <span class="prelude-val">None </span>=&gt; rule_b,
            <span class="kw">_ </span>=&gt; <span class="self">self</span>,
        }
    }
}

<span class="kw">pub struct </span>ParseError&lt;<span class="lifetime">'a</span>&gt; {
    unknown: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>str,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>&gt; Display <span class="kw">for </span>ParseError&lt;<span class="lifetime">'a</span>&gt; {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter) -&gt; fmt::Result {
        f.write_str(<span class="string">"unknown rename rule `rename_all = "</span>)<span class="question-mark">?</span>;
        Debug::fmt(<span class="self">self</span>.unknown, f)<span class="question-mark">?</span>;
        f.write_str(<span class="string">"`, expected one of "</span>)<span class="question-mark">?</span>;
        <span class="kw">for </span>(i, (name, _rule)) <span class="kw">in </span>RENAME_RULES.iter().enumerate() {
            <span class="kw">if </span>i &gt; <span class="number">0 </span>{
                f.write_str(<span class="string">", "</span>)<span class="question-mark">?</span>;
            }
            Debug::fmt(name, f)<span class="question-mark">?</span>;
        }
