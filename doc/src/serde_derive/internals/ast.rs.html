...
</pre></div><pre class="rust"><code><span class="doccomment">//! A Serde ast, parsed from the Syn ast and ready to generate Rust code.

</span><span class="kw">use </span><span class="kw">crate</span>::internals::{attr, check, Ctxt, Derive};
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>&gt; Container&lt;<span class="lifetime">'a</span>&gt; {
    <span class="doccomment">/// Convert the raw Syn ast into a parsed container object, collecting errors in `cx`.
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>data = <span class="kw">match </span><span class="kw-2">&amp;</span>item.data {
            syn::Data::Enum(data) =&gt; Data::Enum(enum_from_ast(cx, <span class="kw-2">&amp;</span>data.variants, attrs.default())),
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>&gt; Data&lt;<span class="lifetime">'a</span>&gt; {
    <span class="kw">pub fn </span>all_fields(<span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="self">self</span>) -&gt; Box&lt;<span class="kw">dyn </span>Iterator&lt;Item = <span class="kw-2">&amp;</span><span class="lifetime">'a </span>Field&lt;<span class="lifetime">'a</span>&gt;&gt; + <span class="lifetime">'a</span>&gt; {
        <span class="kw">match </span><span class="self">self </span>{
            Data::Enum(variants) =&gt; {
                Box::new(variants.iter().flat_map(|variant| variant.fields.iter()))
            }
            Data::Struct(<span class="kw">_</span>, fields) =&gt; Box::new(fields.iter()),
        }
    }
...
<span class="kw">fn </span>enum_from_ast&lt;<span class="lifetime">'a</span>&gt;(
    ...
    <span class="kw">let </span>variants: Vec&lt;Variant&gt; = variants
        .iter()
        .map(|variant| {
    ...
    <span class="kw">let </span>index_of_last_tagged_variant = variants
        .iter()
        .rposition(|variant| !variant.attrs.untagged());
    <span class="kw">if let </span><span class="prelude-val">Some</span>(index_of_last_tagged_variant) = index_of_last_tagged_variant {
        <span class="kw">for </span>variant <span class="kw">in </span><span class="kw-2">&amp;</span>variants[..index_of_last_tagged_variant] {
            <span class="kw">if </span>variant.attrs.untagged() {
                cx.error_spanned_by(<span class="kw-2">&amp;</span>variant.ident, <span class="string">"all variants with the #[serde(untagged)] attribute must be placed at the end of the enum"</span>);
            }
        }
    }
    ...
    fields
        ...
        .map(|(i, field)| Field {
            member: <span class="kw">match </span><span class="kw-2">&amp;</span>field.ident {
                <span class="prelude-val">Some</span>(ident) =&gt; syn::Member::Named(ident.clone()),
