...
<span class="kw">impl </span>Ctxt {
    ...
    <span class="doccomment">/// Add one of Syn's parse errors.
    </span><span class="kw">pub fn </span>syn_error(<span class="kw-2">&amp;</span><span class="self">self</span>, err: syn::Error) {
        <span class="self">self</span>.errors.borrow_mut().as_mut().unwrap().push(err);
    }

    <span class="doccomment">/// Consume this object, producing a formatted error string if there are errors.
    </span><span class="kw">pub fn </span>check(<span class="self">self</span>) -&gt; syn::Result&lt;()&gt; {
        <span class="kw">let </span><span class="kw-2">mut </span>errors = <span class="self">self</span>.errors.borrow_mut().take().unwrap().into_iter();

        <span class="kw">let </span><span class="kw-2">mut </span>combined = <span class="kw">match </span>errors.next() {
        ...
        <span class="kw">for </span>rest <span class="kw">in </span>errors {
            combined.combine(rest);
        }

        <span class="prelude-val">Err</span>(combined)
    }
}
