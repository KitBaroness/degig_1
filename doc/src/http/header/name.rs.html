...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::byte_str::ByteStr;
...
/// [`header`]: index.html
</span><span class="attr">#[derive(Clone, Eq, PartialEq, Hash)]
...
<span class="attr">#[derive(Debug, Clone, Eq, PartialEq, Hash)]
...
<span class="macro">macro_rules!</span> standard_headers {
    (
        ...
        <span class="attr">#[derive(Debug, Clone, Copy, Eq, PartialEq, Hash)]
...
<span class="kw">impl </span>HeaderName {
        ...
        <span class="comment">// Precondition: HEADER_CHARS_H2 is a valid table for parse_hdr()
            ...
            Repr::Custom(MaybeLower { buf, lower: <span class="bool-val">false </span>}) =&gt; {
                <span class="kw">for </span><span class="kw-2">&amp;</span>b <span class="kw">in </span>buf.iter() {
                    <span class="comment">// HEADER_CHARS_H2 maps all bytes that are not valid single-byte
                    // UTF-8 to 0 so this check returns an error for invalid UTF-8.
                    </span><span class="kw">if </span>HEADER_CHARS_H2[b <span class="kw">as </span>usize] == <span class="number">0 </span>{
                        <span class="kw">return </span><span class="prelude-val">Err</span>(InvalidHeaderName::new());
                    }
                }
    ...
    /// ```
    </span><span class="attr">#[allow(unconditional_panic)] </span><span class="comment">// required for the panic circumvention
...
<span class="kw">impl </span>InvalidHeaderName {
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>new() -&gt; InvalidHeaderName {
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_from_lowercase() {
        HeaderName::from_lowercase(<span class="kw-2">&amp;</span>[<span class="number">0</span>; <span class="number">10</span>]).unwrap_err();
        HeaderName::from_lowercase(<span class="kw-2">&amp;</span>[<span class="string">b'A'</span>; <span class="number">10</span>]).unwrap_err();
        HeaderName::from_lowercase(<span class="kw-2">&amp;</span>[<span class="number">0x1</span>; <span class="number">10</span>]).unwrap_err();
        HeaderName::from_lowercase(<span class="kw-2">&amp;</span>[<span class="number">0xFF</span>; <span class="number">10</span>]).unwrap_err();
        <span class="comment">//HeaderName::from_lowercase(&amp;[0; 100]).unwrap_err();
        </span>HeaderName::from_lowercase(<span class="kw-2">&amp;</span>[<span class="string">b'A'</span>; <span class="number">100</span>]).unwrap_err();
        HeaderName::from_lowercase(<span class="kw-2">&amp;</span>[<span class="number">0x1</span>; <span class="number">100</span>]).unwrap_err();
        HeaderName::from_lowercase(<span class="kw-2">&amp;</span>[<span class="number">0xFF</span>; <span class="number">100</span>]).unwrap_err();
    }
}
