...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::job::<span class="kw-2">*</span>;
...
<span class="doccomment">/// Puts the task into the Rayon threadpool's job queue in the "static"
...
<span class="doccomment">/// Spawns an asynchronous job in `registry.`
///
/// Unsafe because `registry` must not yet have terminated.
</span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>spawn_in&lt;F&gt;(func: F, registry: <span class="kw-2">&amp;</span>Arc&lt;Registry&gt;)
<span class="kw">where
    </span>F: FnOnce() + Send + <span class="lifetime">'static</span>,
{
    ...
    </span><span class="kw">let </span>job_ref = spawn_job(func, registry);
    registry.inject_or_push(job_ref);
    mem::forget(abort_guard);
}

<span class="kw">unsafe fn </span>spawn_job&lt;F&gt;(func: F, registry: <span class="kw-2">&amp;</span>Arc&lt;Registry&gt;) -&gt; JobRef
<span class="kw">where
    </span>F: FnOnce() + Send + <span class="lifetime">'static</span>,
{
...
///
/// [ph]: struct.ThreadPoolBuilder.html#method.panic_handler
</span><span class="kw">pub fn </span>spawn_fifo&lt;F&gt;(func: F)
<span class="kw">where
    </span>F: FnOnce() + Send + <span class="lifetime">'static</span>,
{
    <span class="comment">// We assert that current registry has not terminated.
    </span><span class="kw">unsafe </span>{ spawn_fifo_in(func, <span class="kw-2">&amp;</span>Registry::current()) }
}

<span class="doccomment">/// Spawns an asynchronous FIFO job in `registry.`
///
/// Unsafe because `registry` must not yet have terminated.
</span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>spawn_fifo_in&lt;F&gt;(func: F, registry: <span class="kw-2">&amp;</span>Arc&lt;Registry&gt;)
<span class="kw">where
    </span>F: FnOnce() + Send + <span class="lifetime">'static</span>,
{
    <span class="comment">// We assert that this does not hold any references (we know
    ...
    <span class="comment">// If we're in the pool, use our thread's private fifo for this thread to execute
    // in a locally-FIFO order.  Otherwise, just use the pool's global injector.
    </span><span class="kw">match </span>registry.current_thread() {
        <span class="prelude-val">Some</span>(worker) =&gt; worker.push_fifo(job_ref),
        <span class="prelude-val">None </span>=&gt; registry.inject(job_ref),
    }
    mem::forget(abort_guard);
}
