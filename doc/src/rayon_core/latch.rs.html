...
<span class="kw">use </span><span class="kw">crate</span>::sync::{Condvar, Mutex};
...
<span class="kw">impl </span>AsCoreLatch <span class="kw">for </span>CoreLatch {
    <span class="attr">#[inline]
...
<span class="doccomment">/// Once latches are used to implement one-time blocking, primarily
...
</span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">struct </span>OnceLatch {
    core_latch: CoreLatch,
}

<span class="kw">impl </span>OnceLatch {
    <span class="attr">#[inline]
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>new() -&gt; OnceLatch {
        <span class="self">Self </span>{
            core_latch: CoreLatch::new(),
        }
    }

    <span class="doccomment">/// Set the latch, then tickle the specific worker thread,
    /// which should be the one that owns this latch.
        ...
        <span class="kw">if </span>CoreLatch::set(<span class="kw-2">&amp;</span>(<span class="kw-2">*</span>this).core_latch) {
            registry.notify_worker_latch_is_set(target_worker_index);
        }
    }
}

<span class="kw">impl </span>AsCoreLatch <span class="kw">for </span>OnceLatch {
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>as_core_latch(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>CoreLatch {
        <span class="kw-2">&amp;</span><span class="self">self</span>.core_latch
    }
...
<span class="kw">enum </span>CountLatchKind {
    <span class="doccomment">/// A latch for scopes created on a rayon thread which will participate in work-
    /// stealing while it waits for completion. This thread is not necessarily part
    /// of the same registry as the scope itself!
    </span>Stealing {
        latch: CoreLatch,
        <span class="doccomment">/// If a worker thread in registry A calls `in_place_scope` on a ThreadPool
        ...
        <span class="doccomment">/// The index of the worker to wake in `registry`
        </span>worker_index: usize,
    },

    <span class="doccomment">/// A latch for scopes created on a non-rayon thread which will block to wait.
    </span>Blocking { latch: LockLatch },
}

<span class="kw">impl </span>std::fmt::Debug <span class="kw">for </span>CountLatchKind {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>std::fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; std::fmt::Result {
        <span class="kw">match </span><span class="self">self </span>{
            CountLatchKind::Stealing { latch, .. } =&gt; {
                f.debug_tuple(<span class="string">"Stealing"</span>).field(latch).finish()
            }
            CountLatchKind::Blocking { latch, .. } =&gt; {
                f.debug_tuple(<span class="string">"Blocking"</span>).field(latch).finish()
            }
        }
    }
}
...
<span class="kw">impl </span>CountLatch {
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>new(owner: <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>WorkerThread&gt;) -&gt; <span class="self">Self </span>{
        <span class="self">Self</span>::with_count(<span class="number">1</span>, owner)
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>with_count(count: usize, owner: <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>WorkerThread&gt;) -&gt; <span class="self">Self </span>{
        <span class="self">Self </span>{
            counter: AtomicUsize::new(count),
            kind: <span class="kw">match </span>owner {
                <span class="prelude-val">Some</span>(owner) =&gt; CountLatchKind::Stealing {
                    latch: CoreLatch::new(),
                    registry: Arc::clone(owner.registry()),
                    worker_index: owner.index(),
                },
                <span class="prelude-val">None </span>=&gt; CountLatchKind::Blocking {
                    latch: LockLatch::new(),
                },
            },
        }
    ...
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>wait(<span class="kw-2">&amp;</span><span class="self">self</span>, owner: <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>WorkerThread&gt;) {
        <span class="kw">match </span><span class="kw-2">&amp;</span><span class="self">self</span>.kind {
            CountLatchKind::Stealing {
                ...
                <span class="kw">let </span>owner = owner.expect(<span class="string">"owner thread"</span>);
                <span class="macro">debug_assert_eq!</span>(registry.id(), owner.registry().id());
                <span class="macro">debug_assert_eq!</span>(<span class="kw-2">*</span>worker_index, owner.index());
...
<span class="kw">impl </span>Latch <span class="kw">for </span>CountLatch {
    <span class="attr">#[inline]
        ...
        <span class="kw">if </span>(<span class="kw-2">*</span>this).counter.fetch_sub(<span class="number">1</span>, Ordering::SeqCst) == <span class="number">1 </span>{
            <span class="comment">// NOTE: Once we call `set` on the internal `latch`,
            // the target may proceed and invalidate `this`!
            </span><span class="kw">match </span>(<span class="kw-2">*</span>this).kind {
                CountLatchKind::Stealing {
                    <span class="kw-2">ref </span>latch,
                    <span class="kw-2">ref </span>registry,
                    worker_index,
                } =&gt; {
                    <span class="kw">let </span>registry = Arc::clone(registry);
                    <span class="kw">if </span>CoreLatch::set(latch) {
                        registry.notify_worker_latch_is_set(worker_index);
                    }
                }
                CountLatchKind::Blocking { <span class="kw-2">ref </span>latch } =&gt; LockLatch::set(latch),
            }
        }
    }
}
