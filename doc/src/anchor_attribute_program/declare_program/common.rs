...
</pre></div><pre class="rust"><code><span class="kw">use </span>anchor_lang_idl::types::{
...
<span class="comment">// TODO: Impl `ToString` for `IdlType`
    ...
    <span class="kw">match </span>ty {
        ...
        IdlType::Bytes =&gt; <span class="string">"bytes"</span>.into(),
...
<span class="kw">pub fn </span>convert_idl_type_def_to_ts(
    ...
    <span class="kw">let </span>generics = {
        <span class="kw">let </span>generics = ty_def
            ...
            .iter()
            .map(|generic| <span class="kw">match </span>generic {
                IdlTypeDefGeneric::Type { name } =&gt; <span class="macro">format_ident!</span>(<span class="string">"{name}"</span>),
                IdlTypeDefGeneric::Const { name, ty } =&gt; <span class="macro">format_ident!</span>(<span class="string">"{name}: {ty}"</span>),
            })
    ...
    <span class="kw">let </span>ty = <span class="kw">match </span><span class="kw-2">&amp;</span>ty_def.ty {
        ...
        IdlTypeDefTy::Type { alias } =&gt; {
            ...
            <span class="macro">quote!</span> { <span class="kw">pub type </span>#name = #alias; }
        }
    };
