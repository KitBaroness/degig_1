...
</pre></div><pre class="rust"><code><span class="kw">use </span>anchor_lang_idl::types::{Idl, IdlType};
<span class="kw">use </span>quote::{format_ident, quote, ToTokens};

<span class="kw">use </span><span class="kw">super</span>::common::convert_idl_type_to_str;
...
<span class="kw">pub fn </span>gen_constants_mod(idl: <span class="kw-2">&amp;</span>Idl) -&gt; proc_macro2::TokenStream {
    <span class="kw">let </span>constants = idl.constants.iter().map(|c| {
        ...
        <span class="kw">let </span>ty = <span class="kw">match </span><span class="kw-2">&amp;</span>c.ty {
            IdlType::String =&gt; <span class="macro">quote!</span>(<span class="kw-2">&amp;</span>str),
            <span class="kw">_ </span>=&gt; parse_expr_ts(<span class="kw-2">&amp;</span>convert_idl_type_to_str(<span class="kw-2">&amp;</span>c.ty)),
        };
        <span class="kw">let </span>val = parse_expr_ts(<span class="kw-2">&amp;</span>c.value);

        <span class="comment">// TODO: Docs
        </span><span class="macro">quote!</span> { <span class="kw">pub const </span>#name: #ty = #val; }
    });
...
<span class="kw">fn </span>parse_expr_ts(s: <span class="kw-2">&amp;</span>str) -&gt; proc_macro2::TokenStream {
