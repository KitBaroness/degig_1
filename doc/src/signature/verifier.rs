...
</pre></div><pre class="rust"><code><span class="doccomment">//! Trait for verifying digital signatures
...
<span class="doccomment">/// Verify the provided message bytestring using `Self` (e.g. a public key)
</span><span class="kw">pub trait </span>Verifier&lt;S: Signature&gt; {
    <span class="doccomment">/// Use `Self` to verify that the provided signature for a given message
    /// bytestring is authentic.
    ///
    /// Returns `Error` if it is inauthentic, or otherwise returns `()`.
    </span><span class="kw">fn </span>verify(<span class="kw-2">&amp;</span><span class="self">self</span>, msg: <span class="kw-2">&amp;</span>[u8], signature: <span class="kw-2">&amp;</span>S) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt;;
}

<span class="doccomment">/// Verify the provided signature for the given prehashed message [`Digest`]
/// is authentic.
///
/// ## Notes
///
...
</span><span class="kw">pub trait </span>DigestVerifier&lt;D, S&gt;
<span class="kw">where
    </span>D: Digest,
    S: Signature,
{
    <span class="doccomment">/// Verify the signature against the given [`Digest`] output.
    </span><span class="kw">fn </span>verify_digest(<span class="kw-2">&amp;</span><span class="self">self</span>, digest: D, signature: <span class="kw-2">&amp;</span>S) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt;;
}
