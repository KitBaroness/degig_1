...
</pre></div><pre class="rust"><code><span class="kw">use </span>cfg_if::cfg_if;
...
/// might change at any time, so don't rely upon it.
    ...
    <span class="attr">#[cfg(feature = <span class="string">"enable-interning"</span>)]
    </span>intern_str(s);
