...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">trait </span>AsyncBufWrite {
    <span class="doccomment">/// Attempt to return an internal buffer to write to, flushing data out to the inner reader if
    /// it is full.
    ///
    ...
    </span><span class="kw">fn </span>poll_partial_flush_buf(
    ...
    <span class="doccomment">/// Tells this buffer that `amt` bytes have been written to its buffer, so they should be
    ...
    </span><span class="kw">fn </span>produce(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, amt: usize);
}
