...
</span><span class="doccomment">//! A simple logger that can be configured via environment variables, for use
...
//! [`fmt`]: fmt/index.html
    ...
    html_favicon_url = <span class="string">"https://www.rust-lang.org/static/images/favicon.ico"
</span>)]
...
<span class="kw">use </span><span class="self">self</span>::fmt::{FormatFn, Formatter};

<span class="doccomment">/// The default name for the environment variable to read filters from.
</span><span class="kw">pub const </span>DEFAULT_FILTER_ENV: <span class="kw-2">&amp;</span>str = <span class="string">"RUST_LOG"</span>;

<span class="doccomment">/// The default name for the environment variable to read style preferences from.
</span><span class="kw">pub const </span>DEFAULT_WRITE_STYLE_ENV: <span class="kw-2">&amp;</span>str = <span class="string">"RUST_LOG_STYLE"</span>;
...
<span class="kw">impl </span>Builder {
    ...
    <span class="doccomment">/// Applies the configuration from the environment.
    ///
    ...
    /// builder.init();
    /// ```
    </span><span class="kw">pub fn </span>parse_env&lt;<span class="lifetime">'a</span>, E&gt;(<span class="kw-2">&amp;mut </span><span class="self">self</span>, env: E) -&gt; <span class="kw-2">&amp;mut </span><span class="self">Self
    </span><span class="kw">where
        </span>E: Into&lt;Env&lt;<span class="lifetime">'a</span>&gt;&gt;,
    {
        ...
        <span class="kw">if let </span><span class="prelude-val">Some</span>(s) = env.get_filter() {
            <span class="self">self</span>.parse_filters(<span class="kw-2">&amp;</span>s);
        }

        <span class="kw">if let </span><span class="prelude-val">Some</span>(s) = env.get_write_style() {
            <span class="self">self</span>.parse_write_style(<span class="kw-2">&amp;</span>s);
        }
    ...
    <span class="doccomment">/// Applies the configuration from the environment using default variable names.
    ///
    ...
    ///
    /// [default environment variables]: struct.Env.html#default-environment-variables
    </span><span class="kw">pub fn </span>parse_default_env(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;mut </span><span class="self">Self </span>{
        <span class="self">self</span>.parse_env(Env::default())
    }
    ...
    <span class="doccomment">/// Whether or not to write the target in the default format.
    </span><span class="kw">pub fn </span>format_target(<span class="kw-2">&amp;mut </span><span class="self">self</span>, write: bool) -&gt; <span class="kw-2">&amp;mut </span><span class="self">Self </span>{
        <span class="self">self</span>.format.format_target = write;
        <span class="self">self
    </span>}
    ...
    <span class="doccomment">/// Configures the end of line suffix.
    </span><span class="kw">pub fn </span>format_suffix(<span class="kw-2">&amp;mut </span><span class="self">self</span>, suffix: <span class="kw-2">&amp;</span><span class="lifetime">'static </span>str) -&gt; <span class="kw-2">&amp;mut </span><span class="self">Self </span>{
        <span class="self">self</span>.format.format_suffix = suffix;
        <span class="self">self
    </span>}
...
/// The builder can be configured before being initialized.
    ...
    note = <span class="string">"Prefer `env_logger::Builder::from_env()` instead."
</span>)]
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>builder_parse_env_overrides_existing_filters() {
        env::set_var(
            <span class="string">"builder_parse_default_env_overrides_existing_filters"</span>,
            <span class="string">"debug"</span>,
        );
        <span class="kw">let </span>env = Env::new().filter(<span class="string">"builder_parse_default_env_overrides_existing_filters"</span>);

        <span class="kw">let </span><span class="kw-2">mut </span>builder = Builder::new();
        builder.filter_level(LevelFilter::Trace);
        <span class="comment">// Overrides global level to debug
        </span>builder.parse_env(env);

        <span class="macro">assert_eq!</span>(builder.filter.build().filter(), LevelFilter::Debug);
    }
}
