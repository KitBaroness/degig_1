...
</span><span class="doccomment">//! Conversions into the library's time type.
...
</span><span class="attr">#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd)]
</span><span class="kw">pub struct </span>Time(u64);

<span class="kw">impl </span>Time {
    <span class="doccomment">/// Create a `webpki::Time` from a unix timestamp.
    ///
    ...
    </span><span class="kw">pub fn </span>from_seconds_since_unix_epoch(secs: u64) -&gt; <span class="self">Self </span>{
...
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"std"</span>)))]
</span><span class="kw">impl </span>TryFrom&lt;std::time::SystemTime&gt; <span class="kw">for </span>Time {
    <span class="kw">type </span>Error = std::time::SystemTimeError;

    <span class="doccomment">/// Create a `webpki::Time` from a `std::time::SystemTime`.
    ///
    /// # Example:
    ///
    ...
    /// # }
    /// ```
    </span><span class="kw">fn </span>try_from(value: std::time::SystemTime) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, <span class="self">Self</span>::Error&gt; {
        value
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| <span class="self">Self</span>::from_seconds_since_unix_epoch(d.as_secs()))
    }
}
