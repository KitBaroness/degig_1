...
</pre></div><pre class="rust"><code><span class="doccomment">//! Instructions for the [secp256k1 native program][np].
//!
...
<span class="kw">pub const </span>DATA_START: usize = SIGNATURE_OFFSETS_SERIALIZED_SIZE + <span class="number">1</span>;

<span class="doccomment">/// Offsets of signature data within a secp256k1 instruction.
///
/// See the [module documentation][md] for a complete description.
///
/// [md]: self
</span><span class="attr">#[derive(Default, Serialize, Deserialize, Debug, Eq, PartialEq)]
</span><span class="kw">pub struct </span>SecpSignatureOffsets {
    <span class="doccomment">/// Offset to 64-byte signature plus 1-byte recovery ID.
    </span><span class="kw">pub </span>signature_offset: u16,
    <span class="doccomment">/// Within the transaction, the index of the instruction whose instruction data contains the signature.
    </span><span class="kw">pub </span>signature_instruction_index: u8,
    <span class="doccomment">/// Offset to 20-byte Ethereum address.
    </span><span class="kw">pub </span>eth_address_offset: u16,
    <span class="doccomment">/// Within the transaction, the index of the instruction whose instruction data contains the address.
    </span><span class="kw">pub </span>eth_address_instruction_index: u8,
    <span class="doccomment">/// Offset to start of message data.
    </span><span class="kw">pub </span>message_data_offset: u16,
    <span class="doccomment">/// Size of message data in bytes.
    </span><span class="kw">pub </span>message_data_size: u16,
    <span class="doccomment">/// Within the transaction, the index of the instruction whose instruction data contains the message.
    </span><span class="kw">pub </span>message_instruction_index: u8,
}

<span class="doccomment">/// Sign a message and create a secp256k1 program instruction to verify the signature.
///
...
///
/// [`keccak`]: crate::keccak
</span><span class="kw">pub fn </span>new_secp256k1_instruction(
    priv_key: <span class="kw-2">&amp;</span>libsecp256k1::SecretKey,
    message_arr: <span class="kw-2">&amp;</span>[u8],
) -&gt; Instruction {
    <span class="kw">let </span>secp_pubkey = libsecp256k1::PublicKey::from_secret_key(priv_key);
    <span class="kw">let </span>eth_pubkey = construct_eth_pubkey(<span class="kw-2">&amp;</span>secp_pubkey);
    <span class="kw">let </span><span class="kw-2">mut </span>hasher = sha3::Keccak256::new();
    hasher.update(message_arr);
    <span class="kw">let </span>message_hash = hasher.finalize();
    <span class="kw">let </span><span class="kw-2">mut </span>message_hash_arr = [<span class="number">0u8</span>; <span class="number">32</span>];
    message_hash_arr.copy_from_slice(message_hash.as_slice());
    <span class="kw">let </span>message = libsecp256k1::Message::parse(<span class="kw-2">&amp;</span>message_hash_arr);
    <span class="kw">let </span>(signature, recovery_id) = libsecp256k1::sign(<span class="kw-2">&amp;</span>message, priv_key);
    <span class="kw">let </span>signature_arr = signature.serialize();
    <span class="macro">assert_eq!</span>(signature_arr.len(), SIGNATURE_SERIALIZED_SIZE);

    <span class="kw">let </span>instruction_data_len = DATA_START
    ...
    <span class="kw">let </span><span class="kw-2">mut </span>instruction_data = <span class="macro">vec!</span>[<span class="number">0</span>; instruction_data_len];
    ...
    instruction_data[eth_address_offset..eth_address_offset.saturating_add(eth_pubkey.len())]
        .copy_from_slice(<span class="kw-2">&amp;</span>eth_pubkey);

    <span class="kw">let </span>signature_offset = DATA_START.saturating_add(eth_pubkey.len());
    instruction_data[signature_offset..signature_offset.saturating_add(signature_arr.len())]
        .copy_from_slice(<span class="kw-2">&amp;</span>signature_arr);

    instruction_data[signature_offset.saturating_add(signature_arr.len())] =
        recovery_id.serialize();

    <span class="kw">let </span>message_data_offset = signature_offset
        .saturating_add(signature_arr.len())
        .saturating_add(<span class="number">1</span>);
    instruction_data[message_data_offset..].copy_from_slice(message_arr);

    <span class="kw">let </span>num_signatures = <span class="number">1</span>;
    instruction_data[<span class="number">0</span>] = num_signatures;
    <span class="kw">let </span>offsets = SecpSignatureOffsets {
        signature_offset: signature_offset <span class="kw">as </span>u16,
        signature_instruction_index: <span class="number">0</span>,
        eth_address_offset: eth_address_offset <span class="kw">as </span>u16,
        eth_address_instruction_index: <span class="number">0</span>,
        message_data_offset: message_data_offset <span class="kw">as </span>u16,
        message_data_size: message_arr.len() <span class="kw">as </span>u16,
        message_instruction_index: <span class="number">0</span>,
    };
    <span class="kw">let </span>writer = std::io::Cursor::new(<span class="kw-2">&amp;mut </span>instruction_data[<span class="number">1</span>..DATA_START]);
    bincode::serialize_into(writer, <span class="kw-2">&amp;</span>offsets).unwrap();
...
<span class="doccomment">/// Creates an Ethereum address from a secp256k1 public key.
</span><span class="kw">pub fn </span>construct_eth_pubkey(
    pubkey: <span class="kw-2">&amp;</span>libsecp256k1::PublicKey,
) -&gt; [u8; HASHED_PUBKEY_SERIALIZED_SIZE] {
    <span class="kw">let </span><span class="kw-2">mut </span>addr = [<span class="number">0u8</span>; HASHED_PUBKEY_SERIALIZED_SIZE];
    addr.copy_from_slice(<span class="kw-2">&amp;</span>sha3::Keccak256::digest(<span class="kw-2">&amp;</span>pubkey.serialize()[<span class="number">1</span>..])[<span class="number">12</span>..]);
    <span class="macro">assert_eq!</span>(addr.len(), HASHED_PUBKEY_SERIALIZED_SIZE);
    addr
}

<span class="doccomment">/// Verifies the signatures specified in the secp256k1 instruction data.
///
...
/// purposes passing `FeatureSet::all_enabled()` is reasonable.
    ...
    <span class="kw">if </span>data.is_empty() {
        <span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidInstructionDataSize);
    }
    <span class="kw">let </span>count = data[<span class="number">0</span>] <span class="kw">as </span>usize;
    <span class="kw">if </span>(feature_set.is_active(<span class="kw-2">&amp;</span>libsecp256k1_fail_on_bad_count::id())
        || feature_set.is_active(<span class="kw-2">&amp;</span>libsecp256k1_fail_on_bad_count2::id()))
        &amp;&amp; count == <span class="number">0
        </span>&amp;&amp; data.len() &gt; <span class="number">1
    </span>{
        <span class="comment">// count is zero but the instruction data indicates that is probably not
        // correct, fail the instruction to catch probable invalid secp256k1
        // instruction construction.
        </span><span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidInstructionDataSize);
    }
    <span class="kw">let </span>expected_data_size = count
        .saturating_mul(SIGNATURE_OFFSETS_SERIALIZED_SIZE)
        .saturating_add(<span class="number">1</span>);
    <span class="kw">if </span>data.len() &lt; expected_data_size {
        <span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidInstructionDataSize);
    }
    <span class="kw">for </span>i <span class="kw">in </span><span class="number">0</span>..count {
        <span class="kw">let </span>start = i
            .saturating_mul(SIGNATURE_OFFSETS_SERIALIZED_SIZE)
            .saturating_add(<span class="number">1</span>);
        <span class="kw">let </span>end = start.saturating_add(SIGNATURE_OFFSETS_SERIALIZED_SIZE);

        <span class="kw">let </span>offsets: SecpSignatureOffsets = bincode::deserialize(<span class="kw-2">&amp;</span>data[start..end])
            .map_err(|<span class="kw">_</span>| PrecompileError::InvalidSignature)<span class="question-mark">?</span>;

        <span class="comment">// Parse out signature
        </span><span class="kw">let </span>signature_index = offsets.signature_instruction_index <span class="kw">as </span>usize;
        <span class="kw">if </span>signature_index &gt;= instruction_datas.len() {
            <span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidInstructionDataSize);
        }
        <span class="kw">let </span>signature_instruction = instruction_datas[signature_index];
        <span class="kw">let </span>sig_start = offsets.signature_offset <span class="kw">as </span>usize;
        <span class="kw">let </span>sig_end = sig_start.saturating_add(SIGNATURE_SERIALIZED_SIZE);
        <span class="kw">if </span>sig_end &gt;= signature_instruction.len() {
            <span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidSignature);
        }

        <span class="kw">let </span>signature = libsecp256k1::Signature::parse_standard_slice(
            <span class="kw-2">&amp;</span>signature_instruction[sig_start..sig_end],
        )
        .map_err(|<span class="kw">_</span>| PrecompileError::InvalidSignature)<span class="question-mark">?</span>;

        <span class="kw">let </span>recovery_id = libsecp256k1::RecoveryId::parse(signature_instruction[sig_end])
            .map_err(|<span class="kw">_</span>| PrecompileError::InvalidRecoveryId)<span class="question-mark">?</span>;

        <span class="comment">// Parse out pubkey
        </span><span class="kw">let </span>eth_address_slice = get_data_slice(
            instruction_datas,
        ...
        <span class="comment">// Parse out message
        </span><span class="kw">let </span>message_slice = get_data_slice(
            instruction_datas,
            offsets.message_instruction_index,
            offsets.message_data_offset,
            offsets.message_data_size <span class="kw">as </span>usize,
        )<span class="question-mark">?</span>;
        ...
        <span class="kw">let </span>pubkey = libsecp256k1::recover(
            <span class="kw-2">&amp;</span>libsecp256k1::Message::parse_slice(<span class="kw-2">&amp;</span>message_hash).unwrap(),
            <span class="kw-2">&amp;</span>signature,
            <span class="kw-2">&amp;</span>recovery_id,
        )
        .map_err(|<span class="kw">_</span>| PrecompileError::InvalidSignature)<span class="question-mark">?</span>;
        <span class="kw">let </span>eth_address = construct_eth_pubkey(<span class="kw-2">&amp;</span>pubkey);

        <span class="kw">if </span>eth_address_slice != eth_address {
            <span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidSignature);
        }
    }
...
<span class="kw">fn </span>get_data_slice&lt;<span class="lifetime">'a</span>&gt;(
    instruction_datas: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>[<span class="kw-2">&amp;</span>[u8]],
    instruction_index: u8,
    offset_start: u16,
    size: usize,
) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>[u8], PrecompileError&gt; {
    <span class="kw">let </span>signature_index = instruction_index <span class="kw">as </span>usize;
    <span class="kw">if </span>signature_index &gt;= instruction_datas.len() {
        <span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidDataOffsets);
    }
    <span class="kw">let </span>signature_instruction = <span class="kw-2">&amp;</span>instruction_datas[signature_index];
    <span class="kw">let </span>start = offset_start <span class="kw">as </span>usize;
    <span class="kw">let </span>end = start.saturating_add(size);
    <span class="kw">if </span>end &gt; signature_instruction.len() {
        <span class="kw">return </span><span class="prelude-val">Err</span>(PrecompileError::InvalidSignature);
    }

    <span class="prelude-val">Ok</span>(<span class="kw-2">&amp;</span>instruction_datas[signature_index][start..end])
}
...
<span class="attr">#[cfg(test)]
    ...
    <span class="kw">fn </span>test_case(
        num_signatures: u8,
        offsets: <span class="kw-2">&amp;</span>SecpSignatureOffsets,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;(), PrecompileError&gt; {
        <span class="kw">let </span><span class="kw-2">mut </span>instruction_data = <span class="macro">vec!</span>[<span class="number">0u8</span>; DATA_START];
        instruction_data[<span class="number">0</span>] = num_signatures;
        <span class="kw">let </span>writer = std::io::Cursor::new(<span class="kw-2">&amp;mut </span>instruction_data[<span class="number">1</span>..]);
        bincode::serialize_into(writer, <span class="kw-2">&amp;</span>offsets).unwrap();
        <span class="kw">let </span>feature_set = FeatureSet::all_enabled();
        verify(<span class="kw-2">&amp;</span>instruction_data, <span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>[<span class="number">0u8</span>; <span class="number">100</span>]], <span class="kw-2">&amp;</span>feature_set)
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_invalid_offsets() {
        solana_logger::setup();

        <span class="kw">let </span><span class="kw-2">mut </span>instruction_data = <span class="macro">vec!</span>[<span class="number">0u8</span>; DATA_START];
        <span class="kw">let </span>offsets = SecpSignatureOffsets::default();
        instruction_data[<span class="number">0</span>] = <span class="number">1</span>;
        <span class="kw">let </span>writer = std::io::Cursor::new(<span class="kw-2">&amp;mut </span>instruction_data[<span class="number">1</span>..]);
        bincode::serialize_into(writer, <span class="kw-2">&amp;</span>offsets).unwrap();
        instruction_data.truncate(instruction_data.len() - <span class="number">1</span>);
        <span class="kw">let </span>feature_set = FeatureSet::all_enabled();

        <span class="macro">assert_eq!</span>(
            verify(<span class="kw-2">&amp;</span>instruction_data, <span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>[<span class="number">0u8</span>; <span class="number">100</span>]], <span class="kw-2">&amp;</span>feature_set),
            <span class="prelude-val">Err</span>(PrecompileError::InvalidInstructionDataSize)
        );

        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            signature_instruction_index: <span class="number">1</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
            test_case(<span class="number">1</span>, <span class="kw-2">&amp;</span>offsets),
            <span class="prelude-val">Err</span>(PrecompileError::InvalidInstructionDataSize)
        );

        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            message_instruction_index: <span class="number">1</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
            test_case(<span class="number">1</span>, <span class="kw-2">&amp;</span>offsets),
            <span class="prelude-val">Err</span>(PrecompileError::InvalidDataOffsets)
        );

        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            eth_address_instruction_index: <span class="number">1</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
            test_case(<span class="number">1</span>, <span class="kw-2">&amp;</span>offsets),
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_message_data_offsets() {
        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            message_data_offset: <span class="number">99</span>,
            message_data_size: <span class="number">1</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
            test_case(<span class="number">1</span>, <span class="kw-2">&amp;</span>offsets),
            <span class="prelude-val">Err</span>(PrecompileError::InvalidSignature)
        );

        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            message_data_offset: <span class="number">100</span>,
            message_data_size: <span class="number">1</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
        ...
        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            message_data_offset: <span class="number">100</span>,
            message_data_size: <span class="number">1000</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
            test_case(<span class="number">1</span>, <span class="kw-2">&amp;</span>offsets),
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_eth_offset() {
        <span class="kw">let </span>offsets = SecpSignatureOffsets {
        ...
        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            eth_address_offset: <span class="number">100 </span>- HASHED_PUBKEY_SERIALIZED_SIZE <span class="kw">as </span>u16 + <span class="number">1</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
            test_case(<span class="number">1</span>, <span class="kw-2">&amp;</span>offsets),
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_signature_offset() {
        <span class="kw">let </span>offsets = SecpSignatureOffsets {
        ...
        <span class="kw">let </span>offsets = SecpSignatureOffsets {
            signature_offset: <span class="number">100 </span>- SIGNATURE_SERIALIZED_SIZE <span class="kw">as </span>u16 + <span class="number">1</span>,
            ..SecpSignatureOffsets::default()
        };
        <span class="macro">assert_eq!</span>(
            test_case(<span class="number">1</span>, <span class="kw-2">&amp;</span>offsets),
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_count_is_zero_but_sig_data_exists() {
        solana_logger::setup();

        <span class="kw">let </span><span class="kw-2">mut </span>instruction_data = <span class="macro">vec!</span>[<span class="number">0u8</span>; DATA_START];
        <span class="kw">let </span>offsets = SecpSignatureOffsets::default();
        instruction_data[<span class="number">0</span>] = <span class="number">0</span>;
        <span class="kw">let </span>writer = std::io::Cursor::new(<span class="kw-2">&amp;mut </span>instruction_data[<span class="number">1</span>..]);
        bincode::serialize_into(writer, <span class="kw-2">&amp;</span>offsets).unwrap();
        <span class="kw">let </span>feature_set = FeatureSet::all_enabled();
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_secp256k1() {
        solana_logger::setup();
        <span class="kw">let </span>offsets = SecpSignatureOffsets::default();
        <span class="macro">assert_eq!</span>(
            bincode::serialized_size(<span class="kw-2">&amp;</span>offsets).unwrap() <span class="kw">as </span>usize,
            SIGNATURE_OFFSETS_SERIALIZED_SIZE
        );

        <span class="kw">let </span>secp_privkey = libsecp256k1::SecretKey::random(<span class="kw-2">&amp;mut </span>thread_rng());
        <span class="kw">let </span>message_arr = <span class="string">b"hello"</span>;
        <span class="kw">let </span><span class="kw-2">mut </span>secp_instruction = new_secp256k1_instruction(<span class="kw-2">&amp;</span>secp_privkey, message_arr);
        <span class="kw">let </span>mint_keypair = Keypair::new();
        <span class="kw">let </span>feature_set = feature_set::FeatureSet::all_enabled();

        <span class="kw">let </span>tx = Transaction::new_signed_with_payer(
            <span class="kw-2">&amp;</span>[secp_instruction.clone()],
            <span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span>mint_keypair.pubkey()),
            <span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>mint_keypair],
            Hash::default(),
        );

        <span class="macro">assert!</span>(tx.verify_precompiles(<span class="kw-2">&amp;</span>feature_set).is_ok());

        <span class="kw">let </span>index = thread_rng().gen_range(<span class="number">0</span>, secp_instruction.data.len());
        secp_instruction.data[index] = secp_instruction.data[index].wrapping_add(<span class="number">12</span>);
        <span class="kw">let </span>tx = Transaction::new_signed_with_payer(
            <span class="kw-2">&amp;</span>[secp_instruction],
            <span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span>mint_keypair.pubkey()),
            <span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>mint_keypair],
            Hash::default(),
        );
        <span class="macro">assert!</span>(tx.verify_precompiles(<span class="kw-2">&amp;</span>feature_set).is_err());
    }

    <span class="comment">// Signatures are malleable.
    </span><span class="attr">#[test]
    </span><span class="kw">fn </span>test_malleability() {
        solana_logger::setup();

        <span class="kw">let </span>secret_key = libsecp256k1::SecretKey::random(<span class="kw-2">&amp;mut </span>thread_rng());
        <span class="kw">let </span>public_key = libsecp256k1::PublicKey::from_secret_key(<span class="kw-2">&amp;</span>secret_key);
        <span class="kw">let </span>eth_address = construct_eth_pubkey(<span class="kw-2">&amp;</span>public_key);

        <span class="kw">let </span>message = <span class="string">b"hello"</span>;
        ...
        <span class="comment">// Flip the S value in the signature to make a different but valid signature.
        </span><span class="kw">let </span><span class="kw-2">mut </span>alt_signature = signature;
        alt_signature.s = -alt_signature.s;
        <span class="kw">let </span>alt_recovery_id = libsecp256k1::RecoveryId::parse(recovery_id.serialize() ^ <span class="number">1</span>).unwrap();

        <span class="kw">let </span><span class="kw-2">mut </span>data: Vec&lt;u8&gt; = <span class="macro">vec!</span>[];
        <span class="kw">let </span><span class="kw-2">mut </span>both_offsets = <span class="macro">vec!</span>[];

        <span class="comment">// Verify both signatures of the same message.
        </span><span class="kw">let </span>sigs = [(signature, recovery_id), (alt_signature, alt_recovery_id)];
        <span class="kw">for </span>(signature, recovery_id) <span class="kw">in </span>sigs.iter() {
            <span class="kw">let </span>signature_offset = data.len();
            data.extend(signature.serialize());
            data.push(recovery_id.serialize());
            <span class="kw">let </span>eth_address_offset = data.len();
            data.extend(eth_address);
            <span class="kw">let </span>message_data_offset = data.len();
            data.extend(message);

            <span class="kw">let </span>data_start = <span class="number">1 </span>+ SIGNATURE_OFFSETS_SERIALIZED_SIZE * <span class="number">2</span>;

            <span class="kw">let </span>offsets = SecpSignatureOffsets {
                signature_offset: (signature_offset + data_start) <span class="kw">as </span>u16,
                signature_instruction_index: <span class="number">0</span>,
                eth_address_offset: (eth_address_offset + data_start) <span class="kw">as </span>u16,
                eth_address_instruction_index: <span class="number">0</span>,
                message_data_offset: (message_data_offset + data_start) <span class="kw">as </span>u16,
                message_data_size: message.len() <span class="kw">as </span>u16,
                message_instruction_index: <span class="number">0</span>,
            };
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>instruction_data: Vec&lt;u8&gt; = <span class="macro">vec!</span>[<span class="number">2</span>];

        <span class="kw">for </span>offsets <span class="kw">in </span>both_offsets {
            <span class="kw">let </span>offsets = bincode::serialize(<span class="kw-2">&amp;</span>offsets).unwrap();
        ...
        verify(
            ...
            <span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>instruction_data],
            <span class="kw-2">&amp;</span>FeatureSet::all_enabled(),
        )
        .unwrap();
    }
}
