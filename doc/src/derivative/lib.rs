...
</pre></div><pre class="rust"><code><span class="comment">// We need to support Rust 1.34 to stable
</span><span class="attr">#![allow(deprecated)]

#![allow(renamed_and_removed_lints)] </span><span class="comment">// support for multiple Clippy versions
</span><span class="attr">#![allow(clippy::unknown_clippy_lints)] </span><span class="comment">// because of other #![allow]s
</span><span class="attr">#![allow(clippy::mem_replace_with_default)] </span><span class="comment">// needs rustc 1.40
</span><span class="attr">#![allow(clippy::option_as_ref_deref)] </span><span class="comment">// needs rustc 1.40
</span><span class="attr">#![allow(clippy::cyclomatic_complexity)] </span><span class="comment">// old name of cognitive_complexity
</span><span class="attr">#![allow(clippy::cognitive_complexity)] </span><span class="comment">// in code generated by macros
...
<span class="kw">fn </span>derive_impls(
    input: <span class="kw-2">&amp;mut </span>ast::Input,
    ...
    <span class="kw">if </span>input.attrs.partial_ord.is_some() {
        tokens.extend(cmp::derive_partial_ord(input, errors));
    }
    <span class="kw">if </span>input.attrs.ord.is_some() {
...
<span class="attr">#[cfg_attr(not(test), proc_macro_derive(Derivative, attributes(derivative)))]
</span><span class="kw">pub fn </span>derivative(input: TokenStream) -&gt; TokenStream {
    <span class="kw">let </span><span class="kw-2">mut </span>errors = proc_macro2::TokenStream::new();

    <span class="kw">let </span><span class="kw-2">mut </span>output = <span class="kw">match </span>syn::parse::&lt;syn::DeriveInput&gt;(input) {
        <span class="prelude-val">Ok</span>(parsed) =&gt; {
            ast::Input::from_ast(<span class="kw-2">&amp;</span>parsed, <span class="kw-2">&amp;mut </span>errors)
                .map(|<span class="kw-2">mut </span>input| derive_impls(<span class="kw-2">&amp;mut </span>input, <span class="kw-2">&amp;mut </span>errors))
                .unwrap_or_default()
        },
