...
</span><span class="kw">pub struct </span>Combinations&lt;I: Iterator&gt; {
    indices: Vec&lt;usize&gt;,
    pool: LazyBuffer&lt;I&gt;,
    first: bool,
}

<span class="kw">impl</span>&lt;I&gt; Clone <span class="kw">for </span>Combinations&lt;I&gt;
...
<span class="kw">impl</span>&lt;I&gt; fmt::Debug <span class="kw">for </span>Combinations&lt;I&gt;
...
</span><span class="kw">pub fn </span>combinations&lt;I&gt;(iter: I, k: usize) -&gt; Combinations&lt;I&gt;
    <span class="kw">where </span>I: Iterator
{
    <span class="kw">let </span><span class="kw-2">mut </span>pool = LazyBuffer::new(iter);
    pool.prefill(k);

    Combinations {
        indices: (<span class="number">0</span>..k).collect(),
        pool,
...
<span class="kw">impl</span>&lt;I: Iterator&gt; Combinations&lt;I&gt; {
    ...
    </span><span class="kw">pub fn </span>k(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize { <span class="self">self</span>.indices.len() }
    ...
    </span><span class="kw">pub fn </span>n(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize { <span class="self">self</span>.pool.len() }

    <span class="doccomment">/// Returns a reference to the source iterator.
    </span><span class="attr">#[inline]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>src(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>I { <span class="kw-2">&amp;</span><span class="self">self</span>.pool.it }
...
<span class="kw">impl</span>&lt;I&gt; Iterator <span class="kw">for </span>Combinations&lt;I&gt;
    <span class="kw">where </span>I: Iterator,
          I::Item: Clone
{
    ...
    <span class="kw">fn </span>next(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt; {
        <span class="kw">if </span><span class="self">self</span>.first {
        ...
        } <span class="kw">else if </span><span class="self">self</span>.indices.is_empty() {
            <span class="kw">return </span><span class="prelude-val">None</span>;
        } <span class="kw">else </span>{
            <span class="comment">// Scan from the end, looking for an index to increment
            </span><span class="kw">let </span><span class="kw-2">mut </span>i: usize = <span class="self">self</span>.indices.len() - <span class="number">1</span>;

            <span class="comment">// Check if we need to consume more from the iterator
            </span><span class="kw">if </span><span class="self">self</span>.indices[i] == <span class="self">self</span>.pool.len() - <span class="number">1 </span>{
                <span class="self">self</span>.pool.get_next(); <span class="comment">// may change pool size
            </span>}

            <span class="kw">while </span><span class="self">self</span>.indices[i] == i + <span class="self">self</span>.pool.len() - <span class="self">self</span>.indices.len() {
            ...
            </span><span class="self">self</span>.indices[i] += <span class="number">1</span>;
            <span class="kw">for </span>j <span class="kw">in </span>i+<span class="number">1</span>..<span class="self">self</span>.indices.len() {
                <span class="self">self</span>.indices[j] = <span class="self">self</span>.indices[j - <span class="number">1</span>] + <span class="number">1</span>;
            }
        }

        <span class="comment">// Create result vector based on the indices
        </span><span class="prelude-val">Some</span>(<span class="self">self</span>.indices.iter().map(|i| <span class="self">self</span>.pool[<span class="kw-2">*</span>i].clone()).collect())
    }
}

<span class="kw">impl</span>&lt;I&gt; FusedIterator <span class="kw">for </span>Combinations&lt;I&gt;
    <span class="kw">where </span>I: Iterator,
          I::Item: Clone
{}
