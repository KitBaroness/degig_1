...
</pre></div><pre class="rust"><code><span class="kw">use </span>std::io::{<span class="self">self</span>, ErrorKind};
...
#[derive(Debug, PartialEq)]
    ...
    <span class="doccomment">/// A Certificate Revocation List; as specified in RFC5280
    </span>Crl(Vec&lt;u8&gt;),
}
...
<span class="kw">impl </span>Item {
    <span class="kw">fn </span>from_start_line(start_line: <span class="kw-2">&amp;</span>[u8], der: Vec&lt;u8&gt;) -&gt; <span class="prelude-ty">Option</span>&lt;Item&gt; {
        <span class="kw">match </span>start_line {
            ...
            <span class="string">b"X509 CRL" </span>=&gt; <span class="prelude-val">Some</span>(Item::Crl(der)),
...
/// `for item in iter::from_fn(|| read_one(rd).transpose()) { ... }`
    ...
    <span class="kw">loop </span>{
        line.clear();
        <span class="kw">let </span>len = read_until_newline(rd, <span class="kw-2">&amp;mut </span>line)<span class="question-mark">?</span>;
        ...
        <span class="kw">if let </span><span class="prelude-val">Some</span>((section_type, end_marker)) = section.as_ref() {
            <span class="kw">if </span>line.starts_with(end_marker) {
                <span class="kw">let </span>der = base64::ENGINE
                    .decode(<span class="kw-2">&amp;</span>b64buf)
                    .map_err(|err| io::Error::new(ErrorKind::InvalidData, err))<span class="question-mark">?</span>;

                <span class="kw">if let </span><span class="prelude-val">Some</span>(item) = Item::from_start_line(section_type, der) {
...
<span class="comment">// Ported from https://github.com/rust-lang/rust/blob/91cfcb021935853caa06698b759c293c09d1e96a/library/std/src/io/mod.rs#L1990 and
// modified to look for our accepted newlines.
</span><span class="kw">fn </span>read_until_newline&lt;R: io::BufRead + <span class="question-mark">?</span>Sized&gt;(
    ...
    <span class="kw">loop </span>{
        <span class="kw">let </span>(done, used) = {
            <span class="kw">let </span>available = <span class="kw">match </span>r.fill_buf() {
            ...
            <span class="kw">match </span>available
                .iter()
                .copied()
                .position(|b| b == <span class="string">b'\n' </span>|| b == <span class="string">b'\r'</span>)
            {
...
<span class="kw">mod </span>base64 {
    <span class="kw">use </span>base64::alphabet::STANDARD;
    <span class="kw">use </span>base64::engine::general_purpose::{GeneralPurpose, GeneralPurposeConfig};
    <span class="kw">use </span>base64::engine::DecodePaddingMode;
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">use </span>base64::engine::Engine;

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">const </span>ENGINE: GeneralPurpose = GeneralPurpose::new(
        <span class="kw-2">&amp;</span>STANDARD,
        GeneralPurposeConfig::new().with_decode_padding_mode(DecodePaddingMode::Indifferent),
    );
}
<span class="kw">use </span><span class="self">self</span>::base64::Engine;
