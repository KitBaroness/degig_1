...
<span class="kw">use </span><span class="kw">crate</span>::error::{ErrMode, ErrorConvert, ErrorKind, Needed, ParseError};
<span class="kw">use </span><span class="kw">crate</span>::lib::std::ops::{AddAssign, Div, Shl, Shr};
<span class="kw">use </span><span class="kw">crate</span>::stream::{AsBytes, Stream, StreamIsPartial, ToUsize};
<span class="kw">use </span><span class="kw">crate</span>::trace::trace;
<span class="kw">use crate</span>::{IResult, Parser};
...
/// ```
</span><span class="kw">pub fn </span>bits&lt;I, O, E1, E2, P&gt;(<span class="kw-2">mut </span>parser: P) -&gt; <span class="kw">impl </span>Parser&lt;I, O, E2&gt;
<span class="kw">where
...
{
    trace(<span class="string">"bits"</span>, <span class="kw">move </span>|input: I| {
        <span class="kw">match </span>parser.parse_next((input, <span class="number">0</span>)) {
            <span class="prelude-val">Ok</span>(((rest, offset), result)) =&gt; {
                ...
                <span class="kw">let </span>(input, <span class="kw">_</span>) = rest.next_slice(remaining_bytes_index);
                <span class="prelude-val">Ok</span>((input, result))
            }
            <span class="prelude-val">Err</span>(ErrMode::Incomplete(n)) =&gt; <span class="prelude-val">Err</span>(ErrMode::Incomplete(n.map(|u| u.get() / <span class="number">8 </span>+ <span class="number">1</span>))),
            <span class="prelude-val">Err</span>(e) =&gt; <span class="prelude-val">Err</span>(e.convert()),
        }
    })
}
...
/// ```
</span><span class="kw">pub fn </span>bytes&lt;I, O, E1, E2, P&gt;(<span class="kw-2">mut </span>parser: P) -&gt; <span class="kw">impl </span>Parser&lt;(I, usize), O, E2&gt;
<span class="kw">where
    ...
    P: Parser&lt;I, O, E1&gt;,
{
    trace(<span class="string">"bytes"</span>, <span class="kw">move </span>|(input, offset): (I, usize)| {
        <span class="kw">let </span>(inner, <span class="kw">_</span>) = <span class="kw">if </span>offset % <span class="number">8 </span>!= <span class="number">0 </span>{
            input.next_slice(<span class="number">1 </span>+ offset / <span class="number">8</span>)
        } <span class="kw">else </span>{
            input.next_slice(offset / <span class="number">8</span>)
        };
        <span class="kw">let </span>i = (input, offset);
        <span class="kw">match </span>parser.parse_next(inner) {
            <span class="prelude-val">Ok</span>((rest, res)) =&gt; <span class="prelude-val">Ok</span>(((rest, <span class="number">0</span>), res)),
            <span class="prelude-val">Err</span>(ErrMode::Incomplete(Needed::Unknown)) =&gt; <span class="prelude-val">Err</span>(ErrMode::Incomplete(Needed::Unknown)),
            <span class="prelude-val">Err</span>(ErrMode::Incomplete(Needed::Size(sz))) =&gt; <span class="prelude-val">Err</span>(<span class="kw">match </span>sz.get().checked_mul(<span class="number">8</span>) {
                <span class="prelude-val">Some</span>(v) =&gt; ErrMode::Incomplete(Needed::new(v)),
                <span class="prelude-val">None </span>=&gt; ErrMode::Cut(E2::assert(
                    i,
...
</span><span class="kw">pub fn </span>take&lt;I, O, C, E: ParseError&lt;(I, usize)&gt;&gt;(count: C) -&gt; <span class="kw">impl </span>Parser&lt;(I, usize), O, E&gt;
<span class="kw">where
    </span>I: Stream&lt;Token = u8&gt; + AsBytes + StreamIsPartial,
    C: ToUsize,
    O: From&lt;u8&gt; + AddAssign + Shl&lt;usize, Output = O&gt; + Shr&lt;usize, Output = O&gt;,
{
    <span class="kw">let </span>count = count.to_usize();
    trace(<span class="string">"take"</span>, <span class="kw">move </span>|input: (I, usize)| {
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>streaming_take_internal&lt;I, O, E: ParseError&lt;(I, usize)&gt;&gt;(
    (input, bit_offset): (I, usize),
    count: usize,
) -&gt; IResult&lt;(I, usize), O, E&gt;
<span class="kw">where
    </span>I: Stream&lt;Token = u8&gt; + AsBytes,
    O: From&lt;u8&gt; + AddAssign + Shl&lt;usize, Output = O&gt; + Shr&lt;usize, Output = O&gt;,
{
    <span class="kw">if </span>count == <span class="number">0 </span>{
        ...
        <span class="kw">if </span>input.eof_offset() * <span class="number">8 </span>&lt; count + bit_offset {
            <span class="prelude-val">Err</span>(ErrMode::Incomplete(Needed::new(count)))
        } <span class="kw">else </span>{
            ...
            <span class="kw">for </span>byte <span class="kw">in </span>input.as_bytes().iter().copied().take(cnt + <span class="number">1</span>) {
            ...
            <span class="kw">let </span>(input, <span class="kw">_</span>) = input.next_slice(cnt);
            <span class="prelude-val">Ok</span>(((input, end_offset), acc))
        }
    }
}

<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>complete_take_internal&lt;I, O, E: ParseError&lt;(I, usize)&gt;&gt;(
    (input, bit_offset): (I, usize),
    count: usize,
) -&gt; IResult&lt;(I, usize), O, E&gt;
<span class="kw">where
    ...
    O: From&lt;u8&gt; + AddAssign + Shl&lt;usize, Output = O&gt; + Shr&lt;usize, Output = O&gt;,
{
    <span class="kw">if </span>count == <span class="number">0 </span>{
        ...
        <span class="kw">let </span>cnt = (count + bit_offset).div(<span class="number">8</span>);
        <span class="kw">if </span>input.eof_offset() * <span class="number">8 </span>&lt; count + bit_offset {
            <span class="prelude-val">Err</span>(ErrMode::from_error_kind(
                (input, bit_offset),
                ErrorKind::Eof,
            ))
        } <span class="kw">else </span>{
...
#[doc(alias = <span class="string">"just"</span>)]
    ...
    pattern: O,
    count: C,
) -&gt; <span class="kw">impl </span>Parser&lt;(I, usize), O, E&gt;
<span class="kw">where
    </span>I: Stream&lt;Token = u8&gt; + AsBytes + StreamIsPartial,
    C: ToUsize,
...
{
    <span class="kw">let </span>count = count.to_usize();
    trace(<span class="string">"tag"</span>, <span class="kw">move </span>|input: (I, usize)| {
        ...
        take(count).parse_next(input).and_then(|(i, o)| {
            <span class="kw">if </span>pattern == o {
                ...
                <span class="prelude-val">Err</span>(ErrMode::Backtrack(E::from_error_kind(inp, ErrorKind::Tag)))
            }
        })
    })
}
...
</span><span class="kw">pub fn </span>bool&lt;I, E: ParseError&lt;(I, usize)&gt;&gt;(input: (I, usize)) -&gt; IResult&lt;(I, usize), bool, E&gt;
<span class="kw">where
    </span>I: Stream&lt;Token = u8&gt; + AsBytes + StreamIsPartial,
{
    trace(<span class="string">"bool"</span>, |input: (I, usize)| {
        <span class="kw">let </span>(res, bit): (<span class="kw">_</span>, u32) = take(<span class="number">1usize</span>).parse_next(input)<span class="question-mark">?</span>;
        <span class="prelude-val">Ok</span>((res, bit != <span class="number">0</span>))
    })
    .parse_next(input)
}
