...
</span><span class="doccomment">//! An unordered map.
//!
...
<span class="kw">use </span>std::iter::{FromIterator, FusedIterator, Sum};
<span class="kw">use </span>std::mem;
<span class="kw">use </span>std::ops::{Add, Index, IndexMut};

<span class="kw">use </span><span class="kw">crate</span>::nodes::hamt::{
    hash_key, Drain <span class="kw">as </span>NodeDrain, HashBits, HashValue, Iter <span class="kw">as </span>NodeIter, IterMut <span class="kw">as </span>NodeIterMut,
    Node,
};
<span class="kw">use </span><span class="kw">crate</span>::util::{Pool, PoolRef, Ref};

<span class="doccomment">/// Construct a hash map from a sequence of key/value pairs.
///
/// # Examples
///
/// ```
...
</span><span class="macro">macro_rules!</span> hashmap {
    () =&gt; { <span class="macro-nonterminal">$crate::hashmap::HashMap::new</span>() };

    ( $( <span class="macro-nonterminal">$key</span>:expr =&gt; <span class="macro-nonterminal">$value</span>:expr ),* ) =&gt; {{
        <span class="kw">let </span><span class="kw-2">mut </span>map = <span class="macro-nonterminal">$crate::hashmap::HashMap::new</span>();
        $({
...
<span class="macro">def_pool!</span>(HashMapPool&lt;K,V&gt;, Node&lt;(K,V)&gt;);

<span class="doccomment">/// An unordered map.
///
/// An immutable hash map using [hash array mapped tries] [1].
///
/// Most operations on this map are O(log&lt;sub&gt;x&lt;/sub&gt; n) for a
...
</span><span class="kw">pub struct </span>HashMap&lt;K, V, S = RandomState&gt; {
    size: usize,
...
<span class="kw">impl</span>&lt;K, V&gt; HashValue <span class="kw">for </span>(K, V)
<span class="kw">where
    </span>K: Eq,
{
...
<span class="kw">impl</span>&lt;K, V&gt; HashMap&lt;K, V, RandomState&gt; {
    <span class="doccomment">/// Construct an empty hash map.
    ...
    <span class="doccomment">/// Construct an empty hash map using a specific memory pool.
    </span><span class="attr">#[cfg(feature = <span class="string">"pool"</span>)]
    #[must_use]
    </span><span class="kw">pub fn </span>with_pool(pool: <span class="kw-2">&amp;</span>HashMapPool&lt;K, V&gt;) -&gt; <span class="self">Self </span>{
...
<span class="kw">impl</span>&lt;K, V&gt; HashMap&lt;K, V, RandomState&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
{
    <span class="doccomment">/// Construct a hash map with a single mapping.
    ///
    /// # Examples
    ///
    /// ```
    ...
    #[must_use]
    </span><span class="kw">pub fn </span>unit(k: K, v: V) -&gt; HashMap&lt;K, V&gt; {
        HashMap::new().update(k, v)
    }
}

<span class="kw">impl</span>&lt;K, V, S&gt; HashMap&lt;K, V, S&gt; {
    <span class="doccomment">/// Test whether a hash map is empty.
    ///
    /// Time: O(1)
    ///
    /// # Examples
    ...
    <span class="doccomment">/// Get the size of a hash map.
    ///
    /// Time: O(1)
    ///
    /// # Examples
    ...
    </span><span class="kw">pub fn </span>pool(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>HashMapPool&lt;K, V&gt; {
        <span class="kw-2">&amp;</span><span class="self">self</span>.pool
    }

    <span class="doccomment">/// Construct an empty hash map using the provided hasher.
    </span><span class="attr">#[inline]
    #[must_use]
    </span><span class="kw">pub fn </span>with_hasher&lt;RS&gt;(hasher: RS) -&gt; <span class="self">Self
    </span><span class="kw">where
        </span>Ref&lt;S&gt;: From&lt;RS&gt;,
    {
        <span class="kw">let </span>pool = HashMapPool::default();
        <span class="kw">let </span>root = PoolRef::default(<span class="kw-2">&amp;</span>pool.<span class="number">0</span>);
        HashMap {
            size: <span class="number">0</span>,
            hasher: hasher.into(),
    ...
    <span class="doccomment">/// Construct an empty hash map using a specific memory pool and hasher.
    </span><span class="attr">#[cfg(feature = <span class="string">"pool"</span>)]
    #[must_use]
    </span><span class="kw">pub fn </span>with_pool_hasher&lt;RS&gt;(pool: <span class="kw-2">&amp;</span>HashMapPool&lt;K, V&gt;, hasher: RS) -&gt; <span class="self">Self
    </span><span class="kw">where
        </span>Ref&lt;S&gt;: From&lt;RS&gt;,
    {
        <span class="kw">let </span>root = PoolRef::default(<span class="kw-2">&amp;</span>pool.<span class="number">0</span>);
    ...
    <span class="doccomment">/// Get a reference to the map's [`BuildHasher`][BuildHasher].
    ///
    /// [BuildHasher]: https://doc.rust-lang.org/std/hash/trait.BuildHasher.html
    </span><span class="attr">#[must_use]
    </span><span class="kw">pub fn </span>hasher(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>Ref&lt;S&gt; {
        <span class="kw-2">&amp;</span><span class="self">self</span>.hasher
    }

    <span class="doccomment">/// Construct an empty hash map using the same hasher as the
    /// current hash map.
    </span><span class="attr">#[inline]
    #[must_use]
    </span><span class="kw">pub fn </span>new_from&lt;K1, V1&gt;(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; HashMap&lt;K1, V1, S&gt;
    <span class="kw">where
        </span>K1: Hash + Eq + Clone,
        V1: Clone,
    {
        <span class="kw">let </span>pool = HashMapPool::default();
        <span class="kw">let </span>root = PoolRef::default(<span class="kw-2">&amp;</span>pool.<span class="number">0</span>);
        HashMap {
            ...
            pool,
            root,
            hasher: <span class="self">self</span>.hasher.clone(),
        }
    }

    <span class="doccomment">/// Get an iterator over the key/value pairs of a hash map.
    ///
    ...
    <span class="doccomment">/// Get an iterator over a hash map's keys.
    ///
    /// Please note that the order is consistent between maps using
    /// the same hasher, but no other ordering guarantee is offered.
    /// Items will not come out in insertion order or sort order.
    ...
    <span class="doccomment">/// Get an iterator over a hash map's values.
    ///
    /// Please note that the order is consistent between maps using
    /// the same hasher, but no other ordering guarantee is offered.
    /// Items will not come out in insertion order or sort order.
...
<span class="kw">where
    ...
    S: BuildHasher,
{
    <span class="kw">fn </span>test_eq(<span class="kw-2">&amp;</span><span class="self">self</span>, other: <span class="kw-2">&amp;</span><span class="self">Self</span>) -&gt; bool
    <span class="kw">where
        </span>K: Hash + Eq,
        V: PartialEq,
    {
        ...
        <span class="kw">for </span>(key, value) <span class="kw">in </span><span class="self">self</span>.iter() {
            <span class="kw">if </span><span class="prelude-val">Some</span>(value) != other.get(key) {
                <span class="kw">return </span><span class="bool-val">false</span>;
            }
            seen.insert(key);
        }
        <span class="kw">for </span>key <span class="kw">in </span>other.keys() {
            <span class="kw">if </span>!seen.contains(<span class="kw-2">&amp;</span>key) {
                <span class="kw">return </span><span class="bool-val">false</span>;
            }
        }
    ...
    <span class="doccomment">/// Get the value for a key from a hash map.
    ///
    /// Time: O(log n)
    ///
    /// # Examples
    ...
    <span class="kw">where
        </span>BK: Hash + Eq + <span class="question-mark">?</span>Sized,
        K: Borrow&lt;BK&gt;,
    {
        <span class="self">self</span>.root
            .get(hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, key), <span class="number">0</span>, key)
            .map(|<span class="kw-2">&amp;</span>(<span class="kw">_</span>, <span class="kw-2">ref </span>v)| v)
    }

    <span class="doccomment">/// Get the key/value pair for a key from a hash map.
    ///
    /// Time: O(log n)
    ///
    /// # Examples
        ...
        K: Borrow&lt;BK&gt;,
    {
        <span class="self">self</span>.root
            .get(hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, key), <span class="number">0</span>, key)
            .map(|<span class="kw-2">&amp;</span>(<span class="kw-2">ref </span>k, <span class="kw-2">ref </span>v)| (k, v))
    }

    <span class="doccomment">/// Test for the presence of a key in a hash map.
    ///
    /// Time: O(log n)
    ///
    /// # Examples
...
<span class="kw">where
    ...
    S: BuildHasher,
{
    <span class="doccomment">/// Get a mutable iterator over the values of a hash map.
    ///
    /// Please note that the order is consistent between maps using
    /// the same hasher, but no other ordering guarantee is offered.
    /// Items will not come out in insertion order or sort order.
    ...
    #[must_use]
        ...
        IterMut {
            it: NodeIterMut::new(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, root, <span class="self">self</span>.size),
        }
    }

    <span class="doccomment">/// Get a mutable reference to the value for a key from a hash
    /// map.
    ///
    /// Time: O(log n)
    ///
    ...
    {
        <span class="kw">let </span>root = PoolRef::make_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, <span class="kw-2">&amp;mut </span><span class="self">self</span>.root);
        <span class="kw">match </span>root.get_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, key), <span class="number">0</span>, key) {
            <span class="prelude-val">None </span>=&gt; <span class="prelude-val">None</span>,
            <span class="prelude-val">Some</span>(<span class="kw-2">&amp;mut </span>(<span class="kw">_</span>, <span class="kw-2">ref mut </span>value)) =&gt; <span class="prelude-val">Some</span>(value),
        }
    }
    ...
    /// ```
        ...
        <span class="kw">let </span>hash = hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, <span class="kw-2">&amp;</span>k);
        <span class="kw">let </span>root = PoolRef::make_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, <span class="kw-2">&amp;mut </span><span class="self">self</span>.root);
        <span class="kw">let </span>result = root.insert(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, hash, <span class="number">0</span>, (k, v));
        <span class="kw">if </span>result.is_none() {
            <span class="self">self</span>.size += <span class="number">1</span>;
        }
        result.map(|(<span class="kw">_</span>, v)| v)
    }
    ...
    {
        <span class="kw">let </span>root = PoolRef::make_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, <span class="kw-2">&amp;mut </span><span class="self">self</span>.root);
        <span class="kw">let </span>result = root.remove(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, k), <span class="number">0</span>, k);
    ...
    </span><span class="kw">pub fn </span>entry(<span class="kw-2">&amp;mut </span><span class="self">self</span>, key: K) -&gt; Entry&lt;<span class="lifetime">'_</span>, K, V, S&gt; {
        <span class="kw">let </span>hash = hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, <span class="kw-2">&amp;</span>key);
        <span class="kw">if </span><span class="self">self</span>.root.get(hash, <span class="number">0</span>, <span class="kw-2">&amp;</span>key).is_some() {
    ...
    <span class="doccomment">/// Construct a new hash map by inserting a key/value mapping into a map.
    ///
    ...
    #[must_use]
    </span><span class="kw">pub fn </span>update(<span class="kw-2">&amp;</span><span class="self">self</span>, k: K, v: V) -&gt; <span class="self">Self </span>{
        <span class="kw">let </span><span class="kw-2">mut </span>out = <span class="self">self</span>.clone();
        out.insert(k, v);
        out
    }

    <span class="doccomment">/// Construct a new hash map by inserting a key/value mapping into
    /// a map.
    ///
    ...
    </span><span class="kw">pub fn </span>update_with&lt;F&gt;(<span class="kw-2">&amp;</span><span class="self">self</span>, k: K, v: V, f: F) -&gt; <span class="self">Self
    </span><span class="kw">where
        </span>F: FnOnce(V, V) -&gt; V,
    {
        <span class="kw">match </span><span class="self">self</span>.extract_with_key(<span class="kw-2">&amp;</span>k) {
            ...
            <span class="prelude-val">Some</span>((<span class="kw">_</span>, v2, m)) =&gt; m.update(k, f(v2, v)),
        }
    }
    ...
    </span><span class="kw">pub fn </span>update_with_key&lt;F&gt;(<span class="kw-2">&amp;</span><span class="self">self</span>, k: K, v: V, f: F) -&gt; <span class="self">Self
    </span><span class="kw">where
    ...
    </span><span class="kw">pub fn </span>update_lookup_with_key&lt;F&gt;(<span class="kw-2">&amp;</span><span class="self">self</span>, k: K, v: V, f: F) -&gt; (<span class="prelude-ty">Option</span>&lt;V&gt;, <span class="self">Self</span>)
    <span class="kw">where
    ...
    <span class="doccomment">/// Construct a new map without the given key.
    ///
    ...
    /// Time: O(log n)
        ...
        K: Borrow&lt;BK&gt;,
    {
        <span class="kw">match </span><span class="self">self</span>.extract_with_key(k) {
            <span class="prelude-val">None </span>=&gt; <span class="self">self</span>.clone(),
            <span class="prelude-val">Some</span>((<span class="kw">_</span>, <span class="kw">_</span>, map)) =&gt; map,
        }
    }

    <span class="doccomment">/// Filter out values from a map which don't satisfy a predicate.
    ///
    ...
    {
        <span class="kw">let </span>old_root = <span class="self">self</span>.root.clone();
        <span class="kw">let </span>root = PoolRef::make_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, <span class="kw-2">&amp;mut </span><span class="self">self</span>.root);
        <span class="kw">for </span>((key, value), hash) <span class="kw">in </span>NodeIter::new(<span class="kw-2">&amp;</span>old_root, <span class="self">self</span>.size) {
            <span class="kw">if </span>!f(key, value) &amp;&amp; root.remove(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, hash, <span class="number">0</span>, key).is_some() {
                <span class="self">self</span>.size -= <span class="number">1</span>;
            }
        }
    }
    ...
    /// ```
        ...
        I: IntoIterator&lt;Item = <span class="self">Self</span>&gt;,
    {
    ...
    {
        <span class="kw">let </span><span class="kw-2">mut </span>out = <span class="self">self</span>.new_from();
        <span class="kw">for </span>(key, right_value) <span class="kw">in </span>other {
            <span class="kw">match </span><span class="self">self</span>.remove(<span class="kw-2">&amp;</span>key) {
                <span class="prelude-val">None </span>=&gt; {
    ...
    #[must_use]
    </span><span class="kw">pub fn </span>intersection_with&lt;B, C, F&gt;(<span class="self">self</span>, other: HashMap&lt;K, B, S&gt;, <span class="kw-2">mut </span>f: F) -&gt; HashMap&lt;K, C, S&gt;
    <span class="kw">where
        </span>B: Clone,
        C: Clone,
        F: FnMut(V, B) -&gt; C,
    ...
    </span><span class="kw">pub fn </span>intersection_with_key&lt;B, C, F&gt;(
...
</span><span class="kw">pub enum </span>Entry&lt;<span class="lifetime">'a</span>, K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher,
{
    ...
    </span>Occupied(OccupiedEntry&lt;<span class="lifetime">'a</span>, K, V, S&gt;),
    <span class="doccomment">/// An entry which doesn't exist in the map.
    </span>Vacant(VacantEntry&lt;<span class="lifetime">'a</span>, K, V, S&gt;),
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; Entry&lt;<span class="lifetime">'a</span>, K, V, S&gt;
<span class="kw">where
    </span>K: <span class="lifetime">'a </span>+ Hash + Eq + Clone,
    V: <span class="lifetime">'a </span>+ Clone,
    S: <span class="lifetime">'a </span>+ BuildHasher,
{
...
</span><span class="kw">pub struct </span>OccupiedEntry&lt;<span class="lifetime">'a</span>, K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher,
{
    map: <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="kw-2">mut </span>HashMap&lt;K, V, S&gt;,
    hash: HashBits,
    key: K,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; OccupiedEntry&lt;<span class="lifetime">'a</span>, K, V, S&gt;
<span class="kw">where
    </span>K: <span class="lifetime">'a </span>+ Hash + Eq + Clone,
    V: <span class="lifetime">'a </span>+ Clone,
    S: <span class="lifetime">'a </span>+ BuildHasher,
{
    ...
    <span class="doccomment">/// Remove this entry from the map and return the removed mapping.
        ...
        <span class="kw">let </span>root = PoolRef::make_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.map.pool.<span class="number">0</span>, <span class="kw-2">&amp;mut </span><span class="self">self</span>.map.root);
        <span class="kw">let </span>result = root.remove(<span class="kw-2">&amp;</span><span class="self">self</span>.map.pool.<span class="number">0</span>, <span class="self">self</span>.hash, <span class="number">0</span>, <span class="kw-2">&amp;</span><span class="self">self</span>.key);
        <span class="self">self</span>.map.size -= <span class="number">1</span>;
        result.unwrap()
    }
    ...
    <span class="doccomment">/// Get the current value.
        ...
        <span class="kw-2">&amp;</span><span class="self">self</span>.map.root.get(<span class="self">self</span>.hash, <span class="number">0</span>, <span class="kw-2">&amp;</span><span class="self">self</span>.key).unwrap().<span class="number">1
    </span>}
    ...
    <span class="doccomment">/// Get a mutable reference to the current value.
        ...
        <span class="kw-2">&amp;mut </span>root
            .get_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.map.pool.<span class="number">0</span>, <span class="self">self</span>.hash, <span class="number">0</span>, <span class="kw-2">&amp;</span><span class="self">self</span>.key)
            .unwrap()
            .<span class="number">1
    </span>}
...
</span><span class="kw">pub struct </span>VacantEntry&lt;<span class="lifetime">'a</span>, K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher,
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; VacantEntry&lt;<span class="lifetime">'a</span>, K, V, S&gt;
<span class="kw">where
    </span>K: <span class="lifetime">'a </span>+ Hash + Eq + Clone,
    V: <span class="lifetime">'a </span>+ Clone,
    S: <span class="lifetime">'a </span>+ BuildHasher,
{
    ...
    <span class="doccomment">/// Insert a value into this entry.
        ...
        <span class="kw">if </span>root
            .insert(<span class="kw-2">&amp;</span><span class="self">self</span>.map.pool.<span class="number">0</span>, <span class="self">self</span>.hash, <span class="number">0</span>, (<span class="self">self</span>.key.clone(), value))
            .is_none()
        {
            <span class="self">self</span>.map.size += <span class="number">1</span>;
        }
        <span class="comment">// TODO it's unfortunate that we need to look up the key again
        // here to get the mut ref.
        </span><span class="kw-2">&amp;mut </span>root
            .get_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.map.pool.<span class="number">0</span>, <span class="self">self</span>.hash, <span class="number">0</span>, <span class="kw-2">&amp;</span><span class="self">self</span>.key)
            .unwrap()
            .<span class="number">1
    </span>}
...
</span><span class="kw">impl</span>&lt;K, V, S&gt; Clone <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Clone,
    V: Clone,
{
...
</span><span class="kw">impl</span>&lt;K, V, S&gt; PartialEq <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq,
    V: PartialEq,
    S: BuildHasher,
{
    <span class="kw">fn </span>eq(<span class="kw-2">&amp;</span><span class="self">self</span>, other: <span class="kw-2">&amp;</span><span class="self">Self</span>) -&gt; bool {
        <span class="self">self</span>.test_eq(other)
    }
}
...
{
    <span class="kw">fn </span>eq(<span class="kw-2">&amp;</span><span class="self">self</span>, other: <span class="kw-2">&amp;</span><span class="self">Self</span>) -&gt; bool {
        <span class="kw">if </span>PoolRef::ptr_eq(<span class="kw-2">&amp;</span><span class="self">self</span>.root, <span class="kw-2">&amp;</span>other.root) {
...
<span class="kw">impl</span>&lt;K, V, S&gt; Eq <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq,
    V: Eq,
    S: BuildHasher,
...
<span class="kw">impl</span>&lt;K, V, S&gt; PartialOrd <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone + PartialOrd,
    V: PartialOrd + Clone,
    S: BuildHasher,
{
    <span class="kw">fn </span>partial_cmp(<span class="kw-2">&amp;</span><span class="self">self</span>, other: <span class="kw-2">&amp;</span><span class="self">Self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;Ordering&gt; {
        <span class="kw">if </span>Ref::ptr_eq(<span class="kw-2">&amp;</span><span class="self">self</span>.hasher, <span class="kw-2">&amp;</span>other.hasher) {
            <span class="kw">return </span><span class="self">self</span>.iter().partial_cmp(other.iter());
        }
        <span class="self">self</span>.iter().partial_cmp(other.iter())
    }
}

<span class="kw">impl</span>&lt;K, V, S&gt; Ord <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Ord + Clone,
    V: Ord + Clone,
    S: BuildHasher,
{
    <span class="kw">fn </span>cmp(<span class="kw-2">&amp;</span><span class="self">self</span>, other: <span class="kw-2">&amp;</span><span class="self">Self</span>) -&gt; Ordering {
        <span class="kw">if </span>Ref::ptr_eq(<span class="kw-2">&amp;</span><span class="self">self</span>.hasher, <span class="kw-2">&amp;</span>other.hasher) {
            <span class="kw">return </span><span class="self">self</span>.iter().cmp(other.iter());
        }
        <span class="self">self</span>.iter().cmp(other.iter())
    }
}

<span class="kw">impl</span>&lt;K, V, S&gt; Hash <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq,
    V: Hash,
    S: BuildHasher,
...
<span class="kw">impl</span>&lt;K, V, S&gt; Default <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
...
<span class="kw">impl</span>&lt;K, V, S&gt; Add <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher,
{
    <span class="kw">type </span>Output = HashMap&lt;K, V, S&gt;;
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; Add <span class="kw">for </span><span class="kw-2">&amp;</span><span class="lifetime">'a </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher,
...
<span class="kw">impl</span>&lt;K, V, S&gt; Sum <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
...
<span class="kw">impl</span>&lt;K, V, S, RK, RV&gt; Extend&lt;(RK, RV)&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone + From&lt;RK&gt;,
    V: Clone + From&lt;RV&gt;,
    S: BuildHasher,
{
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, BK, K, V, S&gt; Index&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>BK&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>BK: Hash + Eq + <span class="question-mark">?</span>Sized,
    K: Hash + Eq + Borrow&lt;BK&gt;,
    S: BuildHasher,
{
    ...
    <span class="kw">fn </span>index(<span class="kw-2">&amp;</span><span class="self">self</span>, key: <span class="kw-2">&amp;</span>BK) -&gt; <span class="kw-2">&amp;</span><span class="self">Self</span>::Output {
        <span class="kw">match </span><span class="self">self</span>.root.get(hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, key), <span class="number">0</span>, key) {
            <span class="prelude-val">None </span>=&gt; <span class="macro">panic!</span>(<span class="string">"HashMap::index: invalid key"</span>),
            <span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span>(<span class="kw">_</span>, <span class="kw-2">ref </span>value)) =&gt; value,
        }
    }
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, BK, K, V, S&gt; IndexMut&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>BK&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>BK: Hash + Eq + <span class="question-mark">?</span>Sized,
    ...
    S: BuildHasher,
{
    <span class="kw">fn </span>index_mut(<span class="kw-2">&amp;mut </span><span class="self">self</span>, key: <span class="kw-2">&amp;</span>BK) -&gt; <span class="kw-2">&amp;mut </span><span class="self">Self</span>::Output {
        <span class="kw">let </span>root = PoolRef::make_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, <span class="kw-2">&amp;mut </span><span class="self">self</span>.root);
        <span class="kw">match </span>root.get_mut(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, hash_key(<span class="kw-2">&amp;*</span><span class="self">self</span>.hasher, key), <span class="number">0</span>, key) {
            <span class="prelude-val">None </span>=&gt; <span class="macro">panic!</span>(<span class="string">"HashMap::index_mut: invalid key"</span>),
...
<span class="attr">#[cfg(not(has_specialisation))]
</span><span class="kw">impl</span>&lt;K, V, S&gt; Debug <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Debug,
    V: Debug,
    S: BuildHasher,
{
...
<span class="kw">where
    ...
    S: BuildHasher,
{
    default <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt; {
        <span class="kw">let </span><span class="kw-2">mut </span>d = f.debug_map();
        <span class="kw">for </span>(k, v) <span class="kw">in </span><span class="self">self </span>{
            d.entry(k, v);
        }
...
<span class="kw">where
    </span>K: Hash + Eq + Ord + Debug,
    V: Debug,
    S: BuildHasher,
{
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt; {
        <span class="kw">let </span><span class="kw-2">mut </span>keys = collections::BTreeSet::new();
        keys.extend(<span class="self">self</span>.keys());
        <span class="kw">let </span><span class="kw-2">mut </span>d = f.debug_map();
        <span class="kw">for </span>key <span class="kw">in </span>keys {
            d.entry(key, <span class="kw-2">&amp;</span><span class="self">self</span>[key]);
        }
        d.finish()
    }
}
...
</span><span class="doccomment">/// An iterator over the elements of a map.
</span><span class="kw">pub struct </span>Iter&lt;<span class="lifetime">'a</span>, K, V&gt; {
    it: NodeIter&lt;<span class="lifetime">'a</span>, (K, V)&gt;,
}
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; Iterator <span class="kw">for </span>Iter&lt;<span class="lifetime">'a</span>, K, V&gt; {
    ...
    <span class="kw">fn </span>next(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt; {
        <span class="self">self</span>.it.next().map(|((k, v), <span class="kw">_</span>)| (k, v))
    }
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; FusedIterator <span class="kw">for </span>Iter&lt;<span class="lifetime">'a</span>, K, V&gt; {}

<span class="doccomment">/// A mutable iterator over the elements of a map.
</span><span class="kw">pub struct </span>IterMut&lt;<span class="lifetime">'a</span>, K, V&gt;
<span class="kw">where
    </span>K: Clone,
    V: Clone,
{
    it: NodeIterMut&lt;<span class="lifetime">'a</span>, (K, V)&gt;,
}
    ...
    V: Clone,
{
    ...
    <span class="kw">fn </span>next(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt; {
        <span class="self">self</span>.it.next().map(|((k, v), <span class="kw">_</span>)| (<span class="kw-2">&amp;*</span>k, v))
    }
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; FusedIterator <span class="kw">for </span>IterMut&lt;<span class="lifetime">'a</span>, K, V&gt;
<span class="kw">where
    </span>K: Clone,
    V: Clone,
{
...
<span class="doccomment">/// A consuming iterator over the elements of a map.
</span><span class="kw">pub struct </span>ConsumingIter&lt;A: HashValue&gt; {
    it: NodeDrain&lt;A&gt;,
}
    ...
    </span>A: HashValue + Clone,
{
    ...
    <span class="kw">fn </span>next(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt; {
        <span class="self">self</span>.it.next().map(|(p, <span class="kw">_</span>)| p)
    }
...
<span class="kw">impl</span>&lt;A&gt; ExactSizeIterator <span class="kw">for </span>ConsumingIter&lt;A&gt; <span class="kw">where </span>A: HashValue + Clone {}

<span class="kw">impl</span>&lt;A&gt; FusedIterator <span class="kw">for </span>ConsumingIter&lt;A&gt; <span class="kw">where </span>A: HashValue + Clone {}

<span class="doccomment">/// An iterator over the keys of a map.
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; Iterator <span class="kw">for </span>Keys&lt;<span class="lifetime">'a</span>, K, V&gt; {
    ...
    <span class="kw">fn </span>next(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt; {
        <span class="self">self</span>.it.next().map(|((k, <span class="kw">_</span>), <span class="kw">_</span>)| k)
    }
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; FusedIterator <span class="kw">for </span>Keys&lt;<span class="lifetime">'a</span>, K, V&gt; {}
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; Iterator <span class="kw">for </span>Values&lt;<span class="lifetime">'a</span>, K, V&gt; {
    ...
    <span class="kw">fn </span>next(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt; {
        <span class="self">self</span>.it.next().map(|((<span class="kw">_</span>, v), <span class="kw">_</span>)| v)
    }
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; FusedIterator <span class="kw">for </span>Values&lt;<span class="lifetime">'a</span>, K, V&gt; {}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; IntoIterator <span class="kw">for </span><span class="kw-2">&amp;</span><span class="lifetime">'a </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq,
    S: BuildHasher,
{
...
<span class="kw">impl</span>&lt;K, V, S&gt; IntoIterator <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher,
{
    ...
    <span class="attr">#[inline]
        ...
        ConsumingIter {
            it: NodeDrain::new(<span class="kw-2">&amp;</span><span class="self">self</span>.pool.<span class="number">0</span>, <span class="self">self</span>.root, <span class="self">self</span>.size),
        }
    }
}
...
</span><span class="kw">impl</span>&lt;K, V, S&gt; FromIterator&lt;(K, V)&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from_iter&lt;T&gt;(i: T) -&gt; <span class="self">Self
        ...
        </span>T: IntoIterator&lt;Item = (K, V)&gt;,
    {
        <span class="kw">let </span><span class="kw-2">mut </span>map = <span class="self">Self</span>::default();
...
<span class="kw">impl</span>&lt;K, V, S&gt; AsRef&lt;HashMap&lt;K, V, S&gt;&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt; {
    <span class="attr">#[inline]
...
<span class="kw">impl</span>&lt;<span class="lifetime">'m</span>, <span class="lifetime">'k</span>, <span class="lifetime">'v</span>, K, V, OK, OV, SA, SB&gt; From&lt;<span class="kw-2">&amp;</span><span class="lifetime">'m </span>HashMap&lt;<span class="kw-2">&amp;</span><span class="lifetime">'k </span>K, <span class="kw-2">&amp;</span><span class="lifetime">'v </span>V, SA&gt;&gt; <span class="kw">for </span>HashMap&lt;OK, OV, SB&gt;
<span class="kw">where
    </span>K: Hash + Eq + ToOwned&lt;Owned = OK&gt; + <span class="question-mark">?</span>Sized,
...
{
    <span class="kw">fn </span>from(m: <span class="kw-2">&amp;</span>HashMap&lt;<span class="kw-2">&amp;</span>K, <span class="kw-2">&amp;</span>V, SA&gt;) -&gt; <span class="self">Self </span>{
        m.iter()
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; From&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>[(K, V)]&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from(m: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>[(K, V)]) -&gt; <span class="self">Self </span>{
        m.iter().cloned().collect()
    }
}

<span class="kw">impl</span>&lt;K, V, S&gt; From&lt;Vec&lt;(K, V)&gt;&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from(m: Vec&lt;(K, V)&gt;) -&gt; <span class="self">Self </span>{
        m.into_iter().collect()
    }
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; From&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>Vec&lt;(K, V)&gt;&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from(m: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>Vec&lt;(K, V)&gt;) -&gt; <span class="self">Self </span>{
        m.iter().cloned().collect()
    }
}

<span class="kw">impl</span>&lt;K, V, S&gt; From&lt;collections::HashMap&lt;K, V&gt;&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from(m: collections::HashMap&lt;K, V&gt;) -&gt; <span class="self">Self </span>{
        m.into_iter().collect()
    }
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; From&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>collections::HashMap&lt;K, V&gt;&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from(m: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>collections::HashMap&lt;K, V&gt;) -&gt; <span class="self">Self </span>{
        m.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
    }
}

<span class="kw">impl</span>&lt;K, V, S&gt; From&lt;collections::BTreeMap&lt;K, V&gt;&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from(m: collections::BTreeMap&lt;K, V&gt;) -&gt; <span class="self">Self </span>{
        m.into_iter().collect()
    }
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V, S&gt; From&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>collections::BTreeMap&lt;K, V&gt;&gt; <span class="kw">for </span>HashMap&lt;K, V, S&gt;
<span class="kw">where
    </span>K: Hash + Eq + Clone,
    V: Clone,
    S: BuildHasher + Default,
{
    <span class="kw">fn </span>from(m: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>collections::BTreeMap&lt;K, V&gt;) -&gt; <span class="self">Self </span>{
        m.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
    }
}

<span class="comment">// impl&lt;K: Ord + Hash + Eq, V, S&gt; From&lt;OrdMap&lt;K, V&gt;&gt; for HashMap&lt;K, V, S&gt;
// where
//     S: BuildHasher + Default,
// {
...
#[doc(hidden)]
    ...
    </span><span class="kw">pub use </span><span class="kw">crate</span>::proptest::hash_map;
}
...
<span class="comment">// Tests
    ...
    <span class="kw">use </span><span class="kw">crate</span>::test::LolHasher;
    <span class="kw">use </span>::proptest::num::{i16, usize};
    <span class="kw">use </span>::proptest::{collection, proptest};
    <span class="kw">use </span>std::hash::BuildHasherDefault;

    <span class="attr">#[test]
    </span><span class="kw">fn </span>safe_mutation() {
        <span class="kw">let </span>v1: HashMap&lt;usize, usize&gt; = (<span class="number">0</span>..<span class="number">131_072</span>).map(|i| (i, i)).collect::&lt;HashMap&lt;<span class="kw">_</span>, <span class="kw">_</span>&gt;&gt;();
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>map = <span class="macro">hashmap!</span>[<span class="number">1 </span>=&gt; <span class="number">2</span>, <span class="number">3 </span>=&gt; <span class="number">4</span>, <span class="number">5 </span>=&gt; <span class="number">6</span>];
        ...
        <span class="macro">assert_eq!</span>(<span class="macro">hashmap!</span>[<span class="number">1 </span>=&gt; <span class="number">2</span>, <span class="number">3 </span>=&gt; <span class="number">8</span>, <span class="number">5 </span>=&gt; <span class="number">6</span>], map);
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>proper_formatting() {
        <span class="kw">let </span>map = <span class="macro">hashmap!</span>[<span class="number">1 </span>=&gt; <span class="number">2</span>];
        <span class="macro">assert_eq!</span>(<span class="string">"{1: 2}"</span>, <span class="macro">format!</span>(<span class="string">"{:?}"</span>, map));

        <span class="macro">assert_eq!</span>(<span class="string">"{}"</span>, <span class="macro">format!</span>(<span class="string">"{:?}"</span>, HashMap::&lt;(), ()&gt;::new()));
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>remove_failing() {
        <span class="kw">let </span>pairs = [(<span class="number">1469</span>, <span class="number">0</span>), (-<span class="number">67</span>, <span class="number">0</span>)];
        <span class="kw">let </span><span class="kw-2">mut </span>m: collections::HashMap&lt;i16, i16, <span class="kw">_</span>&gt; =
            collections::HashMap::with_hasher(BuildHasherDefault::&lt;LolHasher&gt;::default());
        <span class="kw">for </span><span class="kw-2">&amp;</span>(<span class="kw-2">ref </span>k, <span class="kw-2">ref </span>v) <span class="kw">in </span><span class="kw-2">&amp;</span>pairs {
            m.insert(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
        }
        <span class="kw">let </span><span class="kw-2">mut </span>map: HashMap&lt;i16, i16, <span class="kw">_</span>&gt; =
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>map: HashMap&lt;String, i32&gt; =
            From::from(<span class="kw-2">&amp;</span><span class="macro">hashmap!</span> { <span class="string">"foo" </span>=&gt; <span class="kw-2">&amp;</span><span class="number">1</span>, <span class="string">"bar" </span>=&gt; <span class="kw-2">&amp;</span><span class="number">2</span>, <span class="string">"baz" </span>=&gt; <span class="kw-2">&amp;</span><span class="number">3 </span>});
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span>map1 = <span class="macro">hashmap!</span> {<span class="string">"x" </span>=&gt; <span class="number">1</span>, <span class="string">"y" </span>=&gt; <span class="number">2</span>};
        <span class="kw">let </span>map2 = <span class="macro">hashmap!</span> {
            <span class="string">"x" </span>=&gt; <span class="number">1</span>,
            <span class="string">"y" </span>=&gt; <span class="number">2</span>,
        };
        <span class="macro">assert_eq!</span>(map1, map2);
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>remove_top_level_collisions() {
        <span class="kw">let </span>pairs = <span class="macro">vec!</span>[<span class="number">9</span>, <span class="number">2569</span>, <span class="number">27145</span>];
        <span class="kw">let </span><span class="kw-2">mut </span>map: HashMap&lt;i16, i16, BuildHasherDefault&lt;LolHasher&gt;&gt; = Default::default();
        <span class="kw">for </span>k <span class="kw">in </span>pairs.clone() {
            map.insert(k, k);
        }
        <span class="macro">assert_eq!</span>(pairs.len(), map.len());
        <span class="kw">let </span>keys: Vec&lt;<span class="kw">_</span>&gt; = map.keys().cloned().collect();
        <span class="kw">for </span>k <span class="kw">in </span>keys {
            <span class="kw">let </span>l = map.len();
            <span class="macro">assert_eq!</span>(<span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span>k), map.get(<span class="kw-2">&amp;</span>k));
            map.remove(<span class="kw-2">&amp;</span>k);
            <span class="macro">assert_eq!</span>(<span class="prelude-val">None</span>, map.get(<span class="kw-2">&amp;</span>k));
            <span class="macro">assert_eq!</span>(l - <span class="number">1</span>, map.len());
        }
    }
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>map = <span class="macro">hashmap!</span> {<span class="string">"bar" </span>=&gt; <span class="number">5</span>};
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>refpool_crash() {
        <span class="kw">let </span>_map = HashMap::&lt;u128, usize&gt;::new();
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>large_map() {
        <span class="kw">let </span><span class="kw-2">mut </span>map = HashMap::new();
        <span class="kw">let </span>size = <span class="number">32769</span>;
        <span class="kw">for </span>i <span class="kw">in </span><span class="number">0</span>..size {
            map.insert(i, i);
        }
        <span class="macro">assert_eq!</span>(size, map.len());
        <span class="kw">for </span>i <span class="kw">in </span><span class="number">0</span>..size {
            <span class="macro">assert_eq!</span>(<span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span>i), map.get(<span class="kw-2">&amp;</span>i));
        }
    }

    <span class="macro">proptest!</span> {
        <span class="attr">#[test]
        </span><span class="kw">fn </span>update_and_length(<span class="kw-2">ref </span>m <span class="kw">in </span>collection::hash_map(i16::ANY, i16::ANY, <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span><span class="kw-2">mut </span>map: HashMap&lt;i16, i16, BuildHasherDefault&lt;LolHasher&gt;&gt; = Default::default();
            <span class="kw">for </span>(index, (k, v)) <span class="kw">in </span>m.iter().enumerate() {
                map = map.update(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
                <span class="macro">assert_eq!</span>(<span class="prelude-val">Some</span>(v), map.get(k));
                <span class="macro">assert_eq!</span>(index + <span class="number">1</span>, map.len());
            }
        }

        <span class="attr">#[test]
        </span><span class="kw">fn </span>from_iterator(<span class="kw-2">ref </span>m <span class="kw">in </span>collection::hash_map(i16::ANY, i16::ANY, <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span>map: HashMap&lt;i16, i16&gt; =
        ...
        <span class="attr">#[test]
        </span><span class="kw">fn </span>iterate_over(<span class="kw-2">ref </span>m <span class="kw">in </span>collection::hash_map(i16::ANY, i16::ANY, <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span>map: HashMap&lt;i16, i16&gt; = FromIterator::from_iter(m.iter().map(|(k, v)| (<span class="kw-2">*</span>k, <span class="kw-2">*</span>v)));
            <span class="macro">assert_eq!</span>(m.len(), map.iter().count());
        }

        <span class="attr">#[test]
        </span><span class="kw">fn </span>equality(<span class="kw-2">ref </span>m <span class="kw">in </span>collection::hash_map(i16::ANY, i16::ANY, <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span>map1: HashMap&lt;i16, i16&gt; = FromIterator::from_iter(m.iter().map(|(k, v)| (<span class="kw-2">*</span>k, <span class="kw-2">*</span>v)));
            <span class="kw">let </span>map2: HashMap&lt;i16, i16&gt; = FromIterator::from_iter(m.iter().map(|(k, v)| (<span class="kw-2">*</span>k, <span class="kw-2">*</span>v)));
            <span class="macro">assert_eq!</span>(map1, map2);
        }

        <span class="attr">#[test]
        </span><span class="kw">fn </span>lookup(<span class="kw-2">ref </span>m <span class="kw">in </span>collection::hash_map(i16::ANY, i16::ANY, <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span>map: HashMap&lt;i16, i16&gt; = FromIterator::from_iter(m.iter().map(|(k, v)| (<span class="kw-2">*</span>k, <span class="kw-2">*</span>v)));
            <span class="kw">for </span>(k, v) <span class="kw">in </span>m {
                <span class="macro">assert_eq!</span>(<span class="prelude-val">Some</span>(<span class="kw-2">*</span>v), map.get(k).cloned());
            }
        }

        <span class="attr">#[test]
        </span><span class="kw">fn </span>without(<span class="kw-2">ref </span>pairs <span class="kw">in </span>collection::vec((i16::ANY, i16::ANY), <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span><span class="kw-2">mut </span>m: collections::HashMap&lt;i16, i16, <span class="kw">_</span>&gt; =
                collections::HashMap::with_hasher(BuildHasherDefault::&lt;LolHasher&gt;::default());
            <span class="kw">for </span><span class="kw-2">&amp;</span>(<span class="kw-2">ref </span>k, <span class="kw-2">ref </span>v) <span class="kw">in </span>pairs {
                m.insert(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
            }
            <span class="kw">let </span><span class="kw-2">mut </span>map: HashMap&lt;i16, i16, <span class="kw">_</span>&gt; = HashMap::with_hasher(BuildHasherDefault::&lt;LolHasher&gt;::default());
            <span class="kw">for </span>(k, v) <span class="kw">in </span><span class="kw-2">&amp;</span>m {
                map = map.update(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
            }
            <span class="kw">for </span>k <span class="kw">in </span>m.keys() {
        ...
        <span class="attr">#[test]
        </span><span class="kw">fn </span>insert(<span class="kw-2">ref </span>m <span class="kw">in </span>collection::hash_map(i16::ANY, i16::ANY, <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span><span class="kw-2">mut </span>mut_map: HashMap&lt;i16, i16, BuildHasherDefault&lt;LolHasher&gt;&gt; = Default::default();
            <span class="kw">let </span><span class="kw-2">mut </span>map: HashMap&lt;i16, i16, BuildHasherDefault&lt;LolHasher&gt;&gt; = Default::default();
            <span class="kw">for </span>(count, (k, v)) <span class="kw">in </span>m.iter().enumerate() {
                map = map.update(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
                mut_map.insert(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
                <span class="macro">assert_eq!</span>(count + <span class="number">1</span>, map.len());
                <span class="macro">assert_eq!</span>(count + <span class="number">1</span>, mut_map.len());
            }
            <span class="macro">assert_eq!</span>(map, mut_map);
        }

        <span class="attr">#[test]
        </span><span class="kw">fn </span>remove(<span class="kw-2">ref </span>pairs <span class="kw">in </span>collection::vec((i16::ANY, i16::ANY), <span class="number">0</span>..<span class="number">100</span>)) {
            <span class="kw">let </span><span class="kw-2">mut </span>m: collections::HashMap&lt;i16, i16, <span class="kw">_</span>&gt; =
                collections::HashMap::with_hasher(BuildHasherDefault::&lt;LolHasher&gt;::default());
            <span class="kw">for </span><span class="kw-2">&amp;</span>(<span class="kw-2">ref </span>k, <span class="kw-2">ref </span>v) <span class="kw">in </span>pairs {
                m.insert(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
            ...
            <span class="kw">let </span><span class="kw-2">mut </span>map: HashMap&lt;i16, i16, <span class="kw">_</span>&gt; = HashMap::with_hasher(BuildHasherDefault::&lt;LolHasher&gt;::default());
            <span class="kw">for </span>(k, v) <span class="kw">in </span><span class="kw-2">&amp;</span>m {
                map.insert(<span class="kw-2">*</span>k, <span class="kw-2">*</span>v);
            }
            <span class="kw">for </span>k <span class="kw">in </span>m.keys() {
                <span class="kw">let </span>l = map.len();
                <span class="macro">assert_eq!</span>(m.get(k).cloned(), map.get(k).cloned());
        ...
        <span class="attr">#[test]
            ...
            <span class="kw-2">ref </span>input <span class="kw">in </span>collection::hash_map(i16::ANY, i16::ANY, <span class="number">1</span>..<span class="number">100</span>),
            ...
            <span class="kw">let </span>map1: HashMap&lt;<span class="kw">_</span>, <span class="kw">_</span>&gt; = HashMap::from_iter(input.clone());
            <span class="kw">let </span>(val, map2) = map1.extract(<span class="kw-2">&amp;</span>index).unwrap();
        ...
        <span class="attr">#[test]
        </span><span class="kw">fn </span>proptest_works(<span class="kw-2">ref </span>m <span class="kw">in </span>proptest::hash_map(<span class="number">0</span>..<span class="number">9999</span>, <span class="string">".*"</span>, <span class="number">10</span>..<span class="number">100</span>)) {
        ...
        <span class="attr">#[test]
        </span><span class="kw">fn </span>exact_size_iterator(<span class="kw-2">ref </span>m <span class="kw">in </span>proptest::hash_map(i16::ANY, i16::ANY, <span class="number">0</span>..<span class="number">100</span>)) {
