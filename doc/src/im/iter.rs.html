...
</span><span class="doccomment">//! Iterators over immutable data.
...
</span><span class="kw">pub fn </span>unfold&lt;F, S, A&gt;(value: S, f: F) -&gt; <span class="kw">impl </span>Iterator&lt;Item = A&gt;
<span class="kw">where
    </span>F: Fn(S) -&gt; <span class="prelude-ty">Option</span>&lt;(A, S)&gt;,
{
    <span class="kw">let </span><span class="kw-2">mut </span>value = <span class="prelude-val">Some</span>(value);
    std::iter::from_fn(<span class="kw">move </span>|| {
        f(value.take().unwrap()).map(|(next, state)| {
            value = <span class="prelude-val">Some</span>(state);
            next
        })
    })
}
