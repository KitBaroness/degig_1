...
</pre></div><pre class="rust"><code><span class="kw">use </span>alloc::{sync::Arc, vec, vec::Vec};

<span class="kw">use crate</span>::{packed::pattern::Patterns, util::search::Match, PatternID};
...
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">struct </span>RabinKarp {
    <span class="doccomment">/// The patterns we're searching for.
...
<span class="kw">impl </span>RabinKarp {
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new(patterns: <span class="kw-2">&amp;</span>Arc&lt;Patterns&gt;) -&gt; RabinKarp {
    ...
    /// search at `at`.
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>find_at(
        ...
        <span class="kw">loop </span>{
            ...
            <span class="kw">for </span><span class="kw-2">&amp;</span>(phash, pid) <span class="kw">in </span>bucket {
                <span class="kw">if </span>phash == hash {
                    <span class="kw">if let </span><span class="prelude-val">Some</span>(c) = <span class="self">self</span>.verify(pid, haystack, at) {
    ...
    /// units of bytes.
        ...
        <span class="self">self</span>.buckets.len() * core::mem::size_of::&lt;Vec&lt;(Hash, PatternID)&gt;&gt;()
            + <span class="self">self</span>.patterns.len() * core::mem::size_of::&lt;(Hash, PatternID)&gt;()
    }
    ...
    /// if it does occur a lot, it's going to be slow no matter what we do.
        ...
        <span class="kw">let </span>pat = <span class="self">self</span>.patterns.get(id);
        <span class="kw">if </span>pat.is_prefix(<span class="kw-2">&amp;</span>haystack[at..]) {
            <span class="prelude-val">Some</span>(Match::new(id, at..at + pat.len()))
