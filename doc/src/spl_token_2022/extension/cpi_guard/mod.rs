...
<span class="doccomment">/// CPI Guard extension instructions
</span><span class="kw">pub mod </span>instruction;

<span class="doccomment">/// CPI Guard extension processor
</span><span class="kw">pub mod </span>processor;

<span class="doccomment">/// CPI Guard extension for Accounts
...
</span><span class="kw">pub struct </span>CpiGuard {
    <span class="doccomment">/// Lock privileged token operations from happening via CPI
    </span><span class="kw">pub </span>lock_cpi: PodBool,
}
<span class="kw">impl </span>Extension <span class="kw">for </span>CpiGuard {
    <span class="kw">const </span>TYPE: ExtensionType = ExtensionType::CpiGuard;
}

<span class="doccomment">/// Determine if CPI Guard is enabled for this account
</span><span class="kw">pub fn </span>cpi_guard_enabled(account_state: <span class="kw-2">&amp;</span>StateWithExtensionsMut&lt;Account&gt;) -&gt; bool {
    <span class="kw">if let </span><span class="prelude-val">Ok</span>(extension) = account_state.get_extension::&lt;CpiGuard&gt;() {
        <span class="kw">return </span>extension.lock_cpi.into();
    }
    <span class="bool-val">false
</span>}

<span class="doccomment">/// Determine if we are in CPI
</span><span class="kw">pub fn </span>in_cpi() -&gt; bool {
