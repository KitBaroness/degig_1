...
<span class="kw">use </span>{
    <span class="kw">crate</span>::extension::{BaseState, BaseStateWithExtensions, Extension, ExtensionType},
    bytemuck::{Pod, Zeroable},
    solana_program::pubkey::Pubkey,
    spl_pod::optional_keys::OptionalNonZeroPubkey,
};

<span class="doccomment">/// Permanent delegate extension data for mints.
...
</span><span class="kw">pub struct </span>PermanentDelegate {
    <span class="doccomment">/// Optional permanent delegate for transferring or burning tokens
    </span><span class="kw">pub </span>delegate: OptionalNonZeroPubkey,
}
<span class="kw">impl </span>Extension <span class="kw">for </span>PermanentDelegate {
    <span class="kw">const </span>TYPE: ExtensionType = ExtensionType::PermanentDelegate;
}

<span class="doccomment">/// Attempts to get the permanent delegate from the TLV data, returning None
/// if the extension is not found
</span><span class="kw">pub fn </span>get_permanent_delegate&lt;S: BaseState, BSE: BaseStateWithExtensions&lt;S&gt;&gt;(
    state: <span class="kw-2">&amp;</span>BSE,
) -&gt; <span class="prelude-ty">Option</span>&lt;Pubkey&gt; {
    state
        .get_extension::&lt;PermanentDelegate&gt;()
        .ok()
        .and_then(|e| <span class="prelude-ty">Option</span>::&lt;Pubkey&gt;::from(e.delegate))
}
