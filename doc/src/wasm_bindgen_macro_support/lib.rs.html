...
</pre></div><pre class="rust"><code><span class="doccomment">//! This crate contains the part of the implementation of the `#[wasm_bindgen]` optsibute that is
//! not in the shared backend crate.

</span><span class="attr">#![doc(html_root_url = <span class="string">"https://docs.rs/wasm-bindgen-macro-support/0.2"</span>)]
...
<span class="doccomment">/// Takes the parsed input from a `wasm_bindgen::link_to` macro and returns the generated link
</span><span class="kw">pub fn </span>expand_link_to(input: TokenStream) -&gt; <span class="prelude-ty">Result</span>&lt;TokenStream, Diagnostic&gt; {
    parser::reset_attrs_used();
    <span class="kw">let </span>opts = syn::parse2(input)<span class="question-mark">?</span>;

    <span class="kw">let </span><span class="kw-2">mut </span>tokens = proc_macro2::TokenStream::new();
    <span class="kw">let </span>link = parser::link_to(opts)<span class="question-mark">?</span>;
    link.try_to_tokens(<span class="kw-2">&amp;mut </span>tokens)<span class="question-mark">?</span>;
...
<span class="doccomment">/// Takes the parsed input from a `#[wasm_bindgen]` macro and returns the generated bindings
    ...
    parser::reset_attrs_used();
    <span class="kw">let </span><span class="kw-2">mut </span>item = syn::parse2::&lt;syn::ImplItemFn&gt;(input)<span class="question-mark">?</span>;
    <span class="kw">let </span>opts: ClassMarker = syn::parse2(attr)<span class="question-mark">?</span>;

    <span class="kw">let </span><span class="kw-2">mut </span>program = backend::ast::Program::default();
    item.macro_parse(<span class="kw-2">&amp;mut </span>program, <span class="kw-2">&amp;</span>opts)<span class="question-mark">?</span>;
    ...
    tokens.append_all(
        item.attrs
            .iter()
            .filter(|attr| <span class="macro">matches!</span>(attr.style, syn::AttrStyle::Outer)),
    );
    ...
    item.block.brace_token.surround(<span class="kw-2">&amp;mut </span>tokens, |tokens| {
        ...
        </span>tokens.append_all(
            item.attrs
                .iter()
                .filter(|attr| <span class="macro">matches!</span>(attr.style, syn::AttrStyle::Inner(<span class="kw">_</span>))),
        );
...
<span class="kw">impl </span>Parse <span class="kw">for </span>ClassMarker {
    <span class="kw">fn </span>parse(input: ParseStream) -&gt; SynResult&lt;<span class="self">Self</span>&gt; {
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>wasm_bindgen = <span class="prelude-val">None</span>;
        <span class="kw">let </span><span class="kw-2">mut </span>wasm_bindgen_futures = <span class="prelude-val">None</span>;

        <span class="kw">loop </span>{
            <span class="kw">if </span>input.parse::&lt;<span class="prelude-ty">Option</span>&lt;<span class="macro">Token!</span>[,]&gt;&gt;()<span class="question-mark">?</span>.is_some() {
                <span class="kw">let </span>ident = input.parse::&lt;syn::Ident&gt;()<span class="question-mark">?</span>;

                <span class="kw">if </span>ident == <span class="string">"wasm_bindgen" </span>{
                    <span class="kw">if </span>wasm_bindgen.is_some() {
                        <span class="kw">return </span><span class="prelude-val">Err</span>(syn::Error::new(
                            ident.span(),
                            <span class="string">"found duplicate `wasm_bindgen`"</span>,
                        ));
                    }

                    input.parse::&lt;<span class="macro">Token!</span>[=]&gt;()<span class="question-mark">?</span>;
                    wasm_bindgen = <span class="prelude-val">Some</span>(input.parse::&lt;syn::Path&gt;()<span class="question-mark">?</span>);
                } <span class="kw">else if </span>ident == <span class="string">"wasm_bindgen_futures" </span>{
                    <span class="kw">if </span>wasm_bindgen_futures.is_some() {
                        <span class="kw">return </span><span class="prelude-val">Err</span>(syn::Error::new(
                            ident.span(),
                            <span class="string">"found duplicate `wasm_bindgen_futures`"</span>,
                        ));
                    }

                    input.parse::&lt;<span class="macro">Token!</span>[=]&gt;()<span class="question-mark">?</span>;
                    wasm_bindgen_futures = <span class="prelude-val">Some</span>(input.parse::&lt;syn::Path&gt;()<span class="question-mark">?</span>);
                } <span class="kw">else </span>{
                    <span class="kw">return </span><span class="prelude-val">Err</span>(syn::Error::new(
                        ident.span(),
                        <span class="string">"expected `wasm_bindgen` or `wasm_bindgen_futures`"</span>,
                    ));
                }
            } <span class="kw">else </span>{
        ...
        <span class="prelude-val">Ok</span>(ClassMarker {
            class,
            js_class,
            wasm_bindgen: wasm_bindgen.unwrap_or_else(|| <span class="macro">syn::parse_quote!</span> { wasm_bindgen }),
            wasm_bindgen_futures: wasm_bindgen_futures
                .unwrap_or_else(|| <span class="macro">syn::parse_quote!</span> { wasm_bindgen_futures }),
        })
    }
}
