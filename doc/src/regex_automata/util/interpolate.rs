...
<span class="kw">use </span><span class="kw">crate</span>::util::memchr::memchr;

<span class="doccomment">/// Accepts a replacement string and interpolates capture references with their
...
/// ```
</span><span class="kw">pub fn </span>string(
    <span class="kw-2">mut </span>replacement: <span class="kw-2">&amp;</span>str,
    <span class="kw-2">mut </span>append: <span class="kw">impl </span>FnMut(usize, <span class="kw-2">&amp;mut </span>String),
    <span class="kw-2">mut </span>name_to_index: <span class="kw">impl </span>FnMut(<span class="kw-2">&amp;</span>str) -&gt; <span class="prelude-ty">Option</span>&lt;usize&gt;,
    dst: <span class="kw-2">&amp;mut </span>String,
) {
    <span class="kw">while </span>!replacement.is_empty() {
        <span class="kw">match </span>memchr(<span class="string">b'$'</span>, replacement.as_bytes()) {
        ...
        <span class="comment">// Handle escaping of '$'.
        </span><span class="kw">if </span>replacement.as_bytes().get(<span class="number">1</span>).map_or(<span class="bool-val">false</span>, |<span class="kw-2">&amp;</span>b| b == <span class="string">b'$'</span>) {
        ...
        <span class="kw">match </span>cap_ref.cap {
            ...
            Ref::Named(name) =&gt; {
                <span class="kw">if let </span><span class="prelude-val">Some</span>(i) = name_to_index(name) {
                    append(i, dst);
                }
            }
        }
...
<span class="doccomment">/// Accepts a replacement byte string and interpolates capture references with
...
/// ```
    ...
    <span class="kw-2">mut </span>append: <span class="kw">impl </span>FnMut(usize, <span class="kw-2">&amp;mut </span>Vec&lt;u8&gt;),
    <span class="kw-2">mut </span>name_to_index: <span class="kw">impl </span>FnMut(<span class="kw-2">&amp;</span>str) -&gt; <span class="prelude-ty">Option</span>&lt;usize&gt;,
    dst: <span class="kw-2">&amp;mut </span>Vec&lt;u8&gt;,
) {
    <span class="kw">while </span>!replacement.is_empty() {
        <span class="kw">match </span>memchr(<span class="string">b'$'</span>, replacement) {
            <span class="prelude-val">None </span>=&gt; <span class="kw">break</span>,
            <span class="prelude-val">Some</span>(i) =&gt; {
                dst.extend_from_slice(<span class="kw-2">&amp;</span>replacement[..i]);
                replacement = <span class="kw-2">&amp;</span>replacement[i..];
            }
        }
        <span class="comment">// Handle escaping of '$'.
        </span><span class="kw">if </span>replacement.get(<span class="number">1</span>).map_or(<span class="bool-val">false</span>, |<span class="kw-2">&amp;</span>b| b == <span class="string">b'$'</span>) {
...
/// being a valid reference, then it should be replaced with the empty string.
    ...
    // &amp;[u8].
    </span><span class="kw">let </span>cap = core::str::from_utf8(<span class="kw-2">&amp;</span>rep[i..cap_end])
        .expect(<span class="string">"valid UTF-8 capture name"</span>);
    <span class="prelude-val">Some</span>(CaptureRef {
        cap: <span class="kw">match </span>cap.parse::&lt;usize&gt;() {
            <span class="prelude-val">Ok</span>(i) =&gt; Ref::Number(i),
...
<span class="doccomment">/// Looks for a braced reference, e.g., `${foo1}`. This assumes that an opening
/// brace has been found at `i-1` in `rep`. This then looks for a closing
/// brace and returns the capture reference within the brace.
</span><span class="kw">fn </span>find_cap_ref_braced(rep: <span class="kw-2">&amp;</span>[u8], <span class="kw-2">mut </span>i: usize) -&gt; <span class="prelude-ty">Option</span>&lt;CaptureRef&lt;<span class="lifetime">'_</span>&gt;&gt; {
    <span class="macro">assert_eq!</span>(<span class="string">b'{'</span>, rep[i.checked_sub(<span class="number">1</span>).unwrap()]);
    ...
    // safely return None.
    </span><span class="kw">let </span>cap = <span class="kw">match </span>core::str::from_utf8(<span class="kw-2">&amp;</span>rep[start..i]) {
...
<span class="attr">#[cfg(test)]
    ...
    <span class="macro">find!</span>(find_cap_ref20, <span class="string">"${¾}"</span>, <span class="macro">c!</span>(<span class="string">"¾"</span>, <span class="number">5</span>));
    <span class="macro">find!</span>(find_cap_ref21, <span class="string">"${¾a}"</span>, <span class="macro">c!</span>(<span class="string">"¾a"</span>, <span class="number">6</span>));
    <span class="macro">find!</span>(find_cap_ref22, <span class="string">"${a¾}"</span>, <span class="macro">c!</span>(<span class="string">"a¾"</span>, <span class="number">6</span>));
    <span class="macro">find!</span>(find_cap_ref23, <span class="string">"${☃}"</span>, <span class="macro">c!</span>(<span class="string">"☃"</span>, <span class="number">6</span>));
    <span class="macro">find!</span>(find_cap_ref24, <span class="string">"${a☃}"</span>, <span class="macro">c!</span>(<span class="string">"a☃"</span>, <span class="number">7</span>));
    <span class="macro">find!</span>(find_cap_ref25, <span class="string">"${☃a}"</span>, <span class="macro">c!</span>(<span class="string">"☃a"</span>, <span class="number">7</span>));
    <span class="macro">find!</span>(find_cap_ref26, <span class="string">"${名字}"</span>, <span class="macro">c!</span>(<span class="string">"名字"</span>, <span class="number">9</span>));

    <span class="kw">fn </span>interpolate_string(
        <span class="kw-2">mut </span>name_to_index: Vec&lt;(<span class="kw-2">&amp;</span><span class="lifetime">'static </span>str, usize)&gt;,
        caps: Vec&lt;<span class="kw-2">&amp;</span><span class="lifetime">'static </span>str&gt;,
        replacement: <span class="kw-2">&amp;</span>str,
    ) -&gt; String {
        name_to_index.sort_by_key(|x| x.<span class="number">0</span>);

        <span class="kw">let </span><span class="kw-2">mut </span>dst = String::new();
        <span class="kw">super</span>::string(
            replacement,
            |i, dst| {
                <span class="kw">if let </span><span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span>s) = caps.get(i) {
                    dst.push_str(s);
                }
            },
            |name| -&gt; <span class="prelude-ty">Option</span>&lt;usize&gt; {
                name_to_index
                    .binary_search_by_key(<span class="kw-2">&amp;</span>name, |x| x.<span class="number">0</span>)
                    .ok()
                    .map(|i| name_to_index[i].<span class="number">1</span>)
            },
    ...
    <span class="kw">fn </span>interpolate_bytes(
        <span class="kw-2">mut </span>name_to_index: Vec&lt;(<span class="kw-2">&amp;</span><span class="lifetime">'static </span>str, usize)&gt;,
        caps: Vec&lt;<span class="kw-2">&amp;</span><span class="lifetime">'static </span>str&gt;,
        replacement: <span class="kw-2">&amp;</span>str,
    ) -&gt; String {
        ...
        <span class="kw">super</span>::bytes(
    ...
    <span class="macro">macro_rules!</span> interp {
        (<span class="macro-nonterminal">$name</span>:ident, <span class="macro-nonterminal">$map</span>:expr, <span class="macro-nonterminal">$caps</span>:expr, <span class="macro-nonterminal">$hay</span>:expr, <span class="macro-nonterminal">$expected</span>:expr $(,)<span class="kw-2">*</span>) =&gt; {
            <span class="attr">#[test]
            </span><span class="kw">fn </span><span class="macro-nonterminal">$name</span>() {
                <span class="macro">assert_eq!</span>(
                    <span class="macro-nonterminal">$expected</span>,
                    interpolate_string(<span class="macro-nonterminal">$map</span>, <span class="macro-nonterminal">$caps</span>, <span class="macro-nonterminal">$hay</span>),
                    <span class="string">"interpolate::string failed"</span>,
                );
                <span class="macro">assert_eq!</span>(
                    <span class="macro-nonterminal">$expected</span>,
                    interpolate_bytes(<span class="macro-nonterminal">$map</span>, <span class="macro-nonterminal">$caps</span>, <span class="macro-nonterminal">$hay</span>),
                    <span class="string">"interpolate::bytes failed"</span>,
                );
            }
        };
    }

    <span class="macro">interp!</span>(
        interp1,
        <span class="macro">vec!</span>[(<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">""</span>, <span class="string">"xxx"</span>],
        <span class="string">"test $foo test"</span>,
        <span class="string">"test xxx test"</span>,
    );
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[(<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">""</span>, <span class="string">"xxx"</span>],
        <span class="string">"test$footest"</span>,
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">""</span>, <span class="string">"xxx"</span>],
        <span class="string">"test${foo}test"</span>,
        <span class="string">"testxxxtest"</span>,
    );
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[(<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">""</span>, <span class="string">"xxx"</span>],
        <span class="string">"test$2test"</span>,
        <span class="string">"test"</span>,
    );
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[(<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">""</span>, <span class="string">"xxx"</span>],
        <span class="string">"test${2}test"</span>,
        <span class="string">"testxxxtest"</span>,
    );
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[(<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">""</span>, <span class="string">"xxx"</span>],
        <span class="string">"test $foo"</span>,
        <span class="string">"test xxx"</span>,
    );
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[(<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">""</span>, <span class="string">"xxx"</span>],
        <span class="string">"$foo test"</span>,
        <span class="string">"xxx test"</span>,
    );

    <span class="macro">interp!</span>(
        interp9,
        <span class="macro">vec!</span>[(<span class="string">"bar"</span>, <span class="number">1</span>), (<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">"yyy"</span>, <span class="string">"xxx"</span>],
        <span class="string">"test $bar$foo"</span>,
        <span class="string">"test yyyxxx"</span>,
    );
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[(<span class="string">"bar"</span>, <span class="number">1</span>), (<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">"yyy"</span>, <span class="string">"xxx"</span>],
        <span class="string">"test ${a b} test"</span>,
        <span class="string">"test  test"</span>,
    );
    ...
    <span class="macro">interp!</span>(
        ...
        <span class="macro">vec!</span>[(<span class="string">"bar"</span>, <span class="number">1</span>), (<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">"yyy"</span>, <span class="string">"xxx"</span>],
        <span class="string">"test ${a} test"</span>,
        <span class="string">"test  test"</span>,
    );

    <span class="comment">// This is a funny case where a braced reference is never closed, but
    ...
    </span><span class="macro">interp!</span>(
        interp15,
        <span class="macro">vec!</span>[(<span class="string">"bar"</span>, <span class="number">1</span>), (<span class="string">"foo"</span>, <span class="number">2</span>)],
        <span class="macro">vec!</span>[<span class="string">""</span>, <span class="string">"yyy"</span>, <span class="string">"xxx"</span>],
        <span class="string">"test ${wat $bar ok"</span>,
        <span class="string">"test ${wat yyy ok"</span>,
    );
}
