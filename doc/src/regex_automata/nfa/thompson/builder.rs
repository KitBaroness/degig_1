...
<span class="kw">use crate</span>::{
    nfa::thompson::{
        error::BuildError,
        nfa::{<span class="self">self</span>, SparseTransitions, Transition, NFA},
    },
    util::{
...
<span class="doccomment">/// An intermediate NFA state used during construction.
///
...
/// of states are completely removed (such as [`State::Empty`]).
    ...
    <span class="doccomment">/// An empty state whose only purpose is to forward the automaton to
    ...
    /// accordingly.
        ...
        <span class="doccomment">/// The next state that this state should transition to.
        </span>next: StateID,
    },
    <span class="doccomment">/// A state that only transitions to another state if the current input
    /// byte is in a particular range of bytes.
    </span>ByteRange { trans: Transition },
    ...
    </span>Sparse { transitions: Vec&lt;Transition&gt; },
    <span class="doccomment">/// A conditional epsilon transition satisfied via some sort of
    /// look-around.
    </span>Look { look: Look, next: StateID },
    <span class="doccomment">/// An empty state that records the start of a capture location. This is an
    ...
    /// search.
        ...
        <span class="doccomment">/// The ID of the pattern that this capture was defined.
        </span>pattern_id: PatternID,
        <span class="doccomment">/// The capture group index that this capture state corresponds to.
    ...
    <span class="doccomment">/// An empty state that records the end of a capture location. This is an
    /// unconditional epsilon transition like `Empty`, except it can be used to
    /// record position information for a capture group when using the NFA for
    /// search.
    </span>CaptureEnd {
    ...
    </span>Union { alternates: Vec&lt;StateID&gt; },
    <span class="doccomment">/// An alternation such that there exists an epsilon transition to all
    ...
    </span>UnionReverse { alternates: Vec&lt;StateID&gt; },
    ...
    <span class="doccomment">/// A match state. There is at most one such occurrence of this state in
    ...
    </span>Match { pattern_id: PatternID },
}

<span class="kw">impl </span>State {
    <span class="doccomment">/// If this state is an unconditional epsilon transition, then this returns
    /// the target of the transition.
    </span><span class="kw">fn </span>goto(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;StateID&gt; {
        <span class="kw">match </span><span class="kw-2">*</span><span class="self">self </span>{
            State::Empty { next } =&gt; <span class="prelude-val">Some</span>(next),
            State::Union { <span class="kw-2">ref </span>alternates } <span class="kw">if </span>alternates.len() == <span class="number">1 </span>=&gt; {
                <span class="prelude-val">Some</span>(alternates[<span class="number">0</span>])
            }
            State::UnionReverse { <span class="kw-2">ref </span>alternates }
                <span class="kw">if </span>alternates.len() == <span class="number">1 </span>=&gt;
            {
                <span class="prelude-val">Some</span>(alternates[<span class="number">0</span>])
            }
    ...
    <span class="doccomment">/// Returns the heap memory usage, in bytes, of this state.
        ...
        <span class="kw">match </span><span class="kw-2">*</span><span class="self">self </span>{
            ...
            | State::CaptureEnd { .. }
            | State::Fail
            | State::Match { .. } =&gt; <span class="number">0</span>,
            State::Sparse { <span class="kw-2">ref </span>transitions } =&gt; {
...
<span class="doccomment">/// An abstraction for building Thompson NFAs by hand.
///
...
/// ```
    ...
    <span class="doccomment">/// The ID of the pattern that we're currently building.
    ///
    /// Callers are required to set (and unset) this by calling
    /// {start,finish}_pattern. Otherwise, most methods will panic.
    </span>pattern_id: <span class="prelude-ty">Option</span>&lt;PatternID&gt;,
    <span class="doccomment">/// A sequence of intermediate NFA states. Once a state is added to this
    ...
    <span class="doccomment">/// A map from pattern ID to capture group index to name. (If no name
    ...
    /// always None.
    </span>captures: Vec&lt;Vec&lt;<span class="prelude-ty">Option</span>&lt;Arc&lt;str&gt;&gt;&gt;&gt;,
    <span class="doccomment">/// The combined memory used by each of the 'State's in 'states'. This
    ...
    </span>memory_states: usize,
    <span class="doccomment">/// Whether this NFA only matches UTF-8 and whether regex engines using
    /// this NFA for searching should report empty matches that split a
    /// codepoint.
    </span>utf8: bool,
    <span class="doccomment">/// Whether this NFA should be matched in reverse or not.
    </span>reverse: bool,
    <span class="doccomment">/// The matcher to use for look-around assertions.
    </span>look_matcher: LookMatcher,
    <span class="doccomment">/// A size limit to respect when building an NFA. If the total heap memory
    /// of the intermediate NFA states exceeds (or would exceed) this amount,
    /// then an error is returned.
    </span>size_limit: <span class="prelude-ty">Option</span>&lt;usize&gt;,
}

<span class="kw">impl </span>Builder {
    <span class="doccomment">/// Create a new builder for hand-assembling NFAs.
    </span><span class="kw">pub fn </span>new() -&gt; Builder {
        Builder::default()
    }

    <span class="doccomment">/// Clear this builder.
    ///
    ...
    /// assemble an entirely new NFA.
        ...
        <span class="self">self</span>.pattern_id = <span class="prelude-val">None</span>;
        <span class="self">self</span>.states.clear();
        <span class="self">self</span>.start_pattern.clear();
        <span class="self">self</span>.captures.clear();
        <span class="self">self</span>.memory_states = <span class="number">0</span>;
    }

    <span class="doccomment">/// Assemble a [`NFA`] from the states added so far.
    ///
    ...
    /// state.
        ...
        <span class="macro">assert!</span>(<span class="self">self</span>.pattern_id.is_none(), <span class="string">"must call 'finish_pattern' first"</span>);
        <span class="macro">debug!</span>(
            <span class="string">"intermediate NFA compilation via builder is complete, \
             intermediate NFA size: {} states, {} bytes on heap"</span>,
            <span class="self">self</span>.states.len(),
            <span class="self">self</span>.memory_usage(),
        );

        <span class="kw">let </span><span class="kw-2">mut </span>nfa = nfa::Inner::default();
        nfa.set_utf8(<span class="self">self</span>.utf8);
        nfa.set_reverse(<span class="self">self</span>.reverse);
        nfa.set_look_matcher(<span class="self">self</span>.look_matcher.clone());
        <span class="comment">// A set of compiler internal state IDs that correspond to states
        ...
        </span><span class="kw">let </span><span class="kw-2">mut </span>empties = <span class="macro">vec!</span>[];
        <span class="comment">// A map used to re-map state IDs when translating this builder's
        // internal NFA state representation to the final NFA representation.
        </span><span class="kw">let </span><span class="kw-2">mut </span>remap = <span class="macro">vec!</span>[];
        remap.resize(<span class="self">self</span>.states.len(), StateID::ZERO);

        nfa.set_starts(start_anchored, start_unanchored, <span class="kw-2">&amp;</span><span class="self">self</span>.start_pattern);
        nfa.set_captures(<span class="kw-2">&amp;</span><span class="self">self</span>.captures).map_err(BuildError::captures)<span class="question-mark">?</span>;
        <span class="comment">// The idea here is to convert our intermediate states to their final
        ...
        </span><span class="kw">for </span>(sid, state) <span class="kw">in </span><span class="self">self</span>.states.iter().with_state_ids() {
            <span class="kw">match </span><span class="kw-2">*</span>state {
                State::Empty { next } =&gt; {
                    <span class="comment">// Since we're removing empty states, we need to handle
                    // them later since we don't yet know which new state this
                    // empty state will be mapped to.
                    </span>empties.push((sid, next));
                }
                State::ByteRange { trans } =&gt; {
                    remap[sid] = nfa.add(nfa::State::ByteRange { trans });
                }
                State::Sparse { <span class="kw-2">ref </span>transitions } =&gt; {
                    remap[sid] = <span class="kw">match </span>transitions.len() {
                        <span class="number">0 </span>=&gt; nfa.add(nfa::State::Fail),
                        <span class="number">1 </span>=&gt; nfa.add(nfa::State::ByteRange {
                            trans: transitions[<span class="number">0</span>],
                        }),
                        <span class="kw">_ </span>=&gt; {
                            <span class="kw">let </span>transitions =
                                transitions.to_vec().into_boxed_slice();
                            <span class="kw">let </span>sparse = SparseTransitions { transitions };
                ...
                State::CaptureStart { pattern_id, group_index, next } =&gt; {
                    <span class="comment">// We can't remove this empty state because of the side
                    // effect of capturing an offset for this capture slot.
                    </span><span class="kw">let </span>slot = nfa
                        .group_info()
                        .slot(pattern_id, group_index.as_usize())
                        .expect(<span class="string">"invalid capture index"</span>);
                    <span class="kw">let </span>slot =
                        SmallIndex::new(slot).expect(<span class="string">"a small enough slot"</span>);
                ...
                State::Union { <span class="kw-2">ref </span>alternates } =&gt; {
                    <span class="kw">if </span>alternates.is_empty() {
                        remap[sid] = nfa.add(nfa::State::Fail);
                    } <span class="kw">else if </span>alternates.len() == <span class="number">1 </span>{
                        empties.push((sid, alternates[<span class="number">0</span>]));
                        remap[sid] = alternates[<span class="number">0</span>];
                    } <span class="kw">else if </span>alternates.len() == <span class="number">2 </span>{
                        remap[sid] = nfa.add(nfa::State::BinaryUnion {
                            alt1: alternates[<span class="number">0</span>],
                            alt2: alternates[<span class="number">1</span>],
                        });
                    } <span class="kw">else </span>{
                        <span class="kw">let </span>alternates =
                ...
                State::UnionReverse { <span class="kw-2">ref </span>alternates } =&gt; {
                    <span class="kw">if </span>alternates.is_empty() {
                        ...
                        remap[sid] = nfa.add(nfa::State::BinaryUnion {
                            alt1: alternates[<span class="number">1</span>],
                            alt2: alternates[<span class="number">0</span>],
                        });
                    } <span class="kw">else </span>{
                        <span class="kw">let </span><span class="kw-2">mut </span>alternates =
                            alternates.to_vec().into_boxed_slice();
                        alternates.reverse();
                        remap[sid] = nfa.add(nfa::State::Union { alternates });
                    }
        ...
        <span class="comment">// Some of the new states still point to empty state IDs, so we need to
        ...
        </span><span class="kw">let </span><span class="kw-2">mut </span>remapped = <span class="macro">vec!</span>[<span class="bool-val">false</span>; <span class="self">self</span>.states.len()];
        <span class="kw">for </span><span class="kw-2">&amp;</span>(empty_id, empty_next) <span class="kw">in </span>empties.iter() {
            <span class="kw">if </span>remapped[empty_id] {
                <span class="kw">continue</span>;
            }
            <span class="comment">// empty states can point to other empty states, forming a chain.
            ...
            </span><span class="kw">let </span><span class="kw-2">mut </span>new_next = empty_next;
            <span class="kw">while let </span><span class="prelude-val">Some</span>(next) = <span class="self">self</span>.states[new_next].goto() {
                new_next = next;
            }
            remap[empty_id] = remap[new_next];
            remapped[empty_id] = <span class="bool-val">true</span>;

            <span class="comment">// Now that we've remapped the main 'empty_id' above, we re-follow
            ...
            </span><span class="kw">let </span><span class="kw-2">mut </span>next2 = empty_next;
            <span class="kw">while let </span><span class="prelude-val">Some</span>(next) = <span class="self">self</span>.states[next2].goto() {
                remap[next2] = remap[new_next];
                remapped[next2] = <span class="bool-val">true</span>;
                next2 = next;
            }
        }
        <span class="comment">// Finally remap all of the state IDs.
        </span>nfa.remap(<span class="kw-2">&amp;</span>remap);
        <span class="kw">let </span>final_nfa = nfa.into_nfa();
        <span class="macro">debug!</span>(
            <span class="string">"NFA compilation via builder complete, \
        ...
        <span class="prelude-val">Ok</span>(final_nfa)
    }

    <span class="doccomment">/// Start the assembly of a pattern in this NFA.
    ///
    ...
    </span><span class="kw">pub fn </span>start_pattern(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;PatternID, BuildError&gt; {
        <span class="macro">assert!</span>(<span class="self">self</span>.pattern_id.is_none(), <span class="string">"must call 'finish_pattern' first"</span>);

        <span class="kw">let </span>proposed = <span class="self">self</span>.start_pattern.len();
        <span class="kw">let </span>pid = PatternID::new(proposed)
            .map_err(|<span class="kw">_</span>| BuildError::too_many_patterns(proposed))<span class="question-mark">?</span>;
        <span class="self">self</span>.pattern_id = <span class="prelude-val">Some</span>(pid);
        <span class="comment">// This gets filled in when 'finish_pattern' is called.
        </span><span class="self">self</span>.start_pattern.push(StateID::ZERO);
        <span class="prelude-val">Ok</span>(pid)
    }

    <span class="doccomment">/// Finish the assembly of a pattern in this NFA.
    ///
    /// Upon success, this returns the identifier for the new pattern.
    ...
    /// this panics.
    </span><span class="kw">pub fn </span>finish_pattern(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        start_id: StateID,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;PatternID, BuildError&gt; {
        <span class="kw">let </span>pid = <span class="self">self</span>.current_pattern_id();
        <span class="self">self</span>.start_pattern[pid] = start_id;
        <span class="self">self</span>.pattern_id = <span class="prelude-val">None</span>;
        <span class="prelude-val">Ok</span>(pid)
    }

    <span class="doccomment">/// Returns the pattern identifier of the current pattern.
    ///
    /// # Panics
    ///
    /// If this doesn't occur after a `start_pattern` call and before the
    /// corresponding `finish_pattern` call, then this panics.
    </span><span class="kw">pub fn </span>current_pattern_id(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; PatternID {
        <span class="self">self</span>.pattern_id.expect(<span class="string">"must call 'start_pattern' first"</span>)
    }

    <span class="doccomment">/// Returns the number of patterns added to this builder so far.
    ///
    ...
    /// for them.
        ...
        <span class="self">self</span>.start_pattern.len()
    }

    <span class="doccomment">/// Add an "empty" NFA state.
    ///
    ...
    </span><span class="kw">pub fn </span>add_empty(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="self">self</span>.add(State::Empty { next: StateID::ZERO })
    }

    <span class="doccomment">/// Add a "union" NFA state.
    ///
    ...
    /// This returns an error if the state identifier space is exhausted, or if
    /// the configured heap size limit has been exceeded.
    </span><span class="kw">pub fn </span>add_union(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        alternates: Vec&lt;StateID&gt;,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="self">self</span>.add(State::Union { alternates })
    }

    <span class="doccomment">/// Add a "reverse union" NFA state.
    ///
    ...
    /// This returns an error if the state identifier space is exhausted, or if
    /// the configured heap size limit has been exceeded.
    </span><span class="kw">pub fn </span>add_union_reverse(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        alternates: Vec&lt;StateID&gt;,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="self">self</span>.add(State::UnionReverse { alternates })
    }

    <span class="doccomment">/// Add a "range" NFA state.
    ///
    ...
    /// This returns an error if the state identifier space is exhausted, or if
    /// the configured heap size limit has been exceeded.
    </span><span class="kw">pub fn </span>add_range(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        trans: Transition,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="self">self</span>.add(State::ByteRange { trans })
    }

    <span class="doccomment">/// Add a "sparse" NFA state.
    ///
    ...
    /// in ascending order.
    </span><span class="kw">pub fn </span>add_sparse(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        transitions: Vec&lt;Transition&gt;,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="self">self</span>.add(State::Sparse { transitions })
    }

    <span class="doccomment">/// Add a "look" NFA state.
    ///
    ...
    /// This returns an error if the state identifier space is exhausted, or if
    /// the configured heap size limit has been exceeded.
    </span><span class="kw">pub fn </span>add_look(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        next: StateID,
        look: Look,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="self">self</span>.add(State::Look { look, next })
    }

    <span class="doccomment">/// Add a "start capture" NFA state.
    ///
    ...
    /// # Ok::&lt;(), Box&lt;dyn std::error::Error&gt;&gt;(())
    /// ```
    </span><span class="kw">pub fn </span>add_capture_start(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        next: StateID,
        group_index: u32,
        name: <span class="prelude-ty">Option</span>&lt;Arc&lt;str&gt;&gt;,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="kw">let </span>pid = <span class="self">self</span>.current_pattern_id();
        <span class="kw">let </span>group_index = <span class="kw">match </span>SmallIndex::try_from(group_index) {
            <span class="prelude-val">Err</span>(<span class="kw">_</span>) =&gt; {
                <span class="kw">return </span><span class="prelude-val">Err</span>(BuildError::invalid_capture_index(group_index))
            }
            <span class="prelude-val">Ok</span>(group_index) =&gt; group_index,
        };
        <span class="comment">// Make sure we have space to insert our (pid,index)|--&gt;name mapping.
        </span><span class="kw">if </span>pid.as_usize() &gt;= <span class="self">self</span>.captures.len() {
            <span class="kw">for _ in </span><span class="number">0</span>..=(pid.as_usize() - <span class="self">self</span>.captures.len()) {
                <span class="self">self</span>.captures.push(<span class="macro">vec!</span>[]);
            }
        }
        <span class="comment">// In the case where 'group_index &lt; self.captures[pid].len()', it means
        ...
        </span><span class="kw">if </span>group_index.as_usize() &gt;= <span class="self">self</span>.captures[pid].len() {
            <span class="comment">// For discontiguous indices, push placeholders for earlier capture
            // groups that weren't explicitly added.
            </span><span class="kw">for _ in </span><span class="number">0</span>..(group_index.as_usize() - <span class="self">self</span>.captures[pid].len()) {
                <span class="self">self</span>.captures[pid].push(<span class="prelude-val">None</span>);
            }
            <span class="self">self</span>.captures[pid].push(name);
        }
        <span class="self">self</span>.add(State::CaptureStart { pattern_id: pid, group_index, next })
    }

    <span class="doccomment">/// Add a "end capture" NFA state.
    ///
    ...
    /// See the [`GroupInfo`](crate::util::captures::GroupInfo) type for
    /// more information on what qualifies as valid capturing groups.
    </span><span class="kw">pub fn </span>add_capture_end(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        next: StateID,
        group_index: u32,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        ...
        <span class="self">self</span>.add(State::CaptureEnd { pattern_id: pid, group_index, next })
    }

    <span class="doccomment">/// Adds a "fail" NFA state.
    ///
    ...
    /// This returns an error if the state identifier space is exhausted, or if
    /// the configured heap size limit has been exceeded.
    </span><span class="kw">pub fn </span>add_fail(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="self">self</span>.add(State::Fail)
    }

    <span class="doccomment">/// Adds a "match" NFA state.
    ///
    ...
    </span><span class="kw">pub fn </span>add_match(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="kw">let </span>pattern_id = <span class="self">self</span>.current_pattern_id();
        <span class="kw">let </span>sid = <span class="self">self</span>.add(State::Match { pattern_id })<span class="question-mark">?</span>;
        <span class="prelude-val">Ok</span>(sid)
    }

    <span class="doccomment">/// The common implementation of "add a state." It handles the common
    /// error cases of state ID exhausting (by owning state ID allocation) and
    /// whether the size limit has been exceeded.
    </span><span class="kw">fn </span>add(<span class="kw-2">&amp;mut </span><span class="self">self</span>, state: State) -&gt; <span class="prelude-ty">Result</span>&lt;StateID, BuildError&gt; {
        <span class="kw">let </span>id = StateID::new(<span class="self">self</span>.states.len())
            .map_err(|<span class="kw">_</span>| BuildError::too_many_states(<span class="self">self</span>.states.len()))<span class="question-mark">?</span>;
        <span class="self">self</span>.memory_states += state.memory_usage();
        <span class="self">self</span>.states.push(state);
        <span class="self">self</span>.check_size_limit()<span class="question-mark">?</span>;
        <span class="prelude-val">Ok</span>(id)
    }

    <span class="doccomment">/// Add a transition from one state to another.
    ///
    ...
    /// will likely require a new API.)
        ...
        <span class="kw">let </span>old_memory_states = <span class="self">self</span>.memory_states;
        <span class="kw">match </span><span class="self">self</span>.states[from] {
            State::Empty { <span class="kw-2">ref mut </span>next } =&gt; {
                <span class="kw-2">*</span>next = to;
            }
            State::ByteRange { <span class="kw-2">ref mut </span>trans } =&gt; {
                trans.next = to;
            }
            State::Sparse { .. } =&gt; {
                <span class="macro">panic!</span>(<span class="string">"cannot patch from a sparse NFA state"</span>)
            }
            State::Look { <span class="kw-2">ref mut </span>next, .. } =&gt; {
                <span class="kw-2">*</span>next = to;
            }
            State::Union { <span class="kw-2">ref mut </span>alternates } =&gt; {
                alternates.push(to);
                <span class="self">self</span>.memory_states += mem::size_of::&lt;StateID&gt;();
            }
            State::UnionReverse { <span class="kw-2">ref mut </span>alternates } =&gt; {
                alternates.push(to);
                <span class="self">self</span>.memory_states += mem::size_of::&lt;StateID&gt;();
            }
            State::CaptureStart { <span class="kw-2">ref mut </span>next, .. } =&gt; {
                <span class="kw-2">*</span>next = to;
            }
            State::CaptureEnd { <span class="kw-2">ref mut </span>next, .. } =&gt; {
                <span class="kw-2">*</span>next = to;
            }
            State::Fail =&gt; {}
            State::Match { .. } =&gt; {}
        }
        <span class="kw">if </span>old_memory_states != <span class="self">self</span>.memory_states {
            <span class="self">self</span>.check_size_limit()<span class="question-mark">?</span>;
        }
        <span class="prelude-val">Ok</span>(())
    }

    <span class="doccomment">/// Set whether the NFA produced by this builder should only match UTF-8.
    ///
    ...
    </span><span class="kw">pub fn </span>set_utf8(<span class="kw-2">&amp;mut </span><span class="self">self</span>, yes: bool) {
        <span class="self">self</span>.utf8 = yes;
    }

    <span class="doccomment">/// Returns whether UTF-8 mode is enabled for this builder.
    ///
    ...
    <span class="doccomment">/// Sets whether the NFA produced by this builder should be matched in
    ...
    </span><span class="kw">pub fn </span>set_reverse(<span class="kw-2">&amp;mut </span><span class="self">self</span>, yes: bool) {
        <span class="self">self</span>.reverse = yes;
    }

    <span class="doccomment">/// Returns whether reverse mode is enabled for this builder.
    ///
    ...
    /// is.
        ...
        <span class="self">self</span>.reverse
    }

    <span class="doccomment">/// Sets the look-around matcher that should be used for the resulting NFA.
    ///
    ...
    /// `(?m:$)` assertions.
    </span><span class="kw">pub fn </span>set_look_matcher(<span class="kw-2">&amp;mut </span><span class="self">self</span>, m: LookMatcher) {
        <span class="self">self</span>.look_matcher = m;
    }

    <span class="doccomment">/// Returns the look-around matcher used for this builder.
    ///
    /// If a matcher was not explicitly set, then `LookMatcher::default()` is
    /// returned.
    </span><span class="kw">pub fn </span>get_look_matcher(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>LookMatcher {
        <span class="kw-2">&amp;</span><span class="self">self</span>.look_matcher
    }

    <span class="doccomment">/// Set the size limit on this builder.
    ///
    ...
    </span><span class="kw">pub fn </span>set_size_limit(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        limit: <span class="prelude-ty">Option</span>&lt;usize&gt;,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;(), BuildError&gt; {
        <span class="self">self</span>.size_limit = limit;
        <span class="self">self</span>.check_size_limit()
    }

    <span class="doccomment">/// Return the currently configured size limit.
    ///
    ...
    /// size limit.
        ...
        <span class="self">self</span>.size_limit
    }

    <span class="doccomment">/// Returns the heap memory usage, in bytes, used by the NFA states added
    /// so far.
    ///
    ...
    /// `Box&lt;[StateID]&gt;` instead of `Vec&lt;StateID&gt;`.)
        ...
        <span class="self">self</span>.states.len() * mem::size_of::&lt;State&gt;() + <span class="self">self</span>.memory_states
    }

    <span class="kw">fn </span>check_size_limit(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;(), BuildError&gt; {
        <span class="kw">if let </span><span class="prelude-val">Some</span>(limit) = <span class="self">self</span>.size_limit {
            <span class="kw">if </span><span class="self">self</span>.memory_usage() &gt; limit {
                <span class="kw">return </span><span class="prelude-val">Err</span>(BuildError::exceeded_size_limit(limit));
            }
        }
        <span class="prelude-val">Ok</span>(())
    }
...
<span class="attr">#[cfg(test)]
    ...
    <span class="comment">// This asserts that a builder state doesn't have its size changed. It is
    ...
    // is built.
        ...
        </span><span class="macro">assert_eq!</span>(<span class="number">32</span>, core::mem::size_of::&lt;State&gt;());
        <span class="attr">#[cfg(target_pointer_width = <span class="string">"32"</span>)]
        </span><span class="macro">assert_eq!</span>(<span class="number">16</span>, core::mem::size_of::&lt;State&gt;());
    }
}
