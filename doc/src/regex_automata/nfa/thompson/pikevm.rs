...
</span><span class="attr">#[cfg(feature = <span class="string">"internal-instrument-pikevm"</span>)]
...
<span class="doccomment">/// A simple macro for conditionally executing instrumentation logic when
...
/// to track.
</span><span class="macro">macro_rules!</span> instrument {
    (<span class="macro-nonterminal">$fun</span>:expr) =&gt; {
        <span class="attr">#[cfg(feature = <span class="string">"internal-instrument-pikevm"</span>)]
        </span>{
            <span class="kw">let </span>fun: <span class="kw-2">&amp;mut </span><span class="kw">dyn </span>FnMut(<span class="kw-2">&amp;mut </span>Counters) = <span class="kw-2">&amp;mut </span><span class="macro-nonterminal">$fun</span>;
            COUNTERS.with(|c: <span class="kw-2">&amp;</span>RefCell&lt;Counters&gt;| fun(<span class="kw-2">&amp;mut *</span>c.borrow_mut()));
        }
    };
}
...
<span class="attr">#[cfg(feature = <span class="string">"internal-instrument-pikevm"</span>)]
    ...
    <span class="doccomment">/// Effectively global state used to keep track of instrumentation
    ...
    </span><span class="kw">static </span>COUNTERS: RefCell&lt;Counters&gt; = RefCell::new(Counters::empty());
}

<span class="doccomment">/// The configuration used for building a [`PikeVM`].
///
...
<span class="kw">impl </span>Config {
    <span class="doccomment">/// Return a new default PikeVM configuration.
...
<span class="doccomment">/// A builder for a `PikeVM`.
///
...
<span class="kw">impl </span>Builder {
    <span class="doccomment">/// Create a new PikeVM builder with its default configuration.
    ...
    <span class="doccomment">/// Build a `PikeVM` from the given pattern.
    ///
    /// If there was a problem parsing or compiling the pattern, then an error
    /// is returned.
    </span><span class="attr">#[cfg(feature = <span class="string">"syntax"</span>)]
    </span><span class="kw">pub fn </span>build(<span class="kw-2">&amp;</span><span class="self">self</span>, pattern: <span class="kw-2">&amp;</span>str) -&gt; <span class="prelude-ty">Result</span>&lt;PikeVM, BuildError&gt; {
        <span class="self">self</span>.build_many(<span class="kw-2">&amp;</span>[pattern])
    }

    <span class="doccomment">/// Build a `PikeVM` from the given patterns.
    </span><span class="attr">#[cfg(feature = <span class="string">"syntax"</span>)]
    </span><span class="kw">pub fn </span>build_many&lt;P: AsRef&lt;str&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        patterns: <span class="kw-2">&amp;</span>[P],
    ) -&gt; <span class="prelude-ty">Result</span>&lt;PikeVM, BuildError&gt; {
    ...
    <span class="doccomment">/// Build a `PikeVM` directly from its NFA.
    ///
    ...
    </span><span class="kw">pub fn </span>build_from_nfa(<span class="kw-2">&amp;</span><span class="self">self</span>, nfa: NFA) -&gt; <span class="prelude-ty">Result</span>&lt;PikeVM, BuildError&gt; {
        nfa.look_set_any().available().map_err(BuildError::word)<span class="question-mark">?</span>;
        <span class="prelude-val">Ok</span>(PikeVM { config: <span class="self">self</span>.config.clone(), nfa })
    }

    <span class="doccomment">/// Apply the given `PikeVM` configuration options to this builder.
...
<span class="doccomment">/// A virtual machine for executing regex searches with capturing groups.
///
/// # Infallible APIs
///
...
</span><span class="kw">pub struct </span>PikeVM {
    config: Config,
    nfa: NFA,
}

<span class="kw">impl </span>PikeVM {
    ...
    </span><span class="kw">pub fn </span>new(pattern: <span class="kw-2">&amp;</span>str) -&gt; <span class="prelude-ty">Result</span>&lt;PikeVM, BuildError&gt; {
    ...
    <span class="doccomment">/// Like `new`, but builds a PikeVM directly from an NFA. This is useful
    ...
    /// # Ok::&lt;(), Box&lt;dyn std::error::Error&gt;&gt;(())
    /// ```
    </span><span class="kw">pub fn </span>new_from_nfa(nfa: NFA) -&gt; <span class="prelude-ty">Result</span>&lt;PikeVM, BuildError&gt; {
        PikeVM::builder().build_from_nfa(nfa)
    }

    <span class="doccomment">/// Create a new `PikeVM` that matches every input.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// # Ok::&lt;(), Box&lt;dyn std::error::Error&gt;&gt;(())
    /// ```
    </span><span class="kw">pub fn </span>always_match() -&gt; <span class="prelude-ty">Result</span>&lt;PikeVM, BuildError&gt; {
        <span class="kw">let </span>nfa = thompson::NFA::always_match();
        PikeVM::new_from_nfa(nfa)
    }

    <span class="doccomment">/// Create a new `PikeVM` that never matches any input.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// # Ok::&lt;(), Box&lt;dyn std::error::Error&gt;&gt;(())
    /// ```
    </span><span class="kw">pub fn </span>never_match() -&gt; <span class="prelude-ty">Result</span>&lt;PikeVM, BuildError&gt; {
        <span class="kw">let </span>nfa = thompson::NFA::never_match();
        PikeVM::new_from_nfa(nfa)
    }

    <span class="doccomment">/// Return a default configuration for a `PikeVM`.
    ///
    ...
    <span class="doccomment">/// Return a builder for configuring the construction of a `PikeVM`.
    ///
    ...
    <span class="doccomment">/// Create a new cache for this `PikeVM`.
    ///
    ...
    <span class="doccomment">/// Returns the total number of patterns compiled into this `PikeVM`.
    ///
    ...
    <span class="doccomment">/// Return the config for this `PikeVM`.
...
<span class="kw">impl </span>PikeVM {
    <span class="doccomment">/// Returns true if and only if this `PikeVM` matches the given haystack.
    ///
    ...
    /// ```
        ...
        <span class="self">self</span>.search_slots(cache, <span class="kw-2">&amp;</span>input, <span class="kw-2">&amp;mut </span>[]).is_some()
    }
    ...
    /// ```
        ...
        <span class="kw">if </span><span class="self">self</span>.get_nfa().pattern_len() == <span class="number">1 </span>{
            ...
            <span class="kw">let </span>pid = <span class="self">self</span>.search_slots(cache, <span class="kw-2">&amp;</span>input, <span class="kw-2">&amp;mut </span>slots)<span class="question-mark">?</span>;
            <span class="kw">let </span>start = slots[<span class="number">0</span>]<span class="question-mark">?</span>.get();
            <span class="kw">let </span>end = slots[<span class="number">1</span>]<span class="question-mark">?</span>.get();
        ...
        <span class="kw">let </span>pid = <span class="self">self</span>.search_slots(cache, <span class="kw-2">&amp;</span>input, <span class="kw-2">&amp;mut </span>slots)<span class="question-mark">?</span>;
        <span class="kw">let </span>start = slots[pid.as_usize() * <span class="number">2</span>]<span class="question-mark">?</span>.get();
        <span class="kw">let </span>end = slots[pid.as_usize() * <span class="number">2 </span>+ <span class="number">1</span>]<span class="question-mark">?</span>.get();
    ...
    /// ```
        ...
        <span class="self">self</span>.search(cache, <span class="kw-2">&amp;</span>input.into(), caps)
    }
    ...
    /// ```
    ...
    </span><span class="kw">pub fn </span>find_iter&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>, I: Into&lt;Input&lt;<span class="lifetime">'h</span>&gt;&gt;&gt;(
        ...
        input: I,
    ) -&gt; FindMatches&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>&gt; {
        ...
        FindMatches { re: <span class="self">self</span>, cache, caps, it }
    }
    ...
    </span><span class="kw">pub fn </span>captures_iter&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>, I: Into&lt;Input&lt;<span class="lifetime">'h</span>&gt;&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="lifetime">'r </span><span class="self">self</span>,
        cache: <span class="kw-2">&amp;</span><span class="lifetime">'c </span><span class="kw-2">mut </span>Cache,
        input: I,
    ) -&gt; CapturesMatches&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>&gt; {
...
<span class="kw">impl </span>PikeVM {
    ...
    /// ```
    ...
    </span><span class="kw">pub fn </span>search(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        cache: <span class="kw-2">&amp;mut </span>Cache,
        ...
        <span class="kw">let </span>pid = <span class="self">self</span>.search_slots(cache, input, caps.slots_mut());
    ...
    /// ```
        ...
        <span class="kw">if </span>!utf8empty {
            <span class="kw">let </span>hm = <span class="self">self</span>.search_slots_imp(cache, input, slots)<span class="question-mark">?</span>;
            <span class="kw">return </span><span class="prelude-val">Some</span>(hm.pattern());
        }
        <span class="comment">// There is an unfortunate special case where if the regex can
        ...
        <span class="kw">if </span><span class="self">self</span>.get_nfa().pattern_len() == <span class="number">1 </span>{
            ...
            <span class="kw">let </span>got = <span class="self">self</span>.search_slots_imp(cache, input, <span class="kw-2">&amp;mut </span>enough);
            <span class="comment">// This is OK because we know `enough` is strictly bigger than
            // `slots`, otherwise this special case isn't reached.
            </span>slots.copy_from_slice(<span class="kw-2">&amp;</span>enough[..slots.len()]);
            <span class="kw">return </span>got.map(|hm| hm.pattern());
        }
        <span class="kw">let </span><span class="kw-2">mut </span>enough = <span class="macro">vec!</span>[<span class="prelude-val">None</span>; min];
        <span class="kw">let </span>got = <span class="self">self</span>.search_slots_imp(cache, input, <span class="kw-2">&amp;mut </span>enough);
        <span class="comment">// This is OK because we know `enough` is strictly bigger than `slots`,
        // otherwise this special case isn't reached.
        </span>slots.copy_from_slice(<span class="kw-2">&amp;</span>enough[..slots.len()]);
        got.map(|hm| hm.pattern())
    }

    <span class="doccomment">/// This is the actual implementation of `search_slots_imp` that
    ...
    /// provided an insufficient number of slots to record match offsets.
    ...
    </span><span class="kw">fn </span>search_slots_imp(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        cache: <span class="kw-2">&amp;mut </span>Cache,
        input: <span class="kw-2">&amp;</span>Input&lt;<span class="lifetime">'_</span>&gt;,
        slots: <span class="kw-2">&amp;mut </span>[<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;],
        ...
        <span class="kw">let </span>utf8empty = <span class="self">self</span>.get_nfa().has_empty() &amp;&amp; <span class="self">self</span>.get_nfa().is_utf8();
        <span class="kw">let </span>hm = <span class="kw">match </span><span class="self">self</span>.search_imp(cache, input, slots) {
            <span class="prelude-val">None </span>=&gt; <span class="kw">return </span><span class="prelude-val">None</span>,
            <span class="prelude-val">Some</span>(hm) <span class="kw">if </span>!utf8empty =&gt; <span class="kw">return </span><span class="prelude-val">Some</span>(hm),
            <span class="prelude-val">Some</span>(hm) =&gt; hm,
        };
        empty::skip_splits_fwd(input, hm, hm.offset(), |input| {
            <span class="prelude-val">Ok</span>(<span class="self">self
                </span>.search_imp(cache, input, slots)
                .map(|hm| (hm, hm.offset())))
        })
        <span class="comment">// OK because the PikeVM never errors.
        </span>.unwrap()
    }
    ...
    /// ```
        ...
        <span class="self">self</span>.which_overlapping_imp(cache, input, patset)
    }
}

<span class="kw">impl </span>PikeVM {
    <span class="doccomment">/// The implementation of standard leftmost search.
    ///
    ...
    /// which is out of bounds for the given `slots` is ignored.
        ...
        <span class="comment">// Why do we even care about this? Well, in our 'Captures'
        ...
        // But we put it here to make our assumption explicit.
            ...
            <span class="string">"byte slice lengths must be less than usize MAX"</span>,
        );
        <span class="macro">instrument!</span>(|c| c.reset(<span class="kw-2">&amp;</span><span class="self">self</span>.nfa));

        <span class="comment">// Whether we want to visit all match states instead of emulating the
        // 'leftmost' semantics of typical backtracking regex engines.
        </span><span class="kw">let </span>allmatches =
            <span class="self">self</span>.config.get_match_kind().continue_past_first_match();
        <span class="kw">let </span>(anchored, start_id) = <span class="kw">match </span><span class="self">self</span>.start_config(input) {
            <span class="prelude-val">None </span>=&gt; <span class="kw">return </span><span class="prelude-val">None</span>,
            <span class="prelude-val">Some</span>(config) =&gt; config,
        };

        <span class="kw">let </span>pre =
            <span class="kw">if </span>anchored { <span class="prelude-val">None </span>} <span class="kw">else </span>{ <span class="self">self</span>.get_config().get_prefilter() };
        <span class="kw">let </span>Cache { <span class="kw-2">ref mut </span>stack, <span class="kw-2">ref mut </span>curr, <span class="kw-2">ref mut </span>next } = cache;
        <span class="kw">let </span><span class="kw-2">mut </span>hm = <span class="prelude-val">None</span>;
        <span class="comment">// Yes, our search doesn't end at input.end(), but includes it. This
        ...
        // match state.)
        </span><span class="kw">let </span><span class="kw-2">mut </span>at = input.start();
        <span class="kw">while </span>at &lt;= input.end() {
            <span class="comment">// If we have no states left to visit, then there are some cases
            // where we know we can quit early or even skip ahead.
            </span><span class="kw">if </span>curr.set.is_empty() {
                <span class="comment">// We have a match and we haven't been instructed to continue
                // on even after finding a match, so we can quit.
                </span><span class="kw">if </span>hm.is_some() &amp;&amp; !allmatches {
                    <span class="kw">break</span>;
                }
                <span class="comment">// If we're running an anchored search and we've advanced
                // beyond the start position with no other states to try, then
                // we will never observe a match and thus can stop.
                </span><span class="kw">if </span>anchored &amp;&amp; at &gt; input.start() {
                    <span class="kw">break</span>;
                }
                <span class="comment">// If there no states left to explore at this position and we
                ...
                // forward.
                </span><span class="kw">if let </span><span class="prelude-val">Some</span>(<span class="kw-2">ref </span>pre) = pre {
            ...
            <span class="comment">// Instead of using the NFA's unanchored start state, we actually
            ...
            {
                <span class="comment">// Since we are adding to the 'curr' active states and since
                ...
                </span><span class="kw">let </span>slots = next.slot_table.all_absent();
                <span class="self">self</span>.epsilon_closure(stack, slots, curr, input, at, start_id);
            }
            <span class="kw">if let </span><span class="prelude-val">Some</span>(pid) = <span class="self">self</span>.nexts(stack, curr, next, input, at, slots)
            {
                hm = <span class="prelude-val">Some</span>(HalfMatch::new(pid, at));
            }
            <span class="comment">// Unless the caller asked us to return early, we need to mush on
            ...
            </span><span class="kw">if </span>input.get_earliest() &amp;&amp; hm.is_some() {
                <span class="kw">break</span>;
            }
        ...
        <span class="macro">instrument!</span>(|c| c.eprint(<span class="kw-2">&amp;</span><span class="self">self</span>.nfa));
        hm
    }

    <span class="doccomment">/// The implementation for the 'which_overlapping_matches' API. Basically,
    ...
    /// 'samwise'.
    </span><span class="kw">fn </span>which_overlapping_imp(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        cache: <span class="kw-2">&amp;mut </span>Cache,
        input: <span class="kw-2">&amp;</span>Input&lt;<span class="lifetime">'_</span>&gt;,
        patset: <span class="kw-2">&amp;mut </span>PatternSet,
    ) {
        <span class="comment">// NOTE: This is effectively a copy of 'search_imp' above, but with no
        ...
        </span>cache.setup_search(<span class="number">0</span>);
        <span class="kw">if </span>input.is_done() {
        ...
        <span class="kw">let </span>allmatches =
            <span class="self">self</span>.config.get_match_kind().continue_past_first_match();
        <span class="kw">let </span>(anchored, start_id) = <span class="kw">match </span><span class="self">self</span>.start_config(input) {
        ...
        <span class="kw">for </span>at <span class="kw">in </span>input.start()..=input.end() {
            <span class="kw">let </span>any_matches = !patset.is_empty();
            <span class="kw">if </span>curr.set.is_empty() {
                <span class="kw">if </span>any_matches &amp;&amp; !allmatches {
                    <span class="kw">break</span>;
                }
                <span class="kw">if </span>anchored &amp;&amp; at &gt; input.start() {
                    <span class="kw">break</span>;
                }
            }
            <span class="kw">if </span>!any_matches || allmatches {
                <span class="kw">let </span>slots = <span class="kw-2">&amp;mut </span>[];
                <span class="self">self</span>.epsilon_closure(stack, slots, curr, input, at, start_id);
            }
            <span class="self">self</span>.nexts_overlapping(stack, curr, next, input, at, patset);
            <span class="comment">// If we found a match and filled our set, then there is no more
    ...
    <span class="doccomment">/// Process the active states in 'curr' to find the states (written to
    ...
    </span><span class="kw">fn </span>nexts(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        stack: <span class="kw-2">&amp;mut </span>Vec&lt;FollowEpsilon&gt;,
        curr: <span class="kw-2">&amp;mut </span>ActiveStates,
        next: <span class="kw-2">&amp;mut </span>ActiveStates,
        input: <span class="kw-2">&amp;</span>Input&lt;<span class="lifetime">'_</span>&gt;,
        at: usize,
        slots: <span class="kw-2">&amp;mut </span>[<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;],
    ) -&gt; <span class="prelude-ty">Option</span>&lt;PatternID&gt; {
        <span class="macro">instrument!</span>(|c| c.record_state_set(<span class="kw-2">&amp;</span>curr.set));
        <span class="kw">let </span><span class="kw-2">mut </span>pid = <span class="prelude-val">None</span>;
        <span class="kw">let </span>ActiveStates { <span class="kw-2">ref </span>set, <span class="kw-2">ref mut </span>slot_table } = <span class="kw-2">*</span>curr;
        <span class="kw">for </span>sid <span class="kw">in </span>set.iter() {
            pid = <span class="kw">match </span><span class="self">self</span>.next(stack, slot_table, next, input, at, sid) {
                <span class="prelude-val">None </span>=&gt; <span class="kw">continue</span>,
                <span class="prelude-val">Some</span>(pid) =&gt; <span class="prelude-val">Some</span>(pid),
            };
            slots.copy_from_slice(slot_table.for_state(sid));
            <span class="kw">if </span>!<span class="self">self</span>.config.get_match_kind().continue_past_first_match() {
                <span class="kw">break</span>;
            }
        }
        pid
    ...
    <span class="doccomment">/// Like 'nexts', but for the overlapping case. This doesn't write any
    /// slots, and instead just writes which pattern matched in 'patset'.
    </span><span class="attr">#[cfg_attr(feature = <span class="string">"perf-inline"</span>, inline(always))]
    </span><span class="kw">fn </span>nexts_overlapping(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        stack: <span class="kw-2">&amp;mut </span>Vec&lt;FollowEpsilon&gt;,
        curr: <span class="kw-2">&amp;mut </span>ActiveStates,
        next: <span class="kw-2">&amp;mut </span>ActiveStates,
        ...
        <span class="kw">let </span>ActiveStates { <span class="kw-2">ref </span>set, <span class="kw-2">ref mut </span>slot_table } = <span class="kw-2">*</span>curr;
        <span class="kw">for </span>sid <span class="kw">in </span>set.iter() {
            <span class="kw">let </span>pid = <span class="kw">match </span><span class="self">self</span>.next(stack, slot_table, next, input, at, sid)
            {
                <span class="prelude-val">None </span>=&gt; <span class="kw">continue</span>,
                <span class="prelude-val">Some</span>(pid) =&gt; pid,
            };
            <span class="comment">// This handles the case of finding a zero-width match that splits
            ...
            // not see a match.
            </span><span class="kw">if </span>utf8empty &amp;&amp; !input.is_char_boundary(at) {
                <span class="kw">continue</span>;
            }
            <span class="kw">let _ </span>= patset.try_insert(pid);
            <span class="kw">if </span>!<span class="self">self</span>.config.get_match_kind().continue_past_first_match() {
                <span class="kw">break</span>;
            }
        }
    ...
    <span class="doccomment">/// Starting from 'sid', if the position 'at' in the 'input' haystack has a
    ...
    </span><span class="kw">fn </span>next(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        stack: <span class="kw-2">&amp;mut </span>Vec&lt;FollowEpsilon&gt;,
        curr_slot_table: <span class="kw-2">&amp;mut </span>SlotTable,
        next: <span class="kw-2">&amp;mut </span>ActiveStates,
        input: <span class="kw-2">&amp;</span>Input&lt;<span class="lifetime">'_</span>&gt;,
        at: usize,
        sid: StateID,
    ) -&gt; <span class="prelude-ty">Option</span>&lt;PatternID&gt; {
        <span class="macro">instrument!</span>(|c| c.record_step(sid));
        <span class="kw">match </span><span class="kw-2">*</span><span class="self">self</span>.nfa.state(sid) {
            State::Fail
            | State::Look { .. }
            | State::Union { .. }
            | State::BinaryUnion { .. }
            | State::Capture { .. } =&gt; <span class="prelude-val">None</span>,
            State::ByteRange { <span class="kw-2">ref </span>trans } =&gt; {
                <span class="kw">if </span>trans.matches(input.haystack(), at) {
                    <span class="kw">let </span>slots = curr_slot_table.for_state(sid);
                    <span class="comment">// OK because 'at &lt;= haystack.len() &lt; usize::MAX', so
                    // adding 1 will never wrap.
                    </span><span class="kw">let </span>at = at.wrapping_add(<span class="number">1</span>);
                    <span class="self">self</span>.epsilon_closure(
            ...
            State::Sparse(<span class="kw-2">ref </span>sparse) =&gt; {
                <span class="kw">if let </span><span class="prelude-val">Some</span>(next_sid) = sparse.matches(input.haystack(), at) {
                    <span class="kw">let </span>slots = curr_slot_table.for_state(sid);
                    <span class="comment">// OK because 'at &lt;= haystack.len() &lt; usize::MAX', so
                    // adding 1 will never wrap.
                    </span><span class="kw">let </span>at = at.wrapping_add(<span class="number">1</span>);
            ...
            State::Dense(<span class="kw-2">ref </span>dense) =&gt; {
                <span class="kw">if let </span><span class="prelude-val">Some</span>(next_sid) = dense.matches(input.haystack(), at) {
                    <span class="kw">let </span>slots = curr_slot_table.for_state(sid);
                    <span class="comment">// OK because 'at &lt;= haystack.len() &lt; usize::MAX', so
                    // adding 1 will never wrap.
                    </span><span class="kw">let </span>at = at.wrapping_add(<span class="number">1</span>);
            ...
            State::Match { pattern_id } =&gt; <span class="prelude-val">Some</span>(pattern_id),
        }
    }

    <span class="doccomment">/// Compute the epsilon closure of 'sid', writing the closure into 'next'
    ...
    </span><span class="kw">fn </span>epsilon_closure(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        stack: <span class="kw-2">&amp;mut </span>Vec&lt;FollowEpsilon&gt;,
        curr_slots: <span class="kw-2">&amp;mut </span>[<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;],
        next: <span class="kw-2">&amp;mut </span>ActiveStates,
        input: <span class="kw-2">&amp;</span>Input&lt;<span class="lifetime">'_</span>&gt;,
        at: usize,
        sid: StateID,
    ) {
        <span class="macro">instrument!</span>(|c| {
            c.record_closure(sid);
            c.record_stack_push(sid);
        });
        stack.push(FollowEpsilon::Explore(sid));
        <span class="kw">while let </span><span class="prelude-val">Some</span>(frame) = stack.pop() {
            <span class="kw">match </span>frame {
                FollowEpsilon::RestoreCapture { slot, offset: pos } =&gt; {
                    curr_slots[slot] = pos;
                }
                FollowEpsilon::Explore(sid) =&gt; {
                    <span class="self">self</span>.epsilon_closure_explore(
    ...
    <span class="doccomment">/// Explore all of the epsilon transitions out of 'sid'. This is mostly
    ...
    /// include them.
    ...
    </span><span class="kw">fn </span>epsilon_closure_explore(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        stack: <span class="kw-2">&amp;mut </span>Vec&lt;FollowEpsilon&gt;,
        curr_slots: <span class="kw-2">&amp;mut </span>[<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;],
        next: <span class="kw-2">&amp;mut </span>ActiveStates,
        ...
        <span class="comment">// We can avoid pushing some state IDs on to our stack in precisely
        ...
        // thrashing.
            ...
            <span class="macro">instrument!</span>(|c| c.record_set_insert(sid));
            <span class="comment">// Record this state as part of our next set of active states. If
            // we've already explored it, then no need to do it again.
            </span><span class="kw">if </span>!next.set.insert(sid) {
                <span class="kw">return</span>;
            }
            <span class="kw">match </span><span class="kw-2">*</span><span class="self">self</span>.nfa.state(sid) {
                State::Fail
                ...
                State::Union { <span class="kw-2">ref </span>alternates } =&gt; {
                    ...
                    <span class="macro">instrument!</span>(|c| {
                        <span class="kw">for </span><span class="kw-2">&amp;</span>alt <span class="kw">in </span><span class="kw-2">&amp;</span>alternates[<span class="number">1</span>..] {
                ...
                State::BinaryUnion { alt1, alt2 } =&gt; {
                    sid = alt1;
                    <span class="macro">instrument!</span>(|c| c.record_stack_push(sid));
                    stack.push(FollowEpsilon::Explore(alt2));
                }
                State::Capture { next, slot, .. } =&gt; {
                    <span class="comment">// There's no need to do anything with slots that
                    // ultimately won't be copied into the caller-provided
                    // 'Captures' value. So we just skip dealing with them at
                    // all.
                    </span><span class="kw">if </span>slot.as_usize() &lt; curr_slots.len() {
                        <span class="macro">instrument!</span>(|c| c.record_stack_push(sid));
                        stack.push(FollowEpsilon::RestoreCapture {
                            slot,
                            offset: curr_slots[slot],
                        });
                        <span class="comment">// OK because length of a slice must fit into an isize.
                        </span>curr_slots[slot] = <span class="prelude-val">Some</span>(NonMaxUsize::new(at).unwrap());
                    }
                    sid = next;
                }
            }
    ...
    <span class="doccomment">/// Return the starting configuration of a PikeVM search.
    ///
    ...
    </span><span class="kw">fn </span>start_config(<span class="kw-2">&amp;</span><span class="self">self</span>, input: <span class="kw-2">&amp;</span>Input&lt;<span class="lifetime">'_</span>&gt;) -&gt; <span class="prelude-ty">Option</span>&lt;(bool, StateID)&gt; {
        <span class="kw">match </span>input.get_anchored() {
            ...
            </span>Anchored::No =&gt; <span class="prelude-val">Some</span>((
                <span class="self">self</span>.nfa.is_always_start_anchored(),
                <span class="self">self</span>.nfa.start_anchored(),
            )),
            Anchored::Yes =&gt; <span class="prelude-val">Some</span>((<span class="bool-val">true</span>, <span class="self">self</span>.nfa.start_anchored())),
            Anchored::Pattern(pid) =&gt; {
                <span class="prelude-val">Some</span>((<span class="bool-val">true</span>, <span class="self">self</span>.nfa.start_pattern(pid)<span class="question-mark">?</span>))
            }
        }
    }
}

<span class="doccomment">/// An iterator over all non-overlapping matches for a particular search.
///
...
</span><span class="kw">pub struct </span>FindMatches&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>&gt; {
    re: <span class="kw-2">&amp;</span><span class="lifetime">'r </span>PikeVM,
    cache: <span class="kw-2">&amp;</span><span class="lifetime">'c </span><span class="kw-2">mut </span>Cache,
    caps: Captures,
    it: iter::Searcher&lt;<span class="lifetime">'h</span>&gt;,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>&gt; Iterator <span class="kw">for </span>FindMatches&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>&gt; {
    ...
    <span class="attr">#[inline]
        ...
        </span><span class="kw">let </span>FindMatches { re, <span class="kw-2">ref mut </span>cache, <span class="kw-2">ref mut </span>caps, <span class="kw-2">ref mut </span>it } =
            <span class="kw-2">*</span><span class="self">self</span>;
        <span class="comment">// 'advance' converts errors into panics, which is OK here because
        // the PikeVM can never return an error.
        </span>it.advance(|input| {
...
</span><span class="kw">pub struct </span>CapturesMatches&lt;<span class="lifetime">'r</span>, <span class="lifetime">'c</span>, <span class="lifetime">'h</span>&gt; {
    re: <span class="kw-2">&amp;</span><span class="lifetime">'r </span>PikeVM,
    cache: <span class="kw-2">&amp;</span><span class="lifetime">'c </span><span class="kw-2">mut </span>Cache,
    caps: Captures,
    it: iter::Searcher&lt;<span class="lifetime">'h</span>&gt;,
...
<span class="doccomment">/// A cache represents mutable state that a [`PikeVM`] requires during a
/// search.
///
...
/// only be used with the new `PikeVM` (and not the old one).
    ...
    <span class="doccomment">/// Stack used while computing epsilon closure. This effectively lets us
    /// move what is more naturally expressed through recursion to a stack
    /// on the heap.
    </span>stack: Vec&lt;FollowEpsilon&gt;,
    <span class="doccomment">/// The current active states being explored for the current byte in the
    /// haystack.
    </span>curr: ActiveStates,
    <span class="doccomment">/// The next set of states we're building that will be explored for the
    /// next byte in the haystack.
    </span>next: ActiveStates,
}

<span class="kw">impl </span>Cache {
    <span class="doccomment">/// Create a new [`PikeVM`] cache.
    ///
    ...
    </span><span class="kw">pub fn </span>new(re: <span class="kw-2">&amp;</span>PikeVM) -&gt; Cache {
        Cache {
    ...
    /// # Ok::&lt;(), Box&lt;dyn std::error::Error&gt;&gt;(())
    /// ```
    </span><span class="kw">pub fn </span>reset(<span class="kw-2">&amp;mut </span><span class="self">self</span>, re: <span class="kw-2">&amp;</span>PikeVM) {
        <span class="self">self</span>.curr.reset(re);
        <span class="self">self</span>.next.reset(re);
    }
    ...
    /// compute that, use `std::mem::size_of::&lt;Cache&gt;()`.
        ...
        (<span class="self">self</span>.stack.len() * size_of::&lt;FollowEpsilon&gt;())
            + <span class="self">self</span>.curr.memory_usage()
            + <span class="self">self</span>.next.memory_usage()
    }
    ...
    /// in the PikeVM.
    </span><span class="kw">fn </span>setup_search(<span class="kw-2">&amp;mut </span><span class="self">self</span>, captures_slot_len: usize) {
        <span class="self">self</span>.stack.clear();
        <span class="self">self</span>.curr.setup_search(captures_slot_len);
        <span class="self">self</span>.next.setup_search(captures_slot_len);
    }
}

<span class="doccomment">/// A set of active states used to "simulate" the execution of an NFA via the
/// PikeVM.
///
...
</span><span class="kw">struct </span>ActiveStates {
    <span class="doccomment">/// The set of active NFA states. This set preserves insertion order, which
    /// is critical for simulating the match semantics of backtracking regex
    /// engines.
    </span>set: SparseSet,
    <span class="doccomment">/// The slots for every NFA state, where each slot stores a (possibly
    /// absent) offset. Every capturing group has two slots. One for a start
    /// offset and one for an end offset.
    </span>slot_table: SlotTable,
}

<span class="kw">impl </span>ActiveStates {
    <span class="doccomment">/// Create a new set of active states for the given PikeVM. The active
    /// states returned may only be used with the given PikeVM. (Use 'reset'
    /// to re-purpose the allocation for a different PikeVM.)
    </span><span class="kw">fn </span>new(re: <span class="kw-2">&amp;</span>PikeVM) -&gt; ActiveStates {
        <span class="kw">let </span><span class="kw-2">mut </span>active = ActiveStates {
            set: SparseSet::new(<span class="number">0</span>),
    ...
    <span class="doccomment">/// Reset this set of active states such that it can be used with the given
    /// PikeVM (and only that PikeVM).
    </span><span class="kw">fn </span>reset(<span class="kw-2">&amp;mut </span><span class="self">self</span>, re: <span class="kw-2">&amp;</span>PikeVM) {
        <span class="self">self</span>.set.resize(re.get_nfa().states().len());
        <span class="self">self</span>.slot_table.reset(re);
    }

    <span class="doccomment">/// Return the heap memory usage, in bytes, used by this set of active
    /// states.
    ///
    /// This does not include the stack size of this value.
        ...
        <span class="self">self</span>.set.memory_usage() + <span class="self">self</span>.slot_table.memory_usage()
    }

    <span class="doccomment">/// Setup this set of active states for a new search. The given slot
    /// length should be the number of slots in a caller provided 'Captures'
    /// (and may be zero).
    </span><span class="kw">fn </span>setup_search(<span class="kw-2">&amp;mut </span><span class="self">self</span>, captures_slot_len: usize) {
        <span class="self">self</span>.set.clear();
        <span class="self">self</span>.slot_table.setup_search(captures_slot_len);
    }
}

<span class="doccomment">/// A table of slots, where each row represent a state in an NFA. Thus, the
...
/// was encapsulated well.
...
</span><span class="kw">struct </span>SlotTable {
    <span class="doccomment">/// The actual table of offsets.
    </span>table: Vec&lt;<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;&gt;,
    <span class="doccomment">/// The number of slots per state, i.e., the table's stride or the length
    /// of each row.
    </span>slots_per_state: usize,
    ...
    /// but may be wasteful.
    </span>slots_for_captures: usize,
}

<span class="kw">impl </span>SlotTable {
    <span class="doccomment">/// Create a new slot table.
    ///
    /// One should call 'reset' with the corresponding PikeVM before use.
    </span><span class="kw">fn </span>new() -&gt; SlotTable {
        SlotTable { table: <span class="macro">vec!</span>[], slots_for_captures: <span class="number">0</span>, slots_per_state: <span class="number">0 </span>}
    }

    <span class="doccomment">/// Reset this slot table such that it can be used with the given PikeVM
    /// (and only that PikeVM).
    </span><span class="kw">fn </span>reset(<span class="kw-2">&amp;mut </span><span class="self">self</span>, re: <span class="kw-2">&amp;</span>PikeVM) {
        <span class="kw">let </span>nfa = re.get_nfa();
        <span class="self">self</span>.slots_per_state = nfa.group_info().slot_len();
        <span class="comment">// This is always correct, but may be reduced for a particular search
        // if a 'Captures' has fewer slots, e.g., none at all or only slots
        // for tracking the overall match instead of all slots for every
        // group.
        </span><span class="self">self</span>.slots_for_captures = core::cmp::max(
            <span class="self">self</span>.slots_per_state,
            nfa.pattern_len().checked_mul(<span class="number">2</span>).unwrap(),
        );
        <span class="kw">let </span>len = nfa
            .states()
            .len()
            .checked_mul(<span class="self">self</span>.slots_per_state)
            <span class="comment">// Add space to account for scratch space used during a search.
            </span>.and_then(|x| x.checked_add(<span class="self">self</span>.slots_for_captures))
            <span class="comment">// It seems like this could actually panic on legitimate inputs on
            ...
            // file a bug.
            </span>.expect(<span class="string">"slot table length doesn't overflow"</span>);
        <span class="comment">// This happens about as often as a regex is compiled, so it probably
        // should be at debug level, but I found it quite distracting and not
        // particularly useful.
        </span><span class="macro">trace!</span>(
            <span class="string">"resizing PikeVM active states table to {} entries \
             (slots_per_state={})"</span>,
            len,
            <span class="self">self</span>.slots_per_state,
        );
        <span class="self">self</span>.table.resize(len, <span class="prelude-val">None</span>);
    }

    <span class="doccomment">/// Return the heap memory usage, in bytes, used by this slot table.
    ///
    /// This does not include the stack size of this value.
    </span><span class="kw">fn </span>memory_usage(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.table.len() * core::mem::size_of::&lt;<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;&gt;()
    }

    <span class="doccomment">/// Perform any per-search setup for this slot table.
    ///
    ...
    /// search.
        ...
        <span class="self">self</span>.slots_for_captures = captures_slot_len;
    }

    <span class="doccomment">/// Return a mutable slice of the slots for the given state.
    ///
    ...
    </span><span class="kw">fn </span>for_state(<span class="kw-2">&amp;mut </span><span class="self">self</span>, sid: StateID) -&gt; <span class="kw-2">&amp;mut </span>[<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;] {
        <span class="kw">let </span>i = sid.as_usize() * <span class="self">self</span>.slots_per_state;
        <span class="kw-2">&amp;mut </span><span class="self">self</span>.table[i..i + <span class="self">self</span>.slots_for_captures]
    }

    <span class="doccomment">/// Return a slice of slots of appropriate length where every slot offset
    ...
    </span><span class="kw">fn </span>all_absent(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;mut </span>[<span class="prelude-ty">Option</span>&lt;NonMaxUsize&gt;] {
        <span class="kw">let </span>i = <span class="self">self</span>.table.len() - <span class="self">self</span>.slots_for_captures;
        <span class="kw-2">&amp;mut </span><span class="self">self</span>.table[i..i + <span class="self">self</span>.slots_for_captures]
    }
}

<span class="doccomment">/// Represents a stack frame for use while computing an epsilon closure.
///
...
/// first traversal.
    ...
    <span class="doccomment">/// Explore the epsilon transitions from a state ID.
    </span>Explore(StateID),
...
<span class="doccomment">/// A set of counters that "instruments" a PikeVM search. To enable this, you
...
#[derive(Clone, Debug)]
</span><span class="kw">struct </span>Counters {
    <span class="doccomment">/// The number of times the NFA is in a particular permutation of states.
    </span>state_sets: alloc::collections::BTreeMap&lt;Vec&lt;StateID&gt;, u64&gt;,
    <span class="doccomment">/// The number of times 'step' is called for a particular state ID (which
    /// indexes this array).
    </span>steps: Vec&lt;u64&gt;,
    <span class="doccomment">/// The number of times an epsilon closure was computed for a state.
    </span>closures: Vec&lt;u64&gt;,
    <span class="doccomment">/// The number of times a particular state ID is pushed on to a stack while
    /// computing an epsilon closure.
    </span>stack_pushes: Vec&lt;u64&gt;,
    <span class="doccomment">/// The number of times a particular state ID is inserted into a sparse set
    /// while computing an epsilon closure.
    </span>set_inserts: Vec&lt;u64&gt;,
}

<span class="attr">#[cfg(feature = <span class="string">"internal-instrument-pikevm"</span>)]
</span><span class="kw">impl </span>Counters {
    <span class="kw">fn </span>empty() -&gt; Counters {
        Counters {
            state_sets: alloc::collections::BTreeMap::new(),
            steps: <span class="macro">vec!</span>[],
            closures: <span class="macro">vec!</span>[],
            stack_pushes: <span class="macro">vec!</span>[],
            set_inserts: <span class="macro">vec!</span>[],
        }
    }

    <span class="kw">fn </span>reset(<span class="kw-2">&amp;mut </span><span class="self">self</span>, nfa: <span class="kw-2">&amp;</span>NFA) {
        <span class="kw">let </span>len = nfa.states().len();

        <span class="self">self</span>.state_sets.clear();

        <span class="self">self</span>.steps.clear();
        <span class="self">self</span>.steps.resize(len, <span class="number">0</span>);

        <span class="self">self</span>.closures.clear();
        <span class="self">self</span>.closures.resize(len, <span class="number">0</span>);

        <span class="self">self</span>.stack_pushes.clear();
        <span class="self">self</span>.stack_pushes.resize(len, <span class="number">0</span>);

        <span class="self">self</span>.set_inserts.clear();
        <span class="self">self</span>.set_inserts.resize(len, <span class="number">0</span>);
    }

    <span class="kw">fn </span>eprint(<span class="kw-2">&amp;</span><span class="self">self</span>, nfa: <span class="kw-2">&amp;</span>NFA) {
        <span class="macro">trace!</span>(<span class="string">"===== START PikeVM Instrumentation Output ====="</span>);
        <span class="comment">// We take the top-K most occurring state sets. Otherwise the output
        // is likely to be overwhelming. And we probably only care about the
        // most frequently occurring ones anyway.
        </span><span class="kw">const </span>LIMIT: usize = <span class="number">20</span>;
        <span class="kw">let </span><span class="kw-2">mut </span>set_counts =
            <span class="self">self</span>.state_sets.iter().collect::&lt;Vec&lt;(<span class="kw-2">&amp;</span>Vec&lt;StateID&gt;, <span class="kw-2">&amp;</span>u64)&gt;&gt;();
        set_counts.sort_by_key(|(<span class="kw">_</span>, <span class="kw-2">&amp;</span>count)| core::cmp::Reverse(count));
        <span class="macro">trace!</span>(<span class="string">"## PikeVM frequency of state sets (top {})"</span>, LIMIT);
        <span class="kw">for </span>(set, count) <span class="kw">in </span>set_counts.iter().take(LIMIT) {
            <span class="macro">trace!</span>(<span class="string">"{:?}: {}"</span>, set, count);
        }
        <span class="kw">if </span>set_counts.len() &gt; LIMIT {
            <span class="macro">trace!</span>(
                <span class="string">"... {} sets omitted (out of {} total)"</span>,
                set_counts.len() - LIMIT,
                set_counts.len(),
            );
        }

        <span class="macro">trace!</span>(<span class="string">""</span>);
        <span class="macro">trace!</span>(<span class="string">"## PikeVM total frequency of events"</span>);
        <span class="macro">trace!</span>(
            <span class="string">"steps: {}, closures: {}, stack-pushes: {}, set-inserts: {}"</span>,
            <span class="self">self</span>.steps.iter().copied().sum::&lt;u64&gt;(),
            <span class="self">self</span>.closures.iter().copied().sum::&lt;u64&gt;(),
            <span class="self">self</span>.stack_pushes.iter().copied().sum::&lt;u64&gt;(),
            <span class="self">self</span>.set_inserts.iter().copied().sum::&lt;u64&gt;(),
        );

        <span class="macro">trace!</span>(<span class="string">""</span>);
        <span class="macro">trace!</span>(<span class="string">"## PikeVM frequency of events broken down by state"</span>);
        <span class="kw">for </span>sid <span class="kw">in </span><span class="number">0</span>..<span class="self">self</span>.steps.len() {
            <span class="macro">trace!</span>(
                <span class="string">"{:06}: steps: {}, closures: {}, \
                 stack-pushes: {}, set-inserts: {}"</span>,
                sid,
                <span class="self">self</span>.steps[sid],
                <span class="self">self</span>.closures[sid],
                <span class="self">self</span>.stack_pushes[sid],
                <span class="self">self</span>.set_inserts[sid],
            );
        }

        <span class="macro">trace!</span>(<span class="string">""</span>);
        <span class="macro">trace!</span>(<span class="string">"## NFA debug display"</span>);
        <span class="macro">trace!</span>(<span class="string">"{:?}"</span>, nfa);
        <span class="macro">trace!</span>(<span class="string">"===== END PikeVM Instrumentation Output ====="</span>);
    }

    <span class="kw">fn </span>record_state_set(<span class="kw-2">&amp;mut </span><span class="self">self</span>, set: <span class="kw-2">&amp;</span>SparseSet) {
        <span class="kw">let </span>set = set.iter().collect::&lt;Vec&lt;StateID&gt;&gt;();
        <span class="kw-2">*</span><span class="self">self</span>.state_sets.entry(set).or_insert(<span class="number">0</span>) += <span class="number">1</span>;
    }

    <span class="kw">fn </span>record_step(<span class="kw-2">&amp;mut </span><span class="self">self</span>, sid: StateID) {
        <span class="self">self</span>.steps[sid] += <span class="number">1</span>;
    }

    <span class="kw">fn </span>record_closure(<span class="kw-2">&amp;mut </span><span class="self">self</span>, sid: StateID) {
        <span class="self">self</span>.closures[sid] += <span class="number">1</span>;
    }

    <span class="kw">fn </span>record_stack_push(<span class="kw-2">&amp;mut </span><span class="self">self</span>, sid: StateID) {
        <span class="self">self</span>.stack_pushes[sid] += <span class="number">1</span>;
    }

    <span class="kw">fn </span>record_set_insert(<span class="kw-2">&amp;mut </span><span class="self">self</span>, sid: StateID) {
        <span class="self">self</span>.set_inserts[sid] += <span class="number">1</span>;
    }
}
