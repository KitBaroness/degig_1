...
<span class="attr">#[derive(AnchorSerialize, AnchorDeserialize, Clone, Default)]
</span><span class="kw">pub struct </span>InitializeTokenBondingV0Args {
  ...
  <span class="kw">pub </span>freeze_buy_unix_time: <span class="prelude-ty">Option</span>&lt;i64&gt;, <span class="comment">// Cut this bonding curve off at some time
  // The maximum number of target tokens that can be minted.
  </span><span class="kw">pub </span>mint_cap: <span class="prelude-ty">Option</span>&lt;u64&gt;,
  <span class="comment">// The maximum target tokens per purchase
  </span><span class="kw">pub </span>purchase_cap: <span class="prelude-ty">Option</span>&lt;u64&gt;,
  ...
  <span class="kw">pub </span>index: u16, <span class="comment">// A given target mint can have multiple curves associated with it. Index 0 is reserved for the primary curve that holds mint authority
  </span><span class="kw">pub </span>bump_seed: u8,
   ...
   */
  </span><span class="kw">pub </span>initial_reserves_pad: u64,
  <span class="kw">pub </span>initial_supply_pad: u64,
}

<span class="attr">#[derive(Accounts)]
#[instruction(args: InitializeTokenBondingV0Args)]
</span><span class="kw">pub struct </span>InitializeTokenBondingV0&lt;<span class="lifetime">'info</span>&gt; {
  <span class="attr">#[account(<span class="kw-2">mut</span>)]
  </span><span class="kw">pub </span>payer: Signer&lt;<span class="lifetime">'info</span>&gt;,
  <span class="kw">pub </span>curve: Box&lt;Account&lt;<span class="lifetime">'info</span>, CurveV0&gt;&gt;,
  <span class="attr">#[account(
    init,
    seeds = [<span class="string">b"token-bonding"</span>, target_mint.key().as_ref(), <span class="kw-2">&amp;</span>args.index.to_le_bytes()]</span>,
    bump,
    <span class="comment">// Index 0 is reserved for the primary bonding curve, the one with which new tokens can be minted
    </span>constraint = args.index != <span class="number">0 </span>|| target_mint.mint_authority.unwrap() == token_bonding.key(),
    payer = payer,
    space = <span class="number">512
  </span>)]
  <span class="kw">pub </span>token_bonding: Box&lt;Account&lt;<span class="lifetime">'info</span>, TokenBondingV0&gt;&gt;,
  <span class="attr">#[account(
    constraint = base_mint.is_initialized
  )]
  </span><span class="kw">pub </span>base_mint: Box&lt;Account&lt;<span class="lifetime">'info</span>, Mint&gt;&gt;,
  <span class="attr">#[account(
    constraint = target_mint.is_initialized
  )]
  </span><span class="kw">pub </span>target_mint: Box&lt;Account&lt;<span class="lifetime">'info</span>, Mint&gt;&gt;,
  <span class="attr">#[account(
  ...
  </span><span class="kw">pub </span>base_storage: Box&lt;Account&lt;<span class="lifetime">'info</span>, TokenAccount&gt;&gt;,
...
<span class="kw">pub fn </span>handler(
  ctx: Context&lt;InitializeTokenBondingV0&gt;,
  args: InitializeTokenBondingV0Args,
) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
  verify_empty_or_mint(
    <span class="kw-2">&amp;</span>ctx.accounts.buy_base_royalties,
    <span class="kw-2">&amp;</span>ctx.accounts.base_mint.key(),
  )<span class="question-mark">?</span>;
  verify_empty_or_mint(
    <span class="kw-2">&amp;</span>ctx.accounts.sell_base_royalties,
    <span class="kw-2">&amp;</span>ctx.accounts.base_mint.key(),
  )<span class="question-mark">?</span>;
  verify_empty_or_mint(
    <span class="kw-2">&amp;</span>ctx.accounts.buy_target_royalties,
    <span class="kw-2">&amp;</span>ctx.accounts.target_mint.key(),
  )<span class="question-mark">?</span>;
  verify_empty_or_mint(
    <span class="kw-2">&amp;</span>ctx.accounts.sell_target_royalties,
    <span class="kw-2">&amp;</span>ctx.accounts.target_mint.key(),
  )<span class="question-mark">?</span>;

  <span class="kw">if </span>ctx.accounts.base_storage.mint == spl_token::native_mint::ID {
    <span class="kw">return </span><span class="prelude-val">Err</span>(<span class="macro">error!</span>(ErrorCode::WrappedSolNotAllowed));
  }

  <span class="kw">let </span>target_mint = <span class="kw-2">&amp;</span>ctx.accounts.target_mint;

  <span class="kw">let </span>bonding = <span class="kw-2">&amp;mut </span>ctx.accounts.token_bonding;
  bonding.go_live_unix_time = <span class="kw">if </span>args.go_live_unix_time &lt; ctx.accounts.clock.unix_timestamp {
  ...
  <span class="comment">// We need to own the mint authority if this bonding curve supports buying.
  // This can be a sell only bonding curve
  </span>bonding.buy_frozen = args.buy_frozen
    || bonding.key() != target_mint.mint_authority.unwrap_or_default()
    || (target_mint.freeze_authority.is_some()
      &amp;&amp; bonding.key()
        != target_mint
          .freeze_authority
          .ok_or(<span class="macro">error!</span>(ErrorCode::NoMintAuthority))<span class="question-mark">?</span>);
  ...
  <span class="kw">if </span>args.initial_reserves_pad &gt; <span class="number">0 </span>|| args.initial_supply_pad &gt; <span class="number">0 </span>{
    <span class="kw">if </span>!args.ignore_external_supply_changes || !args.ignore_external_reserve_changes {
      <span class="kw">return </span><span class="prelude-val">Err</span>(<span class="macro">error!</span>(ErrorCode::InvalidPad));
    }
    bonding.reserve_balance_from_bonding = args.initial_reserves_pad;
    bonding.supply_from_bonding = args.initial_supply_pad;
  } <span class="kw">else </span>{
    bonding.reserve_balance_from_bonding = <span class="number">0</span>;
    bonding.supply_from_bonding = <span class="number">0</span>;
  }
