...
<span class="kw">use </span>anchor_spl::token::{<span class="self">self</span>, Burn, Mint, Token, TokenAccount};

<span class="attr">#[derive(AnchorSerialize, AnchorDeserialize, Clone, Default)]
</span><span class="kw">pub struct </span>SellWrappedSolV0Args {
  <span class="kw">pub </span>amount: u64,
  <span class="kw">pub </span>all: bool, <span class="comment">// Optional flag to just sell all of it.
</span>}

<span class="attr">#[derive(Accounts)]
#[instruction(args: SellWrappedSolV0Args)]
</span><span class="kw">pub struct </span>SellWrappedSolV0&lt;<span class="lifetime">'info</span>&gt; {
  ...
  </span><span class="kw">pub </span>source: Box&lt;Account&lt;<span class="lifetime">'info</span>, TokenAccount&gt;&gt;,
  <span class="doccomment">/// CHECK: Manually check signer here instead of using Signer so it can be used in internal CPI
...
<span class="kw">pub fn </span>sell_wrapped_sol(
  accounts: <span class="kw-2">&amp;</span>SellWrappedSolV0,
  args: <span class="kw-2">&amp;</span>SellWrappedSolV0Args,
  seeds: <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>[u8]]]&gt;,
) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
  <span class="kw">let </span>amount = <span class="kw">if </span>args.all {
  ...
  invoke_signed(
    <span class="kw-2">&amp;</span>system_instruction::transfer(
      ...
      <span class="kw-2">&amp;</span>accounts.destination.key(),
      amount,
    ),
    ...
    <span class="kw-2">&amp;</span>[<span class="kw-2">&amp;</span>[
      <span class="string">"sol-storage"</span>.as_bytes(),
      <span class="kw-2">&amp;</span>[accounts.state.sol_storage_bump_seed],
    ]],
  )<span class="question-mark">?</span>;

  <span class="kw">let </span>token_program = accounts.token_program.to_account_info().clone();
  <span class="kw">let </span>command = Burn {
  ...
  <span class="kw">let </span>context = <span class="kw">match </span>seeds {
    <span class="prelude-val">Some</span>(seeds) =&gt; CpiContext::new_with_signer(token_program, command, seeds),
    <span class="prelude-val">None </span>=&gt; CpiContext::new(token_program, command),
  };
  token::burn(context, amount)<span class="question-mark">?</span>;

  <span class="prelude-val">Ok</span>(())
}

<span class="kw">pub fn </span>handler(ctx: Context&lt;SellWrappedSolV0&gt;, args: SellWrappedSolV0Args) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
  sell_wrapped_sol(ctx.accounts, <span class="kw-2">&amp;</span>args, <span class="prelude-val">None</span>)
}
