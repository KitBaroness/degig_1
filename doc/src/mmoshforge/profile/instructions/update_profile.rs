...
<span class="doccomment">/// This is the function used to update profile nft 
    ...
    {
        <span class="comment">//NOTE: updating profile
        </span>ctx.accounts.update(name, symbol, uri_hash)<span class="question-mark">?</span>;
    }
...
<span class="doccomment">/// Struct used to create context while preparing instruction for update profile
...
#[instruction(
...
</span><span class="kw">pub struct </span>AUpdateMint&lt;<span class="lifetime">'info</span>&gt; {
    <span class="doccomment">/// user publickey and mandatory signer while pushing the instruction to solana
    ...
    <span class="doccomment">/// load mainstate account by public key
    </span><span class="attr">#[account(
        <span class="kw-2">mut</span>,
    ...
    <span class="doccomment">/// profile metadata public key
    ///CHECK:
    </span><span class="attr">#[account(
        <span class="kw-2">mut</span>,
        seeds=[
...
<span class="kw">impl</span>&lt;<span class="lifetime">'info</span>&gt; AUpdateMint&lt;<span class="lifetime">'info</span>&gt; {
    <span class="kw">pub fn </span>update(<span class="kw-2">&amp;mut </span><span class="self">self</span>, name: String, symbol: String, uri_hash: String) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
        ...
        <span class="kw">let </span>metadata = <span class="self">self</span>.metadata.to_account_info();
        <span class="kw">let </span>mpl_program = <span class="self">self</span>.mpl_program.to_account_info();
        ...
        <span class="kw">let </span>asset_data = Data {
            name,
            symbol,
            uri: uri_hash,
            seller_fee_basis_points: <span class="number">0</span>, <span class="comment">//EX: 20% (80% goes to seller)
            </span>creators: <span class="prelude-val">Some</span>(creators),
        };

        <span class="kw">let </span>ix = UpdateV1Builder::new()
