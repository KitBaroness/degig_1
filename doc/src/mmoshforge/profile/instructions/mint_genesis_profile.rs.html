...
<span class="kw">use </span>anchor_spl::{
    ...
    token::{<span class="self">self</span>, Approve, Mint, MintTo, Token, TokenAccount},
};
<span class="kw">use </span>mpl_token_metadata::{
    instructions::{
        ApproveCollectionAuthority, Burn, Create, CreateBuilder, Mint <span class="kw">as </span>MintNft, Verify, VerifySizedCollectionItem
    },
    types::{
...
<span class="attr">#[derive(AnchorSerialize, AnchorDeserialize, Default, Clone)]
</span><span class="kw">pub struct </span>MintProfileByAdminInput {
    name: String,
    symbol: String,
    uri: String,
    lineage: LineageInfo,
...
<span class="doccomment">/// This is the function which used to create instruction for mint gensis profile nft
</span><span class="kw">pub fn </span>mint_genesis_profile(
    ctx: Context&lt;AMintProfileByAdmin&gt;,
    input: MintProfileByAdminInput,
) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
    <span class="comment">// let mut nft_creators = Vec::&lt;Creator&gt;::new();
    </span>{
        ...
        <span class="kw">let </span>collection_state = <span class="kw-2">&amp;mut </span>ctx.accounts.collection_state;

        <span class="comment">//verification
        </span><span class="kw">if </span>collection_state.genesis_profile != System::id() {
            <span class="kw">return </span><span class="macro">anchor_lang::err!</span>(MyError::AlreadySet);
        }

        <span class="comment">//state changes
        </span><span class="kw">let </span>profile = ctx.accounts.profile.key();
        ...
        profile_state.lineage.generation = <span class="number">1</span>;
        profile_state.lineage.total_child = <span class="number">0</span>;
        ...
        </span>main_state.total_minted_profile += <span class="number">1</span>;
        main_state.genesis_profile = ctx.accounts.profile.key();
    }
    {
        <span class="comment">//NOTE: minting
        </span>ctx.accounts.mint(input.name, input.symbol, input.uri)<span class="question-mark">?</span>;
    }
    {
    ...
    {
        <span class="comment">// ctx.accounts.approve_sub_collection_authority_to_main()?;
    </span>}
    <span class="prelude-val">Ok</span>(())
}

<span class="attr">#[derive(Accounts)]
</span><span class="kw">pub struct </span>AMintProfileByAdmin&lt;<span class="lifetime">'info</span>&gt; {
    ...
    <span class="doccomment">/// profile associated token account public key
    </span><span class="attr">#[account(
        <span class="kw-2">mut</span>,
        token::mint = profile,
    ...
    <span class="doccomment">/// collection state account from publickey 
    </span><span class="attr">#[account(
        <span class="kw-2">mut</span>,
    ...
    <span class="doccomment">/// collection metdata account public key 
    ///CHECK:
    </span><span class="attr">#[account(
        <span class="kw-2">mut</span>,
        seeds=[
    ...
    <span class="doccomment">/// collection authority account public key 
    ///CHECK:
    </span><span class="attr">#[account(
        <span class="kw-2">mut</span>,
        seeds = [
    ...
    <span class="doccomment">/// sub collection authority account public key 
    ///CHECK:
    </span><span class="attr">#[account(
        <span class="kw-2">mut</span>,
        seeds = [
    ...
    <span class="kw">pub </span>sub_collection_authority_record: AccountInfo&lt;<span class="lifetime">'info</span>&gt;,

    <span class="doccomment">/// system var instruction public key
    </span><span class="comment">//PERF: not sure parent profile nft collection verification are require or not (think it
    //already secure)
    </span><span class="doccomment">///CHECK:
    </span><span class="attr">#[account()]
...
<span class="kw">impl</span>&lt;<span class="lifetime">'info</span>&gt; AMintProfileByAdmin&lt;<span class="lifetime">'info</span>&gt; {
    <span class="kw">pub fn </span>mint(<span class="kw-2">&amp;mut </span><span class="self">self</span>, name: String, symbol: String, uri: String) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
        <span class="kw">let </span>mint = <span class="self">self</span>.profile.to_account_info();
        <span class="kw">let </span>admin = <span class="self">self</span>.admin.to_account_info();
        <span class="kw">let </span>admin_ata = <span class="self">self</span>.admin_ata.to_account_info();
        ...
        // token::mint_to(CpiContext::new(token_program.clone(), cpi_mint_accounts), 1)?;
        ...
        </span><span class="kw">let </span>asset_data = CreateArgs::V1 {
            name,
            symbol,
            uri,
            ...
            creators: <span class="prelude-val">Some</span>(<span class="macro">vec!</span>[Creator {
                verified: <span class="bool-val">false</span>,
                share: <span class="number">100</span>,
                address: <span class="self">self</span>.admin.key(),
            }]),
            collection_details: <span class="prelude-val">Some</span>(mpl_token_metadata::types::CollectionDetails::V1 { size: <span class="number">0 </span>}),
            ...
            print_supply: <span class="prelude-val">Some</span>(PrintSupply::Zero),
        };
    ...
    <span class="kw">pub fn </span>approve_sub_collection_authority_to_main(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
        ...
        <span class="kw">let </span>sysvar_instructions = <span class="self">self</span>.sysvar_instructions.to_account_info();
        <span class="kw">let </span>main_state = <span class="kw-2">&amp;mut </span><span class="self">self</span>.main_state;
        <span class="kw">let </span>sub_collection_authority_record =
            <span class="self">self</span>.sub_collection_authority_record.to_account_info();
