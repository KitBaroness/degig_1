...
</pre></div><pre class="rust"><code><span class="comment">// Based on unstable std::sync::OnceLock.
//
...
<span class="kw">impl</span>&lt;T&gt; OnceLock&lt;T&gt; {
        ...
        </span>F: FnOnce() -&gt; T,
    {
        ...
        </span><span class="kw">if </span><span class="self">self</span>.once.is_completed() {
    ...
    {
        <span class="kw">let </span>slot = <span class="self">self</span>.value.get();

        <span class="self">self</span>.once.call_once(|| {
            <span class="kw">let </span>value = f();
            <span class="kw">unsafe </span>{ slot.write(MaybeUninit::new(value)) }
        });
    }
    ...
    /// The value must be initialized
        ...
        <span class="macro">debug_assert!</span>(<span class="self">self</span>.once.is_completed());
...
<span class="kw">impl</span>&lt;T&gt; Drop <span class="kw">for </span>OnceLock&lt;T&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="kw">if </span><span class="self">self</span>.once.is_completed() {
            <span class="comment">// SAFETY: The inner value has been initialized
            </span><span class="kw">unsafe </span>{ (<span class="kw-2">*</span><span class="self">self</span>.value.get()).assume_init_drop() };
        }
    }
}
