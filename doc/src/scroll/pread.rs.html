...
</pre></div><pre class="rust"><code><span class="kw">use </span>core::result;

<span class="kw">use </span><span class="kw">crate</span>::ctx::TryFromCtx;
<span class="kw">use </span><span class="kw">crate</span>::error;

<span class="doccomment">/// A very generic, contextual pread interface in Rust.
///
...
///
</span><span class="kw">pub trait </span>Pread&lt;Ctx: Copy, E&gt; {
    <span class="attr">#[inline]
    </span><span class="doccomment">/// Reads a value from `self` at `offset` with a default `Ctx`. For the primitive numeric values, this will read at the machine's endianness.
    /// # Example
    /// ```rust
    /// use scroll::Pread;
    /// let bytes = [0x7fu8; 0x01];
    /// let byte = bytes.pread::&lt;u8&gt;(0).unwrap();
    </span><span class="kw">fn </span>pread&lt;<span class="lifetime">'a</span>, N: TryFromCtx&lt;<span class="lifetime">'a</span>, Ctx, <span class="self">Self</span>, Error = E&gt;&gt;(
    ...
    {
        <span class="self">self</span>.pread_with(offset, Ctx::default())
    }

    <span class="attr">#[inline]
    </span><span class="doccomment">/// Reads a value from `self` at `offset` with the given `ctx`
    /// # Example
    /// ```rust
    /// use scroll::Pread;
    ...
    </span><span class="kw">fn </span>pread_with&lt;<span class="lifetime">'a</span>, N: TryFromCtx&lt;<span class="lifetime">'a</span>, Ctx, <span class="self">Self</span>, Error = E&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="self">self</span>,
        offset: usize,
        ctx: Ctx,
    ) -&gt; result::Result&lt;N, E&gt; {
        <span class="kw">let </span><span class="kw-2">mut </span>ignored = offset;
        <span class="self">self</span>.gread_with(<span class="kw-2">&amp;mut </span>ignored, ctx)
    }

    <span class="attr">#[inline]
    </span><span class="doccomment">/// Reads a value from `self` at `offset` with a default `Ctx`. For the primitive numeric values, this will read at the machine's endianness. Updates the offset
    /// # Example
    /// ```rust
    /// use scroll::Pread;
    /// let offset = &amp;mut 0;
    ...
    /// assert_eq!(*offset, 1);
    </span><span class="kw">fn </span>gread&lt;<span class="lifetime">'a</span>, N: TryFromCtx&lt;<span class="lifetime">'a</span>, Ctx, <span class="self">Self</span>, Error = E&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="self">self</span>,
        ...
        </span>Ctx: Default,
    {
        ...
        <span class="self">self</span>.gread_with(offset, ctx)
    }

    <span class="doccomment">/// Reads a value from `self` at `offset` with the given `ctx`, and updates the offset.
    /// # Example
    /// ```rust
    /// use scroll::Pread;
    /// let offset = &amp;mut 0;
    ...
    /// assert_eq!(dead, 0xdeadu16);
    /// assert_eq!(*offset, 2);
    </span><span class="kw">fn </span>gread_with&lt;<span class="lifetime">'a</span>, N: TryFromCtx&lt;<span class="lifetime">'a</span>, Ctx, <span class="self">Self</span>, Error = E&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="self">self</span>,
        offset: <span class="kw-2">&amp;mut </span>usize,
        ctx: Ctx,
    ) -&gt; result::Result&lt;N, E&gt;;

    <span class="doccomment">/// Tries to write `inout.len()` `N`s into `inout` from `Self` starting at `offset`, using the default context for `N`, and updates the offset.
    /// # Example
    /// ```rust
    /// use scroll::Pread;
    ...
    </span><span class="kw">fn </span>gread_inout&lt;<span class="lifetime">'a</span>, N: TryFromCtx&lt;<span class="lifetime">'a</span>, Ctx, <span class="self">Self</span>, Error = E&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="self">self</span>,
        offset: <span class="kw-2">&amp;mut </span>usize,
        inout: <span class="kw-2">&amp;mut </span>[N],
    ) -&gt; result::Result&lt;(), E&gt;
    <span class="kw">where
        </span>Ctx: Default,
    {
        <span class="kw">for </span>i <span class="kw">in </span>inout.iter_mut() {
            <span class="kw-2">*</span>i = <span class="self">self</span>.gread(offset)<span class="question-mark">?</span>;
        }
        <span class="prelude-val">Ok</span>(())
    }

    <span class="doccomment">/// Tries to write `inout.len()` `N`s into `inout` from `Self` starting at `offset`, using the context `ctx`
    /// # Example
    /// ```rust
    ...
    /// assert_eq!(*offset, 2);
    ...
    </span><span class="kw">fn </span>gread_inout_with&lt;<span class="lifetime">'a</span>, N: TryFromCtx&lt;<span class="lifetime">'a</span>, Ctx, <span class="self">Self</span>, Error = E&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="self">self</span>,
        offset: <span class="kw-2">&amp;mut </span>usize,
        inout: <span class="kw-2">&amp;mut </span>[N],
        ctx: Ctx,
        ...
        <span class="kw">for </span>i <span class="kw">in </span>inout.iter_mut() {
            <span class="kw-2">*</span>i = <span class="self">self</span>.gread_with(offset, ctx)<span class="question-mark">?</span>;
        }
        <span class="prelude-val">Ok</span>(())
    }
}

<span class="kw">impl</span>&lt;Ctx: Copy, E: From&lt;error::Error&gt;&gt; Pread&lt;Ctx, E&gt; <span class="kw">for </span>[u8] {
    <span class="kw">fn </span>gread_with&lt;<span class="lifetime">'a</span>, N: TryFromCtx&lt;<span class="lifetime">'a</span>, Ctx, <span class="self">Self</span>, Error = E&gt;&gt;(
        <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="self">self</span>,
        offset: <span class="kw-2">&amp;mut </span>usize,
        ctx: Ctx,
    ) -&gt; result::Result&lt;N, E&gt; {
        <span class="kw">let </span>start = <span class="kw-2">*</span>offset;
        <span class="kw">if </span>start &gt;= <span class="self">self</span>.len() {
            <span class="kw">return </span><span class="prelude-val">Err</span>(error::Error::BadOffset(start).into());
        }
        N::try_from_ctx(<span class="kw-2">&amp;</span><span class="self">self</span>[start..], ctx).map(|(n, size)| {
