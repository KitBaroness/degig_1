...
<span class="comment">// ===== impl Store =====
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>for_each&lt;F&gt;(<span class="kw-2">&amp;mut </span><span class="self">self</span>, <span class="kw-2">mut </span>f: F)
    <span class="kw">where
        </span>F: FnMut(Ptr),
    {
    ...
    </span>N: Next,
{
    ...
    /// If the stream is already contained by the list, return `false`.
        ...
        <span class="macro">tracing::trace!</span>(<span class="string">"Queue::push_back"</span>);
    ...
    ///
    /// If the stream is already contained by the list, return `false`.
    </span><span class="kw">pub fn </span>push_front(<span class="kw-2">&amp;mut </span><span class="self">self</span>, stream: <span class="kw-2">&amp;mut </span>store::Ptr) -&gt; bool {
        <span class="macro">tracing::trace!</span>(<span class="string">"Queue::push_front"</span>);
        ...
        <span class="comment">// Queue the stream
            ...
            <span class="prelude-val">Some</span>(<span class="kw-2">ref mut </span>idxs) =&gt; {
                ...
                <span class="comment">// Update the provided stream to point to the head node
                </span><span class="kw">let </span>head_key = stream.resolve(idxs.head).key();
                N::set_next(stream, <span class="prelude-val">Some</span>(head_key));

                <span class="comment">// Update the head pointer
                </span>idxs.head = stream.key();
            }
            <span class="prelude-val">None </span>=&gt; {
                <span class="macro">tracing::trace!</span>(<span class="string">" -&gt; first entry"</span>);
                <span class="self">self</span>.indices = <span class="prelude-val">Some</span>(store::Indices {
...
<span class="comment">// ===== impl Ptr =====
    ...
    <span class="kw">pub fn </span>store_mut(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;mut </span>Store {
        <span class="self">self</span>.store
    }
