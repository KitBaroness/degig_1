...
<span class="kw">use </span>tokio_util::io::poll_write_buf;
...
<span class="attr">#[derive(Debug)]
    ...
    <span class="doccomment">/// Chain payloads bigger than this.
    </span>chain_threshold: usize,

    <span class="doccomment">/// Min buffer required to attempt to write a frame
    </span>min_buffer_capacity: usize,
}
...
<span class="doccomment">/// Chain payloads bigger than this when vectored I/O is enabled. The remote
...
<span class="doccomment">/// Chain payloads bigger than this when vectored I/O is **not** enabled.
...
</span><span class="kw">const </span>CHAIN_THRESHOLD_WITHOUT_VECTORED_IO: usize = <span class="number">1024</span>;
    ...
    B: Buf,
{
    <span class="kw">pub fn </span>new(inner: T) -&gt; FramedWrite&lt;T, B&gt; {
        <span class="kw">let </span>chain_threshold = <span class="kw">if </span>inner.is_write_vectored() {
            CHAIN_THRESHOLD
            ...
            <span class="kw">while </span>!<span class="self">self</span>.encoder.is_empty() {
                <span class="kw">match </span><span class="self">self</span>.encoder.next {
                    <span class="prelude-val">Some</span>(Next::Data(<span class="kw-2">ref mut </span>frame)) =&gt; {
                        ...
                        <span class="macro">ready!</span>(poll_write_buf(Pin::new(<span class="kw-2">&amp;mut </span><span class="self">self</span>.inner), cx, <span class="kw-2">&amp;mut </span>buf))<span class="question-mark">?
                    </span>}
                    <span class="kw">_ </span>=&gt; {
                        <span class="macro">tracing::trace!</span>(queued_data_frame = <span class="bool-val">false</span>);
                        <span class="macro">ready!</span>(poll_write_buf(
                            Pin::new(<span class="kw-2">&amp;mut </span><span class="self">self</span>.inner),
                            cx,
    ...
    </span>B: Buf,
{
    ...
    <span class="kw">fn </span>buffer(<span class="kw-2">&amp;mut </span><span class="self">self</span>, item: Frame&lt;B&gt;) -&gt; <span class="prelude-ty">Result</span>&lt;(), UserError&gt; {
        ...
        <span class="kw">match </span>item {
            Frame::Data(<span class="kw-2">mut </span>v) =&gt; {
                ...
                <span class="kw">if </span>len &gt;= <span class="self">self</span>.chain_threshold {
                    ...
                    <span class="kw">if </span><span class="self">self</span>.buf.get_ref().remaining() &lt; <span class="self">self</span>.chain_threshold {
                        <span class="kw">let </span>extra_bytes = <span class="self">self</span>.chain_threshold - <span class="self">self</span>.buf.remaining();
                        <span class="self">self</span>.buf.get_mut().put(v.payload_mut().take(extra_bytes));
                    }
    ...
    <span class="kw">fn </span>has_capacity(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.next.is_none()
            &amp;&amp; (<span class="self">self</span>.buf.get_ref().capacity() - <span class="self">self</span>.buf.get_ref().len()
                &gt;= <span class="self">self</span>.min_buffer_capacity)
    }
