...
<span class="doccomment">/// Create a new non-blocking socket.
    ...
    <span class="kw">let </span>socket = <span class="macro">syscall!</span>(socket(domain, socket_type, <span class="number">0</span>))<span class="question-mark">?</span>;
    ...
    <span class="comment">// Mimick `libstd` and set `SO_NOSIGPIPE` on apple systems.
    ...
    </span><span class="kw">if let </span><span class="prelude-val">Err</span>(err) = <span class="macro">syscall!</span>(setsockopt(
        socket,
        libc::SOL_SOCKET,
        libc::SO_NOSIGPIPE,
        ...
        <span class="kw">let _ </span>= <span class="macro">syscall!</span>(close(socket));
    ...
    <span class="comment">// Darwin (and others) doesn't have SOCK_NONBLOCK or SOCK_CLOEXEC.
        ...
        <span class="kw">if let </span><span class="prelude-val">Err</span>(err) = <span class="macro">syscall!</span>(fcntl(socket, libc::F_SETFL, libc::O_NONBLOCK)) {
            <span class="kw">let _ </span>= <span class="macro">syscall!</span>(close(socket));
            <span class="kw">return </span><span class="prelude-val">Err</span>(err);
        }
        <span class="attr">#[cfg(not(any(target_os = <span class="string">"espidf"</span>, target_os = <span class="string">"vita"</span>)))]
        </span><span class="kw">if let </span><span class="prelude-val">Err</span>(err) = <span class="macro">syscall!</span>(fcntl(socket, libc::F_SETFD, libc::FD_CLOEXEC)) {
            <span class="kw">let _ </span>= <span class="macro">syscall!</span>(close(socket));
            <span class="kw">return </span><span class="prelude-val">Err</span>(err);
        }
    }
    ...
    <span class="kw">match </span>addr {
        SocketAddr::V4(<span class="kw-2">ref </span>addr) =&gt; {
            ...
            <span class="kw">let </span>sockaddr_in = libc::sockaddr_in {
                ...
                </span>sin_zero: [<span class="number">0</span>; <span class="number">8</span>],
                <span class="attr">#[cfg(target_os = <span class="string">"vita"</span>)]
                </span>sin_zero: [<span class="number">0</span>; <span class="number">6</span>],
                <span class="attr">#[cfg(any(
                ...
                <span class="attr">#[cfg(target_os = <span class="string">"vita"</span>)]
                </span>sin_vport: addr.port().to_be(),
            };
        ...
        SocketAddr::V6(<span class="kw-2">ref </span>addr) =&gt; {
            <span class="kw">let </span>sockaddr_in6 = libc::sockaddr_in6 {
                ...
                <span class="attr">#[cfg(target_os = <span class="string">"vita"</span>)]
                </span>sin6_vport: addr.port().to_be(),
                <span class="attr">#[cfg(any(target_os = <span class="string">"illumos"</span>, target_os = <span class="string">"solaris"</span>))]
