...
<a href="#339" id="339">339</a>
...
</span><span class="kw">mod </span>fdbased {
    <span class="attr">#[cfg(all(
        not(mio_unsupported_force_waker_pipe),
        any(target_os = <span class="string">"linux"</span>, target_os = <span class="string">"android"</span>),
    ))]
    </span><span class="kw">use </span><span class="kw">crate</span>::sys::unix::waker::eventfd::WakerInternal;
    ...
    </span><span class="kw">use </span><span class="kw">crate</span>::sys::unix::waker::pipe::WakerInternal;
    ...
    <span class="kw">impl </span>Waker {
        <span class="kw">pub fn </span>new(selector: <span class="kw-2">&amp;</span>Selector, token: Token) -&gt; io::Result&lt;Waker&gt; {
            <span class="kw">let </span>waker = WakerInternal::new()<span class="question-mark">?</span>;
            selector.register(waker.as_raw_fd(), token, Interest::READABLE)<span class="question-mark">?</span>;
            <span class="prelude-val">Ok</span>(Waker { waker })
        }

        <span class="kw">pub fn </span>wake(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; io::Result&lt;()&gt; {
...
</span><span class="kw">pub use </span><span class="self">self</span>::fdbased::Waker;

<span class="attr">#[cfg(all(
    not(mio_unsupported_force_waker_pipe),
    any(target_os = <span class="string">"linux"</span>, target_os = <span class="string">"android"</span>, target_os = <span class="string">"espidf"</span>)
))]
    ...
    </span><span class="kw">pub struct </span>WakerInternal {
        fd: File,
    }

    <span class="kw">impl </span>WakerInternal {
        <span class="kw">pub fn </span>new() -&gt; io::Result&lt;WakerInternal&gt; {
            ...
            <span class="prelude-val">Ok</span>(WakerInternal { fd: file })
        }
        ...
        <span class="attr">#[cfg(mio_unsupported_force_poll_poll)]
        </span><span class="kw">pub fn </span>ack_and_reset(<span class="kw-2">&amp;</span><span class="self">self</span>) {
    ...
    <span class="kw">impl </span>AsRawFd <span class="kw">for </span>WakerInternal {
...
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span><span class="self">self</span>::eventfd::WakerInternal;
...
<span class="attr">#[cfg(all(
    ...
    <span class="doccomment">/// Waker backed by kqueue user space notifications (`EVFILT_USER`).
    ///
    ...
    <span class="kw">impl </span>Waker {
        <span class="kw">pub fn </span>new(selector: <span class="kw-2">&amp;</span>Selector, token: Token) -&gt; io::Result&lt;Waker&gt; {
            <span class="kw">let </span>selector = selector.try_clone()<span class="question-mark">?</span>;
            selector.setup_waker(token)<span class="question-mark">?</span>;
            <span class="prelude-val">Ok</span>(Waker { selector, token })
        }

        <span class="kw">pub fn </span>wake(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; io::Result&lt;()&gt; {
            <span class="self">self</span>.selector.wake(<span class="self">self</span>.token)
        }
    }
}
...
</span><span class="kw">pub use </span><span class="self">self</span>::kqueue::Waker;
...
<span class="attr">#[cfg(any(
    ...
    <span class="kw">use </span><span class="kw">crate</span>::sys::unix::pipe;
    <span class="kw">use </span>std::fs::File;
    <span class="kw">use </span>std::io::{<span class="self">self</span>, Read, Write};
    <span class="kw">use </span>std::os::unix::io::{AsRawFd, FromRawFd, RawFd};

    <span class="doccomment">/// Waker backed by a unix pipe.
    ///
    ...
    <span class="kw">impl </span>WakerInternal {
        <span class="kw">pub fn </span>new() -&gt; io::Result&lt;WakerInternal&gt; {
            <span class="kw">let </span>[receiver, sender] = pipe::new_raw()<span class="question-mark">?</span>;
            <span class="kw">let </span>sender = <span class="kw">unsafe </span>{ File::from_raw_fd(sender) };
            <span class="kw">let </span>receiver = <span class="kw">unsafe </span>{ File::from_raw_fd(receiver) };
            <span class="prelude-val">Ok</span>(WakerInternal { sender, receiver })
        }
        ...
        <span class="kw">pub fn </span>wake(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; io::Result&lt;()&gt; {
            ...
            </span><span class="self">self</span>.empty();

            <span class="kw">match </span>(<span class="kw-2">&amp;</span><span class="self">self</span>.sender).write(<span class="kw-2">&amp;</span>[<span class="number">1</span>]) {
                <span class="prelude-val">Ok</span>(<span class="kw">_</span>) =&gt; <span class="prelude-val">Ok</span>(()),
                <span class="prelude-val">Err</span>(<span class="kw-2">ref </span>err) <span class="kw">if </span>err.kind() == io::ErrorKind::WouldBlock =&gt; {
                    <span class="comment">// The reading end is full so we'll empty the buffer and try
                    // again.
                    </span><span class="self">self</span>.empty();
                    <span class="self">self</span>.wake()
                }
                <span class="prelude-val">Err</span>(<span class="kw-2">ref </span>err) <span class="kw">if </span>err.kind() == io::ErrorKind::Interrupted =&gt; <span class="self">self</span>.wake(),
                <span class="prelude-val">Err</span>(err) =&gt; <span class="prelude-val">Err</span>(err),
            }
        }
        ...
        <span class="doccomment">/// Empty the pipe's buffer, only need to call this if `wake` fails.
        /// This ignores any errors.
        </span><span class="kw">fn </span>empty(<span class="kw-2">&amp;</span><span class="self">self</span>) {
            <span class="kw">let </span><span class="kw-2">mut </span>buf = [<span class="number">0</span>; <span class="number">4096</span>];
            <span class="kw">loop </span>{
                <span class="kw">match </span>(<span class="kw-2">&amp;</span><span class="self">self</span>.receiver).read(<span class="kw-2">&amp;mut </span>buf) {
                    <span class="prelude-val">Ok</span>(n) <span class="kw">if </span>n &gt; <span class="number">0 </span>=&gt; <span class="kw">continue</span>,
    ...
    <span class="kw">impl </span>AsRawFd <span class="kw">for </span>WakerInternal {
        <span class="kw">fn </span>as_raw_fd(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; RawFd {
            <span class="self">self</span>.receiver.as_raw_fd()
        }
    }
}
...
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span><span class="self">self</span>::pipe::WakerInternal;
    ...
    <span class="kw">impl </span>Waker {
        <span class="kw">pub fn </span>new(selector: <span class="kw-2">&amp;</span>Selector, token: Token) -&gt; io::Result&lt;Waker&gt; {
            <span class="prelude-val">Ok</span>(Waker {
                selector: selector.try_clone()<span class="question-mark">?</span>,
                token,
            })
        }
...
</span><span class="kw">pub use </span><span class="self">self</span>::poll::Waker;
