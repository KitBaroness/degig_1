...
</pre></div><pre class="rust"><code><span class="doccomment">//! Unix pipe.
//!
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new_raw() -&gt; io::Result&lt;[RawFd; <span class="number">2</span>]&gt; {
    <span class="kw">let </span><span class="kw-2">mut </span>fds: [RawFd; <span class="number">2</span>] = [-<span class="number">1</span>, -<span class="number">1</span>];
    ...
    <span class="attr">#[cfg(any(
        ...
        <span class="kw">if </span>libc::pipe2(fds.as_mut_ptr(), libc::O_CLOEXEC | libc::O_NONBLOCK) != <span class="number">0 </span>{
    ...
    <span class="attr">#[cfg(any(
        ...
        <span class="comment">// For platforms that don't have `pipe2(2)` we need to manually set the
        // correct flags on the file descriptor.
        </span><span class="kw">if </span>libc::pipe(fds.as_mut_ptr()) != <span class="number">0 </span>{
            <span class="kw">return </span><span class="prelude-val">Err</span>(io::Error::last_os_error());
        }

        <span class="kw">for </span>fd <span class="kw">in </span><span class="kw-2">&amp;</span>fds {
            <span class="kw">if </span>libc::fcntl(<span class="kw-2">*</span>fd, libc::F_SETFL, libc::O_NONBLOCK) != <span class="number">0
                </span>|| libc::fcntl(<span class="kw-2">*</span>fd, libc::F_SETFD, libc::FD_CLOEXEC) != <span class="number">0
            </span>{
                <span class="kw">let </span>err = io::Error::last_os_error();
                <span class="comment">// Don't leak file descriptors. Can't handle closing error though.
                </span><span class="kw">let _ </span>= libc::close(fds[<span class="number">0</span>]);
                <span class="kw">let _ </span>= libc::close(fds[<span class="number">1</span>]);
    ...
    </span><span class="macro">compile_error!</span>(<span class="string">"unsupported target for `mio::unix::pipe`"</span>);

    <span class="prelude-val">Ok</span>(fds)
}

<span class="macro">cfg_os_ext!</span> {
<span class="kw">use </span>std::fs::File;
<span class="kw">use </span>std::io::{IoSlice, IoSliceMut, Read, Write};
<span class="kw">use </span>std::os::unix::io::{AsRawFd, FromRawFd, IntoRawFd};
<span class="kw">use </span>std::process::{ChildStderr, ChildStdin, ChildStdout};
...
<span class="doccomment">/// Create a new non-blocking Unix pipe.
///
...
/// # }
/// ```
</span><span class="kw">pub fn </span>new() -&gt; io::Result&lt;(Sender, Receiver)&gt; {
    <span class="kw">let </span>fds = new_raw()<span class="question-mark">?</span>;
    <span class="comment">// SAFETY: `new_raw` initialised the `fds` above.
    </span><span class="kw">let </span>r = <span class="kw">unsafe </span>{ Receiver::from_raw_fd(fds[<span class="number">0</span>]) };
    <span class="kw">let </span>w = <span class="kw">unsafe </span>{ Sender::from_raw_fd(fds[<span class="number">1</span>]) };
    <span class="prelude-val">Ok</span>((w, r))
}

<span class="doccomment">/// Sending end of an Unix pipe.
///
...
<span class="kw">impl </span>Sender {
    <span class="doccomment">/// Set the `Sender` into or out of non-blocking mode.
    </span><span class="kw">pub fn </span>set_nonblocking(<span class="kw-2">&amp;</span><span class="self">self</span>, nonblocking: bool) -&gt; io::Result&lt;()&gt; {
        set_nonblocking(<span class="self">self</span>.inner.as_raw_fd(), nonblocking)
    }
...
<span class="kw">impl </span>event::Source <span class="kw">for </span>Sender {
...
<span class="kw">impl </span>Write <span class="kw">for </span>Sender {
    <span class="kw">fn </span>write(<span class="kw-2">&amp;mut </span><span class="self">self</span>, buf: <span class="kw-2">&amp;</span>[u8]) -&gt; io::Result&lt;usize&gt; {
        <span class="self">self</span>.inner.do_io(|<span class="kw-2">mut </span>sender| sender.write(buf))
    }
    ...
    <span class="kw">fn </span>write_vectored(<span class="kw-2">&amp;mut </span><span class="self">self</span>, bufs: <span class="kw-2">&amp;</span>[IoSlice&lt;<span class="lifetime">'_</span>&gt;]) -&gt; io::Result&lt;usize&gt; {
        <span class="self">self</span>.inner.do_io(|<span class="kw-2">mut </span>sender| sender.write_vectored(bufs))
    }

    <span class="kw">fn </span>flush(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; io::Result&lt;()&gt; {
        <span class="self">self</span>.inner.do_io(|<span class="kw-2">mut </span>sender| sender.flush())
    }
}

<span class="kw">impl </span>Write <span class="kw">for </span><span class="kw-2">&amp;</span>Sender {
    <span class="kw">fn </span>write(<span class="kw-2">&amp;mut </span><span class="self">self</span>, buf: <span class="kw-2">&amp;</span>[u8]) -&gt; io::Result&lt;usize&gt; {
        <span class="self">self</span>.inner.do_io(|<span class="kw-2">mut </span>sender| sender.write(buf))
    }
...
</span><span class="kw">impl </span>From&lt;ChildStdin&gt; <span class="kw">for </span>Sender {
    <span class="kw">fn </span>from(stdin: ChildStdin) -&gt; Sender {
        <span class="comment">// Safety: `ChildStdin` is guaranteed to be a valid file descriptor.
        </span><span class="kw">unsafe </span>{ Sender::from_raw_fd(stdin.into_raw_fd()) }
    }
}

<span class="kw">impl </span>FromRawFd <span class="kw">for </span>Sender {
    <span class="kw">unsafe fn </span>from_raw_fd(fd: RawFd) -&gt; Sender {
        Sender {
...
<span class="kw">impl </span>Receiver {
    <span class="doccomment">/// Set the `Receiver` into or out of non-blocking mode.
    </span><span class="kw">pub fn </span>set_nonblocking(<span class="kw-2">&amp;</span><span class="self">self</span>, nonblocking: bool) -&gt; io::Result&lt;()&gt; {
        set_nonblocking(<span class="self">self</span>.inner.as_raw_fd(), nonblocking)
    }
...
<span class="kw">impl </span>event::Source <span class="kw">for </span>Receiver {
    <span class="kw">fn </span>register(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        registry: <span class="kw-2">&amp;</span>Registry,
        token: Token,
...
<span class="kw">impl </span>Read <span class="kw">for </span>Receiver {
    <span class="kw">fn </span>read(<span class="kw-2">&amp;mut </span><span class="self">self</span>, buf: <span class="kw-2">&amp;mut </span>[u8]) -&gt; io::Result&lt;usize&gt; {
        <span class="self">self</span>.inner.do_io(|<span class="kw-2">mut </span>sender| sender.read(buf))
    }
    ...
    <span class="kw">fn </span>read_vectored(<span class="kw-2">&amp;mut </span><span class="self">self</span>, bufs: <span class="kw-2">&amp;mut </span>[IoSliceMut&lt;<span class="lifetime">'_</span>&gt;]) -&gt; io::Result&lt;usize&gt; {
        <span class="self">self</span>.inner.do_io(|<span class="kw-2">mut </span>sender| sender.read_vectored(bufs))
    }
}

<span class="kw">impl </span>Read <span class="kw">for </span><span class="kw-2">&amp;</span>Receiver {
    <span class="kw">fn </span>read(<span class="kw-2">&amp;mut </span><span class="self">self</span>, buf: <span class="kw-2">&amp;mut </span>[u8]) -&gt; io::Result&lt;usize&gt; {
        <span class="self">self</span>.inner.do_io(|<span class="kw-2">mut </span>sender| sender.read(buf))
    }
...
///
/// The underlying pipe is **not** set to non-blocking.
</span><span class="kw">impl </span>From&lt;ChildStdout&gt; <span class="kw">for </span>Receiver {
    <span class="kw">fn </span>from(stdout: ChildStdout) -&gt; Receiver {
        <span class="comment">// Safety: `ChildStdout` is guaranteed to be a valid file descriptor.
        </span><span class="kw">unsafe </span>{ Receiver::from_raw_fd(stdout.into_raw_fd()) }
    }
}
...
///
/// The underlying pipe is **not** set to non-blocking.
</span><span class="kw">impl </span>From&lt;ChildStderr&gt; <span class="kw">for </span>Receiver {
    <span class="kw">fn </span>from(stderr: ChildStderr) -&gt; Receiver {
        <span class="comment">// Safety: `ChildStderr` is guaranteed to be a valid file descriptor.
        </span><span class="kw">unsafe </span>{ Receiver::from_raw_fd(stderr.into_raw_fd()) }
    }
}

<span class="kw">impl </span>FromRawFd <span class="kw">for </span>Receiver {
    <span class="kw">unsafe fn </span>from_raw_fd(fd: RawFd) -&gt; Receiver {
        Receiver {
            inner: IoSource::new(File::from_raw_fd(fd)),
        }
    }
...
<span class="kw">impl </span>IntoRawFd <span class="kw">for </span>Receiver {
    <span class="kw">fn </span>into_raw_fd(<span class="self">self</span>) -&gt; RawFd {
        <span class="self">self</span>.inner.into_inner().into_raw_fd()
    }
}

<span class="attr">#[cfg(not(any(target_os = <span class="string">"illumos"</span>, target_os = <span class="string">"solaris"</span>, target_os = <span class="string">"vita"</span>)))]
</span><span class="kw">fn </span>set_nonblocking(fd: RawFd, nonblocking: bool) -&gt; io::Result&lt;()&gt; {
    <span class="kw">let </span>value = nonblocking <span class="kw">as </span>libc::c_int;
    <span class="kw">if unsafe </span>{ libc::ioctl(fd, libc::FIONBIO, <span class="kw-2">&amp;</span>value) } == -<span class="number">1 </span>{
...
<span class="attr">#[cfg(any(target_os = <span class="string">"illumos"</span>, target_os = <span class="string">"solaris"</span>, target_os = <span class="string">"vita"</span>))]
</span><span class="kw">fn </span>set_nonblocking(fd: RawFd, nonblocking: bool) -&gt; io::Result&lt;()&gt; {
    ...
    <span class="kw">let </span>nflags = <span class="kw">if </span>nonblocking {
        flags | libc::O_NONBLOCK
    } <span class="kw">else </span>{
        flags &amp; !libc::O_NONBLOCK
    };

    <span class="kw">if </span>flags != nflags {
        <span class="kw">if unsafe </span>{ libc::fcntl(fd, libc::F_SETFL, nflags) } &lt; <span class="number">0 </span>{
            <span class="kw">return </span><span class="prelude-val">Err</span>(io::Error::last_os_error());
        }
    }
