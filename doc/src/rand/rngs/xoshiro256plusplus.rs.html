...
</span><span class="kw">pub struct </span>Xoshiro256PlusPlus {
    s: [u64; <span class="number">4</span>],
}

<span class="kw">impl </span>SeedableRng <span class="kw">for </span>Xoshiro256PlusPlus {
    <span class="kw">type </span>Seed = [u8; <span class="number">32</span>];

    <span class="doccomment">/// Create a new `Xoshiro256PlusPlus`.  If `seed` is entirely 0, it will be
    /// mapped to a different seed.
    </span><span class="attr">#[inline]
    </span><span class="kw">fn </span>from_seed(seed: [u8; <span class="number">32</span>]) -&gt; Xoshiro256PlusPlus {
        <span class="kw">if </span>seed.iter().all(|<span class="kw-2">&amp;</span>x| x == <span class="number">0</span>) {
            <span class="kw">return </span><span class="self">Self</span>::seed_from_u64(<span class="number">0</span>);
        }
    ...
    <span class="doccomment">/// Create a new `Xoshiro256PlusPlus` from a `u64` seed.
    ///
    /// This uses the SplitMix64 generator internally.
        ...
        <span class="kw">for </span>chunk <span class="kw">in </span>seed.as_mut().chunks_mut(<span class="number">8</span>) {
            state = state.wrapping_add(PHI);
            <span class="kw">let </span><span class="kw-2">mut </span>z = state;
            ...
            z = z ^ (z &gt;&gt; <span class="number">31</span>);
            chunk.copy_from_slice(<span class="kw-2">&amp;</span>z.to_le_bytes());
        }
        <span class="self">Self</span>::from_seed(seed)
    }
}

<span class="kw">impl </span>RngCore <span class="kw">for </span>Xoshiro256PlusPlus {
    ...
    <span class="attr">#[inline]
        ...
        <span class="kw">let </span>result_plusplus = <span class="self">self</span>.s[<span class="number">0</span>]
            .wrapping_add(<span class="self">self</span>.s[<span class="number">3</span>])
            .rotate_left(<span class="number">23</span>)
            .wrapping_add(<span class="self">self</span>.s[<span class="number">0</span>]);

        <span class="kw">let </span>t = <span class="self">self</span>.s[<span class="number">1</span>] &lt;&lt; <span class="number">17</span>;

        <span class="self">self</span>.s[<span class="number">2</span>] ^= <span class="self">self</span>.s[<span class="number">0</span>];
        <span class="self">self</span>.s[<span class="number">3</span>] ^= <span class="self">self</span>.s[<span class="number">1</span>];
        <span class="self">self</span>.s[<span class="number">1</span>] ^= <span class="self">self</span>.s[<span class="number">2</span>];
        <span class="self">self</span>.s[<span class="number">0</span>] ^= <span class="self">self</span>.s[<span class="number">3</span>];

        <span class="self">self</span>.s[<span class="number">2</span>] ^= t;

        <span class="self">self</span>.s[<span class="number">3</span>] = <span class="self">self</span>.s[<span class="number">3</span>].rotate_left(<span class="number">45</span>);
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>rng = Xoshiro256PlusPlus::from_seed(
