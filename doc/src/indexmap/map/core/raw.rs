...
<span class="kw">impl</span>&lt;K, V&gt; IndexMapCore&lt;K, V&gt; {
    ...
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>raw_entry(
        ...
        <span class="kw">match </span><span class="self">self</span>.indices.find(hash.get(), eq) {
            <span class="comment">// SAFETY: The entry is created with a live raw bucket, at the same time
            // we have a &amp;mut reference to the map, so it can not be modified further.
            </span><span class="prelude-val">Some</span>(raw_bucket) =&gt; <span class="prelude-val">Ok</span>(RawTableEntry {
