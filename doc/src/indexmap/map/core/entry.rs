...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">super</span>::raw::RawTableEntry;
<span class="kw">use </span><span class="kw">super</span>::IndexMapCore;
<span class="kw">use </span><span class="kw">crate</span>::HashValue;
<span class="kw">use </span>core::{fmt, mem};

<span class="kw">impl</span>&lt;K, V&gt; IndexMapCore&lt;K, V&gt; {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>entry(<span class="kw-2">&amp;mut </span><span class="self">self</span>, hash: HashValue, key: K) -&gt; Entry&lt;<span class="lifetime">'_</span>, K, V&gt;
    <span class="kw">where
        </span>K: Eq,
    {
        <span class="kw">match </span><span class="self">self</span>.raw_entry(hash, |k| <span class="kw-2">*</span>k == key) {
            <span class="prelude-val">Ok</span>(raw) =&gt; Entry::Occupied(OccupiedEntry { raw }),
            <span class="prelude-val">Err</span>(map) =&gt; Entry::Vacant(VacantEntry { map, hash, key }),
        }
    }
}

<span class="doccomment">/// Entry for an existing key-value pair in an [`IndexMap`][crate::IndexMap]
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; Entry&lt;<span class="lifetime">'a</span>, K, V&gt; {
    ...
    {
        <span class="kw">if let </span>Entry::Occupied(entry) = <span class="kw-2">&amp;mut </span><span class="self">self </span>{
            f(entry.get_mut());
        }
...
<span class="kw">impl</span>&lt;K: fmt::Debug, V: fmt::Debug&gt; fmt::Debug <span class="kw">for </span>Entry&lt;<span class="lifetime">'_</span>, K, V&gt; {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="kw">let </span><span class="kw-2">mut </span>tuple = f.debug_tuple(<span class="string">"Entry"</span>);
        <span class="kw">match </span><span class="self">self </span>{
...
<span class="doccomment">/// A view into an occupied entry in an [`IndexMap`][crate::IndexMap].
...
<span class="doccomment">/// A view into a vacant entry in an [`IndexMap`][crate::IndexMap].
/// It is part of the [`Entry`] enum.
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; VacantEntry&lt;<span class="lifetime">'a</span>, K, V&gt; {
    ...
    /// to the value.
        ...
        <span class="kw">let </span><span class="self">Self </span>{ map, hash, key } = <span class="self">self</span>;
        <span class="kw">let </span>i = map.insert_unique(hash, key, value);
        <span class="kw-2">&amp;mut </span>map.entries[i].value
    }

    <span class="doccomment">/// Inserts the entry's key and the given value into the map at its ordered
    ...
    ///
    /// Computes in **O(n)** time (average).
    </span><span class="kw">pub fn </span>insert_sorted(<span class="self">self</span>, value: V) -&gt; (usize, <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="kw-2">mut </span>V)
    <span class="kw">where
        </span>K: Ord,
    {
        <span class="kw">let </span>slice = <span class="kw">crate</span>::map::Slice::from_slice(<span class="kw-2">&amp;</span><span class="self">self</span>.map.entries);
        <span class="kw">let </span>i = slice.binary_search_keys(<span class="kw-2">&amp;</span><span class="self">self</span>.key).unwrap_err();
        (i, <span class="self">self</span>.shift_insert(i, value))
    }

    <span class="doccomment">/// Inserts the entry's key and the given value into the map at the given index,
    /// shifting others to the right, and returns a mutable reference to the value.
    ///
    /// ***Panics*** if `index` is out of bounds.
    ///
    /// Computes in **O(n)** time (average).
    </span><span class="kw">pub fn </span>shift_insert(<span class="self">self</span>, index: usize, value: V) -&gt; <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="kw-2">mut </span>V {
        <span class="kw">let </span><span class="self">Self </span>{ map, hash, key } = <span class="self">self</span>;
        map.shift_insert_unique(index, hash, key, value);
        <span class="kw-2">&amp;mut </span>map.entries[index].value
    }
}
...
<span class="doccomment">/// A view into an occupied entry in an [`IndexMap`][crate::IndexMap] obtained by index.
///
/// This `struct` is created from the [`get_index_entry`][crate::IndexMap::get_index_entry] method.
</span><span class="kw">pub struct </span>IndexedEntry&lt;<span class="lifetime">'a</span>, K, V&gt; {
    map: <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="kw-2">mut </span>IndexMapCore&lt;K, V&gt;,
    <span class="comment">// We have a mutable reference to the map, which keeps the index
    // valid and pointing to the correct entry.
    </span>index: usize,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, K, V&gt; IndexedEntry&lt;<span class="lifetime">'a</span>, K, V&gt; {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new(map: <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="kw-2">mut </span>IndexMapCore&lt;K, V&gt;, index: usize) -&gt; <span class="self">Self </span>{
    ...
    /// Computes in **O(1)** time (average).
        ...
        <span class="self">self</span>.map.swap_remove_index(<span class="self">self</span>.index).unwrap()
    }
    ...
    /// Computes in **O(n)** time (average).
        ...
        <span class="self">self</span>.map.shift_remove_index(<span class="self">self</span>.index).unwrap()
    }
    ...
    /// Computes in **O(n)** time (average).
        ...
        <span class="self">self</span>.map.move_index(<span class="self">self</span>.index, to);
    }
    ...
    /// Computes in **O(1)** time (average).
        ...
        <span class="self">self</span>.map.swap_indices(<span class="self">self</span>.index, other)
    }
}

<span class="kw">impl</span>&lt;K: fmt::Debug, V: fmt::Debug&gt; fmt::Debug <span class="kw">for </span>IndexedEntry&lt;<span class="lifetime">'_</span>, K, V&gt; {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        f.debug_struct(<span class="string">"IndexedEntry"</span>)
