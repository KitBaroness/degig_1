...
</pre></div><pre class="rust"><code><span class="doccomment">//! The ciphertext-commitment equality sigma proof system.
//!
...
</span><span class="kw">const </span>CIPHERTEXT_COMMITMENT_EQUALITY_PROOF_LEN: usize = UNIT_LEN * <span class="number">6</span>;

<span class="doccomment">/// Equality proof.
///
/// Contains all the elliptic curve and scalar components that make up the sigma protocol.
</span><span class="attr">#[allow(non_snake_case)]
#[derive(Clone)]
</span><span class="kw">pub struct </span>CiphertextCommitmentEqualityProof {
...
#[cfg(not(target_os = <span class="string">"solana"</span>))]
</span><span class="kw">impl </span>CiphertextCommitmentEqualityProof {
    <span class="doccomment">/// Creates a ciphertext-commitment equality proof.
    ///
    ...
    <span class="doccomment">/// Verifies a ciphertext-commitment equality proof.
    ///
    ...
    /// * `transcript` - The transcript that does the bookkeeping for the Fiat-Shamir heuristic
        ...
        source_ciphertext: <span class="kw-2">&amp;</span>ElGamalCiphertext,
        destination_commitment: <span class="kw-2">&amp;</span>PedersenCommitment,
        transcript: <span class="kw-2">&amp;mut </span>Transcript,
        ...
        <span class="kw">let </span>D_source = source_ciphertext.handle.get_point();
        <span class="kw">let </span>C_destination = destination_commitment.get_point();
    ...
    <span class="kw">pub fn </span>to_bytes(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; [u8; CIPHERTEXT_COMMITMENT_EQUALITY_PROOF_LEN] {
        <span class="kw">let </span><span class="kw-2">mut </span>buf = [<span class="number">0_u8</span>; CIPHERTEXT_COMMITMENT_EQUALITY_PROOF_LEN];
    ...
    <span class="kw">pub fn </span>from_bytes(bytes: <span class="kw-2">&amp;</span>[u8]) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, EqualityProofVerificationError&gt; {
        ...
        <span class="prelude-val">Ok</span>(CiphertextCommitmentEqualityProof {
            Y_0,
            Y_1,
            Y_2,
            z_s,
...
<span class="attr">#[cfg(test)]
    ...
    <span class="kw">use </span>{
        ...
        <span class="kw">crate</span>::encryption::{elgamal::ElGamalSecretKey, pedersen::Pedersen},
    };

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_ciphertext_commitment_equality_proof_correctness() {
        ...
        <span class="kw">let </span>(destination_commitment, destination_opening) = Pedersen::new(message);
        ...
        <span class="macro">assert!</span>(proof
            .verify(
                ...
                <span class="kw-2">&amp;</span>source_ciphertext,
                <span class="kw-2">&amp;</span>destination_commitment,
        ...
        <span class="kw">let </span>encrypted_message: u64 = <span class="number">55</span>;
        <span class="kw">let </span>committed_message: u64 = <span class="number">77</span>;

        <span class="kw">let </span>source_ciphertext = source_keypair.pubkey().encrypt(encrypted_message);
        <span class="kw">let </span>(destination_commitment, destination_opening) = Pedersen::new(committed_message);
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_ciphertext_commitment_equality_proof_edge_cases() {
        <span class="comment">// if ElGamal public key zero (public key is invalid), then the proof should always reject
        </span><span class="kw">let </span>public = ElGamalPubkey::from_bytes(<span class="kw-2">&amp;</span>[<span class="number">0u8</span>; <span class="number">32</span>]).unwrap();
        <span class="kw">let </span>secret = ElGamalSecretKey::new_rand();

        <span class="kw">let </span>elgamal_keypair = ElGamalKeypair::new_for_tests(public, secret);

        <span class="kw">let </span>message: u64 = <span class="number">55</span>;
        <span class="kw">let </span>ciphertext = elgamal_keypair.pubkey().encrypt(message);
        <span class="kw">let </span>(commitment, opening) = Pedersen::new(message);

        <span class="kw">let </span><span class="kw-2">mut </span>prover_transcript = Transcript::new(<span class="string">b"Test"</span>);
        <span class="kw">let </span><span class="kw-2">mut </span>verifier_transcript = Transcript::new(<span class="string">b"Test"</span>);

        <span class="kw">let </span>proof = CiphertextCommitmentEqualityProof::new(
            <span class="kw-2">&amp;</span>elgamal_keypair,
        ...
        <span class="comment">// if ciphertext is all-zero (valid commitment of 0) and commitment is also all-zero, then
        // the proof should still accept
        </span><span class="kw">let </span>elgamal_keypair = ElGamalKeypair::new_rand();

        <span class="kw">let </span>message: u64 = <span class="number">0</span>;
        <span class="kw">let </span>ciphertext = ElGamalCiphertext::from_bytes(<span class="kw-2">&amp;</span>[<span class="number">0u8</span>; <span class="number">64</span>]).unwrap();
        ...
        <span class="comment">// if commitment is all-zero and the ciphertext is a correct encryption of 0, then the
        ...
        <span class="comment">// if ciphertext is all zero and commitment correctly encodes 0, then the proof should
        // still accept
        </span><span class="kw">let </span>elgamal_keypair = ElGamalKeypair::new_rand();
