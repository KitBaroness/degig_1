...
</pre></div><pre class="rust"><code><span class="doccomment">//! The discrete log implementation for the twisted ElGamal decryption.
//!
...
/// The goal of discrete log is to find x such that x * generator = target.
    ...
    </span>compression_batch_size: usize,
}
...
<span class="doccomment">/// Solves the discrete log instance using a 16/16 bit offline/online split
    ...
    /// Default number of threads set to 1.
        ...
        <span class="self">Self </span>{
            ...
            range_bound: TWO16 <span class="kw">as </span>usize,
            step_point: G,
            compression_batch_size: <span class="number">32</span>,
        }
    }

    <span class="doccomment">/// Adjusts number of threads in a discrete log instance.
    </span><span class="kw">pub fn </span>num_threads(<span class="kw-2">&amp;mut </span><span class="self">self</span>, num_threads: usize) -&gt; <span class="prelude-ty">Result</span>&lt;(), DiscreteLogError&gt; {
        <span class="comment">// number of threads must be a positive power-of-two integer
        </span><span class="kw">if </span>num_threads == <span class="number">0 </span>|| (num_threads &amp; (num_threads - <span class="number">1</span>)) != <span class="number">0 </span>|| num_threads &gt; MAX_THREAD {
        ...
        <span class="self">self</span>.range_bound = (TWO16 <span class="kw">as </span>usize).checked_div(num_threads).unwrap();
        <span class="self">self</span>.step_point = Scalar::from(num_threads <span class="kw">as </span>u64) * G;
    ...
    <span class="doccomment">/// Adjusts inversion batch size in a discrete log instance.
        ...
        <span class="kw">if </span>compression_batch_size &gt;= TWO16 <span class="kw">as </span>usize || compression_batch_size == <span class="number">0 </span>{
    ...
    /// is a positive 32-bit number.
        ...
        <span class="kw">let </span>handles = (<span class="number">0</span>..<span class="self">self</span>.num_threads)
            .map(|i| {
                <span class="kw">let </span>ristretto_iterator = RistrettoIterator::new(
                    (starting_point, i <span class="kw">as </span>u64),
                    (-(<span class="kw-2">&amp;</span><span class="self">self</span>.step_point), <span class="self">self</span>.num_threads <span class="kw">as </span>u64),
                );
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>solution = <span class="prelude-val">None</span>;
        <span class="kw">for </span>handle <span class="kw">in </span>handles {
            <span class="kw">let </span>discrete_log = handle.join().unwrap();
            <span class="kw">if </span>discrete_log.is_some() {
