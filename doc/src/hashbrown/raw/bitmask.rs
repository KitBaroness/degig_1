...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">super</span>::imp::{
...
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">struct </span>BitMask(<span class="kw">pub</span>(<span class="kw">crate</span>) BitMaskWord);
...
<span class="attr">#[allow(clippy::use_self)]
    ...
    #[must_use]
    #[allow(dead_code)]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>invert(<span class="self">self</span>) -&gt; <span class="self">Self </span>{
    ...
    #[must_use]
    </span><span class="kw">fn </span>remove_lowest_bit(<span class="self">self</span>) -&gt; <span class="self">Self </span>{
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>any_bit_set(<span class="self">self</span>) -&gt; bool {
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>lowest_set_bit(<span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;usize&gt; {
        <span class="kw">if let </span><span class="prelude-val">Some</span>(nonzero) = NonZeroBitMaskWord::new(<span class="self">self</span>.<span class="number">0</span>) {
            <span class="prelude-val">Some</span>(<span class="self">Self</span>::nonzero_trailing_zeros(nonzero))
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>trailing_zeros(<span class="self">self</span>) -&gt; usize {
    ...
    <span class="doccomment">/// Same as above but takes a `NonZeroBitMaskWord`.
    </span><span class="attr">#[inline]
    </span><span class="kw">fn </span>nonzero_trailing_zeros(nonzero: NonZeroBitMaskWord) -&gt; usize {
        <span class="kw">if </span><span class="macro">cfg!</span>(target_arch = <span class="string">"arm"</span>) &amp;&amp; BITMASK_STRIDE % <span class="number">8 </span>== <span class="number">0 </span>{
            <span class="comment">// SAFETY: A byte-swapped non-zero value is still non-zero.
            </span><span class="kw">let </span>swapped = <span class="kw">unsafe </span>{ NonZeroBitMaskWord::new_unchecked(nonzero.get().swap_bytes()) };
            swapped.leading_zeros() <span class="kw">as </span>usize / BITMASK_STRIDE
        } <span class="kw">else </span>{
            nonzero.trailing_zeros() <span class="kw">as </span>usize / BITMASK_STRIDE
        }
    }

    <span class="doccomment">/// Returns the number of leading zeroes in the `BitMask`.
    </span><span class="attr">#[inline]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>leading_zeros(<span class="self">self</span>) -&gt; usize {
...
<span class="kw">impl </span>IntoIterator <span class="kw">for </span>BitMask {
    ...
    <span class="attr">#[inline]
        ...
        <span class="comment">// A BitMask only requires each element (group of bits) to be non-zero.
        // However for iteration we need each element to only contain 1 bit.
        </span>BitMaskIter(BitMask(<span class="self">self</span>.<span class="number">0 </span>&amp; BITMASK_ITER_MASK))
    }
}

<span class="doccomment">/// Iterator over the contents of a `BitMask`, returning the indices of set
/// bits.
</span><span class="attr">#[derive(Copy, Clone)]
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">struct </span>BitMaskIter(<span class="kw">pub</span>(<span class="kw">crate</span>) BitMask);

<span class="kw">impl </span>BitMaskIter {
    ...
    #[cfg(feature = <span class="string">"raw"</span>)]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">unsafe fn </span>flip(<span class="kw-2">&amp;mut </span><span class="self">self</span>, index: usize) -&gt; bool {
        ...
        <span class="self">self</span>.<span class="number">0 </span>.<span class="number">0 </span>^= mask;
        <span class="comment">// The bit was set if the bit is now 0.
        </span><span class="self">self</span>.<span class="number">0 </span>.<span class="number">0 </span>&amp; mask == <span class="number">0
    </span>}
}
