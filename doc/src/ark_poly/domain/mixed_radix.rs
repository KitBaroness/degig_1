...
</pre></div><pre class="rust"><code><span class="doccomment">//! This module contains a `MixedRadixEvaluationDomain` for
...
<span class="kw">use </span>ark_ff::{fields::utils::k_adicity, FftField};
<span class="kw">use </span>ark_serialize::{CanonicalDeserialize, CanonicalSerialize};
<span class="kw">use </span>ark_std::{cmp::min, convert::TryFrom, fmt, vec::Vec};
...
</span><span class="kw">pub struct </span>MixedRadixEvaluationDomain&lt;F: FftField&gt; {
...
<span class="kw">impl</span>&lt;F: FftField&gt; fmt::Debug <span class="kw">for </span>MixedRadixEvaluationDomain&lt;F&gt; {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="macro">write!</span>(
            f,
            <span class="string">"Mixed-radix multiplicative subgroup of size {}"</span>,
            <span class="self">self</span>.size
        )
    }
}

<span class="kw">impl</span>&lt;F: FftField&gt; EvaluationDomain&lt;F&gt; <span class="kw">for </span>MixedRadixEvaluationDomain&lt;F&gt; {
    ...
    /// having `num_coeffs` coefficients.
        ...
        <span class="comment">// Compute the best size of our evaluation domain.
        </span><span class="kw">let </span>size = best_mixed_domain_size::&lt;F&gt;(num_coeffs) <span class="kw">as </span>u64;
        <span class="kw">let </span>small_subgroup_base = F::SMALL_SUBGROUP_BASE<span class="question-mark">?</span>;

        <span class="comment">// Compute the size of our evaluation domain
        </span><span class="kw">let </span>q = u64::from(small_subgroup_base);
        <span class="kw">let </span>q_adicity = k_adicity(q, size);
        <span class="kw">let </span>q_part = q.checked_pow(q_adicity)<span class="question-mark">?</span>;

        <span class="kw">let </span>two_adicity = k_adicity(<span class="number">2</span>, size);
        <span class="kw">let </span>log_size_of_group = two_adicity;
        <span class="kw">let </span>two_part = <span class="number">2u64</span>.checked_pow(two_adicity)<span class="question-mark">?</span>;

        <span class="kw">if </span>size != q_part * two_part {
        ...
        <span class="comment">// Check that it is indeed the requested root of unity.
        ...
        <span class="prelude-val">Some</span>(MixedRadixEvaluationDomain {
            size,
            log_size_of_group,
    ...
    <span class="kw">fn </span>compute_size_of_domain(num_coeffs: usize) -&gt; <span class="prelude-ty">Option</span>&lt;usize&gt; {
        ...
        <span class="comment">// Compute the best size of our evaluation domain.
        </span><span class="kw">let </span>num_coeffs = best_mixed_domain_size::&lt;F&gt;(num_coeffs) <span class="kw">as </span>u64;

        <span class="kw">let </span>q = u64::from(small_subgroup_base);
        <span class="kw">let </span>q_adicity = k_adicity(q, num_coeffs);
        <span class="kw">let </span>q_part = q.checked_pow(q_adicity)<span class="question-mark">?</span>;

        <span class="kw">let </span>two_adicity = k_adicity(<span class="number">2</span>, num_coeffs);
        <span class="kw">let </span>two_part = <span class="number">2u64</span>.checked_pow(two_adicity)<span class="question-mark">?</span>;

        <span class="kw">if </span>num_coeffs == q_part * two_part {
            <span class="prelude-val">Some</span>(num_coeffs <span class="kw">as </span>usize)
    ...
    <span class="attr">#[inline]
        ...
        <span class="kw">if </span>!<span class="self">self</span>.offset.is_one() {
            <span class="self">Self</span>::distribute_powers(coeffs, <span class="self">self</span>.offset);
        }
        coeffs.resize(<span class="self">self</span>.size(), T::zero());
        best_fft(
            coeffs,
            <span class="self">self</span>.group_gen,
            <span class="self">self</span>.log_size_of_group,
    ...
    <span class="attr">#[inline]
        ...
        <span class="kw">if </span><span class="self">self</span>.offset.is_one() {
            <span class="macro">ark_std::cfg_iter_mut!</span>(evals).for_each(|val| <span class="kw-2">*</span>val <span class="kw-2">*</span>= <span class="self">self</span>.size_inv);
        } <span class="kw">else </span>{
            <span class="self">Self</span>::distribute_powers_and_mul_by_const(evals, <span class="self">self</span>.offset_inv, <span class="self">self</span>.size_inv);
        }
    }
...
<span class="kw">fn </span>mixed_radix_fft_permute(
    two_adicity: u32,
    q_adicity: u32,
    q: usize,
    n: usize,
    ...
    <span class="comment">// This is the permutation obtained by splitting into 2 groups two_adicity times
    ...
    // q^q_adicity)
    </span><span class="kw">let </span><span class="kw-2">mut </span>res = <span class="number">0</span>;
    <span class="kw">let </span><span class="kw-2">mut </span>shift = n;

    <span class="kw">for _ in </span><span class="number">0</span>..two_adicity {
        shift /= <span class="number">2</span>;
        res += (i % <span class="number">2</span>) * shift;
        i /= <span class="number">2</span>;
    }

    <span class="kw">for _ in </span><span class="number">0</span>..q_adicity {
        shift /= q;
        res += (i % q) * shift;
        i /= q;
    }
...
<span class="kw">fn </span>best_mixed_domain_size&lt;F: FftField&gt;(min_size: usize) -&gt; usize {
    <span class="kw">let </span><span class="kw-2">mut </span>best = usize::max_value();
    <span class="kw">let </span>small_subgroup_base_adicity = F::SMALL_SUBGROUP_BASE_ADICITY.unwrap();
    <span class="kw">let </span>small_subgroup_base = usize::try_from(F::SMALL_SUBGROUP_BASE.unwrap()).unwrap();

    <span class="kw">for </span>b <span class="kw">in </span><span class="number">0</span>..=small_subgroup_base_adicity {
        <span class="kw">let </span><span class="kw-2">mut </span>r = small_subgroup_base.pow(b);

        <span class="kw">let </span><span class="kw-2">mut </span>two_adicity = <span class="number">0</span>;
        <span class="kw">while </span>r &lt; min_size {
            r <span class="kw-2">*</span>= <span class="number">2</span>;
            two_adicity += <span class="number">1</span>;
        }

        <span class="kw">if </span>two_adicity &lt;= F::TWO_ADICITY {
            best = min(best, r);
        }
    }
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>serial_mixed_radix_fft&lt;T: DomainCoeff&lt;F&gt;, F: FftField&gt;(
    a: <span class="kw-2">&amp;mut </span>[T],
    omega: F,
    two_adicity: u32,
) {
    <span class="comment">// Conceptually, this FFT first splits into 2 sub-arrays two_adicity many times,
    // and then splits into q sub-arrays q_adicity many times.

    </span><span class="kw">let </span>n = a.len();
    <span class="kw">let </span>q = usize::try_from(F::SMALL_SUBGROUP_BASE.unwrap()).unwrap();
    <span class="kw">let </span>q_u64 = u64::from(F::SMALL_SUBGROUP_BASE.unwrap());
    <span class="kw">let </span>n_u64 = n <span class="kw">as </span>u64;

    <span class="kw">let </span>q_adicity = k_adicity(q_u64, n_u64);
    <span class="kw">let </span>q_part = q_u64.checked_pow(q_adicity).unwrap();
    <span class="kw">let </span>two_part = <span class="number">2u64</span>.checked_pow(two_adicity).unwrap();

    <span class="macro">assert_eq!</span>(n_u64, q_part * two_part);

    <span class="kw">let </span><span class="kw-2">mut </span>m = <span class="number">1</span>; <span class="comment">// invariant: m = 2^{s-1}

    </span><span class="kw">if </span>q_adicity &gt; <span class="number">0 </span>{
        <span class="comment">// If we're using the other radix, we have to do two things differently than in
        ...
        </span><span class="kw">let </span><span class="kw-2">mut </span>seen = <span class="macro">vec!</span>[<span class="bool-val">false</span>; n];
        <span class="kw">for </span>k <span class="kw">in </span><span class="number">0</span>..n {
            <span class="kw">let </span><span class="kw-2">mut </span>i = k;
            <span class="kw">let </span><span class="kw-2">mut </span>a_i = a[i];
            <span class="kw">while </span>!seen[i] {
                <span class="kw">let </span>dest = mixed_radix_fft_permute(two_adicity, q_adicity, q, n, i);

                <span class="kw">let </span>a_dest = a[dest];
                a[dest] = a_i;

                seen[i] = <span class="bool-val">true</span>;
        ...
        <span class="kw">let </span>omega_q = omega.pow([(n / q) <span class="kw">as </span>u64]);
        <span class="kw">let </span><span class="kw-2">mut </span>qth_roots = Vec::with_capacity(q);
        qth_roots.push(F::one());
        <span class="kw">for </span>i <span class="kw">in </span><span class="number">1</span>..q {
            qth_roots.push(qth_roots[i - <span class="number">1</span>] * omega_q);
        }

        <span class="kw">let </span><span class="kw-2">mut </span>terms = <span class="macro">vec!</span>[T::zero(); q - <span class="number">1</span>];

        <span class="comment">// Doing the q_adicity passes.
        </span><span class="kw">for _ in </span><span class="number">0</span>..q_adicity {
            <span class="kw">let </span>w_m = omega.pow([(n / (q * m)) <span class="kw">as </span>u64]);
            <span class="kw">let </span><span class="kw-2">mut </span>k = <span class="number">0</span>;
            <span class="kw">while </span>k &lt; n {
                <span class="kw">let </span><span class="kw-2">mut </span>w_j = F::one(); <span class="comment">// w_j is omega_m ^ j
                </span><span class="kw">for </span>j <span class="kw">in </span><span class="number">0</span>..m {
                    <span class="kw">let </span>base_term = a[k + j];
                    <span class="kw">let </span><span class="kw-2">mut </span>w_j_i = w_j;
                    <span class="kw">for </span>i <span class="kw">in </span><span class="number">1</span>..q {
                        terms[i - <span class="number">1</span>] = a[k + j + i * m];
                        terms[i - <span class="number">1</span>] <span class="kw-2">*</span>= w_j_i;
                        w_j_i <span class="kw-2">*</span>= w_j;
                    }

                    <span class="kw">for </span>i <span class="kw">in </span><span class="number">0</span>..q {
                        a[k + j + i * m] = base_term;
                        <span class="kw">for </span>l <span class="kw">in </span><span class="number">1</span>..q {
                            <span class="kw">let </span><span class="kw-2">mut </span>tmp = terms[l - <span class="number">1</span>];
                            tmp <span class="kw-2">*</span>= qth_roots[(i * l) % q];
                            a[k + j + i * m] += tmp;
                        }
                    }

                    w_j <span class="kw-2">*</span>= w_m;
                }

                k += q * m;
            }
            m <span class="kw-2">*</span>= q;
        }
    } <span class="kw">else </span>{
        <span class="comment">// swapping in place (from Storer's book)
        </span><span class="kw">for </span>k <span class="kw">in </span><span class="number">0</span>..n {
            <span class="kw">let </span>rk = bitreverse(k <span class="kw">as </span>u32, two_adicity) <span class="kw">as </span>usize;
...
<span class="attr">#[cfg(test)]
    ...
    <span class="kw">use </span>ark_test_curves::bn384_small_two_adicity::Fq <span class="kw">as </span>Fr;
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">for </span>coeffs <span class="kw">in </span><span class="number">0</span>..<span class="number">12 </span>{
            <span class="kw">let </span>domain = MixedRadixEvaluationDomain::&lt;Fr&gt;::new(coeffs).unwrap();
            ...
            <span class="kw">let </span>coset_z = coset_domain.vanishing_polynomial();
            <span class="kw">for _ in </span><span class="number">0</span>..<span class="number">100 </span>{
                ...
                <span class="macro">assert_eq!</span>(
                    coset_z.evaluate(<span class="kw-2">&amp;</span>point),
    ...
    /// point works.
        ...
        <span class="kw">for </span>domain_dim <span class="kw">in </span><span class="number">1</span>..<span class="number">10 </span>{
            ...
            <span class="kw">let </span>domain = MixedRadixEvaluationDomain::&lt;Fr&gt;::new(domain_size).unwrap();
            <span class="kw">let </span>coset_domain = domain.get_coset(Fr::GENERATOR).unwrap();
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">for </span>coeffs <span class="kw">in </span><span class="number">1</span>..<span class="number">12 </span>{
            <span class="kw">let </span>size = <span class="number">1 </span>&lt;&lt; coeffs;
            <span class="kw">let </span>domain = MixedRadixEvaluationDomain::&lt;Fr&gt;::new(size).unwrap();
    ...
    #[cfg(feature = <span class="string">"parallel"</span>)]
        ...
        <span class="kw">use </span><span class="kw">super</span>::serial_mixed_radix_fft;
        <span class="kw">use </span><span class="kw">crate</span>::domain::utils::parallel_fft;
        ...
        <span class="kw">fn </span>test_consistency&lt;F: PrimeField, R: Rng&gt;(rng: <span class="kw-2">&amp;mut </span>R, max_coeffs: u32) {
            <span class="kw">for _ in </span><span class="number">0</span>..<span class="number">5 </span>{
                <span class="kw">for </span>log_d <span class="kw">in </span><span class="number">0</span>..max_coeffs {
                    ...
                    <span class="kw">let </span><span class="kw-2">mut </span>v1 = (<span class="number">0</span>..d).map(|<span class="kw">_</span>| F::rand(rng)).collect::&lt;Vec&lt;<span class="kw">_</span>&gt;&gt;();
                    <span class="kw">let </span><span class="kw-2">mut </span>v2 = v1.clone();

                    <span class="kw">let </span>domain = MixedRadixEvaluationDomain::new(v1.len()).unwrap();

                    <span class="kw">for </span>log_cpus <span class="kw">in </span>log_d..min(log_d + <span class="number">1</span>, <span class="number">3</span>) {
                        parallel_fft::&lt;F, F&gt;(
                            <span class="kw-2">&amp;mut </span>v1,
                            domain.group_gen,
                            log_d,
                            log_cpus,
                            serial_mixed_radix_fft::&lt;F, F&gt;,
                        );
                        serial_mixed_radix_fft::&lt;F, F&gt;(<span class="kw-2">&amp;mut </span>v2, domain.group_gen, log_d);
        ...
        test_consistency::&lt;Fr, <span class="kw">_</span>&gt;(rng, <span class="number">16</span>);
    }
}
