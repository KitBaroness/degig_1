...
</pre></div><pre class="rust"><code><span class="doccomment">//! Streaming decompression functionality.
...
<span class="kw">impl </span>HuffmanTable {
    ...
    <span class="doccomment">/// Get the symbol and the code length from the huffman tree.
        ...
        // symbol.
            ...
            </span><span class="kw">let </span>tree_index = (!symbol + ((bit_buf &gt;&gt; code_len) &amp; <span class="number">1</span>) <span class="kw">as </span>i32) <span class="kw">as </span>usize;
            <span class="macro">debug_assert!</span>(tree_index &lt; <span class="self">self</span>.tree.len());
            <span class="kw">if </span>tree_index &gt;= <span class="self">self</span>.tree.len() {
                <span class="kw">break</span>;
            }
            symbol = i32::from(<span class="self">self</span>.tree[tree_index]);
    ...
    </span>F: FnOnce(<span class="kw-2">&amp;mut </span>DecompressorOxide, <span class="kw-2">&amp;mut </span>LocalVars, i32) -&gt; Action,
{
    ...
    <span class="comment">// If the symbol was found in the fast lookup table.
        ...
        code_len = res.<span class="number">1</span>;
    };
...
<span class="kw">fn </span>init_tree(r: <span class="kw-2">&amp;mut </span>DecompressorOxide, l: <span class="kw-2">&amp;mut </span>LocalVars) -&gt; <span class="prelude-ty">Option</span>&lt;Action&gt; {
    <span class="kw">loop </span>{
        <span class="kw">let </span>bt = r.block_type <span class="kw">as </span>usize;
        <span class="kw">if </span>bt &gt;= r.tables.len() {
            <span class="kw">return </span><span class="prelude-val">None</span>;
        }
        <span class="kw">let </span>table = <span class="kw-2">&amp;mut </span>r.tables[bt];
        <span class="kw">let </span>table_size = r.table_sizes[bt] <span class="kw">as </span>usize;
        <span class="kw">if </span>table_size &gt; table.code_size.len() {
            <span class="kw">return </span><span class="prelude-val">None</span>;
        }
        ...
        <span class="kw">for </span><span class="kw-2">&amp;</span>code_size <span class="kw">in </span><span class="kw-2">&amp;</span>table.code_size[..table_size] {
            <span class="kw">let </span>cs = code_size <span class="kw">as </span>usize;
            <span class="kw">if </span>cs &gt;= total_symbols.len() {
                <span class="kw">return </span><span class="prelude-val">None</span>;
            }
            total_symbols[cs] += <span class="number">1</span>;
        }
        ...
        <span class="kw">for </span>(ts, next) <span class="kw">in </span>total_symbols
            .iter()
            .copied()
            .zip(next_code.iter_mut().skip(<span class="number">1</span>))
            .skip(<span class="number">1</span>)
        {
            used_symbols += ts;
            total += ts;
            total &lt;&lt;= <span class="number">1</span>;
            <span class="kw-2">*</span>next = total;
        }

        <span class="kw">if </span>total != <span class="number">65_536 </span>&amp;&amp; used_symbols &gt; <span class="number">1 </span>{
            <span class="kw">return </span><span class="prelude-val">Some</span>(Action::Jump(BadTotalSymbols));
        }
        ...
        <span class="kw">for </span>symbol_index <span class="kw">in </span><span class="number">0</span>..table_size {
            ...
            <span class="kw">if </span>code_size == <span class="number">0 </span>|| usize::from(code_size) &gt;= next_code.len() {
            ...
            <span class="kw">if </span>tree_cur == <span class="number">0 </span>{
                table.look_up[(rev_code &amp; (FAST_LOOKUP_SIZE - <span class="number">1</span>)) <span class="kw">as </span>usize] = tree_next;
                tree_cur = tree_next;
            ...
            <span class="kw">for _ in </span>FAST_LOOKUP_BITS + <span class="number">1</span>..code_size {
                ...
                <span class="kw">let </span>tree_index = (-tree_cur - <span class="number">1</span>) <span class="kw">as </span>usize;
                <span class="kw">if </span>tree_index &gt;= table.tree.len() {
                    <span class="kw">return </span><span class="prelude-val">None</span>;
                }
                <span class="kw">if </span>table.tree[tree_index] == <span class="number">0 </span>{
            ...
            table.tree[tree_index] = symbol_index <span class="kw">as </span>i16;
        }

        <span class="kw">if </span>r.block_type == <span class="number">2 </span>{
            l.counter = <span class="number">0</span>;
            <span class="kw">return </span><span class="prelude-val">Some</span>(Action::Jump(ReadLitlenDistTablesCodeSize));
        }
    ...
    l.counter = <span class="number">0</span>;
    <span class="prelude-val">Some</span>(Action::Jump(DecodeLitlen))
}
...
<span class="doccomment">/// Presumes that there is at least match_len bytes in output left.
    ...
    <span class="macro">debug_assert!</span>(out_pos.checked_add(match_len).unwrap() &lt;= out_slice.len());
    ...
    <span class="kw">if </span>match_len == <span class="number">3 </span>{
        <span class="kw">let </span>out_slice = Cell::from_mut(out_slice).as_slice_of_cells();
        <span class="kw">if let </span><span class="prelude-val">Some</span>(dst) = out_slice.get(out_pos..out_pos + <span class="number">3</span>) {
            <span class="comment">// Moving bounds checks before any memory mutation allows the optimizer
            // combine them together.
            </span><span class="kw">let </span>src = out_slice
                .get(source_pos)
                .zip(out_slice.get((source_pos + <span class="number">1</span>) &amp; out_buf_size_mask))
                .zip(out_slice.get((source_pos + <span class="number">2</span>) &amp; out_buf_size_mask));
            <span class="kw">if let </span><span class="prelude-val">Some</span>(((a, b), c)) = src {
                <span class="comment">// For correctness, the memory reads and writes have to be interleaved.
                // Cells make it possible for read and write references to overlap.
                </span>dst[<span class="number">0</span>].set(a.get());
                dst[<span class="number">1</span>].set(b.get());
                dst[<span class="number">2</span>].set(c.get());
            }
        }
        ...
        <span class="kw">match </span>state {
            ...
            RawMemcpy2 =&gt; <span class="macro">generate_state!</span>(state, <span class="lifetime">'state_machine</span>, {
                <span class="kw">if </span>in_iter.len() &gt; <span class="number">0 </span>{
                    ...
                    in_iter.nth(bytes_to_copy - <span class="number">1</span>);
            ...
            // to decode the lengths of the main tables.
                ...
                <span class="kw">if </span>l.counter &lt; r.table_sizes[HUFFLEN_TABLE] {
                    ...
                    init_tree(r, <span class="kw-2">&amp;mut </span>l).unwrap_or(Action::End(TINFLStatus::Failed))
                }
            }),
            ...
            HuffDecodeOuterLoop2 =&gt; <span class="macro">generate_state!</span>(state, <span class="lifetime">'state_machine</span>, {
                    ...
                    (flags &amp; TINFL_FLAG_USING_NON_WRAPPING_OUTPUT_BUF != <span class="number">0</span>)
                {
                    ...
                    <span class="kw">let </span>out_len = out_buf.get_ref().len();
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>b_buf = [<span class="number">0</span>; LEN];

        <span class="comment">// This should fail with the out buffer being to small.
        </span><span class="kw">let </span>b_status = tinfl_decompress_oxide(<span class="kw-2">&amp;mut </span>b, <span class="kw-2">&amp;</span>encoded[..], <span class="kw-2">&amp;mut </span>b_buf, flags);
    ...
    #[test]
        ...
        <span class="kw">let </span>b_status = tinfl_decompress_oxide(<span class="kw-2">&amp;mut </span>b, <span class="kw-2">&amp;</span>encoded[..], <span class="kw-2">&amp;mut </span>b_buf, flags);
    ...
    <span class="attr">#[test]
        ...
        init_tree(<span class="kw-2">&amp;mut </span>d, <span class="kw-2">&amp;mut </span>l).unwrap();
    ...
    <span class="comment">// Only run this test with alloc enabled as it uses a larger buffer.
    </span><span class="attr">#[cfg(feature = <span class="string">"with-alloc"</span>)]
    </span><span class="kw">fn </span>check_result(input: <span class="kw-2">&amp;</span>[u8], expected_status: TINFLStatus, expected_state: State, zlib: bool) {
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>output_buf: [u8; <span class="number">0</span>] = [];
