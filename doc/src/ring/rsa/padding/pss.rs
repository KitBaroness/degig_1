...
</span><span class="kw">use super</span>::{<span class="kw">super</span>::PUBLIC_KEY_PUBLIC_MODULUS_MAX_LEN, mgf1, Padding, RsaEncoding, Verification};
<span class="kw">use crate</span>::{bits, digest, error, rand};
...
</span><span class="attr">#[allow(clippy::upper_case_acronyms)] </span><span class="comment">// TODO: Until we implement cargo-semver-checks
...
<span class="kw">impl </span>RsaEncoding <span class="kw">for </span>PSS {
    ...
    // https://tools.ietf.org/html/rfc3447#section-9.1.
        ...
        </span><span class="kw">let </span>(db, digest_terminator) = em.split_at_mut(metrics.db_len);

        <span class="kw">let </span>separator_pos = db.len() - <span class="number">1 </span>- metrics.s_len;

        <span class="comment">// Step 4.
        </span><span class="kw">let </span>salt: <span class="kw-2">&amp;</span>[u8] = {
            <span class="kw">let </span>salt = <span class="kw-2">&amp;mut </span>db[(separator_pos + <span class="number">1</span>)..];
            rng.fill(salt)<span class="question-mark">?</span>; <span class="comment">// salt
            </span>salt
        };

        <span class="comment">// Steps 5 and 6.
        </span><span class="kw">let </span>h = pss_digest(<span class="self">self</span>.digest_alg, m_hash, salt);

        <span class="comment">// Step 7.
        </span>db[..separator_pos].fill(<span class="number">0</span>); <span class="comment">// ps

        // Step 8.
        </span>db[separator_pos] = <span class="number">0x01</span>;

        <span class="comment">// Steps 9 and 10.
        </span>mgf1(<span class="self">self</span>.digest_alg, h.as_ref(), db);

        <span class="comment">// Step 11.
        </span>db[<span class="number">0</span>] &amp;= metrics.top_byte_mask;

        <span class="comment">// Step 12.
        </span>digest_terminator[..metrics.h_len].copy_from_slice(h.as_ref());
...
<span class="kw">impl </span>Verification <span class="kw">for </span>PSS {
    ...
    // where steps 1, 2(a), and 2(b) have been done for us.
        ...
        mgf1(<span class="self">self</span>.digest_alg, h_hash.as_slice_less_safe(), db);
        ...
        masked_db.read_all(error::Unspecified, |masked_bytes| {
            ...
            </span><span class="kw">for </span>db <span class="kw">in </span>db[<span class="number">1</span>..].iter_mut() {
                <span class="kw-2">*</span>db ^= masked_bytes.read_byte()<span class="question-mark">?</span>;
            }
            <span class="prelude-val">Ok</span>(())
        ...
        <span class="kw">if </span>db[<span class="number">0</span>..ps_len].iter().any(|<span class="kw-2">&amp;</span>db| db != <span class="number">0</span>) {
            <span class="kw">return </span><span class="prelude-val">Err</span>(error::Unspecified);
        }
        ...
        </span><span class="kw">if </span>h_hash.as_slice_less_safe() != h_prime.as_ref() {
            <span class="kw">return </span><span class="prelude-val">Err</span>(error::Unspecified);
        }
...
<span class="kw">struct </span>PSSMetrics {
    <span class="attr">#[cfg_attr(not(feature = <span class="string">"alloc"</span>), allow(dead_code))]
    </span>em_len: usize,
    db_len: usize,
    ps_len: usize,
    s_len: usize,
...
<span class="kw">impl </span>PSSMetrics {
    <span class="kw">fn </span>new(
        ...
        <span class="kw">let </span>leading_zero_bits = (<span class="number">8 </span>* em_len) - em_bits.as_bits();
        ...
        <span class="macro">debug_assert!</span>(em_bits.as_bits() &gt;= (<span class="number">8 </span>* h_len) + (<span class="number">8 </span>* s_len) + <span class="number">9</span>);
...
<span class="macro">macro_rules!</span> rsa_pss_padding {
    ( <span class="macro-nonterminal">$vis</span>:vis <span class="macro-nonterminal">$PADDING_ALGORITHM</span>:ident, <span class="macro-nonterminal">$digest_alg</span>:expr, <span class="macro-nonterminal">$doc_str</span>:expr ) =&gt; {
        <span class="attr">#[doc=<span class="macro-nonterminal">$doc_str</span>]
        </span><span class="macro-nonterminal">$vis </span><span class="kw">static </span><span class="macro-nonterminal">$PADDING_ALGORITHM</span>: PSS = PSS {
...
<span class="macro">rsa_pss_padding!</span>(
    <span class="kw">pub </span>RSA_PSS_SHA256,
...
<span class="macro">rsa_pss_padding!</span>(
    <span class="kw">pub </span>RSA_PSS_SHA384,
...
<span class="macro">rsa_pss_padding!</span>(
    <span class="kw">pub </span>RSA_PSS_SHA512,
