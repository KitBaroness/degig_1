...
<span class="kw">use </span><span class="kw">crate</span>::polyfill::{unwrap_const, ArrayFlatMap, LeadingZerosStripped};
<span class="kw">use </span>core::num::NonZeroU64;

<span class="doccomment">/// The exponent `e` of an RSA public key.
</span><span class="attr">#[derive(Clone, Copy)]
</span><span class="kw">pub struct </span>PublicExponent(NonZeroU64);

<span class="kw">impl </span>PublicExponent {
    <span class="attr">#[cfg(test)]
    </span><span class="kw">const </span>ALL_CONSTANTS: [<span class="self">Self</span>; <span class="number">3</span>] = [<span class="self">Self</span>::_3, <span class="self">Self</span>::_65537, <span class="self">Self</span>::MAX];

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">const </span>_3: <span class="self">Self </span>= <span class="self">Self</span>(unwrap_const(NonZeroU64::new(<span class="number">3</span>)));
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">const </span>_65537: <span class="self">Self </span>= <span class="self">Self</span>(unwrap_const(NonZeroU64::new(<span class="number">65537</span>)));
    ...
    </span><span class="kw">const </span>MAX: <span class="self">Self </span>= <span class="self">Self</span>(unwrap_const(NonZeroU64::new((<span class="number">1u64 </span>&lt;&lt; <span class="number">33</span>) - <span class="number">1</span>)));

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>from_be_bytes(
        input: untrusted::Input,
        min_value: <span class="self">Self</span>,
        ...
        </span><span class="kw">if </span>input.len() &gt; <span class="number">5 </span>{
        ...
        // supported.
        </span><span class="kw">let </span>value = NonZeroU64::new(value).ok_or_else(error::KeyRejected::too_small)<span class="question-mark">?</span>;
        <span class="kw">if </span>value &lt; min_value.<span class="number">0 </span>{
            <span class="kw">return </span><span class="prelude-val">Err</span>(error::KeyRejected::too_small());
        }
        <span class="kw">if </span>value &gt; <span class="self">Self</span>::MAX.<span class="number">0 </span>{
            <span class="kw">return </span><span class="prelude-val">Err</span>(error::KeyRejected::too_large());
        }

        <span class="comment">// Step 3 / Step c.
        </span><span class="kw">if </span>value.get() &amp; <span class="number">1 </span>!= <span class="number">1 </span>{
    ...
    <span class="doccomment">/// The big-endian encoding of the exponent.
    ///
    /// There are no leading zeros.
        ...
        <span class="comment">// The `unwrap()` won't fail as `self.0` is only a few bytes long.
        </span><span class="kw">let </span>bytes = ArrayFlatMap::new(core::iter::once(<span class="self">self</span>.<span class="number">0</span>.get()), u64::to_be_bytes).unwrap();
        LeadingZerosStripped::new(bytes)
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>value(<span class="self">self</span>) -&gt; NonZeroU64 {
...
<span class="attr">#[cfg(test)]
    ...
    </span><span class="kw">fn </span>test_public_exponent_constants() {
        <span class="kw">for </span>value <span class="kw">in </span>PublicExponent::ALL_CONSTANTS.iter() {
            <span class="kw">let </span>value: u64 = value.<span class="number">0</span>.into();
            <span class="macro">assert_eq!</span>(value &amp; <span class="number">1</span>, <span class="number">1</span>);
            <span class="macro">assert!</span>(value &gt;= PublicExponent::_3.<span class="number">0</span>.into()); <span class="comment">// The absolute minimum.
            </span><span class="macro">assert!</span>(value &lt;= PublicExponent::MAX.<span class="number">0</span>.into());
        }
    }
}
