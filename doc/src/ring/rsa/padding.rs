...
</span><span class="kw">use crate</span>::{bits, digest, error, rand};

<span class="kw">mod </span>pkcs1;
<span class="kw">mod </span>pss;
...
<span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">use </span>pkcs1::RSA_PKCS1_SHA1_FOR_LEGACY_USE_ONLY;
...
<span class="comment">// Masks `out` with the output of the mask-generating function MGF1 as
// described in https://tools.ietf.org/html/rfc3447#appendix-B.2.1.
</span><span class="kw">fn </span>mgf1(digest_alg: <span class="kw-2">&amp;</span><span class="lifetime">'static </span>digest::Algorithm, seed: <span class="kw-2">&amp;</span>[u8], out: <span class="kw-2">&amp;mut </span>[u8]) {
    ...
    </span><span class="kw">for </span>(i, out) <span class="kw">in </span>out.chunks_mut(digest_len).enumerate() {
        <span class="kw">let </span><span class="kw-2">mut </span>ctx = digest::Context::new(digest_alg);
        ctx.update(seed);
        <span class="comment">// The counter will always fit in a `u32` because we reject absurdly
        // long inputs very early.
        </span>ctx.update(<span class="kw-2">&amp;</span>u32::to_be_bytes(i.try_into().unwrap()));
        <span class="kw">let </span>digest = ctx.finish();
        <span class="comment">// `zip` does the right thing as the the last chunk may legitimately be
        // shorter than `digest`, and `digest` will never be shorter than `out`.
        </span><span class="kw">for </span>(m, <span class="kw-2">&amp;</span>d) <span class="kw">in </span>out.iter_mut().zip(digest.as_ref().iter()) {
            <span class="kw-2">*</span>m ^= d;
        }
    }
}
    ...
    #[test]
        ...
        test::run(
            ...
            |section, test_case| {
                ...
                alg.encode(digest, <span class="kw-2">&amp;mut </span>m_out, bit_len, <span class="kw-2">&amp;</span>rng).unwrap();
