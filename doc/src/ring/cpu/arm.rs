...
</pre></div><pre class="rust"><code><span class="comment">// Copyright 2016-2021 Brian Smith.
//
...
// CONNECTION WITH THE USE OR PERFORMANCE OF THIS SOFTWARE.
    ...
    not(any(target_arch = <span class="string">"aarch64"</span>, target_arch = <span class="string">"arm"</span>)),
    allow(dead_code)
)]

#[cfg(any(target_arch = <span class="string">"arm"</span>, target_arch = <span class="string">"aarch64"</span>))]
</span><span class="kw">mod </span>abi_assumptions {
    ...
    <span class="kw">const </span>_ASSUMED_USIZE_SIZE: () = <span class="macro">assert!</span>(core::mem::size_of::&lt;usize&gt;() == _ASSUMED_POINTER_SIZE);
    <span class="kw">const </span>_ASSUMED_REF_SIZE: () =
        <span class="macro">assert!</span>(core::mem::size_of::&lt;<span class="kw-2">&amp;</span><span class="lifetime">'static </span>u8&gt;() == _ASSUMED_POINTER_SIZE);
...
// detection.
    ...
    not(target_env = <span class="string">"uclibc"</span>)
))]
</span><span class="kw">fn </span>detect_features() -&gt; u32 {
    ...
    <span class="kw">let </span><span class="kw-2">mut </span>features = <span class="number">0</span>;
...
<span class="attr">#[cfg(all(target_os = <span class="string">"fuchsia"</span>, target_arch = <span class="string">"aarch64"</span>))]
</span><span class="kw">fn </span>detect_features() -&gt; u32 {
...
<span class="attr">#[cfg(all(target_os = <span class="string">"windows"</span>, target_arch = <span class="string">"aarch64"</span>))]
</span><span class="kw">fn </span>detect_features() -&gt; u32 {
    <span class="comment">// We do not need to check for the presence of NEON, as Armv8-A always has it
    </span><span class="kw">const </span>_ASSERT_NEON_DETECTED: () = <span class="macro">assert!</span>((ARMCAP_STATIC &amp; NEON.mask) == NEON.mask);
    <span class="kw">let </span><span class="kw-2">mut </span>features = ARMCAP_STATIC;
    ...
    <span class="kw">if </span>result != <span class="number">0 </span>{
        <span class="comment">// These are all covered by one call in Windows
        </span>features |= AES.mask;
        features |= PMULL.mask;
        features |= SHA256.mask;
    }
...
<span class="attr">#[cfg(all(
    ...
    not(any(
        ...
        target_os = <span class="string">"windows"
    </span>))
))]
</span><span class="kw">fn </span>detect_features() -&gt; u32 {
    <span class="number">0
...
<span class="macro">macro_rules!</span> features {
    {
        $(
            <span class="macro-nonterminal">$target_feature_name</span>:expr =&gt; <span class="macro-nonterminal">$name</span>:ident {
        ...
        <span class="kw">const </span>ARMCAP_STATIC: u32 = <span class="number">0
            </span>$(
                | (
                    <span class="kw">if </span><span class="macro">cfg!</span>(all(any(target_arch = <span class="string">"aarch64"</span>, target_arch = <span class="string">"arm"</span>),
                                target_feature = <span class="macro-nonterminal">$target_feature_name</span>)) {
                        <span class="macro-nonterminal">$name</span>.mask
        ...
        <span class="attr">#[cfg(all(test, any(target_arch = <span class="string">"arm"</span>, target_arch = <span class="string">"aarch64"</span>)))]
        </span><span class="kw">const </span>ALL_FEATURES: [Feature; <span class="number">4</span>] = [
            $(
...
<span class="kw">impl </span>Feature {
    <span class="attr">#[inline(always)]
        ...
        <span class="attr">#[cfg(all(
            ...
            <span class="comment">// SAFETY: See `OPENSSL_armcap_P`'s safety documentation.
            </span><span class="kw">if </span><span class="self">self</span>.mask == <span class="self">self</span>.mask &amp; <span class="kw">unsafe </span>{ OPENSSL_armcap_P } {
                <span class="kw">return </span><span class="bool-val">true</span>;
            }
        }
...
<span class="comment">// Assumes all target feature names are the same for ARM and AAarch64.
    ...
    // that is supported.
    </span><span class="string">"aes" </span>=&gt; PMULL {
...
</span><span class="attr">#[cfg(any(target_arch = <span class="string">"aarch64"</span>, target_arch = <span class="string">"arm"</span>))]
</span><span class="kw">pub unsafe fn </span>init_global_shared_with_assembly() {
    <span class="kw">let </span>detected = detect_features();
...
// with a `OnceCell&lt;u32&gt;` that will provide all the necessary safety guarantees.
    ...
    <span class="kw">static </span><span class="kw-2">mut </span>OPENSSL_armcap_P: u32;
}

<span class="comment">// MSRV: Enforce 1.61.0 on some aarch64-* targets (aarch64-apple-*, in particular) prior to. Earlier
...
// target_feature="sha3"
// ```
</span><span class="attr">#[allow(clippy::assertions_on_constants)]
</span><span class="kw">const </span>_AARCH64_HAS_NEON: () =
    <span class="macro">assert!</span>(((ARMCAP_STATIC &amp; NEON.mask) == NEON.mask) || !<span class="macro">cfg!</span>(target_arch = <span class="string">"aarch64"</span>));
<span class="attr">#[allow(clippy::assertions_on_constants)]
</span><span class="kw">const </span>_AARCH64_APPLE_FEATURES: u32 = NEON.mask | AES.mask | SHA256.mask | PMULL.mask;
<span class="attr">#[allow(clippy::assertions_on_constants)]
</span><span class="kw">const </span>_AARCH64_APPLE_TARGETS_EXPECTED_FEATURES: () = <span class="macro">assert!</span>(
    ((ARMCAP_STATIC &amp; _AARCH64_APPLE_FEATURES) == _AARCH64_APPLE_FEATURES)
        || !<span class="macro">cfg!</span>(all(target_arch = <span class="string">"aarch64"</span>, target_vendor = <span class="string">"apple"</span>))
);
...
<span class="attr">#[cfg(all(test, any(target_arch = <span class="string">"arm"</span>, target_arch = <span class="string">"aarch64"</span>)))]
    ...
    </span><span class="kw">fn </span>test_mask_abi() {
        <span class="macro">assert_eq!</span>(NEON.mask, <span class="number">1</span>);
        <span class="macro">assert_eq!</span>(AES.mask, <span class="number">4</span>);
        <span class="macro">assert_eq!</span>(SHA256.mask, <span class="number">16</span>);
        <span class="macro">assert_eq!</span>(PMULL.mask, <span class="number">32</span>);
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_armcap_static_is_subset_of_armcap_dynamic() {
        <span class="comment">// Ensure `OPENSSL_armcap_P` is initialized.
        </span><span class="kw">let </span>cpu = <span class="kw">crate</span>::cpu::features();

        <span class="kw">let </span>armcap_dynamic = <span class="kw">unsafe </span>{ OPENSSL_armcap_P };
        <span class="macro">assert_eq!</span>(armcap_dynamic &amp; ARMCAP_STATIC, ARMCAP_STATIC);

        ALL_FEATURES.iter().for_each(|feature| {
            <span class="kw">if </span>(ARMCAP_STATIC &amp; feature.mask) != <span class="number">0 </span>{
                <span class="macro">assert!</span>(feature.available(cpu));
            }
        })
    }
}
