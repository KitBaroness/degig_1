...
</pre></div><pre class="rust"><code><span class="comment">// Copyright 2017-2023 Brian Smith.
//
...
</span><span class="kw">pub use </span><span class="kw">super</span>::n0::N0;
<span class="kw">use </span><span class="kw">crate</span>::cpu;

<span class="comment">// Indicates that the element is not encoded; there is no *R* factor
...
<span class="comment">// Indicates the element is encoded three times; the value has three
// *R* factors that need to be canceled out.
</span><span class="attr">#[allow(clippy::upper_case_acronyms)]
#[derive(Copy, Clone)]
</span><span class="kw">pub enum </span>RRR {}
...
<span class="kw">impl </span>Encoding <span class="kw">for </span>RRR {}
...
<span class="kw">impl </span>ReductionEncoding <span class="kw">for </span>RRR {
    <span class="kw">type </span>Output = RR;
}
...
<span class="kw">impl </span>ProductEncoding <span class="kw">for </span>(RR, RR) {
    <span class="kw">type </span>Output = RRR;
}
...
<span class="kw">impl </span>ProductEncoding <span class="kw">for </span>(RRR, RInverse) {
    <span class="kw">type </span>Output = &lt;(RInverse, RRR) <span class="kw">as </span>ProductEncoding&gt;::Output;
}

<span class="attr">#[allow(unused_imports)]
</span><span class="kw">use crate</span>::{bssl, c, limb::Limb};

<span class="attr">#[inline(always)]
</span><span class="kw">unsafe fn </span>mul_mont(
...
</span><span class="comment">// TODO: Stop calling this from C and un-export it.
</span><span class="attr">#[allow(deprecated)]
</span><span class="macro">prefixed_export!</span> {
    <span class="kw">unsafe fn </span>bn_mul_mont(
        r: <span class="kw-2">*mut </span>Limb,
        a: <span class="kw-2">*const </span>Limb,
        b: <span class="kw-2">*const </span>Limb,
        n: <span class="kw-2">*const </span>Limb,
        ...
        <span class="comment">// The mutable pointer `r` may alias `a` and/or `b`, so the lifetimes of
        // any slices for `a` or `b` must not overlap with the lifetime of any
        // mutable for `r`.

        // Nothing aliases `n`
        </span><span class="kw">let </span>n = <span class="kw">unsafe </span>{ core::slice::from_raw_parts(n, num_limbs) };

        <span class="kw">let </span><span class="kw-2">mut </span>tmp = [<span class="number">0</span>; <span class="number">2 </span>* <span class="kw">super</span>::BIGINT_MODULUS_MAX_LIMBS];
        <span class="kw">let </span>tmp = <span class="kw-2">&amp;mut </span>tmp[..(<span class="number">2 </span>* num_limbs)];
        {
            <span class="kw">let </span>a: <span class="kw-2">&amp;</span>[Limb] = <span class="kw">unsafe </span>{ core::slice::from_raw_parts(a, num_limbs) };
            <span class="kw">let </span>b: <span class="kw-2">&amp;</span>[Limb] = <span class="kw">unsafe </span>{ core::slice::from_raw_parts(b, num_limbs) };
            limbs_mul(tmp, a, b);
        }
        <span class="kw">let </span>r: <span class="kw-2">&amp;mut </span>[Limb] = <span class="kw">unsafe </span>{ core::slice::from_raw_parts_mut(r, num_limbs) };
        limbs_from_mont_in_place(r, tmp, n, n0);
    }
}

<span class="comment">// `bigint` needs then when the `alloc` feature is enabled. `bn_mul_mont` above needs this when
...
</span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>limbs_from_mont_in_place(r: <span class="kw-2">&amp;mut </span>[Limb], tmp: <span class="kw-2">&amp;mut </span>[Limb], m: <span class="kw-2">&amp;</span>[Limb], n0: <span class="kw-2">&amp;</span>N0) {
    <span class="macro">prefixed_extern!</span> {
        <span class="kw">fn </span>bn_from_montgomery_in_place(
            r: <span class="kw-2">*mut </span>Limb,
            num_r: c::size_t,
...
<span class="attr">#[cfg(not(any(
    ...
    r[..ab_len].fill(<span class="number">0</span>);
...
<span class="attr">#[cfg(any(
    ...
    </span><span class="kw">fn </span>limbs_mul_add_limb(r: <span class="kw-2">*mut </span>Limb, a: <span class="kw-2">*const </span>Limb, b: Limb, num_limbs: c::size_t) -&gt; Limb;
}
...
<span class="attr">#[cfg(any(
    ...
    <span class="comment">// `r` and/or 'a' and/or 'b' may alias.
    </span><span class="kw">fn </span>bn_mul_mont(
        r: <span class="kw-2">*mut </span>Limb,
        a: <span class="kw-2">*const </span>Limb,
        b: <span class="kw-2">*const </span>Limb,
        n: <span class="kw-2">*const </span>Limb,
...
</span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>limbs_mont_mul(
    r: <span class="kw-2">&amp;mut </span>[Limb],
    a: <span class="kw-2">&amp;</span>[Limb],
...
</span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>limbs_mont_product(
    r: <span class="kw-2">&amp;mut </span>[Limb],
    a: <span class="kw-2">&amp;</span>[Limb],
    b: <span class="kw-2">&amp;</span>[Limb],
    m: <span class="kw-2">&amp;</span>[Limb],
    n0: <span class="kw-2">&amp;</span>N0,
    cpu_features: cpu::Features,
) {
...
</span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>limbs_mont_square(r: <span class="kw-2">&amp;mut </span>[Limb], m: <span class="kw-2">&amp;</span>[Limb], n0: <span class="kw-2">&amp;</span>N0, cpu_features: cpu::Features) {
    <span class="macro">debug_assert_eq!</span>(r.len(), m.len());
    <span class="kw">unsafe </span>{
        mul_mont(
            r.as_mut_ptr(),
...
<span class="attr">#[cfg(test)]
    ...
    <span class="kw">use </span><span class="kw">crate</span>::limb::Limb;
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">for </span>(i, (r_input, a, w, expected_retval, expected_r)) <span class="kw">in </span>TEST_CASES.iter().enumerate() {
            <span class="kw">let </span><span class="kw-2">mut </span>r = [<span class="number">0</span>; <span class="kw">super</span>::<span class="kw">super</span>::BIGINT_MODULUS_MAX_LIMBS];
            <span class="kw">let </span>r = {
                <span class="kw">let </span>r = <span class="kw-2">&amp;mut </span>r[..r_input.len()];
            ...
            <span class="macro">assert_eq!</span>(r.len(), a.len()); <span class="comment">// Sanity check
                ...
                <span class="kw">unsafe </span>{ limbs_mul_add_limb(r.as_mut_ptr(), a.as_ptr(), <span class="kw-2">*</span>w, a.len()) };
            <span class="macro">assert_eq!</span>(<span class="kw-2">&amp;</span>r, expected_r, <span class="string">"{}: {:x?} != {:x?}"</span>, i, r, expected_r);
