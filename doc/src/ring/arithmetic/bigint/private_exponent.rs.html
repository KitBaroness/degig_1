...
</span><span class="kw">use super</span>::{limb, BoxedLimbs, Limb, LimbMask, Modulus};
<span class="kw">use </span><span class="kw">crate</span>::error;
<span class="kw">use </span>alloc::boxed::Box;

<span class="kw">pub struct </span>PrivateExponent {
    limbs: Box&lt;[Limb]&gt;,
}

<span class="kw">impl </span>PrivateExponent {
    <span class="comment">// `p` is the modulus for which the exponent is in the interval [1, `p` - 1).
    </span><span class="kw">pub fn </span>from_be_bytes_padded&lt;M&gt;(
    ...
    </span><span class="kw">pub fn </span>from_be_bytes_for_test_only&lt;M&gt;(
        input: untrusted::Input,
        p: <span class="kw-2">&amp;</span>Modulus&lt;M&gt;,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, error::Unspecified&gt; {
        <span class="comment">// Do exactly what `from_be_bytes_padded` does for any inputs it accepts.
        </span><span class="kw">if let </span>r @ <span class="prelude-val">Ok</span>(<span class="kw">_</span>) = <span class="self">Self</span>::from_be_bytes_padded(input, p) {
            <span class="kw">return </span>r;
        }

        <span class="kw">let </span>dP = BoxedLimbs::&lt;M&gt;::positive_minimal_width_from_be_bytes(input)<span class="question-mark">?</span>;
