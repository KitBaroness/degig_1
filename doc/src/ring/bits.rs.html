...
</span><span class="kw">use crate</span>::{error, polyfill};
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">trait </span>FromUsizeBytes: Sized {
    <span class="doccomment">/// Constructs a `BitLength` from the given length in bytes.
    ///
    /// Fails if `bytes * 8` is too large for a `T`.
    </span><span class="kw">fn </span>from_usize_bytes(bytes: usize) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, error::Unspecified&gt;;
}

<span class="kw">impl </span>FromUsizeBytes <span class="kw">for </span>BitLength&lt;usize&gt; {
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>from_usize_bytes(bytes: usize) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, error::Unspecified&gt; {
        <span class="kw">let </span>bits = bytes.checked_shl(<span class="number">3</span>).ok_or(error::Unspecified)<span class="question-mark">?</span>;
        <span class="prelude-val">Ok</span>(<span class="self">Self</span>(bits))
    }
}

<span class="kw">impl </span>FromUsizeBytes <span class="kw">for </span>BitLength&lt;u64&gt; {
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>from_usize_bytes(bytes: usize) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, error::Unspecified&gt; {
        <span class="kw">let </span>bytes = polyfill::u64_from_usize(bytes);
        <span class="kw">let </span>bits = bytes.checked_shl(<span class="number">3</span>).ok_or(error::Unspecified)<span class="question-mark">?</span>;
        <span class="prelude-val">Ok</span>(<span class="self">Self</span>(bits))
    }
}
...
<span class="kw">impl</span>&lt;T: Copy&gt; BitLength&lt;T&gt; {
    <span class="doccomment">/// The number of bits this bit length represents, as a `usize`.
...
// overflow.
    ...
    </span><span class="kw">pub const fn </span>from_usize_bits(bits: usize) -&gt; <span class="self">Self </span>{
    ...
    </span><span class="attr">#[cfg(any(target_arch = <span class="string">"aarch64"</span>, feature = <span class="string">"alloc"</span>))]
    #[inline]
    </span><span class="kw">pub fn </span>as_usize_bytes_rounded_up(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
    ...
    <span class="attr">#[cfg(feature = <span class="string">"alloc"</span>)]
    #[inline]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>try_sub_1(<span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, error::Unspecified&gt; {
        <span class="kw">let </span>sum = <span class="self">self</span>.<span class="number">0</span>.checked_sub(<span class="number">1</span>).ok_or(error::Unspecified)<span class="question-mark">?</span>;
