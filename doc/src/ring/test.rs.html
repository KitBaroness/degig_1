...
<span class="kw">use </span>alloc::{format, string::String, vec::Vec};

<span class="kw">use crate</span>::{bits, digest, error};

<span class="attr">#[cfg(any(feature = <span class="string">"std"</span>, feature = <span class="string">"test_logging"</span>))]
...
<span class="doccomment">/// `compile_time_assert_eq::&lt;T&gt;();` fails to compile if `T` doesn't
/// implement `Eq`.
</span><span class="kw">pub fn </span>compile_time_assert_eq&lt;T: Eq&gt;() {}
...
<span class="kw">impl </span>TestCase {
    ...
    /// empty (zero-length) value is represented as "".
        ...
        <span class="self">self</span>.consume_optional_bytes(key)
            .unwrap_or_else(|| <span class="macro">panic!</span>(<span class="string">"No attribute named \"{}\""</span>, key))
    }

    <span class="doccomment">/// Like `consume_bytes()` except it returns `None` if the test case
    /// doesn't have the attribute.
    </span><span class="kw">pub fn </span>consume_optional_bytes(<span class="kw-2">&amp;mut </span><span class="self">self</span>, key: <span class="kw-2">&amp;</span>str) -&gt; <span class="prelude-ty">Option</span>&lt;Vec&lt;u8&gt;&gt; {
        <span class="kw">let </span>s = <span class="self">self</span>.consume_optional_string(key)<span class="question-mark">?</span>;
        <span class="kw">let </span>result = <span class="kw">if </span>s.starts_with(<span class="string">'\"'</span>) {
...
<span class="doccomment">/// References a test input file.
    ...
    (<span class="macro-nonterminal">$file_name</span>:expr) =&gt; {
        <span class="macro-nonterminal">$crate::test::File </span>{
    ...
    </span>F: FnMut(<span class="kw-2">&amp;</span>str, <span class="kw-2">&amp;mut </span>TestCase) -&gt; <span class="prelude-ty">Result</span>&lt;(), error::Unspecified&gt;,
{
    ...
    <span class="kw">while let </span><span class="prelude-val">Some</span>(<span class="kw-2">mut </span>test_case) = parse_test_case(<span class="kw-2">&amp;mut </span>current_section, lines) {
        ...
        <span class="attr">#[cfg(feature = <span class="string">"test_logging"</span>)]
        </span><span class="kw">if let </span><span class="prelude-val">Err</span>(msg) = result {
...
<span class="kw">fn </span>from_hex_digit(d: u8) -&gt; <span class="prelude-ty">Result</span>&lt;u8, String&gt; {
...
<span class="kw">fn </span>parse_test_case(
    ...
    <span class="kw">loop </span>{
        ...
        <span class="attr">#[cfg(feature = <span class="string">"test_logging"</span>)]
        </span><span class="kw">if let </span><span class="prelude-val">Some</span>(text) = <span class="kw-2">&amp;</span>line {
        ...
        <span class="kw">match </span>line {
            ...
            </span><span class="prelude-val">Some</span>(line) <span class="kw">if </span>line.starts_with(<span class="string">'#'</span>) =&gt; (),

            <span class="prelude-val">Some</span>(line) <span class="kw">if </span>line.starts_with(<span class="string">'['</span>) =&gt; {
...
/// useful for some types of fuzzing.
    ...
    <span class="kw">use crate</span>::{error, rand};
    ...
    <span class="kw">impl </span>rand::sealed::SecureRandom <span class="kw">for </span>FixedByteRandom {
        <span class="kw">fn </span>fill_impl(<span class="kw-2">&amp;</span><span class="self">self</span>, dest: <span class="kw-2">&amp;mut </span>[u8]) -&gt; <span class="prelude-ty">Result</span>&lt;(), error::Unspecified&gt; {
            dest.fill(<span class="self">self</span>.byte);
            <span class="prelude-val">Ok</span>(())
        }
    }
