...
</span><span class="kw">use super</span>::{Algorithm, LessSafeKey, MAX_KEY_LEN};
<span class="kw">use crate</span>::{error, hkdf};

<span class="doccomment">/// An AEAD key without a designated role or nonce sequence.
</span><span class="kw">pub struct </span>UnboundKey {
    inner: LessSafeKey,
}

<span class="kw">impl </span>UnboundKey {
    <span class="doccomment">/// Constructs a `UnboundKey`.
    ///
    /// Fails if `key_bytes.len() != algorithm.key_len()`.
        ...
        <span class="prelude-val">Ok</span>(<span class="self">Self </span>{
            inner: LessSafeKey::new_(algorithm, key_bytes)<span class="question-mark">?</span>,
        })
    }
    ...
    <span class="doccomment">/// The key's AEAD algorithm.
        ...
        <span class="self">self</span>.inner.algorithm()
    }

    <span class="attr">#[inline]
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>into_inner(<span class="self">self</span>) -&gt; LessSafeKey {
        <span class="self">self</span>.inner
    }
}

<span class="kw">impl </span>core::fmt::Debug <span class="kw">for </span>UnboundKey {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>core::fmt::Formatter) -&gt; <span class="prelude-ty">Result</span>&lt;(), core::fmt::Error&gt; {
        <span class="self">self</span>.inner.fmt_debug(<span class="string">"UnboundKey"</span>, f)
    }
}

<span class="kw">impl </span>From&lt;hkdf::Okm&lt;<span class="lifetime">'_</span>, <span class="kw-2">&amp;</span><span class="lifetime">'static </span>Algorithm&gt;&gt; <span class="kw">for </span>UnboundKey {
    <span class="kw">fn </span>from(okm: hkdf::Okm&lt;<span class="kw-2">&amp;</span><span class="lifetime">'static </span>Algorithm&gt;) -&gt; <span class="self">Self </span>{
        <span class="kw">let </span><span class="kw-2">mut </span>key_bytes = [<span class="number">0</span>; MAX_KEY_LEN];
        <span class="kw">let </span>key_bytes = <span class="kw-2">&amp;mut </span>key_bytes[..okm.len().key_len];
