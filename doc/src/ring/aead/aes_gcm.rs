...
<span class="kw">use crate</span>::{aead, cpu, error, polyfill::usize_from_u64_saturated};
...
<span class="kw">fn </span>init(
    ...
    <span class="kw">let </span>gcm_key = gcm::Key::new(
        aes_key.encrypt_block(Block::zero(), cpu_features),
        cpu_features,
    );
    <span class="prelude-val">Ok</span>(aead::KeyInner::AesGcm(Key { gcm_key, aes_key }))
}
...
<span class="kw">fn </span>aes_gcm_seal(
    ...
    <span class="kw">let </span>Key { gcm_key, aes_key } = <span class="kw">match </span>key {
        aead::KeyInner::AesGcm(key) =&gt; key,
        <span class="kw">_ </span>=&gt; <span class="macro">unreachable!</span>(),
    };

    <span class="kw">let </span><span class="kw-2">mut </span>auth = gcm::Context::new(gcm_key, aad, in_out.len(), cpu_features)<span class="question-mark">?</span>;
    ...
    </span><span class="kw">let </span>in_out = {
        <span class="kw">if </span>!aes_key.is_aes_hw(cpu_features) || !auth.is_avx() {
            in_out
            ...
            <span class="macro">prefixed_extern!</span> {
                <span class="comment">// `HTable` and `Xi` should be 128-bit aligned. TODO: Can we shrink `HTable`? The
                // assembly says it needs just nine values in that array.
                </span><span class="kw">fn </span>aesni_gcm_encrypt(
                    ...
                    Xi: <span class="kw-2">&amp;mut </span>gcm::Xi) -&gt; c::size_t;
            }
            <span class="kw">let </span>processed = <span class="kw">unsafe </span>{
                aesni_gcm_encrypt(
                    in_out.as_ptr(),
                    in_out.as_mut_ptr(),
                    in_out.len(),
    ...
    <span class="attr">#[cfg(target_arch = <span class="string">"aarch64"</span>)]
        ...
        <span class="kw">if </span>!aes_key.is_aes_hw(cpu_features) || !auth.is_clmul() {
            in_out
        } <span class="kw">else </span>{
            <span class="kw">let </span>whole_block_bits = auth.in_out_whole_block_bits();
            <span class="kw">if </span>whole_block_bits.as_bits() &gt; <span class="number">0 </span>{
                <span class="kw">use crate</span>::{bits::BitLength, c};
                <span class="kw">let </span>(htable, xi) = auth.inner();
                <span class="macro">prefixed_extern!</span> {
            ...
            <span class="kw-2">&amp;mut </span>in_out[whole_block_bits.as_usize_bytes_rounded_up()..]
        }
    };

    <span class="kw">let </span>(whole, remainder) = {
        <span class="kw">let </span>in_out_len = in_out.len();
        <span class="kw">let </span>whole_len = in_out_len - (in_out_len % BLOCK_LEN);
        in_out.split_at_mut(whole_len)
    };

    <span class="kw">for </span>chunk <span class="kw">in </span>whole.chunks_mut(CHUNK_BLOCKS * BLOCK_LEN) {
        aes_key.ctr32_encrypt_within(chunk, <span class="number">0</span>.., <span class="kw-2">&amp;mut </span>ctr, cpu_features);
    ...
    <span class="kw">if </span>!remainder.is_empty() {
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>output = aes_key.encrypt_iv_xor_block(ctr.into(), input, cpu_features);
        output.zero_from(remainder.len());
        auth.update_block(output);
        remainder.copy_from_slice(<span class="kw-2">&amp;</span>output.as_ref()[..remainder.len()]);
    }

    <span class="prelude-val">Ok</span>(finish(aes_key, auth, tag_iv))
}
...
<span class="kw">fn </span>aes_gcm_open(
    ...
    <span class="kw">let </span>in_prefix_len = src.start;
    ...
    <span class="attr">#[cfg(target_arch = <span class="string">"x86_64"</span>)]
        ...
        <span class="kw">if </span>!aes_key.is_aes_hw(cpu_features) || !auth.is_avx() {
            ...
            <span class="macro">prefixed_extern!</span> {
                <span class="comment">// `HTable` and `Xi` should be 128-bit aligned. TODO: Can we shrink `HTable`? The
                // assembly says it needs just nine values in that array.
                </span><span class="kw">fn </span>aesni_gcm_decrypt(
                    input: <span class="kw-2">*const </span>u8,
                    output: <span class="kw-2">*mut </span>u8,
                    len: c::size_t,
                    key: <span class="kw-2">&amp;</span>aes::AES_KEY,
    ...
    <span class="kw">let </span>whole_len = {
    ...
    };
    {
        ...
        <span class="kw">loop </span>{
            ...
            auth.update_blocks(<span class="kw-2">&amp;</span>in_out[input..][..chunk_len]);
    ...
    <span class="kw">let </span>remainder = <span class="kw-2">&amp;mut </span>in_out[whole_len..];
...
<span class="kw">fn </span>finish(aes_key: <span class="kw-2">&amp;</span>aes::Key, gcm_ctx: gcm::Context, tag_iv: aes::Iv) -&gt; Tag {
    <span class="comment">// Finalize the tag and return it.
    </span>gcm_ctx.pre_finish(|pre_tag, cpu_features| {
        <span class="kw">let </span>encrypted_iv = aes_key.encrypt_block(tag_iv.into_block_less_safe(), cpu_features);
        <span class="kw">let </span>tag = pre_tag ^ encrypted_iv;
        Tag(<span class="kw-2">*</span>tag.as_ref())
    })
}

<span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">const </span>MAX_IN_OUT_LEN: usize = <span class="kw">super</span>::max_input_len(BLOCK_LEN, <span class="number">2</span>);
...
</span><span class="kw">const </span>_MAX_INPUT_LEN_BOUNDED_BY_NIST: () =
    <span class="macro">assert!</span>(MAX_IN_OUT_LEN == usize_from_u64_saturated(((<span class="number">1u64 </span>&lt;&lt; <span class="number">39</span>) - <span class="number">256</span>) / <span class="number">8</span>));
