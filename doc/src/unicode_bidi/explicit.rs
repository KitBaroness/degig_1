...
<span class="kw">use </span><span class="kw">super</span>::TextSource;
...
/// for each char in `text`.
...
</span><span class="kw">pub fn </span>compute&lt;<span class="lifetime">'a</span>, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(
    ...
    <span class="kw">for </span>(i, len) <span class="kw">in </span>text.indices_lengths() {
        <span class="kw">match </span>original_classes[i] {
            <span class="comment">// Rules X2-X5c
                ...
                <span class="kw">if </span>is_isolate {
                    <span class="comment">// Redundant due to "Retaining explicit formatting characters" step.
                    // levels[i] = last_level;
                    </span><span class="kw">match </span>stack.last().status {
                ...
                <span class="kw">if </span>!is_isolate {
                    <span class="comment">// X9 +
                    // &lt;https://www.unicode.org/reports/tr9/#Retaining_Explicit_Formatting_Characters&gt;
                    // (PDF handled below)
                    </span>processing_classes[i] = BN;
                }
            }
            ...
            <span class="comment">// &lt;http://www.unicode.org/reports/tr9/#X7&gt;
                ...
                </span>} <span class="kw">else if </span>overflow_embedding_count &gt; <span class="number">0 </span>{
                    overflow_embedding_count -= <span class="number">1</span>;
                } <span class="kw">else if </span>stack.last().status != OverrideStatus::Isolate &amp;&amp; stack.vec.len() &gt;= <span class="number">2 </span>{
                    stack.vec.pop();
                }
                <span class="comment">// &lt;https://www.unicode.org/reports/tr9/#Retaining_Explicit_Formatting_Characters&gt;
                </span>levels[i] = stack.last().level;
                <span class="comment">// X9 part of retaining explicit formatting characters.
                </span>processing_classes[i] = BN;
            }
            ...
            <span class="comment">// &lt;http://www.unicode.org/reports/tr9/#X6&gt;
                ...
                <span class="kw">let </span>last = stack.last();
                levels[i] = last.level;
                <span class="comment">// This condition is not in the spec, but I am pretty sure that is a spec bug.
                // https://www.unicode.org/L2/L2023/23014-amd-to-uax9.pdf
                </span><span class="kw">if </span>original_classes[i] != BN {
                    <span class="kw">match </span>last.status {
                        OverrideStatus::RTL =&gt; processing_classes[i] = R,
                        OverrideStatus::LTR =&gt; processing_classes[i] = L,
                        <span class="kw">_ </span>=&gt; {}
        ...
        </span><span class="kw">for </span>j <span class="kw">in </span><span class="number">1</span>..len {
            levels[i + j] = levels[i];
