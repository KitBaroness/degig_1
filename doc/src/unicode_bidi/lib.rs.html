...
<span class="kw">pub mod </span>utf16;
...
<span class="kw">use </span>core::str::CharIndices;
...
<span class="doccomment">/// Trait that abstracts over a text source for use by the bidi algorithms.
...
/// This trait is sealed and cannot be implemented for types outside this crate.
</span><span class="kw">pub trait </span>TextSource&lt;<span class="lifetime">'text</span>&gt;: private::Sealed {
    <span class="kw">type </span>CharIter: Iterator&lt;Item = char&gt;;
    <span class="kw">type </span>CharIndexIter: Iterator&lt;Item = (usize, char)&gt;;
    <span class="kw">type </span>IndexLenIter: Iterator&lt;Item = (usize, usize)&gt;;

    <span class="doccomment">/// Return the length of the text in code units.
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">fn </span>len(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize;

    <span class="doccomment">/// Get the character at a given code unit index, along with its length in code units.
    ...
    </span><span class="kw">fn </span>char_at(<span class="kw-2">&amp;</span><span class="self">self</span>, index: usize) -&gt; <span class="prelude-ty">Option</span>&lt;(char, usize)&gt;;

    <span class="doccomment">/// Return a subrange of the text, indexed by code units.
    /// (We don't implement all of the Index trait, just the minimum we use.)
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">fn </span>subrange(<span class="kw-2">&amp;</span><span class="self">self</span>, range: Range&lt;usize&gt;) -&gt; <span class="kw-2">&amp;</span><span class="self">Self</span>;

    <span class="doccomment">/// An iterator over the text returning Unicode characters,
    /// REPLACEMENT_CHAR for invalid code units.
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">fn </span>chars(<span class="kw-2">&amp;</span><span class="lifetime">'text </span><span class="self">self</span>) -&gt; <span class="self">Self</span>::CharIter;

    <span class="doccomment">/// An iterator over the text returning (index, char) tuples,
    /// where index is the starting code-unit index of the character,
    /// and char is its Unicode value (or REPLACEMENT_CHAR if invalid).
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">fn </span>char_indices(<span class="kw-2">&amp;</span><span class="lifetime">'text </span><span class="self">self</span>) -&gt; <span class="self">Self</span>::CharIndexIter;

    <span class="doccomment">/// An iterator over the text returning (index, length) tuples,
    /// where index is the starting code-unit index of the character,
    /// and length is its length in code units.
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">fn </span>indices_lengths(<span class="kw-2">&amp;</span><span class="lifetime">'text </span><span class="self">self</span>) -&gt; <span class="self">Self</span>::IndexLenIter;

    <span class="doccomment">/// Number of code units the given character uses.
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">fn </span>char_len(ch: char) -&gt; usize;
}

<span class="kw">mod </span>private {
    <span class="kw">pub trait </span>Sealed {}

    <span class="comment">// Implement for str and [u16] only.
    </span><span class="kw">impl </span>Sealed <span class="kw">for </span>str {}
    <span class="kw">impl </span>Sealed <span class="kw">for </span>[u16] {}
}
...
<span class="doccomment">/// Extended version of InitialInfo (not public API).
</span><span class="attr">#[derive(PartialEq, Debug)]
</span><span class="kw">struct </span>InitialInfoExt&lt;<span class="lifetime">'text</span>&gt; {
    <span class="doccomment">/// The base InitialInfo for the text, recording its paragraphs and bidi classes.
    </span>base: InitialInfo&lt;<span class="lifetime">'text</span>&gt;,

    <span class="doccomment">/// Parallel to base.paragraphs, records whether each paragraph is "pure LTR" that
    /// requires no further bidi processing (i.e. there are no RTL characters or bidi
    /// control codes present).
    </span>pure_ltr: Vec&lt;bool&gt;,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'text</span>&gt; InitialInfoExt&lt;<span class="lifetime">'text</span>&gt; {
    <span class="doccomment">/// Find the paragraphs and BidiClasses in a string of text, with a custom [`BidiDataSource`]
    /// for Bidi data. If you just wish to use the hardcoded Bidi data, please use [`InitialInfo::new()`]
    /// instead (enabled with tbe default `hardcoded-data` Cargo feature)
    ///
    ...
    /// remain FSI, and it's up to later stages to treat these as LRI when needed.
        ...
        text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>str,
        default_para_level: <span class="prelude-ty">Option</span>&lt;Level&gt;,
    ) -&gt; InitialInfoExt&lt;<span class="lifetime">'a</span>&gt; {
        <span class="kw">let </span><span class="kw-2">mut </span>paragraphs = Vec::&lt;ParagraphInfo&gt;::new();
        <span class="kw">let </span><span class="kw-2">mut </span>pure_ltr = Vec::&lt;bool&gt;::new();
        <span class="kw">let </span>(original_classes, <span class="kw">_</span>, <span class="kw">_</span>) = compute_initial_info(
            data_source,
            text,
            default_para_level,
            <span class="prelude-val">Some</span>((<span class="kw-2">&amp;mut </span>paragraphs, <span class="kw-2">&amp;mut </span>pure_ltr)),
        );
...
<span class="doccomment">/// Implementation of initial-info computation for both BidiInfo and ParagraphBidiInfo.
...
/// the embedding level and pure-ltr flag for the *last* (or only) paragraph.
</span><span class="kw">fn </span>compute_initial_info&lt;<span class="lifetime">'a</span>, D: BidiDataSource, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(
    data_source: <span class="kw-2">&amp;</span>D,
    text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T,
    default_para_level: <span class="prelude-ty">Option</span>&lt;Level&gt;,
    <span class="kw-2">mut </span>split_paragraphs: <span class="prelude-ty">Option</span>&lt;(<span class="kw-2">&amp;mut </span>Vec&lt;ParagraphInfo&gt;, <span class="kw-2">&amp;mut </span>Vec&lt;bool&gt;)&gt;,
) -&gt; (Vec&lt;BidiClass&gt;, Level, bool) {
    <span class="kw">let </span><span class="kw-2">mut </span>original_classes = Vec::with_capacity(text.len());

    <span class="comment">// The stack contains the starting code unit index for each nested isolate we're inside.
    </span><span class="kw">let </span><span class="kw-2">mut </span>isolate_stack = Vec::new();

    <span class="macro">debug_assert!</span>(
        <span class="kw">if let </span><span class="prelude-val">Some</span>((<span class="kw-2">ref </span>paragraphs, <span class="kw-2">ref </span>pure_ltr)) = split_paragraphs {
    ...
    <span class="comment">// Per-paragraph flag: can subsequent processing be skipped? Set to false if any
    // RTL characters or bidi control characters are encountered in the paragraph.
    </span><span class="kw">let </span><span class="kw-2">mut </span>is_pure_ltr = <span class="bool-val">true</span>;

    <span class="attr">#[cfg(feature = <span class="string">"flame_it"</span>)]
    </span>flame::start(<span class="string">"compute_initial_info(): iter text.char_indices()"</span>);
    ...
    <span class="kw">for </span>(i, c) <span class="kw">in </span>text.char_indices() {
        ...
        <span class="kw">let </span>len = T::char_len(c);
        ...
        <span class="kw">match </span>class {
            B =&gt; {
                <span class="kw">if let </span><span class="prelude-val">Some</span>((<span class="kw-2">ref mut </span>paragraphs, <span class="kw-2">ref mut </span>pure_ltr)) = split_paragraphs {
                    <span class="comment">// P1. Split the text into separate paragraphs. The paragraph separator is kept
                    // with the previous paragraph.
                    </span><span class="kw">let </span>para_end = i + len;
                    ...
                    is_pure_ltr = <span class="bool-val">true</span>;
                    isolate_stack.clear();
                }
            }

            L | R | AL =&gt; {
                <span class="kw">if </span>class != L {
                    is_pure_ltr = <span class="bool-val">false</span>;
                }
                <span class="kw">match </span>isolate_stack.last() {
                    <span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span>start) =&gt; {
                        <span class="kw">if </span>original_classes[start] == FSI {
                            ...
                            </span><span class="kw">for </span>j <span class="kw">in </span><span class="number">0</span>..T::char_len(chars::FSI) {
                                original_classes[start + j] = <span class="kw">if </span>class == L { LRI } <span class="kw">else </span>{ RLI };
                            }
                        }
                    }
    ...
    <span class="kw">if let </span><span class="prelude-val">Some</span>((paragraphs, pure_ltr)) = split_paragraphs {
        ...
        <span class="macro">debug_assert_eq!</span>(paragraphs.len(), pure_ltr.len());
    }
    <span class="macro">debug_assert_eq!</span>(original_classes.len(), text.len());

    <span class="attr">#[cfg(feature = <span class="string">"flame_it"</span>)]
    </span>flame::end(<span class="string">"compute_initial_info(): iter text.char_indices()"</span>);
...
<span class="kw">impl</span>&lt;<span class="lifetime">'text</span>&gt; BidiInfo&lt;<span class="lifetime">'text</span>&gt; {
    ...
    /// TODO: Support auto-RTL base direction
        ...
        <span class="kw">let </span>InitialInfoExt { base, pure_ltr, .. } =
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>processing_classes = base.original_classes.clone();

        <span class="kw">for </span>(para, is_pure_ltr) <span class="kw">in </span>base.paragraphs.iter().zip(pure_ltr.iter()) {
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>text[para.range.clone()];
            <span class="kw">let </span>original_classes = <span class="kw-2">&amp;</span>base.original_classes[para.range.clone()];

            compute_bidi_info_for_para(
                data_source,
                para,
                <span class="kw-2">*</span>is_pure_ltr,
                text,
                original_classes,
                <span class="kw-2">&amp;mut </span>processing_classes,
                <span class="kw-2">&amp;mut </span>levels,
            );
        }
    ...
    <span class="doccomment">/// Produce the levels for this paragraph as needed for reordering, one level per *byte*
    ...
    /// [Rule L2]: https://www.unicode.org/reports/tr9/#L2
        ...
        reorder_levels(
            line_classes,
            line_levels,
            <span class="self">self</span>.text.subrange(line),
            para.level,
        );
    ...
    <span class="doccomment">/// Produce the levels for this paragraph as needed for reordering, one level per *character*
    ...
    /// [Rule L4]: https://www.unicode.org/reports/tr9/#L4
        ...
        <span class="kw">if </span>!level::has_rtl(<span class="kw-2">&amp;</span><span class="self">self</span>.levels[line.clone()]) {
        ...
        reorder_line(<span class="self">self</span>.text, line, levels, runs)
    }
    ...
    #[inline]
        ...
        <span class="kw">let </span>levels = <span class="self">self</span>.reordered_levels(para, line.clone());
        visual_runs_for_line(levels, <span class="kw-2">&amp;</span>line)
    }
...
<span class="doccomment">/// Bidi information of text treated as a single paragraph.
///
/// The `original_classes` and `levels` vectors are indexed by byte offsets into the text.  If a
/// character is multiple bytes wide, then its class and level will appear multiple times in these
/// vectors.
</span><span class="attr">#[derive(Debug, PartialEq)]
</span><span class="kw">pub struct </span>ParagraphBidiInfo&lt;<span class="lifetime">'text</span>&gt; {
    <span class="doccomment">/// The text
    </span><span class="kw">pub </span>text: <span class="kw-2">&amp;</span><span class="lifetime">'text </span>str,
    ...
    <span class="doccomment">/// The paragraph embedding level.
    </span><span class="kw">pub </span>paragraph_level: Level,

    <span class="doccomment">/// Whether the paragraph is purely LTR.
    </span><span class="kw">pub </span>is_pure_ltr: bool,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'text</span>&gt; ParagraphBidiInfo&lt;<span class="lifetime">'text</span>&gt; {
    <span class="doccomment">/// Determine the bidi embedding level.
    ///
    ///
    /// The `hardcoded-data` Cargo feature (enabled by default) must be enabled to use this.
    ///
    ...
    #[cfg(feature = <span class="string">"hardcoded-data"</span>)]
    #[inline]
    </span><span class="kw">pub fn </span>new(text: <span class="kw-2">&amp;</span>str, default_para_level: <span class="prelude-ty">Option</span>&lt;Level&gt;) -&gt; ParagraphBidiInfo&lt;<span class="lifetime">'_</span>&gt; {
        <span class="self">Self</span>::new_with_data_source(<span class="kw-2">&amp;</span>HardcodedBidiData, text, default_para_level)
    }

    <span class="doccomment">/// Determine the bidi embedding level, with a custom [`BidiDataSource`]
    /// for Bidi data. If you just wish to use the hardcoded Bidi data, please use [`BidiInfo::new()`]
    /// instead (enabled with tbe default `hardcoded-data` Cargo feature).
    ///
    ...
    /// and should be kept in sync with it.
        ...
        text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>str,
        default_para_level: <span class="prelude-ty">Option</span>&lt;Level&gt;,
    ) -&gt; ParagraphBidiInfo&lt;<span class="lifetime">'a</span>&gt; {
        <span class="comment">// Here we could create a ParagraphInitialInfo struct to parallel the one
        // used by BidiInfo, but there doesn't seem any compelling reason for it.
        </span><span class="kw">let </span>(original_classes, paragraph_level, is_pure_ltr) =
            compute_initial_info(data_source, text, default_para_level, <span class="prelude-val">None</span>);

        <span class="kw">let </span><span class="kw-2">mut </span>levels = Vec::&lt;Level&gt;::with_capacity(text.len());
        <span class="kw">let </span><span class="kw-2">mut </span>processing_classes = original_classes.clone();

        <span class="kw">let </span>para_info = ParagraphInfo {
            range: Range {
        ...
        compute_bidi_info_for_para(
            data_source,
            <span class="kw-2">&amp;</span>para_info,
            is_pure_ltr,
            text,
            <span class="kw-2">&amp;</span>original_classes,
            <span class="kw-2">&amp;mut </span>processing_classes,
            <span class="kw-2">&amp;mut </span>levels,
        );
    ...
    /// (This should be kept in sync with BidiInfo::reordered_levels.)
    ...
    </span><span class="kw">pub fn </span>reordered_levels(<span class="kw-2">&amp;</span><span class="self">self</span>, line: Range&lt;usize&gt;) -&gt; Vec&lt;Level&gt; {
        <span class="macro">assert!</span>(line.start &lt;= <span class="self">self</span>.levels.len());
        <span class="macro">assert!</span>(line.end &lt;= <span class="self">self</span>.levels.len());
        ...
        reorder_levels(
            ...
            line_levels,
            <span class="self">self</span>.text.subrange(line),
            <span class="self">self</span>.paragraph_level,
        );
    ...
    </span><span class="kw">pub fn </span>reordered_levels_per_char(<span class="kw-2">&amp;</span><span class="self">self</span>, line: Range&lt;usize&gt;) -&gt; Vec&lt;Level&gt; {
        <span class="kw">let </span>levels = <span class="self">self</span>.reordered_levels(line);
        <span class="self">self</span>.text.char_indices().map(|(i, <span class="kw">_</span>)| levels[i]).collect()
    }
    ...
    </span><span class="kw">pub fn </span>reorder_line(<span class="kw-2">&amp;</span><span class="self">self</span>, line: Range&lt;usize&gt;) -&gt; Cow&lt;<span class="lifetime">'text</span>, str&gt; {
        <span class="kw">if </span>!level::has_rtl(<span class="kw-2">&amp;</span><span class="self">self</span>.levels[line.clone()]) {
            <span class="kw">return </span><span class="self">self</span>.text[line].into();
        }

        <span class="kw">let </span>(levels, runs) = <span class="self">self</span>.visual_runs(line.clone());
    ...
    #[inline]
    </span><span class="kw">pub fn </span>visual_runs(<span class="kw-2">&amp;</span><span class="self">self</span>, line: Range&lt;usize&gt;) -&gt; (Vec&lt;Level&gt;, Vec&lt;LevelRun&gt;) {
        <span class="kw">let </span>levels = <span class="self">self</span>.reordered_levels(line.clone());
        visual_runs_for_line(levels, <span class="kw-2">&amp;</span>line)
    }
    ...
    /// This information is usually used to skip re-ordering of text when no RTL level is present
        ...
        !<span class="self">self</span>.is_pure_ltr
    }

    <span class="doccomment">/// Return the paragraph's Direction (Ltr, Rtl, or Mixed) based on its levels.
    </span><span class="attr">#[inline]
    </span><span class="kw">pub fn </span>direction(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; Direction {
        para_direction(<span class="kw-2">&amp;</span><span class="self">self</span>.levels)
    }
}

<span class="doccomment">/// Return a line of the text in display order based on resolved levels.
///
...
/// [Rule L3]: https://www.unicode.org/reports/tr9/#L3
/// [Rule L4]: https://www.unicode.org/reports/tr9/#L4
</span><span class="kw">fn </span>reorder_line&lt;<span class="lifetime">'text</span>&gt;(
    text: <span class="kw-2">&amp;</span><span class="lifetime">'text </span>str,
    line: Range&lt;usize&gt;,
    levels: Vec&lt;Level&gt;,
    runs: Vec&lt;LevelRun&gt;,
) -&gt; Cow&lt;<span class="lifetime">'text</span>, str&gt; {
    <span class="comment">// If all isolating run sequences are LTR, no reordering is needed
        ...
        <span class="kw">return </span>text[line].into();
    }
    ...
    <span class="kw">for </span>run <span class="kw">in </span>runs {
        <span class="kw">if </span>levels[run.start].is_rtl() {
            ...
            result.push_str(<span class="kw-2">&amp;</span>text[run]);
        }
    }
    result.into()
}
...
/// [Rule L3]: https://www.unicode.org/reports/tr9/#L3
/// [Rule L4]: https://www.unicode.org/reports/tr9/#L4
</span><span class="kw">fn </span>visual_runs_for_line(levels: Vec&lt;Level&gt;, line: <span class="kw-2">&amp;</span>Range&lt;usize&gt;) -&gt; (Vec&lt;Level&gt;, Vec&lt;LevelRun&gt;) {
...
/// reversed. You may need to fix up the map to deal with this. Alternatively, only pass in arrays where each `Level`
/// is for a single code point.
</span><span class="kw">fn </span>reorder_visual(levels: <span class="kw-2">&amp;</span>[Level]) -&gt; Vec&lt;usize&gt; {
...
<span class="doccomment">/// The core of BidiInfo initialization, factored out into a function that both
/// the utf-8 and utf-16 versions of BidiInfo can use.
</span><span class="kw">fn </span>compute_bidi_info_for_para&lt;<span class="lifetime">'a</span>, D: BidiDataSource, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(
    data_source: <span class="kw-2">&amp;</span>D,
    para: <span class="kw-2">&amp;</span>ParagraphInfo,
    is_pure_ltr: bool,
    text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T,
    ...
    levels: <span class="kw-2">&amp;mut </span>Vec&lt;Level&gt;,
) {
    <span class="kw">let </span>new_len = levels.len() + para.range.len();
    levels.resize(new_len, para.level);
    <span class="kw">if </span>para.level == LTR_LEVEL &amp;&amp; is_pure_ltr {
...
<span class="doccomment">/// Produce the levels for this paragraph as needed for reordering, one level per *code unit*
...
///
/// [Rule L1]: https://www.unicode.org/reports/tr9/#L1
</span><span class="kw">fn </span>reorder_levels&lt;<span class="lifetime">'a</span>, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(
    line_classes: <span class="kw-2">&amp;</span>[BidiClass],
    line_levels: <span class="kw-2">&amp;mut </span>[Level],
    line_text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T,
    para_level: Level,
) {
    <span class="comment">// Reset some whitespace chars to paragraph level.
    // &lt;http://www.unicode.org/reports/tr9/#L1&gt;
    </span><span class="kw">let </span><span class="kw-2">mut </span>reset_from: <span class="prelude-ty">Option</span>&lt;usize&gt; = <span class="prelude-val">Some</span>(<span class="number">0</span>);
    ...
    <span class="kw">let </span><span class="kw-2">mut </span>prev_level = para_level;
    <span class="kw">for </span>(i, c) <span class="kw">in </span>line_text.char_indices() {
        <span class="kw">match </span>line_classes[i] {
            <span class="comment">// Segment separator, Paragraph separator
                ...
                reset_to = <span class="prelude-val">Some</span>(i + T::char_len(c));
        ...
        <span class="kw">if let </span>(<span class="prelude-val">Some</span>(from), <span class="prelude-val">Some</span>(to)) = (reset_from, reset_to) {
            <span class="kw">for </span>level <span class="kw">in </span><span class="kw-2">&amp;mut </span>line_levels[from..to] {
                <span class="kw-2">*</span>level = para_level;
            }
            reset_from = <span class="prelude-val">None</span>;
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, <span class="lifetime">'text</span>&gt; Paragraph&lt;<span class="lifetime">'a</span>, <span class="lifetime">'text</span>&gt; {
    ...
    </span><span class="kw">pub fn </span>new(info: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>BidiInfo&lt;<span class="lifetime">'text</span>&gt;, para: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>ParagraphInfo) -&gt; Paragraph&lt;<span class="lifetime">'a</span>, <span class="lifetime">'text</span>&gt; {
        Paragraph { info, para }
    }
    ...
    <span class="doccomment">/// Returns if the paragraph is Left direction, right direction or mixed.
        ...
        para_direction(<span class="kw-2">&amp;</span><span class="self">self</span>.info.levels[<span class="self">self</span>.para.range.clone()])
    }
...
<span class="doccomment">/// Return the directionality of the paragraph (Left, Right or Mixed) from its levels.
</span><span class="attr">#[cfg_attr(feature = <span class="string">"flame_it"</span>, flamer::flame)]
</span><span class="kw">fn </span>para_direction(levels: <span class="kw-2">&amp;</span>[Level]) -&gt; Direction {
    ...
    <span class="kw">for </span>level <span class="kw">in </span>levels {
        <span class="kw">if </span>level.is_ltr() {
            ltr = <span class="bool-val">true</span>;
            <span class="kw">if </span>rtl {
                <span class="kw">return </span>Direction::Mixed;
            }
        }

        <span class="kw">if </span>level.is_rtl() {
...
<span class="doccomment">/// Get the base direction of the text provided according to the Unicode Bidirectional Algorithm.
///
/// See rules P2 and P3.
///
...
</span><span class="attr">#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
#[inline]
</span><span class="kw">pub fn </span>get_base_direction&lt;<span class="lifetime">'a</span>, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T) -&gt; Direction {
    get_base_direction_with_data_source(<span class="kw-2">&amp;</span>HardcodedBidiData, text)
}
...
#[inline]
</span><span class="kw">pub fn </span>get_base_direction_full&lt;<span class="lifetime">'a</span>, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T) -&gt; Direction {
    get_base_direction_full_with_data_source(<span class="kw-2">&amp;</span>HardcodedBidiData, text)
}

<span class="attr">#[inline]
</span><span class="kw">pub fn </span>get_base_direction_with_data_source&lt;<span class="lifetime">'a</span>, D: BidiDataSource, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(
    data_source: <span class="kw-2">&amp;</span>D,
    text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T,
) -&gt; Direction {
    get_base_direction_impl(data_source, text, <span class="bool-val">false</span>)
}

<span class="attr">#[inline]
</span><span class="kw">pub fn </span>get_base_direction_full_with_data_source&lt;
    <span class="lifetime">'a</span>,
    D: BidiDataSource,
    T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized,
&gt;(
    data_source: <span class="kw-2">&amp;</span>D,
    text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T,
) -&gt; Direction {
    get_base_direction_impl(data_source, text, <span class="bool-val">true</span>)
}

<span class="kw">fn </span>get_base_direction_impl&lt;<span class="lifetime">'a</span>, D: BidiDataSource, T: TextSource&lt;<span class="lifetime">'a</span>&gt; + <span class="question-mark">?</span>Sized&gt;(
    data_source: <span class="kw-2">&amp;</span>D,
    text: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>T,
    use_full_text: bool,
) -&gt; Direction {
    <span class="kw">let </span><span class="kw-2">mut </span>isolate_level = <span class="number">0</span>;
    <span class="kw">for </span>c <span class="kw">in </span>text.chars() {
        <span class="kw">match </span>data_source.bidi_class(c) {
            LRI | RLI | FSI =&gt; isolate_level = isolate_level + <span class="number">1</span>,
            PDI <span class="kw">if </span>isolate_level &gt; <span class="number">0 </span>=&gt; isolate_level = isolate_level - <span class="number">1</span>,
            L <span class="kw">if </span>isolate_level == <span class="number">0 </span>=&gt; <span class="kw">return </span>Direction::Ltr,
            R | AL <span class="kw">if </span>isolate_level == <span class="number">0 </span>=&gt; <span class="kw">return </span>Direction::Rtl,
            B <span class="kw">if </span>!use_full_text =&gt; <span class="kw">break</span>,
            B <span class="kw">if </span>use_full_text =&gt; isolate_level = <span class="number">0</span>,
            <span class="kw">_ </span>=&gt; (),
        }
    }
    <span class="comment">// If no strong char was found, return Mixed. Normally this will be treated as Ltr by callers
    ...
    </span>Direction::Mixed
}

<span class="doccomment">/// Implementation of TextSource for UTF-8 text (a string slice).
</span><span class="kw">impl</span>&lt;<span class="lifetime">'text</span>&gt; TextSource&lt;<span class="lifetime">'text</span>&gt; <span class="kw">for </span>str {
    <span class="kw">type </span>CharIter = core::str::Chars&lt;<span class="lifetime">'text</span>&gt;;
    <span class="kw">type </span>CharIndexIter = core::str::CharIndices&lt;<span class="lifetime">'text</span>&gt;;
    <span class="kw">type </span>IndexLenIter = Utf8IndexLenIter&lt;<span class="lifetime">'text</span>&gt;;

    <span class="attr">#[inline]
    </span><span class="kw">fn </span>len(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        (<span class="self">self </span><span class="kw">as </span><span class="kw-2">&amp;</span>str).len()
    }
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>char_at(<span class="kw-2">&amp;</span><span class="self">self</span>, index: usize) -&gt; <span class="prelude-ty">Option</span>&lt;(char, usize)&gt; {
        <span class="kw">if let </span><span class="prelude-val">Some</span>(slice) = <span class="self">self</span>.get(index..) {
            <span class="kw">if let </span><span class="prelude-val">Some</span>(ch) = slice.chars().next() {
                <span class="kw">return </span><span class="prelude-val">Some</span>((ch, ch.len_utf8()));
            }
        }
    ...
    <span class="attr">#[inline]
        ...
        <span class="kw-2">&amp;</span>(<span class="self">self </span><span class="kw">as </span><span class="kw-2">&amp;</span>str)[range]
    }
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>chars(<span class="kw-2">&amp;</span><span class="lifetime">'text </span><span class="self">self</span>) -&gt; <span class="self">Self</span>::CharIter {
        (<span class="self">self </span><span class="kw">as </span><span class="kw-2">&amp;</span>str).chars()
    }
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>char_indices(<span class="kw-2">&amp;</span><span class="lifetime">'text </span><span class="self">self</span>) -&gt; <span class="self">Self</span>::CharIndexIter {
        (<span class="self">self </span><span class="kw">as </span><span class="kw-2">&amp;</span>str).char_indices()
    }
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>indices_lengths(<span class="kw-2">&amp;</span><span class="lifetime">'text </span><span class="self">self</span>) -&gt; <span class="self">Self</span>::IndexLenIter {
        Utf8IndexLenIter::new(<span class="kw-2">&amp;</span><span class="self">self</span>)
    }
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>char_len(ch: char) -&gt; usize {
        ch.len_utf8()
...
<span class="doccomment">/// Iterator over (UTF-8) string slices returning (index, char_len) tuple.
</span><span class="attr">#[derive(Debug)]
</span><span class="kw">pub struct </span>Utf8IndexLenIter&lt;<span class="lifetime">'text</span>&gt; {
    iter: CharIndices&lt;<span class="lifetime">'text</span>&gt;,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'text</span>&gt; Utf8IndexLenIter&lt;<span class="lifetime">'text</span>&gt; {
    <span class="attr">#[inline]
    </span><span class="kw">pub fn </span>new(text: <span class="kw-2">&amp;</span><span class="lifetime">'text </span>str) -&gt; <span class="self">Self </span>{
        Utf8IndexLenIter {
...
<span class="kw">impl </span>Iterator <span class="kw">for </span>Utf8IndexLenIter&lt;<span class="lifetime">'_</span>&gt; {
    ...
    <span class="attr">#[inline]
        ...
        <span class="kw">if let </span><span class="prelude-val">Some</span>((pos, ch)) = <span class="self">self</span>.iter.next() {
            <span class="kw">return </span><span class="prelude-val">Some</span>((pos, ch.len_utf8()));
        }
        <span class="prelude-val">None
    </span>}
}
...
</span><span class="kw">fn </span>to_utf16(s: <span class="kw-2">&amp;</span>str) -&gt; Vec&lt;u16&gt; {
...
#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
    ...
    <span class="kw">use </span>utf16::{
        BidiInfo <span class="kw">as </span>BidiInfoU16, InitialInfo <span class="kw">as </span>InitialInfoU16, Paragraph <span class="kw">as </span>ParagraphU16,
        ParagraphBidiInfo <span class="kw">as </span>ParagraphBidiInfoU16,
    };

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_utf16_text_source() {
        <span class="kw">let </span>text: <span class="kw-2">&amp;</span>[u16] =
            <span class="kw-2">&amp;</span>[<span class="number">0x41</span>, <span class="number">0xD801</span>, <span class="number">0xDC01</span>, <span class="number">0x20</span>, <span class="number">0xD800</span>, <span class="number">0x20</span>, <span class="number">0xDFFF</span>, <span class="number">0x20</span>, <span class="number">0xDC00</span>, <span class="number">0xD800</span>];
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">0</span>), <span class="prelude-val">Some</span>((<span class="string">'A'</span>, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">1</span>), <span class="prelude-val">Some</span>((<span class="string">'\u{10401}'</span>, <span class="number">2</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">2</span>), <span class="prelude-val">None</span>);
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">3</span>), <span class="prelude-val">Some</span>((<span class="string">' '</span>, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">4</span>), <span class="prelude-val">Some</span>((char::REPLACEMENT_CHARACTER, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">5</span>), <span class="prelude-val">Some</span>((<span class="string">' '</span>, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">6</span>), <span class="prelude-val">Some</span>((char::REPLACEMENT_CHARACTER, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">7</span>), <span class="prelude-val">Some</span>((<span class="string">' '</span>, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">8</span>), <span class="prelude-val">Some</span>((char::REPLACEMENT_CHARACTER, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">9</span>), <span class="prelude-val">Some</span>((char::REPLACEMENT_CHARACTER, <span class="number">1</span>)));
        <span class="macro">assert_eq!</span>(text.char_at(<span class="number">10</span>), <span class="prelude-val">None</span>);
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_utf16_char_iter() {
        <span class="kw">let </span>text: <span class="kw-2">&amp;</span>[u16] =
            <span class="kw-2">&amp;</span>[<span class="number">0x41</span>, <span class="number">0xD801</span>, <span class="number">0xDC01</span>, <span class="number">0x20</span>, <span class="number">0xD800</span>, <span class="number">0x20</span>, <span class="number">0xDFFF</span>, <span class="number">0x20</span>, <span class="number">0xDC00</span>, <span class="number">0xD800</span>];
        <span class="macro">assert_eq!</span>(text.len(), <span class="number">10</span>);
        <span class="macro">assert_eq!</span>(text.chars().count(), <span class="number">9</span>);
        <span class="kw">let </span><span class="kw-2">mut </span>chars = text.chars();
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">'A'</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">'\u{10401}'</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">' '</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">'\u{FFFD}'</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">' '</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">'\u{FFFD}'</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">' '</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">'\u{FFFD}'</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">Some</span>(<span class="string">'\u{FFFD}'</span>));
        <span class="macro">assert_eq!</span>(chars.next(), <span class="prelude-val">None</span>);
    }
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            (
                ...
                </span><span class="string">"a1"</span>,
                <span class="comment">// expected bidi classes per utf-8 byte
                </span><span class="macro">vec!</span>[L, EN],
                <span class="comment">// expected paragraph-info for utf-8
                </span><span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">2</span>,
                    level: LTR_LEVEL,
                }],
                <span class="comment">// expected bidi classes per utf-16 code unit
                </span><span class="macro">vec!</span>[L, EN],
                <span class="comment">// expected paragraph-info for utf-16
                </span><span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">2</span>,
                    level: LTR_LEVEL,
                }],
            ...
            (
                <span class="comment">// Arabic, space, Hebrew
                </span><span class="string">"\u{0639} \u{05D0}"</span>,
                <span class="macro">vec!</span>[AL, AL, WS, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">5</span>,
                    level: RTL_LEVEL,
                }],
                <span class="macro">vec!</span>[AL, WS, R],
                <span class="macro">vec!</span>[ParagraphInfo {
            ...
            (
                <span class="comment">// SMP characters from Kharoshthi, Cuneiform, Adlam:
                </span><span class="string">"\u{10A00}\u{12000}\u{1E900}"</span>,
                <span class="macro">vec!</span>[R, R, R, R, L, L, L, L, R, R, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">12</span>,
                    level: RTL_LEVEL,
                }],
                <span class="macro">vec!</span>[R, R, L, L, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
            ...
            (
                <span class="string">"a\u{2029}b"</span>,
                <span class="macro">vec!</span>[L, B, B, B, L],
                ...
                <span class="macro">vec!</span>[L, B, L],
                <span class="macro">vec!</span>[
                    ParagraphInfo {
                        range: <span class="number">0</span>..<span class="number">2</span>,
                        level: LTR_LEVEL,
                    ...
                    ParagraphInfo {
                        range: <span class="number">2</span>..<span class="number">3</span>,
                        level: LTR_LEVEL,
                    },
                ],
            ),
            (
                <span class="string">"\u{2068}א\u{2069}a"</span>, <span class="comment">// U+2068 FSI, U+2069 PDI
                </span><span class="macro">vec!</span>[RLI, RLI, RLI, R, R, PDI, PDI, PDI, L],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">9</span>,
                    level: LTR_LEVEL,
                }],
                <span class="macro">vec!</span>[RLI, R, PDI, L],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">4</span>,
                    level: LTR_LEVEL,
                }],
        ...
        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(
                InitialInfo::new(t.<span class="number">0</span>, <span class="prelude-val">None</span>),
                InitialInfo {
                    text: t.<span class="number">0</span>,
                    original_classes: t.<span class="number">1</span>,
                    paragraphs: t.<span class="number">2</span>,
                }
            );
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>);
            <span class="macro">assert_eq!</span>(
                InitialInfoU16::new(text, <span class="prelude-val">None</span>),
                InitialInfoU16 {
                    text,
                    original_classes: t.<span class="number">3</span>,
                    paragraphs: t.<span class="number">4</span>,
                }
            );
        }
    }
    ...
    #[cfg(feature = <span class="string">"hardcoded-data"</span>)]
        ...
        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            (
                <span class="comment">// text
                </span><span class="string">"abc123"</span>,
                <span class="comment">// base level
                </span><span class="prelude-val">Some</span>(LTR_LEVEL),
                <span class="comment">// levels
                </span>Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>]),
                <span class="comment">// original_classes
                </span><span class="macro">vec!</span>[L, L, L, EN, EN, EN],
                <span class="comment">// paragraphs
                </span><span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">6</span>,
                    level: LTR_LEVEL,
                }],
                <span class="comment">// levels_u16
                </span>Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>]),
                <span class="comment">// original_classes_u16
                </span><span class="macro">vec!</span>[L, L, L, EN, EN, EN],
                <span class="comment">// paragraphs_u16
                </span><span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">6</span>,
                    level: LTR_LEVEL,
                }],
            ...
            (
                <span class="string">"abc \u{05D0}\u{05D1}\u{05D2}"</span>,
                <span class="prelude-val">Some</span>(LTR_LEVEL),
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="macro">vec!</span>[L, L, L, WS, R, R, R, R, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">10</span>,
                    level: LTR_LEVEL,
                }],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="macro">vec!</span>[L, L, L, WS, R, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">7</span>,
                    level: LTR_LEVEL,
                }],
            ),
            (
                <span class="string">"abc \u{05D0}\u{05D1}\u{05D2}"</span>,
                <span class="prelude-val">Some</span>(RTL_LEVEL),
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="macro">vec!</span>[L, L, L, WS, R, R, R, R, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">10</span>,
                    level: RTL_LEVEL,
                }],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="macro">vec!</span>[L, L, L, WS, R, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">7</span>,
                    level: RTL_LEVEL,
            ...
            (
                <span class="string">"\u{05D0}\u{05D1}\u{05D2} abc"</span>,
                <span class="prelude-val">Some</span>(LTR_LEVEL),
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>]),
                <span class="macro">vec!</span>[R, R, R, R, R, R, WS, L, L, L],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">10</span>,
                    level: LTR_LEVEL,
                }],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>]),
                <span class="macro">vec!</span>[R, R, R, WS, L, L, L],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">7</span>,
                    level: LTR_LEVEL,
                }],
            ...
            (
                <span class="string">"\u{05D0}\u{05D1}\u{05D2} abc"</span>,
                <span class="prelude-val">None</span>,
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>]),
                <span class="macro">vec!</span>[R, R, R, R, R, R, WS, L, L, L],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">10</span>,
                    level: RTL_LEVEL,
                }],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>]),
                <span class="macro">vec!</span>[R, R, R, WS, L, L, L],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">7</span>,
                    level: RTL_LEVEL,
            ...
            (
                <span class="string">"\u{063A}2\u{0638} \u{05D0}2\u{05D2}"</span>,
                <span class="prelude-val">Some</span>(LTR_LEVEL),
                ...
                <span class="macro">vec!</span>[AL, AL, EN, AL, AL, WS, R, R, EN, R, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">11</span>,
                    level: LTR_LEVEL,
                }],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">2</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">1</span>]),
                <span class="macro">vec!</span>[AL, EN, AL, WS, R, EN, R],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">7</span>,
                    level: LTR_LEVEL,
                }],
            ...
            (
                <span class="string">"a א.\nג"</span>,
                <span class="prelude-val">None</span>,
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="macro">vec!</span>[L, WS, R, R, CS, B, R, R],
                <span class="macro">vec!</span>[
                    ParagraphInfo {
                        range: <span class="number">0</span>..<span class="number">6</span>,
                        level: LTR_LEVEL,
                ...
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>]),
                <span class="macro">vec!</span>[L, WS, R, CS, B, R],
                <span class="macro">vec!</span>[
                    ParagraphInfo {
                        range: <span class="number">0</span>..<span class="number">5</span>,
                        level: LTR_LEVEL,
                    ...
                    ParagraphInfo {
                        range: <span class="number">5</span>..<span class="number">6</span>,
                        level: RTL_LEVEL,
                    },
                ],
            ),
            <span class="comment">// BidiTest:69635 (AL ET EN)
                ...
                <span class="string">"\u{060B}\u{20CF}\u{06F9}"</span>,
                <span class="prelude-val">None</span>,
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>]),
                <span class="macro">vec!</span>[AL, AL, ET, ET, ET, EN, EN],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">7</span>,
                    level: RTL_LEVEL,
                }],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>]),
                <span class="macro">vec!</span>[AL, ET, EN],
                <span class="macro">vec!</span>[ParagraphInfo {
                    range: <span class="number">0</span>..<span class="number">3</span>,
                    level: RTL_LEVEL,
                }],
        ...
        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(
                BidiInfo::new(t.<span class="number">0</span>, t.<span class="number">1</span>),
                BidiInfo {
                    text: t.<span class="number">0</span>,
                    levels: t.<span class="number">2</span>.clone(),
                    original_classes: t.<span class="number">3</span>.clone(),
                    paragraphs: t.<span class="number">4</span>.clone(),
                }
            );
            <span class="comment">// If it was a single paragraph, also test ParagraphBidiInfo.
            </span><span class="kw">if </span>t.<span class="number">4</span>.len() == <span class="number">1 </span>{
                <span class="macro">assert_eq!</span>(
                    ParagraphBidiInfo::new(t.<span class="number">0</span>, t.<span class="number">1</span>),
                    ParagraphBidiInfo {
                        text: t.<span class="number">0</span>,
                        original_classes: t.<span class="number">3</span>,
                        levels: t.<span class="number">2</span>.clone(),
                        paragraph_level: t.<span class="number">4</span>[<span class="number">0</span>].level,
                        is_pure_ltr: !level::has_rtl(<span class="kw-2">&amp;</span>t.<span class="number">2</span>),
                    }
                )
            }
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>);
            <span class="macro">assert_eq!</span>(
                BidiInfoU16::new(text, t.<span class="number">1</span>),
                BidiInfoU16 {
                    text,
                    levels: t.<span class="number">5</span>.clone(),
                    original_classes: t.<span class="number">6</span>.clone(),
                    paragraphs: t.<span class="number">7</span>.clone(),
                }
            );
            <span class="kw">if </span>t.<span class="number">7</span>.len() == <span class="number">1 </span>{
                <span class="macro">assert_eq!</span>(
                    ParagraphBidiInfoU16::new(text, t.<span class="number">1</span>),
                    ParagraphBidiInfoU16 {
                        text: text,
                        original_classes: t.<span class="number">6</span>.clone(),
                        levels: t.<span class="number">5</span>.clone(),
                        paragraph_level: t.<span class="number">7</span>[<span class="number">0</span>].level,
                        is_pure_ltr: !level::has_rtl(<span class="kw-2">&amp;</span>t.<span class="number">5</span>),
                    }
                )
            }
        }
    ...
    <span class="attr">#[test]
    #[cfg(feature = <span class="string">"hardcoded-data"</span>)]
    </span><span class="kw">fn </span>test_paragraph_bidi_info() {
        <span class="comment">// Passing text that includes a paragraph break to the ParagraphBidiInfo API:
        ...
        // consistent. The embedded paragraph break acts like a separator (tab) would.
            ...
            (
                <span class="string">"a א.\nג"</span>,
                <span class="prelude-val">None</span>,
                <span class="comment">// utf-8 results:
                </span><span class="macro">vec!</span>[L, WS, R, R, CS, B, R, R],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="comment">// utf-16 results:
                </span><span class="macro">vec!</span>[L, WS, R, CS, B, R],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="comment">// paragraph level; is_pure_ltr
                </span>LTR_LEVEL,
                <span class="bool-val">false</span>,
            ),
            (
                <span class="string">"\u{5d1} a.\nb."</span>,
                <span class="prelude-val">None</span>,
                <span class="comment">// utf-8 results:
                </span><span class="macro">vec!</span>[R, R, WS, L, CS, B, L, CS],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">1</span>]),
                <span class="comment">// utf-16 results:
                </span><span class="macro">vec!</span>[R, WS, L, CS, B, L, CS],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">1</span>]),
                <span class="comment">// paragraph level; is_pure_ltr
                </span>RTL_LEVEL,
                <span class="bool-val">false</span>,
            ),
            (
                <span class="string">"a א.\tג"</span>,
                <span class="prelude-val">None</span>,
                <span class="comment">// utf-8 results:
                </span><span class="macro">vec!</span>[L, WS, R, R, CS, S, R, R],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="comment">// utf-16 results:
                </span><span class="macro">vec!</span>[L, WS, R, CS, S, R],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>]),
                <span class="comment">// paragraph level; is_pure_ltr
                </span>LTR_LEVEL,
                <span class="bool-val">false</span>,
            ...
            (
                <span class="string">"\u{5d1} a.\tb."</span>,
                <span class="prelude-val">None</span>,
                <span class="comment">// utf-8 results:
                </span><span class="macro">vec!</span>[R, R, WS, L, CS, S, L, CS],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">1</span>]),
                <span class="comment">// utf-16 results:
                </span><span class="macro">vec!</span>[R, WS, L, CS, S, L, CS],
                Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">2</span>, <span class="number">1</span>]),
                <span class="comment">// paragraph level; is_pure_ltr
                </span>RTL_LEVEL,
                <span class="bool-val">false</span>,
        ...
        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(
                ...
                ParagraphBidiInfo {
                    text: t.<span class="number">0</span>,
                    original_classes: t.<span class="number">2</span>,
                    levels: t.<span class="number">3</span>,
                    paragraph_level: t.<span class="number">6</span>,
                    is_pure_ltr: t.<span class="number">7</span>,
                }
            );
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>);
            <span class="macro">assert_eq!</span>(
                ...
                ParagraphBidiInfoU16 {
                    text: text,
                    original_classes: t.<span class="number">4</span>,
                    levels: t.<span class="number">5</span>,
                    paragraph_level: t.<span class="number">6</span>,
                    is_pure_ltr: t.<span class="number">7</span>,
                }
            );
    ...
    #[cfg(feature = <span class="string">"hardcoded-data"</span>)]
        ...
        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            ...
            </span>(<span class="string">"123"</span>, <span class="prelude-val">None</span>, <span class="bool-val">false</span>),
            (<span class="string">"123"</span>, <span class="prelude-val">Some</span>(LTR_LEVEL), <span class="bool-val">false</span>),
            (<span class="string">"123"</span>, <span class="prelude-val">Some</span>(RTL_LEVEL), <span class="bool-val">false</span>),
            (<span class="string">"abc"</span>, <span class="prelude-val">None</span>, <span class="bool-val">false</span>),
            (<span class="string">"abc"</span>, <span class="prelude-val">Some</span>(LTR_LEVEL), <span class="bool-val">false</span>),
            (<span class="string">"abc"</span>, <span class="prelude-val">Some</span>(RTL_LEVEL), <span class="bool-val">false</span>),
            (<span class="string">"abc 123"</span>, <span class="prelude-val">None</span>, <span class="bool-val">false</span>),
            (<span class="string">"abc\n123"</span>, <span class="prelude-val">None</span>, <span class="bool-val">false</span>),
            <span class="comment">// With Hebrew
            </span>(<span class="string">"\u{05D0}\u{05D1}\u{05BC}\u{05D2}"</span>, <span class="prelude-val">None</span>, <span class="bool-val">true</span>),
            (<span class="string">"\u{05D0}\u{05D1}\u{05BC}\u{05D2}"</span>, <span class="prelude-val">Some</span>(LTR_LEVEL), <span class="bool-val">true</span>),
            (<span class="string">"\u{05D0}\u{05D1}\u{05BC}\u{05D2}"</span>, <span class="prelude-val">Some</span>(RTL_LEVEL), <span class="bool-val">true</span>),
            (<span class="string">"abc \u{05D0}\u{05D1}\u{05BC}\u{05D2}"</span>, <span class="prelude-val">None</span>, <span class="bool-val">true</span>),
            (<span class="string">"abc\n\u{05D0}\u{05D1}\u{05BC}\u{05D2}"</span>, <span class="prelude-val">None</span>, <span class="bool-val">true</span>),
            (<span class="string">"\u{05D0}\u{05D1}\u{05BC}\u{05D2} abc"</span>, <span class="prelude-val">None</span>, <span class="bool-val">true</span>),
            (<span class="string">"\u{05D0}\u{05D1}\u{05BC}\u{05D2}\nabc"</span>, <span class="prelude-val">None</span>, <span class="bool-val">true</span>),
            (<span class="string">"\u{05D0}\u{05D1}\u{05BC}\u{05D2} 123"</span>, <span class="prelude-val">None</span>, <span class="bool-val">true</span>),
            (<span class="string">"\u{05D0}\u{05D1}\u{05BC}\u{05D2}\n123"</span>, <span class="prelude-val">None</span>, <span class="bool-val">true</span>),
        ];

        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(BidiInfo::new(t.<span class="number">0</span>, t.<span class="number">1</span>).has_rtl(), t.<span class="number">2</span>);
            <span class="macro">assert_eq!</span>(BidiInfoU16::new(<span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>), t.<span class="number">1</span>).has_rtl(), t.<span class="number">2</span>);
        }
    }
    ...
    <span class="attr">#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
    </span><span class="kw">fn </span>reorder_paras_u16(text: <span class="kw-2">&amp;</span>[u16]) -&gt; Vec&lt;Cow&lt;<span class="lifetime">'_</span>, [u16]&gt;&gt; {
        <span class="kw">let </span>bidi_info = BidiInfoU16::new(text, <span class="prelude-val">None</span>);
        bidi_info
            .paragraphs
            .iter()
            .map(|para| bidi_info.reorder_line(para, para.range.clone()))
    ...
    #[cfg(feature = <span class="string">"hardcoded-data"</span>)]
        ...
        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            ...
            <span class="comment">// Bidi_Class: AL AL AL B L L L
                ...
                <span class="string">"\u{0627}\u{0628}\u{062C}\nabc"</span>,
                <span class="macro">vec!</span>[<span class="string">"\n\u{062C}\u{0628}\u{0627}"</span>, <span class="string">"abc"</span>],
            ),
            (<span class="string">"1.-2"</span>, <span class="macro">vec!</span>[<span class="string">"1.-2"</span>]),
            (<span class="string">"1-.2"</span>, <span class="macro">vec!</span>[<span class="string">"1-.2"</span>]),
            (<span class="string">"abc אבג"</span>, <span class="macro">vec!</span>[<span class="string">"abc גבא"</span>]),
            <span class="comment">// Numbers being weak LTR characters, cannot reorder strong RTL
            </span>(<span class="string">"123 \u{05D0}\u{05D1}\u{05D2}"</span>, <span class="macro">vec!</span>[<span class="string">"גבא 123"</span>]),
            (<span class="string">"abc\u{202A}def"</span>, <span class="macro">vec!</span>[<span class="string">"abc\u{202A}def"</span>]),
            (
                <span class="string">"abc\u{202A}def\u{202C}ghi"</span>,
                <span class="macro">vec!</span>[<span class="string">"abc\u{202A}def\u{202C}ghi"</span>],
            ),
            (
                <span class="string">"abc\u{2066}def\u{2069}ghi"</span>,
                <span class="macro">vec!</span>[<span class="string">"abc\u{2066}def\u{2069}ghi"</span>],
            ),
            <span class="comment">// Testing for RLE Character
            </span>(<span class="string">"\u{202B}abc אבג\u{202C}"</span>, <span class="macro">vec!</span>[<span class="string">"\u{202b}גבא abc\u{202c}"</span>]),
            <span class="comment">// Testing neutral characters
            </span>(<span class="string">"\u{05D0}בג? אבג"</span>, <span class="macro">vec!</span>[<span class="string">"גבא ?גבא"</span>]),
            <span class="comment">// Testing neutral characters with special case
            </span>(<span class="string">"A אבג?"</span>, <span class="macro">vec!</span>[<span class="string">"A גבא?"</span>]),
            <span class="comment">// Testing neutral characters with Implicit RTL Marker
            </span>(<span class="string">"A אבג?\u{200F}"</span>, <span class="macro">vec!</span>[<span class="string">"A \u{200F}?גבא"</span>]),
            (<span class="string">"\u{05D0}בג abc"</span>, <span class="macro">vec!</span>[<span class="string">"abc גבא"</span>]),
            (<span class="string">"abc\u{2067}.-\u{2069}ghi"</span>, <span class="macro">vec!</span>[<span class="string">"abc\u{2067}-.\u{2069}ghi"</span>]),
            (
                <span class="string">"Hello, \u{2068}\u{202E}world\u{202C}\u{2069}!"</span>,
            ...
            </span>(<span class="string">"\u{05D0}(ב)ג."</span>, <span class="macro">vec!</span>[<span class="string">".ג)ב(א"</span>]),
            <span class="comment">// With mirrorable characters on level boundary
            </span>(<span class="string">"\u{05D0}ב(גד[&amp;ef].)gh"</span>, <span class="macro">vec!</span>[<span class="string">"gh).]ef&amp;[דג(בא"</span>]),
        ];

        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(reorder_paras(t.<span class="number">0</span>), t.<span class="number">1</span>);
            <span class="kw">let </span>expect_utf16 = t.<span class="number">1</span>.iter().map(|v| to_utf16(v)).collect::&lt;Vec&lt;<span class="kw">_</span>&gt;&gt;();
            <span class="macro">assert_eq!</span>(reorder_paras_u16(<span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>)), expect_utf16);
        }
    }
    ...
    <span class="kw">fn </span>reordered_levels_for_paras_u16(text: <span class="kw-2">&amp;</span>[u16]) -&gt; Vec&lt;Vec&lt;Level&gt;&gt; {
        <span class="kw">let </span>bidi_info = BidiInfoU16::new(text, <span class="prelude-val">None</span>);
        bidi_info
            .paragraphs
            .iter()
    ...
    <span class="kw">fn </span>reordered_levels_per_char_for_paras_u16(text: <span class="kw-2">&amp;</span>[u16]) -&gt; Vec&lt;Vec&lt;Level&gt;&gt; {
        <span class="kw">let </span>bidi_info = BidiInfoU16::new(text, <span class="prelude-val">None</span>);
        bidi_info
            .paragraphs
            .iter()
    ...
    #[cfg(feature = <span class="string">"hardcoded-data"</span>)]
        ...
        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            <span class="comment">// BidiTest:946 (LRI PDI)
                ...
                <span class="string">"\u{2067}\u{2069}"</span>,
            ...
            <span class="comment">// BidiTest:69635 (AL ET EN)
                ...
                <span class="string">"\u{060B}\u{20CF}\u{06F9}"</span>,
                <span class="macro">vec!</span>[Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>, <span class="number">2</span>])],
                <span class="macro">vec!</span>[Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>])],
                <span class="macro">vec!</span>[Level::vec(<span class="kw-2">&amp;</span>[<span class="number">1</span>, <span class="number">1</span>, <span class="number">2</span>])],
            ),
        ];

        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(reordered_levels_for_paras(t.<span class="number">0</span>), t.<span class="number">1</span>);
            <span class="macro">assert_eq!</span>(reordered_levels_per_char_for_paras(t.<span class="number">0</span>), t.<span class="number">2</span>);
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>);
            <span class="macro">assert_eq!</span>(reordered_levels_for_paras_u16(text), t.<span class="number">3</span>);
            <span class="macro">assert_eq!</span>(reordered_levels_per_char_for_paras_u16(text), t.<span class="number">2</span>);
        }

        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            <span class="comment">// BidiTest:291284 (AN RLI PDF R)
            </span>(
                <span class="string">"\u{0605}\u{2067}\u{202C}\u{0590}"</span>,
                <span class="macro">vec!</span>[<span class="kw-2">&amp;</span>[<span class="string">"2"</span>, <span class="string">"2"</span>, <span class="string">"0"</span>, <span class="string">"0"</span>, <span class="string">"0"</span>, <span class="string">"x"</span>, <span class="string">"x"</span>, <span class="string">"x"</span>, <span class="string">"1"</span>, <span class="string">"1"</span>]],
                <span class="macro">vec!</span>[<span class="kw-2">&amp;</span>[<span class="string">"2"</span>, <span class="string">"0"</span>, <span class="string">"x"</span>, <span class="string">"1"</span>]],
                <span class="macro">vec!</span>[<span class="kw-2">&amp;</span>[<span class="string">"2"</span>, <span class="string">"0"</span>, <span class="string">"x"</span>, <span class="string">"1"</span>]],
            ),
        ];
        ...
        <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(text);
        <span class="kw">let </span>bidi_info = BidiInfoU16::new(text, <span class="prelude-val">None</span>);
        <span class="macro">assert_eq!</span>(
            bidi_info.reordered_levels(<span class="kw-2">&amp;</span>bidi_info.paragraphs[<span class="number">0</span>], <span class="number">1</span>..<span class="number">4</span>),
            Level::vec(<span class="kw-2">&amp;</span>[<span class="number">0</span>, <span class="number">0</span>, <span class="number">0</span>, <span class="number">1</span>, <span class="number">1</span>]),
        );
    }
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span>text2 = <span class="kw-2">&amp;</span>to_utf16(text2);
        <span class="kw">let </span>whole_text = <span class="kw-2">&amp;</span>to_utf16(<span class="kw-2">&amp;</span>whole_text);
        <span class="kw">let </span>bidi_info = BidiInfoU16::new(<span class="kw-2">&amp;</span>whole_text, <span class="prelude-val">None</span>);
        <span class="macro">assert_eq!</span>(bidi_info.paragraphs.len(), <span class="number">2</span>);

        <span class="macro">assert_eq!</span>(bidi_info.paragraphs[<span class="number">0</span>].len(), text.len() + <span class="number">1</span>);
        <span class="macro">assert_eq!</span>(bidi_info.paragraphs[<span class="number">1</span>].len(), text2.len());
    }
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span>all_paragraphs = <span class="kw-2">&amp;</span>to_utf16(<span class="kw-2">&amp;</span>all_paragraphs);
        <span class="kw">let </span>bidi_info = BidiInfoU16::new(<span class="kw-2">&amp;</span>all_paragraphs, <span class="prelude-val">None</span>);
        <span class="macro">assert_eq!</span>(bidi_info.paragraphs.len(), <span class="number">3</span>);
        <span class="kw">let </span>p_ltr = ParagraphU16::new(<span class="kw-2">&amp;</span>bidi_info, <span class="kw-2">&amp;</span>bidi_info.paragraphs[<span class="number">0</span>]);
        <span class="kw">let </span>p_rtl = ParagraphU16::new(<span class="kw-2">&amp;</span>bidi_info, <span class="kw-2">&amp;</span>bidi_info.paragraphs[<span class="number">1</span>]);
        <span class="kw">let </span>p_mixed = ParagraphU16::new(<span class="kw-2">&amp;</span>bidi_info, <span class="kw-2">&amp;</span>bidi_info.paragraphs[<span class="number">2</span>]);
        <span class="macro">assert_eq!</span>(p_ltr.direction(), Direction::Ltr);
        <span class="macro">assert_eq!</span>(p_rtl.direction(), Direction::Rtl);
        <span class="macro">assert_eq!</span>(p_mixed.direction(), Direction::Mixed);
    }
    ...
    <span class="attr">#[test]
        ...
        <span class="kw">let </span>empty = <span class="kw-2">&amp;</span>to_utf16(empty);
        <span class="kw">let </span>bidi_info = BidiInfoU16::new(empty, Option::from(RTL_LEVEL));
        <span class="macro">assert_eq!</span>(bidi_info.paragraphs.len(), <span class="number">0</span>);

        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            <span class="comment">// The paragraph separator will take the value of the default direction
            // which is left to right.
            </span>(<span class="string">"\n"</span>, <span class="prelude-val">None</span>, Direction::Ltr),
            <span class="comment">// The paragraph separator will take the value of the given initial direction
            // which is left to right.
            </span>(<span class="string">"\n"</span>, Option::from(LTR_LEVEL), Direction::Ltr),
            <span class="comment">// The paragraph separator will take the value of the given initial direction
            // which is right to left.
            </span>(<span class="string">"\n"</span>, Option::from(RTL_LEVEL), Direction::Rtl),
        ];

        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="kw">let </span>bidi_info = BidiInfo::new(t.<span class="number">0</span>, t.<span class="number">1</span>);
            <span class="macro">assert_eq!</span>(bidi_info.paragraphs.len(), <span class="number">1</span>);
            ...
            <span class="macro">assert_eq!</span>(p.direction(), t.<span class="number">2</span>);
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>);
            <span class="kw">let </span>bidi_info = BidiInfoU16::new(text, t.<span class="number">1</span>);
            <span class="kw">let </span>p = ParagraphU16::new(<span class="kw-2">&amp;</span>bidi_info, <span class="kw-2">&amp;</span>bidi_info.paragraphs[<span class="number">0</span>]);
            <span class="macro">assert_eq!</span>(p.direction(), t.<span class="number">2</span>);
        }
    }
    ...
    <span class="attr">#[test]
        ...
        <span class="macro">assert_eq!</span>(p_rtl.level_at(<span class="number">0</span>), RTL_LEVEL);
        <span class="macro">assert_eq!</span>(p_mixed.level_at(<span class="number">0</span>), LTR_LEVEL);
        <span class="macro">assert_eq!</span>(p_mixed.info.levels.len(), <span class="number">40</span>);
        <span class="macro">assert_eq!</span>(p_mixed.para.range.start, <span class="number">21</span>);
        <span class="macro">assert_eq!</span>(p_mixed.level_at(ltr_text.len()), RTL_LEVEL);
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_get_base_direction() {
        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            (<span class="string">""</span>, Direction::Mixed), <span class="comment">// return Mixed if no strong character found
            </span>(<span class="string">"123[]-+\u{2019}\u{2060}\u{00bf}?"</span>, Direction::Mixed),
            (<span class="string">"3.14\npi"</span>, Direction::Mixed), <span class="comment">// only first paragraph is considered
            </span>(<span class="string">"[123 'abc']"</span>, Direction::Ltr),
            (<span class="string">"[123 '\u{0628}' abc"</span>, Direction::Rtl),
            (<span class="string">"[123 '\u{2066}abc\u{2069}'\u{0628}]"</span>, Direction::Rtl), <span class="comment">// embedded isolate is ignored
            </span>(<span class="string">"[123 '\u{2066}abc\u{2068}'\u{0628}]"</span>, Direction::Mixed),
        ];

        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(get_base_direction(t.<span class="number">0</span>), t.<span class="number">1</span>);
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>);
            <span class="macro">assert_eq!</span>(get_base_direction(text.as_slice()), t.<span class="number">1</span>);
        }
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_get_base_direction_full() {
        <span class="kw">let </span>tests = <span class="macro">vec!</span>[
            (<span class="string">""</span>, Direction::Mixed), <span class="comment">// return Mixed if no strong character found
            </span>(<span class="string">"123[]-+\u{2019}\u{2060}\u{00bf}?"</span>, Direction::Mixed),
            (<span class="string">"3.14\npi"</span>, Direction::Ltr), <span class="comment">// direction taken from the second paragraph
            </span>(<span class="string">"3.14\n\u{05D0}"</span>, Direction::Rtl), <span class="comment">// direction taken from the second paragraph
            </span>(<span class="string">"[123 'abc']"</span>, Direction::Ltr),
            (<span class="string">"[123 '\u{0628}' abc"</span>, Direction::Rtl),
            (<span class="string">"[123 '\u{2066}abc\u{2069}'\u{0628}]"</span>, Direction::Rtl), <span class="comment">// embedded isolate is ignored
            </span>(<span class="string">"[123 '\u{2066}abc\u{2068}'\u{0628}]"</span>, Direction::Mixed),
            (<span class="string">"[123 '\u{2066}abc\u{2068}'\n\u{0628}]"</span>, Direction::Rtl), <span class="comment">// \n resets embedding level
        </span>];

        <span class="kw">for </span>t <span class="kw">in </span>tests {
            <span class="macro">assert_eq!</span>(get_base_direction_full(t.<span class="number">0</span>), t.<span class="number">1</span>);
            <span class="kw">let </span>text = <span class="kw-2">&amp;</span>to_utf16(t.<span class="number">0</span>);
            <span class="macro">assert_eq!</span>(get_base_direction_full(text.as_slice()), t.<span class="number">1</span>);
        }
    }
}

<span class="attr">#[cfg(all(feature = <span class="string">"serde"</span>, feature = <span class="string">"hardcoded-data"</span>, test))]
