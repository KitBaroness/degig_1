...
<span class="attr">#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
</span><span class="kw">use </span>core::cmp::Ordering::{Equal, Greater, Less};

<span class="attr">#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
</span><span class="kw">use </span><span class="self">self</span>::tables::bidi_class_table;
<span class="kw">use </span><span class="kw">crate</span>::data_source::BidiMatchedOpeningBracket;
<span class="kw">use </span><span class="kw">crate</span>::BidiClass::<span class="kw-2">*</span>;
<span class="attr">#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
</span><span class="kw">use </span><span class="kw">crate</span>::BidiDataSource;
<span class="doccomment">/// Hardcoded Bidi data that ships with the unicode-bidi crate.
///
...
</span><span class="kw">pub struct </span>HardcodedBidiData;

<span class="attr">#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
</span><span class="kw">impl </span>BidiDataSource <span class="kw">for </span>HardcodedBidiData {
    <span class="kw">fn </span>bidi_class(<span class="kw-2">&amp;</span><span class="self">self</span>, c: char) -&gt; BidiClass {
...
<span class="doccomment">/// If this character is a bracket according to BidiBrackets.txt,
/// return the corresponding *normalized* *opening bracket* of the pair,
/// and whether or not it itself is an opening bracket.
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>bidi_matched_opening_bracket(c: char) -&gt; <span class="prelude-ty">Option</span>&lt;BidiMatchedOpeningBracket&gt; {
    <span class="kw">for </span>pair <span class="kw">in </span><span class="self">self</span>::tables::bidi_pairs_table {
        <span class="kw">if </span>pair.<span class="number">0 </span>== c || pair.<span class="number">1 </span>== c {
            <span class="kw">let </span>skeleton = pair.<span class="number">2</span>.unwrap_or(pair.<span class="number">0</span>);
            <span class="kw">return </span><span class="prelude-val">Some</span>(BidiMatchedOpeningBracket {
                opening: skeleton,
                is_open: pair.<span class="number">0 </span>== c,
            });
        }
    }
...
<span class="attr">#[cfg(feature = <span class="string">"hardcoded-data"</span>)]
</span><span class="kw">fn </span>bsearch_range_value_table(c: char, r: <span class="kw-2">&amp;</span><span class="lifetime">'static </span>[(char, char, BidiClass)]) -&gt; BidiClass {
    <span class="kw">match </span>r.binary_search_by(|<span class="kw-2">&amp;</span>(lo, hi, <span class="kw">_</span>)| {
        <span class="kw">if </span>lo &lt;= c &amp;&amp; c &lt;= hi {
            Equal
...
<span class="attr">#[cfg(all(test, feature = <span class="string">"hardcoded-data"</span>))]
    ...
    <span class="attr">#[test]
        ...
        <span class="macro">assert_eq!</span>(bidi_class(<span class="string">'\u{0870}'</span>), AL);
        <span class="macro">assert_eq!</span>(bidi_class(<span class="string">'\u{089F}'</span>), NSM);
        ...
        <span class="macro">assert_eq!</span>(bidi_class(<span class="string">'\u{FDCF}'</span>), ON);
        <span class="macro">assert_eq!</span>(bidi_class(<span class="string">'\u{FDF0}'</span>), AL);
        <span class="macro">assert_eq!</span>(bidi_class(<span class="string">'\u{FDFF}'</span>), ON);
