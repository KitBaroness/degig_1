...
</pre></div><pre class="rust"><code><span class="comment">// Allow `unreachable_pub` warnings when sync is not enabled
...
</span><span class="attr">#![cfg_attr(not(feature = <span class="string">"sync"</span>), allow(unreachable_pub, dead_code))]
...
<span class="kw">use </span><span class="kw">crate</span>::util::linked_list::{<span class="self">self</span>, GuardedLinkedList, LinkedList};
...
<span class="kw">use </span>std::sync::atomic::Ordering::{<span class="self">self</span>, Acquire, Relaxed, Release, SeqCst};
...
<span class="kw">type </span>GuardedWaitList = GuardedLinkedList&lt;Waiter, &lt;Waiter <span class="kw">as </span>linked_list::Link&gt;::Target&gt;;
...
/// [`Semaphore`]: crate::sync::Semaphore
    ...
    <span class="comment">// `state` uses 2 bits to store one of `EMPTY`,
...
<span class="attr">#[derive(Debug)]
    ...
    <span class="doccomment">/// Waiting task's waker. Depending on the value of `notification`,
    ...
    <span class="doccomment">/// Notification for this waiter.
    ...
    </span>notification: AtomicNotification,
...
<span class="kw">impl </span>Waiter {
    <span class="kw">fn </span>new() -&gt; Waiter {
        Waiter {
...
<span class="comment">// No notification.
</span><span class="kw">const </span>NOTIFICATION_NONE: usize = <span class="number">0</span>;

<span class="comment">// Notification type used by `notify_one`.
</span><span class="kw">const </span>NOTIFICATION_ONE: usize = <span class="number">1</span>;

<span class="comment">// Notification type used by `notify_waiters`.
</span><span class="kw">const </span>NOTIFICATION_ALL: usize = <span class="number">2</span>;

<span class="doccomment">/// Notification for a `Waiter`.
/// This struct is equivalent to `Option&lt;Notification&gt;`, but uses
/// `AtomicUsize` inside for atomic operations.
</span><span class="attr">#[derive(Debug)]
</span><span class="kw">struct </span>AtomicNotification(AtomicUsize);

<span class="kw">impl </span>AtomicNotification {
    <span class="kw">fn </span>none() -&gt; <span class="self">Self </span>{
        AtomicNotification(AtomicUsize::new(NOTIFICATION_NONE))
    }

    <span class="doccomment">/// Store-release a notification.
    /// This method should be called exactly once.
    </span><span class="kw">fn </span>store_release(<span class="kw-2">&amp;</span><span class="self">self</span>, notification: Notification) {
        <span class="self">self</span>.<span class="number">0</span>.store(notification <span class="kw">as </span>usize, Release);
    }

    <span class="kw">fn </span>load(<span class="kw-2">&amp;</span><span class="self">self</span>, ordering: Ordering) -&gt; <span class="prelude-ty">Option</span>&lt;Notification&gt; {
        <span class="kw">match </span><span class="self">self</span>.<span class="number">0</span>.load(ordering) {
            NOTIFICATION_NONE =&gt; <span class="prelude-val">None</span>,
            NOTIFICATION_ONE =&gt; <span class="prelude-val">Some</span>(Notification::One),
            NOTIFICATION_ALL =&gt; <span class="prelude-val">Some</span>(Notification::All),
            <span class="kw">_ </span>=&gt; <span class="macro">unreachable!</span>(),
        }
    }

    <span class="doccomment">/// Clears the notification.
    ...
    /// used once the atomic notification is no longer shared.
        ...
        <span class="self">self</span>.<span class="number">0</span>.store(NOTIFICATION_NONE, Relaxed);
    }
}

<span class="attr">#[derive(Debug, PartialEq, Eq)]
#[repr(usize)]
</span><span class="kw">enum </span>Notification {
    One = NOTIFICATION_ONE,
    All = NOTIFICATION_ALL,
}

<span class="doccomment">/// List used in `Notify::notify_waiters`. It wraps a guarded linked list
/// and gates the access to it on `notify.waiters` mutex. It also empties
/// the list on drop.
</span><span class="kw">struct </span>NotifyWaitersList&lt;<span class="lifetime">'a</span>&gt; {
    list: GuardedWaitList,
    is_empty: bool,
    notify: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>Notify,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>&gt; NotifyWaitersList&lt;<span class="lifetime">'a</span>&gt; {
    <span class="kw">fn </span>new(
        unguarded_list: WaitList,
        guard: Pin&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>Waiter&gt;,
        notify: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>Notify,
    ) -&gt; NotifyWaitersList&lt;<span class="lifetime">'a</span>&gt; {
        <span class="kw">let </span>guard_ptr = NonNull::from(guard.get_ref());
        <span class="kw">let </span>list = unguarded_list.into_guarded(guard_ptr);
        NotifyWaitersList {
            list,
            is_empty: <span class="bool-val">false</span>,
            notify,
        }
    }

    <span class="doccomment">/// Removes the last element from the guarded list. Modifying this list
    /// requires an exclusive access to the main list in `Notify`.
    </span><span class="kw">fn </span>pop_back_locked(<span class="kw-2">&amp;mut </span><span class="self">self</span>, _waiters: <span class="kw-2">&amp;mut </span>WaitList) -&gt; <span class="prelude-ty">Option</span>&lt;NonNull&lt;Waiter&gt;&gt; {
        <span class="kw">let </span>result = <span class="self">self</span>.list.pop_back();
        <span class="kw">if </span>result.is_none() {
            <span class="comment">// Save information about emptiness to avoid waiting for lock
            // in the destructor.
            </span><span class="self">self</span>.is_empty = <span class="bool-val">true</span>;
        }
        result
    }
}

<span class="kw">impl </span>Drop <span class="kw">for </span>NotifyWaitersList&lt;<span class="lifetime">'_</span>&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="comment">// If the list is not empty, we unlink all waiters from it.
        // We do not wake the waiters to avoid double panics.
        </span><span class="kw">if </span>!<span class="self">self</span>.is_empty {
            <span class="kw">let </span>_lock_guard = <span class="self">self</span>.notify.waiters.lock();
            <span class="kw">while let </span><span class="prelude-val">Some</span>(waiter) = <span class="self">self</span>.list.pop_back() {
                <span class="comment">// Safety: we never make mutable references to waiters.
                </span><span class="kw">let </span>waiter = <span class="kw">unsafe </span>{ waiter.as_ref() };
...
/// will immediately return `Poll::Ready`.
    ...
    <span class="doccomment">/// Number of calls to `notify_waiters` at the time of creation.
    </span>notify_waiters_calls: usize,
...
<span class="kw">impl </span>Notify {
    ...
    </span><span class="attr">#[cfg_attr(docsrs, doc(alias = <span class="string">"notify"</span>))]
    ...
    /// ```
        ...
        <span class="comment">// The state must be loaded while the lock is held. The state may only
        // transition out of WAITING while the lock is held.
        ...
        <span class="comment">// Increment the number of times this method was called
        // and transition to empty.
        </span><span class="kw">let </span>new_state = set_state(inc_num_notify_waiters_calls(curr), EMPTY);
        <span class="self">self</span>.state.store(new_state, SeqCst);

        <span class="comment">// It is critical for `GuardedLinkedList` safety that the guard node is
        // pinned in memory and is not dropped until the guarded list is dropped.
        </span><span class="kw">let </span>guard = Waiter::new();
        <span class="macro">pin!</span>(guard);

        <span class="comment">// We move all waiters to a secondary list. It uses a `GuardedLinkedList`
        ...
        </span><span class="kw">let </span><span class="kw-2">mut </span>list = NotifyWaitersList::new(std::mem::take(<span class="kw-2">&amp;mut *</span>waiters), guard.as_ref(), <span class="self">self</span>);
        ...
        <span class="lifetime">'outer</span>: <span class="kw">loop </span>{
            <span class="kw">while </span>wakers.can_push() {
                <span class="kw">match </span>list.pop_back_locked(<span class="kw-2">&amp;mut </span>waiters) {
                    <span class="prelude-val">Some</span>(waiter) =&gt; {
                        <span class="comment">// Safety: we never make mutable references to waiters.
                        </span><span class="kw">let </span>waiter = <span class="kw">unsafe </span>{ waiter.as_ref() };

                        <span class="comment">// Safety: we hold the lock, so we can access the waker.
                        </span><span class="kw">if let </span><span class="prelude-val">Some</span>(waker) =
                            <span class="kw">unsafe </span>{ waiter.waker.with_mut(|waker| (<span class="kw-2">*</span>waker).take()) }
                        {
                            wakers.push(waker);
                        }

                        <span class="comment">// This waiter is unlinked and will not be shared ever again, release it.
                        </span>waiter.notification.store_release(Notification::All);
                    }
            ...
            <span class="comment">// One of the wakers may panic, but the remaining waiters will still
...
<span class="kw">fn </span>notify_locked(waiters: <span class="kw-2">&amp;mut </span>WaitList, state: <span class="kw-2">&amp;</span>AtomicUsize, curr: usize) -&gt; <span class="prelude-ty">Option</span>&lt;Waker&gt; {
    <span class="kw">match </span>get_state(curr) {
        ...
        WAITING =&gt; {
            ...
            </span><span class="kw">let </span>waiter = waiters.pop_back().unwrap();

            <span class="comment">// Safety: we never make mutable references to waiters.
            </span><span class="kw">let </span>waiter = <span class="kw">unsafe </span>{ waiter.as_ref() };

            <span class="comment">// Safety: we hold the lock, so we can access the waker.
            </span><span class="kw">let </span>waker = <span class="kw">unsafe </span>{ waiter.waker.with_mut(|waker| (<span class="kw-2">*</span>waker).take()) };

            <span class="comment">// This waiter is unlinked and will not be shared ever again, release it.
            </span>waiter.notification.store_release(Notification::One);
...
<span class="comment">// ===== impl Notified =====
    ...
    </span><span class="kw">fn </span>project(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;) -&gt; (<span class="kw-2">&amp;</span>Notify, <span class="kw-2">&amp;mut </span>State, <span class="kw-2">&amp;</span>usize, <span class="kw-2">&amp;</span>Waiter) {
        <span class="kw">unsafe </span>{
            <span class="comment">// Safety: `notify`, `state` and `notify_waiters_calls` are `Unpin`.
            ...
            (
                me.notify,
                <span class="kw-2">&amp;mut </span>me.state,
                <span class="kw-2">&amp;</span>me.notify_waiters_calls,
                <span class="kw-2">&amp;</span>me.waiter,
            )
        }
    }
    ...
    <span class="kw">fn </span>poll_notified(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, waker: <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>Waker&gt;) -&gt; Poll&lt;()&gt; {
        <span class="kw">let </span>(notify, state, notify_waiters_calls, waiter) = <span class="self">self</span>.project();

        <span class="lifetime">'outer_loop</span>: <span class="kw">loop </span>{
            <span class="kw">match </span><span class="kw-2">*</span>state {
                State::Init =&gt; {
                    ...
                    <span class="kw">if </span>res.is_ok() {
                        ...
                        <span class="kw">continue </span><span class="lifetime">'outer_loop</span>;
                    }
                    ...
                    </span><span class="kw">if </span>get_num_notify_waiters_calls(curr) != <span class="kw-2">*</span>notify_waiters_calls {
                    ...
                    <span class="kw">if </span>waker.is_some() {
                        ...
                        // None when we reach this line.
                            ...
                            old_waker =
                                waiter.waker.with_mut(|v| std::mem::replace(<span class="kw-2">&amp;mut *</span>v, waker));
                        }
                    }

                    <span class="comment">// Insert the waiter into the linked list
                    </span>waiters.push_front(NonNull::from(waiter));
                ...
                State::Waiting =&gt; {
                    <span class="attr">#[cfg(tokio_taskdump)]
                        ...
                        <span class="kw">let </span><span class="kw-2">mut </span>ctx = Context::from_waker(waker);
                        <span class="macro">ready!</span>(<span class="kw">crate</span>::trace::trace_leaf(<span class="kw-2">&amp;mut </span>ctx));
                    }

                    <span class="kw">if </span>waiter.notification.load(Acquire).is_some() {
                        <span class="comment">// Safety: waiter is already unlinked and will not be shared again,
                        // so we have an exclusive access to `waker`.
                        </span>drop(<span class="kw">unsafe </span>{ waiter.waker.with_mut(|waker| (<span class="kw-2">*</span>waker).take()) });
                    ...
                    <span class="comment">// Our waiter was not notified, implying it is still stored in a waiter
                    // list (guarded by `notify.waiters`). In order to access the waker
                    // fields, we must acquire the lock.

                    </span><span class="kw">let </span><span class="kw-2">mut </span>old_waker = <span class="prelude-val">None</span>;
                    <span class="kw">let </span><span class="kw-2">mut </span>waiters = notify.waiters.lock();

                    <span class="comment">// We hold the lock and notifications are set only with the lock held,
                    // so this can be relaxed, because the happens-before relationship is
                    // established through the mutex.
                    </span><span class="kw">if </span>waiter.notification.load(Relaxed).is_some() {
                        <span class="comment">// Safety: waiter is already unlinked and will not be shared again,
                        // so we have an exclusive access to `waker`.
                        </span>old_waker = <span class="kw">unsafe </span>{ waiter.waker.with_mut(|waker| (<span class="kw-2">*</span>waker).take()) };

                        waiter.notification.clear();

                        <span class="comment">// Drop the old waker after releasing the lock.
                        </span>drop(waiters);
                        drop(old_waker);
                    ...
                    <span class="comment">// Load the state with the lock held.
                    </span><span class="kw">let </span>curr = notify.state.load(SeqCst);

                    <span class="kw">if </span>get_num_notify_waiters_calls(curr) != <span class="kw-2">*</span>notify_waiters_calls {
                        <span class="comment">// Before we add a waiter to the list we check if these numbers are
                        ...
                        <span class="comment">// Safety: we hold the lock, so we have an exclusive access to the list.
                        // The list is used in `notify_waiters`, so it must be guarded.
                        </span><span class="kw">unsafe </span>{ waiters.remove(NonNull::from(waiter)) };

                        <span class="kw-2">*</span>state = State::Done;
                    } <span class="kw">else </span>{
                        <span class="comment">// Safety: we hold the lock, so we can modify the waker.
                        </span><span class="kw">unsafe </span>{
                            waiter.waker.with_mut(|v| {
                                <span class="kw">if let </span><span class="prelude-val">Some</span>(waker) = waker {
                                    <span class="kw">let </span>should_update = <span class="kw">match </span><span class="kw-2">&amp;*</span>v {
                                    ...
                                    <span class="kw">if </span>should_update {
                                        old_waker = std::mem::replace(<span class="kw-2">&amp;mut *</span>v, <span class="prelude-val">Some</span>(waker.clone()));
                                    }
                                }
                            });
                        }
...
<span class="kw">impl </span>Drop <span class="kw">for </span>Notified&lt;<span class="lifetime">'_</span>&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        ...
        </span><span class="kw">let </span>(notify, state, <span class="kw">_</span>, waiter) = <span class="kw">unsafe </span>{ Pin::new_unchecked(<span class="self">self</span>).project() };
        ...
        // longer stored in the linked list.
        </span><span class="kw">if </span><span class="macro">matches!</span>(<span class="kw-2">*</span>state, State::Waiting) {
            <span class="kw">let </span><span class="kw-2">mut </span>waiters = notify.waiters.lock();
            <span class="kw">let </span><span class="kw-2">mut </span>notify_state = notify.state.load(SeqCst);

            <span class="comment">// We hold the lock, so this field is not concurrently accessed by
            // `notify_*` functions and we can use the relaxed ordering.
            </span><span class="kw">let </span>notification = waiter.notification.load(Relaxed);
            ...
            // to the next waiter.
            </span><span class="kw">if </span>notification == <span class="prelude-val">Some</span>(Notification::One) {
