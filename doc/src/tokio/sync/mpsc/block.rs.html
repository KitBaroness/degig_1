...
/// Each block in the list can hold up to `BLOCK_CAP` messages.
    ...
    <span class="doccomment">/// The header fields.
    </span>header: BlockHeader&lt;T&gt;,
...
<span class="doccomment">/// Extra fields for a `Block&lt;T&gt;`.
</span><span class="kw">struct </span>BlockHeader&lt;T&gt; {
...
</span><span class="kw">struct </span>Values&lt;T&gt;([UnsafeCell&lt;MaybeUninit&lt;T&gt;&gt;; BLOCK_CAP]);
...
<span class="macro">generate_addr_of_methods!</span> {
    <span class="kw">impl</span>&lt;T&gt; Block&lt;T&gt; {
        <span class="kw">unsafe fn </span>addr_of_header(<span class="self">self</span>: NonNull&lt;<span class="self">Self</span>&gt;) -&gt; NonNull&lt;BlockHeader&lt;T&gt;&gt; {
            <span class="kw-2">&amp;</span><span class="self">self</span>.header
        }

        <span class="kw">unsafe fn </span>addr_of_values(<span class="self">self</span>: NonNull&lt;<span class="self">Self</span>&gt;) -&gt; NonNull&lt;Values&lt;T&gt;&gt; {
...
<span class="kw">impl</span>&lt;T&gt; Block&lt;T&gt; {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new(start_index: usize) -&gt; Box&lt;Block&lt;T&gt;&gt; {
        <span class="kw">unsafe </span>{
            <span class="comment">// Allocate the block on the heap.
            // SAFETY: The size of the Block&lt;T&gt; is non-zero, since it is at least the size of the header.
            </span><span class="kw">let </span>block = std::alloc::alloc(Layout::new::&lt;Block&lt;T&gt;&gt;()) <span class="kw">as </span><span class="kw-2">*mut </span>Block&lt;T&gt;;
            <span class="kw">let </span>block = <span class="kw">match </span>NonNull::new(block) {
                <span class="prelude-val">Some</span>(block) =&gt; block,
                <span class="prelude-val">None </span>=&gt; std::alloc::handle_alloc_error(Layout::new::&lt;Block&lt;T&gt;&gt;()),
            };

            <span class="comment">// Write the header to the block.
            </span>Block::addr_of_header(block).as_ptr().write(BlockHeader {
            ...
            <span class="comment">// Initialize the values array.
            </span>Values::initialize(Block::addr_of_values(block));

            <span class="comment">// Convert the pointer to a `Box`.
            ...
            </span>Box::from_raw(block.as_ptr())
        }
    }
    ...
    <span class="doccomment">/// Returns `true` if the block matches the given index.
        ...
        <span class="self">self</span>.header.start_index == index
    }
    ...
    /// `start_index` must represent a block *after* `self`.
        ...
        other_index.wrapping_sub(<span class="self">self</span>.header.start_index) / BLOCK_CAP
    }
    ...
    /// * No concurrent access to the slot.
        ...
        <span class="kw">let </span>ready_bits = <span class="self">self</span>.header.ready_slots.load(Acquire);
    ...
    <span class="doccomment">/// Returns true if there is a value in the slot to be consumed
    ///
    /// # Safety
    ///
    /// To maintain safety, the caller must ensure:
    ///
    /// * No concurrent access to the slot.
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>has_value(<span class="kw-2">&amp;</span><span class="self">self</span>, slot_index: usize) -&gt; bool {
        <span class="kw">let </span>offset = offset(slot_index);
        <span class="kw">let </span>ready_bits = <span class="self">self</span>.header.ready_slots.load(Acquire);
    ...
    <span class="doccomment">/// Signal to the receiver that the sender half of the list is closed.
        ...
        <span class="self">self</span>.header.ready_slots.fetch_or(TX_CLOSED, Release);
    }

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">unsafe fn </span>is_closed(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="kw">let </span>ready_bits = <span class="self">self</span>.header.ready_slots.load(Acquire);
    ...
    /// * The caller holds a unique pointer to the block.
        ...
        <span class="self">self</span>.header.start_index = <span class="number">0</span>;
        <span class="self">self</span>.header.next = AtomicPtr::new(ptr::null_mut());
        <span class="self">self</span>.header.ready_slots = AtomicUsize::new(<span class="number">0</span>);
    }
    ...
    /// * The block will no longer be accessed by any sender.
        ...
        </span><span class="self">self</span>.header
        ...
        </span><span class="self">self</span>.header.ready_slots.fetch_or(RELEASED, Release);
    }
    ...
    <span class="doccomment">/// Mark a slot as ready
        ...
        <span class="self">self</span>.header.ready_slots.fetch_or(mask, Release);
    }
    ...
    /// be mutated.
        ...
        <span class="self">self</span>.header.ready_slots.load(Acquire) &amp; READY_MASK == READY_MASK
    }
    ...
    <span class="doccomment">/// Returns the `observed_tail_position` value, if set
        ...
        <span class="kw">if </span><span class="number">0 </span>== RELEASED &amp; <span class="self">self</span>.header.ready_slots.load(Acquire) {
            <span class="prelude-val">None
        </span>} <span class="kw">else </span>{
            <span class="prelude-val">Some</span>(
                <span class="self">self</span>.header
                    .observed_tail_position
                    .with(|ptr| <span class="kw">unsafe </span>{ <span class="kw-2">*</span>ptr }),
            )
        }
    }
    ...
    <span class="doccomment">/// Loads the next block
        ...
        <span class="kw">let </span>ret = NonNull::new(<span class="self">self</span>.header.next.load(ordering));

        <span class="macro">debug_assert!</span>(<span class="kw">unsafe </span>{
            ret.map_or(<span class="bool-val">true</span>, |block| {
                block.as_ref().header.start_index == <span class="self">self</span>.header.start_index.wrapping_add(BLOCK_CAP)
            })
        });
    ...
    /// * `block` is not freed until it has been removed from the list.
        ...
        block.as_mut().header.start_index = <span class="self">self</span>.header.start_index.wrapping_add(BLOCK_CAP);
    ...
    /// the slots.
        ...
        </span><span class="kw">let </span>new_block = Block::new(<span class="self">self</span>.header.start_index + BLOCK_CAP);
...
<span class="kw">impl</span>&lt;T&gt; Values&lt;T&gt; {
    <span class="doccomment">/// Initialize a `Values` struct from a pointer.
    ///
    /// # Safety
    ///
    /// The raw pointer must be valid for writing a `Values&lt;T&gt;`.
    </span><span class="kw">unsafe fn </span>initialize(_value: NonNull&lt;Values&lt;T&gt;&gt;) {
        <span class="comment">// When fuzzing, `UnsafeCell` needs to be initialized.
        </span><span class="macro">if_loom!</span> {
            <span class="kw">let </span>p = _value.as_ptr() <span class="kw">as </span><span class="kw-2">*mut </span>UnsafeCell&lt;MaybeUninit&lt;T&gt;&gt;;
...
#[test]
</span><span class="kw">fn </span>assert_no_stack_overflow() {
    <span class="comment">// https://github.com/tokio-rs/tokio/issues/5293

    </span><span class="kw">struct </span>Foo {
        _a: [u8; <span class="number">2_000_000</span>],
    }
    ...
    <span class="kw">let </span>_block = Block::&lt;Foo&gt;::new(<span class="number">0</span>);
}
