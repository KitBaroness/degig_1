...
</pre></div><pre class="rust"><code><span class="doccomment">//! A concurrent, lock-free, FIFO list.
...
<span class="kw">impl</span>&lt;T&gt; Tx&lt;T&gt; {
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>is_closed(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="kw">let </span>tail = <span class="self">self</span>.block_tail.load(Acquire);

        <span class="kw">unsafe </span>{
            <span class="kw">let </span>tail_block = <span class="kw-2">&amp;*</span>tail;
...
<span class="kw">impl</span>&lt;T&gt; Rx&lt;T&gt; {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>is_empty(<span class="kw-2">&amp;</span><span class="self">self</span>, tx: <span class="kw-2">&amp;</span>Tx&lt;T&gt;) -&gt; bool {
        <span class="kw">let </span>block = <span class="kw">unsafe </span>{ <span class="self">self</span>.head.as_ref() };
        <span class="kw">if </span>block.has_value(<span class="self">self</span>.index) {
            <span class="kw">return </span><span class="bool-val">false</span>;
        }

        <span class="comment">// It is possible that a block has no value "now" but the list is still not empty.
        // To be sure, it is necessary to check the length of the list.
        </span><span class="self">self</span>.len(tx) == <span class="number">0
    </span>}

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>len(<span class="kw-2">&amp;</span><span class="self">self</span>, tx: <span class="kw-2">&amp;</span>Tx&lt;T&gt;) -&gt; usize {
        <span class="comment">// When all the senders are dropped, there will be a last block in the tail position,
        // but it will be closed
        </span><span class="kw">let </span>tail_position = tx.tail_position.load(Acquire);
        tail_position - <span class="self">self</span>.index - (tx.is_closed() <span class="kw">as </span>usize)
    }
