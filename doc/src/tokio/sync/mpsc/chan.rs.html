...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::loom::cell::UnsafeCell;
...
<span class="kw">use </span><span class="kw">crate</span>::runtime::park::CachedParkThread;
<span class="kw">use </span><span class="kw">crate</span>::sync::mpsc::error::TryRecvError;
<span class="kw">use </span><span class="kw">crate</span>::sync::mpsc::{bounded, list, unbounded};
<span class="kw">use </span><span class="kw">crate</span>::sync::notify::Notify;
<span class="kw">use </span><span class="kw">crate</span>::util::cacheline::CachePadded;
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">trait </span>Semaphore {
    ...
    <span class="kw">fn </span>add_permits(<span class="kw-2">&amp;</span><span class="self">self</span>, n: usize);
...
<span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">struct </span>Chan&lt;T, S&gt; {
    ...
    <span class="doccomment">/// Tracks the number of outstanding weak sender handles.
    </span>tx_weak_count: AtomicUsize,
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>channel&lt;T, S: Semaphore&gt;(semaphore: S) -&gt; (Tx&lt;T, S&gt;, Rx&lt;T, S&gt;) {
    ...
    <span class="kw">let </span>chan = Arc::new(Chan {
        ...
        tx_weak_count: AtomicUsize::new(<span class="number">0</span>),
...
<span class="comment">// ===== impl Tx =====
    ...
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>strong_count(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.inner.tx_count.load(Acquire)
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>weak_count(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.inner.tx_weak_count.load(Relaxed)
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>downgrade(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; Arc&lt;Chan&lt;T, S&gt;&gt; {
        <span class="self">self</span>.inner.increment_weak_count();

        <span class="self">self</span>.inner.clone()
    }

    <span class="comment">// Returns the upgraded channel or None if the upgrade failed.
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>upgrade(chan: Arc&lt;Chan&lt;T, S&gt;&gt;) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>&gt; {
        <span class="kw">let </span><span class="kw-2">mut </span>tx_count = chan.tx_count.load(Acquire);

        <span class="kw">loop </span>{
            <span class="kw">if </span>tx_count == <span class="number">0 </span>{
                <span class="comment">// channel is closed
                </span><span class="kw">return </span><span class="prelude-val">None</span>;
            }

            <span class="kw">match </span>chan
                .tx_count
                .compare_exchange_weak(tx_count, tx_count + <span class="number">1</span>, AcqRel, Acquire)
            {
                <span class="prelude-val">Ok</span>(<span class="kw">_</span>) =&gt; <span class="kw">return </span><span class="prelude-val">Some</span>(Tx { inner: chan }),
                <span class="prelude-val">Err</span>(prev_count) =&gt; tx_count = prev_count,
            }
        }
    }
...
<span class="comment">// ===== impl Rx =====
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>is_closed(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="comment">// There two internal states that can represent a closed channel
        //
        ...
        //  `closed`.
        </span><span class="self">self</span>.inner.semaphore.is_closed() || <span class="self">self</span>.inner.tx_count.load(Acquire) == <span class="number">0
    </span>}
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>is_empty(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.inner.rx_fields.with(|rx_fields_ptr| {
            <span class="kw">let </span>rx_fields = <span class="kw">unsafe </span>{ <span class="kw-2">&amp;*</span>rx_fields_ptr };
            rx_fields.list.is_empty(<span class="kw-2">&amp;</span><span class="self">self</span>.inner.tx)
        })
    }
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>len(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.inner.rx_fields.with(|rx_fields_ptr| {
            <span class="kw">let </span>rx_fields = <span class="kw">unsafe </span>{ <span class="kw-2">&amp;*</span>rx_fields_ptr };
            rx_fields.list.len(<span class="kw-2">&amp;</span><span class="self">self</span>.inner.tx)
        })
    }
    ...
    <span class="doccomment">/// Receive the next value
        ...
        <span class="kw">use </span><span class="kw">super</span>::block::Read;
            ...
            <span class="macro">macro_rules!</span> try_recv {
                () =&gt; {
                    <span class="kw">match </span>rx_fields.list.pop(<span class="kw-2">&amp;</span><span class="self">self</span>.inner.tx) {
                        <span class="prelude-val">Some</span>(Read::Value(value)) =&gt; {
                        ...
                        <span class="prelude-val">Some</span>(Read::Closed) =&gt; {
    ...
    <span class="doccomment">/// Receives up to `limit` values into `buffer`
    ///
    /// For `limit &gt; 0`, receives up to limit values into `buffer`.
    /// For `limit == 0`, immediately returns Ready(0).
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>recv_many(
        ...
        <span class="kw">if </span>limit == <span class="number">0 </span>{
            coop.made_progress();
            <span class="kw">return </span>Ready(<span class="number">0usize</span>);
        }

        <span class="kw">let </span><span class="kw-2">mut </span>remaining = limit;
        <span class="kw">let </span>initial_length = buffer.len();

        <span class="self">self</span>.inner.rx_fields.with_mut(|rx_fields_ptr| {
            <span class="kw">let </span>rx_fields = <span class="kw">unsafe </span>{ <span class="kw-2">&amp;mut *</span>rx_fields_ptr };
            <span class="macro">macro_rules!</span> try_recv {
                () =&gt; {
                    <span class="kw">while </span>remaining &gt; <span class="number">0 </span>{
                        <span class="kw">match </span>rx_fields.list.pop(<span class="kw-2">&amp;</span><span class="self">self</span>.inner.tx) {
                            <span class="prelude-val">Some</span>(Read::Value(value)) =&gt; {
                            ...
                            <span class="prelude-val">Some</span>(Read::Closed) =&gt; {
                                <span class="kw">let </span>number_added = buffer.len() - initial_length;
                                <span class="kw">if </span>number_added &gt; <span class="number">0 </span>{
                                    <span class="self">self</span>.inner.semaphore.add_permits(number_added);
                                }
                                <span class="comment">// TODO: This check may not be required as it most
                                // likely can only return `true` at this point. A
                                // channel is closed when all tx handles are
                                ...
                                coop.made_progress();
                                <span class="kw">return </span>Ready(number_added);
                            }

                            <span class="prelude-val">None </span>=&gt; {
                                <span class="kw">break</span>; <span class="comment">// fall through
                            </span>}
                        }
                    }
                    <span class="kw">let </span>number_added = buffer.len() - initial_length;
            ...
            <span class="kw">if </span>rx_fields.rx_closed &amp;&amp; <span class="self">self</span>.inner.semaphore.is_idle() {
                ...
                coop.made_progress();
                Ready(<span class="number">0usize</span>)
            } <span class="kw">else </span>{
                Pending
            }
        })
    ...
    <span class="doccomment">/// Try to receive the next value.
        ...
        <span class="self">self</span>.inner.rx_fields.with_mut(|rx_fields_ptr| {
            ...
            <span class="kw">let </span>waker = park.waker().unwrap();
            <span class="kw">loop </span>{
...
<span class="comment">// ===== impl Chan =====
    ...
    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>decrement_weak_count(<span class="kw-2">&amp;</span><span class="self">self</span>) {
        <span class="self">self</span>.tx_weak_count.fetch_sub(<span class="number">1</span>, Relaxed);
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>increment_weak_count(<span class="kw-2">&amp;</span><span class="self">self</span>) {
        <span class="self">self</span>.tx_weak_count.fetch_add(<span class="number">1</span>, Relaxed);
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>strong_count(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.tx_count.load(Acquire)
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>weak_count(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.tx_weak_count.load(Relaxed)
    }
}
...
<span class="kw">impl</span>&lt;T, S&gt; Drop <span class="kw">for </span>Chan&lt;T, S&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        ...
        <span class="comment">// Safety: the only owner of the rx fields is Chan, and being
...
<span class="comment">// ===== impl Semaphore for (::Semaphore, capacity) =====
...
</span><span class="kw">impl </span>Semaphore <span class="kw">for </span>bounded::Semaphore {
    <span class="kw">fn </span>add_permit(<span class="kw-2">&amp;</span><span class="self">self</span>) {
        <span class="self">self</span>.semaphore.release(<span class="number">1</span>);
    }

    <span class="kw">fn </span>add_permits(<span class="kw-2">&amp;</span><span class="self">self</span>, n: usize) {
        <span class="self">self</span>.semaphore.release(n)
    }

    <span class="kw">fn </span>is_idle(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.semaphore.available_permits() == <span class="self">self</span>.bound
    }
    ...
    <span class="kw">fn </span>is_closed(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.semaphore.is_closed()
    }
}

<span class="comment">// ===== impl Semaphore for AtomicUsize =====

</span><span class="kw">impl </span>Semaphore <span class="kw">for </span>unbounded::Semaphore {
    <span class="kw">fn </span>add_permit(<span class="kw-2">&amp;</span><span class="self">self</span>) {
        <span class="kw">let </span>prev = <span class="self">self</span>.<span class="number">0</span>.fetch_sub(<span class="number">2</span>, Release);
    ...
    <span class="kw">fn </span>add_permits(<span class="kw-2">&amp;</span><span class="self">self</span>, n: usize) {
        <span class="kw">let </span>prev = <span class="self">self</span>.<span class="number">0</span>.fetch_sub(n &lt;&lt; <span class="number">1</span>, Release);

        <span class="kw">if </span>(prev &gt;&gt; <span class="number">1</span>) &lt; n {
            <span class="comment">// Something went wrong
            </span>process::abort();
        }
    }
    ...
    <span class="kw">fn </span>is_idle(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.<span class="number">0</span>.load(Acquire) &gt;&gt; <span class="number">1 </span>== <span class="number">0
    </span>}

    <span class="kw">fn </span>close(<span class="kw-2">&amp;</span><span class="self">self</span>) {
        <span class="self">self</span>.<span class="number">0</span>.fetch_or(<span class="number">1</span>, Release);
    }

    <span class="kw">fn </span>is_closed(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.<span class="number">0</span>.load(Acquire) &amp; <span class="number">1 </span>== <span class="number">1
    </span>}
}
