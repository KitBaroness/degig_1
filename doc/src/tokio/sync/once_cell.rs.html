...
</pre></div><pre class="rust"><code><span class="kw">use super</span>::{Semaphore, SemaphorePermit, TryAcquireError};
...
<span class="comment">// This file contains an implementation of an OnceCell. The principle
...
</span><span class="doccomment">/// A thread-safe cell that can be written to only once.
///
...
<span class="kw">impl</span>&lt;T&gt; Drop <span class="kw">for </span>OnceCell&lt;T&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="kw">if </span><span class="self">self</span>.initialized_mut() {
            <span class="kw">unsafe </span>{
                <span class="self">self</span>.value
                    .with_mut(|ptr| ptr::drop_in_place((<span class="kw-2">*</span>ptr).as_mut_ptr()));
            };
        }
    }
}
...
<span class="kw">impl</span>&lt;T&gt; OnceCell&lt;T&gt; {
    <span class="doccomment">/// Creates a new empty `OnceCell` instance.
    ...
    <span class="doccomment">/// Creates a new `OnceCell` that contains the provided value, if any.
    ///
    /// If the `Option` is `None`, this is equivalent to `OnceCell::new`.
    ///
    /// [`OnceCell::new`]: crate::sync::OnceCell::new
    </span><span class="comment">// Once https://github.com/rust-lang/rust/issues/73255 lands
    ...
    <span class="doccomment">/// Creates a new `OnceCell` that contains the provided value.
    ///
    /// # Example
    ///
    /// When using the `tracing` [unstable feature], a `OnceCell` created with
    ...
    </span><span class="kw">pub const fn </span>const_new_with(value: T) -&gt; <span class="self">Self </span>{
        OnceCell {
            value_set: AtomicBool::new(<span class="bool-val">true</span>),
            value: UnsafeCell::new(MaybeUninit::new(value)),
    ...
    <span class="doccomment">/// Returns `true` if the `OnceCell` currently contains a value, and `false`
    /// otherwise.
    </span><span class="kw">pub fn </span>initialized(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="comment">// Using acquire ordering so any threads that read a true from this
        // atomic is able to read the value.
        </span><span class="self">self</span>.value_set.load(Ordering::Acquire)
    }

    <span class="doccomment">/// Returns `true` if the `OnceCell` currently contains a value, and `false`
    /// otherwise.
    </span><span class="kw">fn </span>initialized_mut(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; bool {
        <span class="kw-2">*</span><span class="self">self</span>.value_set.get_mut()
    }

    <span class="comment">// SAFETY: The OnceCell must not be empty.
    ...
    <span class="kw">fn </span>set_value(<span class="kw-2">&amp;</span><span class="self">self</span>, value: T, permit: SemaphorePermit&lt;<span class="lifetime">'_</span>&gt;) -&gt; <span class="kw-2">&amp;</span>T {
        <span class="comment">// SAFETY: We are holding the only permit on the semaphore.
        </span><span class="kw">unsafe </span>{
            <span class="self">self</span>.value.with_mut(|ptr| (<span class="kw-2">*</span>ptr).as_mut_ptr().write(value));
        }

        <span class="comment">// Using release ordering so any threads that read a true from this
        // atomic is able to read the value we just stored.
        </span><span class="self">self</span>.value_set.store(<span class="bool-val">true</span>, Ordering::Release);
        <span class="self">self</span>.semaphore.close();
        permit.forget();

        <span class="comment">// SAFETY: We just initialized the cell.
        </span><span class="kw">unsafe </span>{ <span class="self">self</span>.get_unchecked() }
    }

    <span class="doccomment">/// Returns a reference to the value currently stored in the `OnceCell`, or
    ...
    <span class="doccomment">/// Returns a mutable reference to the value currently stored in the
    ...
    <span class="doccomment">/// Sets the value of the `OnceCell` to the given value if the `OnceCell` is
    /// empty.
    ///
    ...
    /// [`SetError::InitializingError`]: crate::sync::SetError::InitializingError
        ...
        <span class="kw">if </span><span class="self">self</span>.initialized() {
            <span class="kw">return </span><span class="prelude-val">Err</span>(SetError::AlreadyInitializedError(value));
        }

        <span class="comment">// Another task might be initializing the cell, in which case
        ...
        // permit, then we can set the value.
            ...
            <span class="prelude-val">Ok</span>(permit) =&gt; {
                <span class="macro">debug_assert!</span>(!<span class="self">self</span>.initialized());
                <span class="self">self</span>.set_value(value, permit);
                <span class="prelude-val">Ok</span>(())
            }
            <span class="prelude-val">Err</span>(TryAcquireError::NoPermits) =&gt; {
                <span class="comment">// Some other task is holding the permit. That task is
                // currently trying to initialize the value.
                </span><span class="prelude-val">Err</span>(SetError::InitializingError(value))
            }
            <span class="prelude-val">Err</span>(TryAcquireError::Closed) =&gt; {
                <span class="comment">// The semaphore was closed. Some other task has initialized
                // the value.
                </span><span class="prelude-val">Err</span>(SetError::AlreadyInitializedError(value))
            }
        }
    }

    <span class="doccomment">/// Gets the value currently in the `OnceCell`, or initialize it with the
        ...
        Fut: Future&lt;Output = T&gt;,
    {
        ...
        <span class="kw">if </span><span class="self">self</span>.initialized() {
            <span class="comment">// SAFETY: The OnceCell has been fully initialized.
            </span><span class="kw">unsafe </span>{ <span class="self">self</span>.get_unchecked() }
        } <span class="kw">else </span>{
            <span class="comment">// Here we try to acquire the semaphore permit. Holding the permit
            ...
            // it.
                ...
                <span class="prelude-val">Ok</span>(permit) =&gt; {
                    ...
                    <span class="comment">// If `f()` panics or `select!` is called, this
                ...
                <span class="prelude-val">Err</span>(<span class="kw">_</span>) =&gt; {
                    ...
                    <span class="comment">// SAFETY: The semaphore has been closed. This only happens
            ...
            // it.
                ...
                <span class="prelude-val">Ok</span>(permit) =&gt; {
                    ...
                    <span class="kw">match </span>value {
                        <span class="prelude-val">Ok</span>(value) =&gt; <span class="prelude-val">Ok</span>(<span class="self">self</span>.set_value(value, permit)),
    ...
    <span class="doccomment">/// Takes the value from the cell, destroying the cell in the process.
    ...
    <span class="doccomment">/// Takes ownership of the current value, leaving the cell empty.  Returns
...
<span class="comment">// Since `get` gives us access to immutable references of the OnceCell, OnceCell
...
/// [`OnceCell::set`]: crate::sync::OnceCell::set
    ...
    <span class="doccomment">/// The cell was already initialized when [`OnceCell::set`] was called.
    ///
    /// [`OnceCell::set`]: crate::sync::OnceCell::set
    </span>AlreadyInitializedError(T),

    <span class="doccomment">/// The cell is currently being initialized.
