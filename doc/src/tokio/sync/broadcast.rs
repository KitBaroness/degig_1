...
</pre></div><pre class="rust"><code><span class="doccomment">//! A multi-producer, multi-consumer broadcast queue. Each sent value is seen by
//! all consumers.
//!
...
<span class="kw">use </span><span class="kw">crate</span>::loom::sync::atomic::{AtomicBool, AtomicUsize};
...
<span class="kw">use </span>std::sync::atomic::Ordering::{Acquire, Relaxed, Release, SeqCst};
...
<span class="kw">pub mod </span>error {
    ...
    <span class="doccomment">/// Error returned by the [`send`] function on a [`Sender`].
    ///
...
<span class="kw">use </span><span class="self">self</span>::error::{RecvError, SendError, TryRecvError};
...
<span class="kw">impl </span>Waiter {
    <span class="kw">fn </span>new() -&gt; <span class="self">Self </span>{
        <span class="self">Self </span>{
            queued: AtomicBool::new(<span class="bool-val">false</span>),
...
</span><span class="kw">pub fn </span>channel&lt;T: Clone&gt;(capacity: usize) -&gt; (Sender&lt;T&gt;, Receiver&lt;T&gt;) {
    <span class="comment">// SAFETY: In the line below we are creating one extra receiver, so there will be 1 in total.
    </span><span class="kw">let </span>tx = <span class="kw">unsafe </span>{ Sender::new_with_receiver_count(<span class="number">1</span>, capacity) };
...
<span class="kw">impl</span>&lt;T&gt; Sender&lt;T&gt; {
    <span class="doccomment">/// Creates the sending-half of the [`broadcast`] channel.
    ///
    ...
    /// [`broadcast::channel`]: crate::sync::broadcast::channel
        ...
        <span class="comment">// SAFETY: We don't create extra receivers, so there are 0.
        </span><span class="kw">unsafe </span>{ <span class="self">Self</span>::new_with_receiver_count(<span class="number">0</span>, capacity) }
    }

    <span class="doccomment">/// Creates the sending-half of the [`broadcast`](self) channel, and provide the receiver
    /// count.
    ///
    ...
    </span><span class="kw">unsafe fn </span>new_with_receiver_count(receiver_count: usize, <span class="kw-2">mut </span>capacity: usize) -&gt; <span class="self">Self </span>{
        <span class="macro">assert!</span>(capacity &gt; <span class="number">0</span>, <span class="string">"broadcast channel capacity cannot be zero"</span>);
        <span class="macro">assert!</span>(
            capacity &lt;= usize::MAX &gt;&gt; <span class="number">1</span>,
            <span class="string">"broadcast channel capacity exceeded `usize::MAX / 2`"
        </span>);
...
<span class="doccomment">/// Create a new `Receiver` which reads starting from the tail.
    ...
    <span class="macro">assert!</span>(tail.rx_cnt != MAX_RECEIVERS, <span class="string">"max receivers"</span>);
...
<span class="doccomment">/// List used in `Shared::notify_rx`. It wraps a guarded linked list
/// and gates the access to it on the `Shared.tail` mutex. It also empties
/// the list on drop.
</span><span class="kw">struct </span>WaitersList&lt;<span class="lifetime">'a</span>, T&gt; {
    list: GuardedLinkedList&lt;Waiter, &lt;Waiter <span class="kw">as </span>linked_list::Link&gt;::Target&gt;,
    is_empty: bool,
    shared: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>Shared&lt;T&gt;,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, T&gt; Drop <span class="kw">for </span>WaitersList&lt;<span class="lifetime">'a</span>, T&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        ...
        // We do not wake the waiters to avoid double panics.
            ...
            <span class="kw">let </span>_lock_guard = <span class="self">self</span>.shared.tail.lock();
            <span class="kw">while </span><span class="self">self</span>.list.pop_back().is_some() {}
        }
    }
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, T&gt; WaitersList&lt;<span class="lifetime">'a</span>, T&gt; {
    <span class="kw">fn </span>new(
        unguarded_list: LinkedList&lt;Waiter, &lt;Waiter <span class="kw">as </span>linked_list::Link&gt;::Target&gt;,
    ...
    </span><span class="kw">fn </span>pop_back_locked(<span class="kw-2">&amp;mut </span><span class="self">self</span>, _tail: <span class="kw-2">&amp;mut </span>Tail) -&gt; <span class="prelude-ty">Option</span>&lt;NonNull&lt;Waiter&gt;&gt; {
...
<span class="kw">impl</span>&lt;T&gt; Shared&lt;T&gt; {
    <span class="kw">fn </span>notify_rx&lt;<span class="lifetime">'a</span>, <span class="lifetime">'b</span>: <span class="lifetime">'a</span>&gt;(<span class="kw-2">&amp;</span><span class="lifetime">'b </span><span class="self">self</span>, <span class="kw-2">mut </span>tail: MutexGuard&lt;<span class="lifetime">'a</span>, Tail&gt;) {
        ...
        </span><span class="kw">let </span><span class="kw-2">mut </span>list = WaitersList::new(std::mem::take(<span class="kw-2">&amp;mut </span>tail.waiters), guard.as_ref(), <span class="self">self</span>);
        ...
        <span class="lifetime">'outer</span>: <span class="kw">loop </span>{
            <span class="kw">while </span>wakers.can_push() {
                <span class="kw">match </span>list.pop_back_locked(<span class="kw-2">&amp;mut </span>tail) {
                    <span class="prelude-val">Some</span>(waiter) =&gt; {
                        <span class="kw">unsafe </span>{
                            <span class="comment">// Safety: accessing `waker` is safe because
                            // the tail lock is held.
                            </span><span class="kw">if let </span><span class="prelude-val">Some</span>(waker) = (<span class="kw-2">*</span>waiter.as_ptr()).waker.take() {
                                wakers.push(waker);
                            }

                            <span class="comment">// Safety: `queued` is atomic.
                            </span><span class="kw">let </span>queued = <span class="kw-2">&amp;</span>(<span class="kw-2">*</span>waiter.as_ptr()).queued;
                            <span class="comment">// `Relaxed` suffices because the tail lock is held.
                            </span><span class="macro">assert!</span>(queued.load(Relaxed));
                            <span class="comment">// `Release` is needed to synchronize with `Recv::drop`.
                            // It is critical to set this variable **after** waker
                            // is extracted, otherwise we may data race with `Recv::drop`.
                            </span>queued.store(<span class="bool-val">false</span>, Release);
                        }
                    }
                    ...
                    <span class="comment">// Store the waker
                        ...
                        <span class="comment">// Safety: called while locked.
                            ...
                            <span class="comment">// Only queue if not already queued
                                ...
                                <span class="comment">// If the waiter is not already queued, enqueue it.
                                // `Relaxed` order suffices: we have synchronized with
                                // all writers through the tail lock that we hold.
                                </span><span class="kw">if </span>!(<span class="kw-2">*</span>ptr).queued.load(Relaxed) {
                                    <span class="comment">// `Relaxed` order suffices: all the readers will
                                    // synchronize with this write through the tail lock.
                                    </span>(<span class="kw-2">*</span>ptr).queued.store(<span class="bool-val">true</span>, Relaxed);
...
<span class="kw">impl</span>&lt;T: Clone&gt; Receiver&lt;T&gt; {
    ...
    /// }
    /// ```
    </span><span class="kw">pub fn </span>blocking_recv(<span class="kw-2">&amp;mut </span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;T, RecvError&gt; {
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>, T&gt; Drop <span class="kw">for </span>Recv&lt;<span class="lifetime">'a</span>, T&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="comment">// Safety: `waiter.queued` is atomic.
        // Acquire ordering is required to synchronize with
        // `Shared::notify_rx` before we drop the object.
        </span><span class="kw">let </span>queued = <span class="self">self
            </span>.waiter
            .with(|ptr| <span class="kw">unsafe </span>{ (<span class="kw-2">*</span>ptr).queued.load(Acquire) });

        <span class="comment">// If the waiter is queued, we need to unlink it from the waiters list.
        ...
        // is not in the list and, as such, is not shared with any other threads.
            ...
            // `Relaxed` order suffices because we hold the tail lock.
                ...
                .with_mut(|ptr| <span class="kw">unsafe </span>{ (<span class="kw-2">*</span>ptr).queued.load(Relaxed) });
...
#[cfg(test)]
    ...
    </span><span class="kw">fn </span>receiver_count_on_sender_constructor() {
        <span class="kw">let </span>sender = Sender::&lt;i32&gt;::new(<span class="number">16</span>);
        <span class="macro">assert_eq!</span>(sender.receiver_count(), <span class="number">0</span>);

        <span class="kw">let </span>rx_1 = sender.subscribe();
        <span class="macro">assert_eq!</span>(sender.receiver_count(), <span class="number">1</span>);

        <span class="kw">let </span>rx_2 = rx_1.resubscribe();
        <span class="macro">assert_eq!</span>(sender.receiver_count(), <span class="number">2</span>);

        <span class="kw">let </span>rx_3 = sender.subscribe();
        <span class="macro">assert_eq!</span>(sender.receiver_count(), <span class="number">3</span>);
    ...
    <span class="attr">#[cfg(not(loom))]
    #[test]
    </span><span class="kw">fn </span>receiver_count_on_channel_constructor() {
        <span class="kw">let </span>(sender, rx) = channel::&lt;i32&gt;(<span class="number">16</span>);
        <span class="macro">assert_eq!</span>(sender.receiver_count(), <span class="number">1</span>);

        <span class="kw">let </span>_rx_2 = rx.resubscribe();
        <span class="macro">assert_eq!</span>(sender.receiver_count(), <span class="number">2</span>);
    }
}
