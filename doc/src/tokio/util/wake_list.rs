...
</pre></div><pre class="rust"><code><span class="kw">use </span>core::mem::MaybeUninit;
...
<span class="kw">impl </span>WakeList {
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>wake_all(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="macro">assert!</span>(<span class="self">self</span>.curr &lt;= NUM_WAKERS);
        <span class="kw">while </span><span class="self">self</span>.curr &gt; <span class="number">0 </span>{
            <span class="self">self</span>.curr -= <span class="number">1</span>;
            <span class="comment">// SAFETY: The first `curr` elements of `WakeList` are initialized, so by decrementing
            // `curr`, we can take ownership of the last item.
            </span><span class="kw">let </span>waker = <span class="kw">unsafe </span>{ ptr::read(<span class="self">self</span>.inner[<span class="self">self</span>.curr].as_mut_ptr()) };
            waker.wake();
        }
    }
}
