...
</pre></div><pre class="rust"><code><span class="attr">#![cfg_attr(any(not(feature = <span class="string">"full"</span>), loom), allow(unused_imports, dead_code))]

</span><span class="kw">mod </span>atomic_u16;
...
<span class="attr">#[cfg(all(feature = <span class="string">"parking_lot"</span>, not(miri)))]
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">mod </span>sync {
    ...
    </span><span class="attr">#[cfg(all(feature = <span class="string">"parking_lot"</span>, not(miri)))]
    ...
    <span class="attr">#[cfg(not(all(feature = <span class="string">"parking_lot"</span>, not(miri))))]
    #[allow(unused_imports)]
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">mod </span>atomic {
        ...
        <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span><span class="kw">crate</span>::loom::std::atomic_u64::{AtomicU64, StaticAtomicU64};
        ...
        <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span>std::sync::atomic::{fence, AtomicBool, AtomicPtr, AtomicU8, Ordering};
    }

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span><span class="kw">super</span>::barrier::Barrier;
}
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">mod </span>sys {
    <span class="attr">#[cfg(feature = <span class="string">"rt-multi-thread"</span>)]
        ...
        <span class="kw">const </span>ENV_WORKER_THREADS: <span class="kw-2">&amp;</span>str = <span class="string">"TOKIO_WORKER_THREADS"</span>;

        <span class="kw">match </span>std::env::var(ENV_WORKER_THREADS) {
            <span class="prelude-val">Ok</span>(s) =&gt; {
                <span class="kw">let </span>n = s.parse().unwrap_or_else(|e| {
                    <span class="macro">panic!</span>(
                        <span class="string">"\"{}\" must be usize, error: {}, value: {}"</span>,
                        ENV_WORKER_THREADS, e, s
                    )
                });
                <span class="macro">assert!</span>(n &gt; <span class="number">0</span>, <span class="string">"\"{}\" cannot be set to 0"</span>, ENV_WORKER_THREADS);
                n
            }
            <span class="prelude-val">Err</span>(std::env::VarError::NotPresent) =&gt; usize::max(<span class="number">1</span>, num_cpus::get()),
            <span class="prelude-val">Err</span>(std::env::VarError::NotUnicode(e)) =&gt; {
                <span class="macro">panic!</span>(
                    <span class="string">"\"{}\" must be valid unicode, error: {:?}"</span>,
    ...
    <span class="attr">#[cfg(not(feature = <span class="string">"rt-multi-thread"</span>))]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>num_cpus() -&gt; usize {
        <span class="number">1
    </span>}
}
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">mod </span>thread {
    ...
    <span class="attr">#[allow(unused_imports)]
        ...
        LocalKey, <span class="prelude-ty">Result</span>, Thread, ThreadId,
    };
}
