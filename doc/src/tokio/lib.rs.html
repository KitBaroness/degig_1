...
</pre></div><pre class="rust"><code><span class="attr">#![allow(
...
</span><span class="doccomment">//! A runtime for writing reliable network applications without compromising speed.
//!
...
</span><span class="comment">// Test that pointer width is compatible. This asserts that e.g. usize is at
...
// TODO: improve once we have MSRV access to const eval to make more flexible.
    ...
    target_pointer_width = <span class="string">"128"
</span>)))]
</span><span class="macro">compile_error!</span> {
    <span class="string">"Tokio requires the platform pointer width to be 32, 64, or 128 bits"
</span>}
...
<span class="attr">#[cfg(all(
    ...
    any(
        ...
        feature = <span class="string">"rt-multi-thread"</span>,
        feature = <span class="string">"signal"
    </span>)
))]
</span><span class="macro">compile_error!</span>(<span class="string">"Only features sync,macros,io-util,rt,time are supported on wasm."</span>);

<span class="attr">#[cfg(all(not(tokio_unstable), tokio_taskdump))]
</span><span class="macro">compile_error!</span>(<span class="string">"The `tokio_taskdump` feature requires `--cfg tokio_unstable`."</span>);
...
<span class="attr">#[cfg(all(
    ...
    <span class="string">"The `tokio_taskdump` feature is only currently supported on \
linux, on `aarch64`, `x86` and `x86_64`."
</span>);

<span class="comment">// Includes re-exports used by macros.
//
...
<span class="macro">cfg_not_rt!</span> {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">mod </span>runtime;
}

<span class="macro">cfg_signal!</span> {
    <span class="kw">pub mod </span>signal;
}

<span class="macro">cfg_signal_internal!</span> {
    <span class="attr">#[cfg(not(feature = <span class="string">"signal"</span>))]
    #[allow(dead_code)]
    #[allow(unreachable_pub)]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">mod </span>signal;
}

<span class="macro">cfg_sync!</span> {
...
<span class="macro">cfg_rt!</span> {
    <span class="kw">pub use </span>task::spawn;
}
...
<span class="kw">mod </span>trace {
    <span class="kw">use </span>std::future::Future;
    <span class="kw">use </span>std::pin::Pin;
    <span class="kw">use </span>std::task::{Context, Poll};

    <span class="macro">cfg_taskdump!</span> {
        <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span><span class="kw">crate</span>::runtime::task::trace::trace_leaf;
    }

    <span class="macro">cfg_not_taskdump!</span> {
        <span class="attr">#[inline(always)]
        #[allow(dead_code)]
        </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>trace_leaf(<span class="kw">_</span>: <span class="kw-2">&amp;mut </span>std::task::Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; std::task::Poll&lt;()&gt; {
            std::task::Poll::Ready(())
        }
    }

    <span class="attr">#[cfg_attr(not(feature = <span class="string">"sync"</span>), allow(dead_code))]
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>async_trace_leaf() -&gt; <span class="kw">impl </span>Future&lt;Output = ()&gt; {
        <span class="kw">struct </span>Trace;

        <span class="kw">impl </span>Future <span class="kw">for </span>Trace {
            <span class="kw">type </span>Output = ();

            <span class="attr">#[inline(always)]
            </span><span class="kw">fn </span>poll(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;()&gt; {
                trace_leaf(cx)
            }
        }
...
<span class="doccomment">/// Due to the `Stream` trait's inclusion in `std` landing later than Tokio's 1.0
...
<span class="comment">// local re-exports of platform specific things, allowing for decent
...
#[cfg(docsrs)]
#[allow(unused)]
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span><span class="self">self</span>::doc::os;

<span class="attr">#[cfg(not(docsrs))]
#[allow(unused)]
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">use </span>std::os;

<span class="macro">cfg_macros!</span> {
    <span class="doccomment">/// Implementation detail of the `select!` macro. This macro is **not**
    /// intended to be used as part of the public API and is permitted to
    /// change.
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">pub use </span>tokio_macros::select_priv_declare_output_enum;

    <span class="doccomment">/// Implementation detail of the `select!` macro. This macro is **not**
    /// intended to be used as part of the public API and is permitted to
    /// change.
    </span><span class="attr">#[doc(hidden)]
    </span><span class="kw">pub use </span>tokio_macros::select_priv_clean_pattern;

    <span class="macro">cfg_rt!</span> {
        <span class="attr">#[cfg(feature = <span class="string">"rt-multi-thread"</span>)]
        #[cfg(not(test))] </span><span class="comment">// Work around for rust-lang/rust#62127
        </span><span class="attr">#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"macros"</span>)))]
        #[doc(inline)]
        </span><span class="kw">pub use </span>tokio_macros::main;

        <span class="attr">#[cfg(feature = <span class="string">"rt-multi-thread"</span>)]
        #[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"macros"</span>)))]
        #[doc(inline)]
        </span><span class="kw">pub use </span>tokio_macros::test;

        <span class="macro">cfg_not_rt_multi_thread!</span> {
            <span class="attr">#[cfg(not(test))] </span><span class="comment">// Work around for rust-lang/rust#62127
            </span><span class="attr">#[doc(inline)]
            </span><span class="kw">pub use </span>tokio_macros::main_rt <span class="kw">as </span>main;

            <span class="attr">#[doc(inline)]
            </span><span class="kw">pub use </span>tokio_macros::test_rt <span class="kw">as </span>test;
        }
    }

    <span class="comment">// Always fail if rt is not enabled.
    </span><span class="macro">cfg_not_rt!</span> {
        <span class="attr">#[cfg(not(test))]
        #[doc(inline)]
        </span><span class="kw">pub use </span>tokio_macros::main_fail <span class="kw">as </span>main;

        <span class="attr">#[doc(inline)]
        </span><span class="kw">pub use </span>tokio_macros::test_fail <span class="kw">as </span>test;
    }
}

<span class="comment">// TODO: rm
</span><span class="attr">#[cfg(feature = <span class="string">"io-util"</span>)]
#[cfg(test)]
</span><span class="kw">fn </span>is_unpin&lt;T: Unpin&gt;() {}

<span class="doccomment">/// fuzz test (`fuzz_linked_list`)
</span><span class="attr">#[cfg(fuzzing)]
</span><span class="kw">pub mod </span>fuzz;
