...
</pre></div><pre class="rust"><code><span class="doccomment">//! Unix pipe types.
...
<span class="kw">use </span>std::os::unix::io::{AsFd, AsRawFd, BorrowedFd, FromRawFd, IntoRawFd, OwnedFd, RawFd};
...
<span class="doccomment">/// Creates a new anonymous Unix pipe.
///
...
</span><span class="kw">pub fn </span>pipe() -&gt; io::Result&lt;(Sender, Receiver)&gt; {
    <span class="kw">let </span>(tx, rx) = mio_pipe::new()<span class="question-mark">?</span>;
    <span class="prelude-val">Ok</span>((Sender::from_mio(tx)<span class="question-mark">?</span>, Receiver::from_mio(rx)<span class="question-mark">?</span>))
}
...
<span class="kw">impl </span>OpenOptions {
    ...
    <span class="kw">fn </span>open(<span class="kw-2">&amp;</span><span class="self">self</span>, path: <span class="kw-2">&amp;</span>Path, pipe_end: PipeEnd) -&gt; io::Result&lt;File&gt; {
        ...
        <span class="kw">if </span>!<span class="self">self</span>.unchecked &amp;&amp; !is_pipe(file.as_fd())<span class="question-mark">? </span>{
...
<span class="kw">impl </span>Sender {
    ...
    /// from a future driven by a tokio runtime, otherwise runtime can be set
    /// explicitly with [`Runtime::enter`](crate::runtime::Runtime::enter) function.
    </span><span class="kw">pub fn </span>from_file(file: File) -&gt; io::Result&lt;Sender&gt; {
        Sender::from_owned_fd(file.into())
    }

    <span class="doccomment">/// Creates a new `Sender` from an [`OwnedFd`].
    ///
    ...
    /// from a future driven by a tokio runtime, otherwise runtime can be set
    /// explicitly with [`Runtime::enter`](crate::runtime::Runtime::enter) function.
    </span><span class="kw">pub fn </span>from_owned_fd(owned_fd: OwnedFd) -&gt; io::Result&lt;Sender&gt; {
        <span class="kw">if </span>!is_pipe(owned_fd.as_fd())<span class="question-mark">? </span>{
            <span class="kw">return </span><span class="prelude-val">Err</span>(io::Error::new(io::ErrorKind::InvalidInput, <span class="string">"not a pipe"</span>));
        }

        <span class="kw">let </span>flags = get_file_flags(owned_fd.as_fd())<span class="question-mark">?</span>;
        <span class="kw">if </span>has_write_access(flags) {
            set_nonblocking(owned_fd.as_fd(), flags)<span class="question-mark">?</span>;
    ...
    <span class="doccomment">/// Creates a new `Sender` from an [`OwnedFd`] without checking pipe properties.
    ///
    /// This function is intended to construct a pipe from an [`OwnedFd`] representing
    ...
    /// from a future driven by a tokio runtime, otherwise runtime can be set
    /// explicitly with [`Runtime::enter`](crate::runtime::Runtime::enter) function.
    </span><span class="kw">pub fn </span>from_owned_fd_unchecked(owned_fd: OwnedFd) -&gt; io::Result&lt;Sender&gt; {
        <span class="comment">// Safety: OwnedFd represents a valid, open file descriptor.
        </span><span class="kw">let </span>mio_tx = <span class="kw">unsafe </span>{ mio_pipe::Sender::from_raw_fd(owned_fd.into_raw_fd()) };
    ...
    <span class="doccomment">/// Converts the pipe into an [`OwnedFd`] in blocking mode.
    ///
    /// This function will deregister this pipe end from the event loop, set
    /// it in blocking mode and perform the conversion.
    </span><span class="kw">pub fn </span>into_blocking_fd(<span class="self">self</span>) -&gt; io::Result&lt;OwnedFd&gt; {
        <span class="kw">let </span>fd = <span class="self">self</span>.into_nonblocking_fd()<span class="question-mark">?</span>;
        set_blocking(<span class="kw-2">&amp;</span>fd)<span class="question-mark">?</span>;
        <span class="prelude-val">Ok</span>(fd)
    }

    <span class="doccomment">/// Converts the pipe into an [`OwnedFd`] in nonblocking mode.
    ///
    ...
    /// mode.
    </span><span class="kw">pub fn </span>into_nonblocking_fd(<span class="self">self</span>) -&gt; io::Result&lt;OwnedFd&gt; {
        <span class="kw">let </span>mio_pipe = <span class="self">self</span>.io.into_inner()<span class="question-mark">?</span>;

        <span class="comment">// Safety: the pipe is now deregistered from the event loop
        // and we are the only owner of this pipe end.
        </span><span class="kw">let </span>owned_fd = <span class="kw">unsafe </span>{ OwnedFd::from_raw_fd(mio_pipe.into_raw_fd()) };

        <span class="prelude-val">Ok</span>(owned_fd)
    }
}
...
<span class="kw">impl </span>AsFd <span class="kw">for </span>Sender {
...
<span class="kw">impl </span>Receiver {
    ...
    /// from a future driven by a tokio runtime, otherwise runtime can be set
    /// explicitly with [`Runtime::enter`](crate::runtime::Runtime::enter) function.
    </span><span class="kw">pub fn </span>from_file(file: File) -&gt; io::Result&lt;Receiver&gt; {
        Receiver::from_owned_fd(file.into())
    }

    <span class="doccomment">/// Creates a new `Receiver` from an [`OwnedFd`].
    ///
    /// This function is intended to construct a pipe from an [`OwnedFd`] representing
    /// an anonymous pipe or a special FIFO file. It will check if the file descriptor
    ...
    /// from a future driven by a tokio runtime, otherwise runtime can be set
    /// explicitly with [`Runtime::enter`](crate::runtime::Runtime::enter) function.
    </span><span class="kw">pub fn </span>from_owned_fd(owned_fd: OwnedFd) -&gt; io::Result&lt;Receiver&gt; {
        <span class="kw">if </span>!is_pipe(owned_fd.as_fd())<span class="question-mark">? </span>{
            <span class="kw">return </span><span class="prelude-val">Err</span>(io::Error::new(io::ErrorKind::InvalidInput, <span class="string">"not a pipe"</span>));
        }
    ...
    <span class="doccomment">/// Creates a new `Receiver` from an [`OwnedFd`] without checking pipe properties.
    ///
    /// This function is intended to construct a pipe from an [`OwnedFd`] representing
    /// an anonymous pipe or a special FIFO file. The conversion assumes nothing about
    /// the underlying pipe; it is left up to the user to make sure that the file
    ...
    /// from a future driven by a tokio runtime, otherwise runtime can be set
    /// explicitly with [`Runtime::enter`](crate::runtime::Runtime::enter) function.
    </span><span class="kw">pub fn </span>from_owned_fd_unchecked(owned_fd: OwnedFd) -&gt; io::Result&lt;Receiver&gt; {
        <span class="comment">// Safety: OwnedFd represents a valid, open file descriptor.
        </span><span class="kw">let </span>mio_rx = <span class="kw">unsafe </span>{ mio_pipe::Receiver::from_raw_fd(owned_fd.into_raw_fd()) };
...
<span class="kw">impl </span>AsFd <span class="kw">for </span>Receiver {
    <span class="kw">fn </span>as_fd(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; BorrowedFd&lt;<span class="lifetime">'_</span>&gt; {
        <span class="kw">unsafe </span>{ BorrowedFd::borrow_raw(<span class="self">self</span>.as_raw_fd()) }
    }
}

<span class="doccomment">/// Checks if the file descriptor is a pipe or a FIFO.
</span><span class="kw">fn </span>is_pipe(fd: BorrowedFd&lt;<span class="lifetime">'_</span>&gt;) -&gt; io::Result&lt;bool&gt; {
    <span class="comment">// Safety: `libc::stat` is C-like struct used for syscalls and all-zero
    // byte pattern forms a valid value.
    </span><span class="kw">let </span><span class="kw-2">mut </span>stat: libc::stat = <span class="kw">unsafe </span>{ std::mem::zeroed() };

    <span class="comment">// Safety: it's safe to call `fstat` with a valid, open file descriptor
    // and a valid pointer to a `stat` struct.
    </span><span class="kw">let </span>r = <span class="kw">unsafe </span>{ libc::fstat(fd.as_raw_fd(), <span class="kw-2">&amp;mut </span>stat) };

    <span class="kw">if </span>r == -<span class="number">1 </span>{
        <span class="prelude-val">Err</span>(io::Error::last_os_error())
    } <span class="kw">else </span>{
        <span class="prelude-val">Ok</span>((stat.st_mode <span class="kw">as </span>libc::mode_t &amp; libc::S_IFMT) == libc::S_IFIFO)
    }
}

<span class="doccomment">/// Gets file descriptor's flags by fcntl.
</span><span class="kw">fn </span>get_file_flags(fd: BorrowedFd&lt;<span class="lifetime">'_</span>&gt;) -&gt; io::Result&lt;libc::c_int&gt; {
    <span class="comment">// Safety: it's safe to use `fcntl` to read flags of a valid, open file descriptor.
    </span><span class="kw">let </span>flags = <span class="kw">unsafe </span>{ libc::fcntl(fd.as_raw_fd(), libc::F_GETFL) };
...
<span class="doccomment">/// Sets file descriptor's flags with `O_NONBLOCK` by fcntl.
</span><span class="kw">fn </span>set_nonblocking(fd: BorrowedFd&lt;<span class="lifetime">'_</span>&gt;, current_flags: libc::c_int) -&gt; io::Result&lt;()&gt; {
    <span class="kw">let </span>flags = current_flags | libc::O_NONBLOCK;

    <span class="kw">if </span>flags != current_flags {
        <span class="comment">// Safety: it's safe to use `fcntl` to set the `O_NONBLOCK` flag of a valid,
        // open file descriptor.
        </span><span class="kw">let </span>ret = <span class="kw">unsafe </span>{ libc::fcntl(fd.as_raw_fd(), libc::F_SETFL, flags) };
...
<span class="doccomment">/// Removes `O_NONBLOCK` from fd's flags.
</span><span class="kw">fn </span>set_blocking&lt;T: AsRawFd&gt;(fd: <span class="kw-2">&amp;</span>T) -&gt; io::Result&lt;()&gt; {
    <span class="comment">// Safety: it's safe to use `fcntl` to read flags of a valid, open file descriptor.
    </span><span class="kw">let </span>previous = <span class="kw">unsafe </span>{ libc::fcntl(fd.as_raw_fd(), libc::F_GETFL) };
    <span class="kw">if </span>previous == -<span class="number">1 </span>{
        <span class="kw">return </span><span class="prelude-val">Err</span>(io::Error::last_os_error());
    }

    <span class="kw">let </span>new = previous &amp; !libc::O_NONBLOCK;

    <span class="comment">// Safety: it's safe to use `fcntl` to unset the `O_NONBLOCK` flag of a valid,
    // open file descriptor.
    </span><span class="kw">let </span>r = <span class="kw">unsafe </span>{ libc::fcntl(fd.as_raw_fd(), libc::F_SETFL, new) };
    <span class="kw">if </span>r == -<span class="number">1 </span>{
        <span class="prelude-val">Err</span>(io::Error::last_os_error())
    } <span class="kw">else </span>{
        <span class="prelude-val">Ok</span>(())
