...
</pre></div><pre class="rust"><code><span class="doccomment">//! `UnixStream` owned split support.
//!
...
<span class="doccomment">/// Owned read half of a [`UnixStream`], created by [`into_split`].
///
...
<span class="doccomment">/// Owned write half of a [`UnixStream`], created by [`into_split`].
///
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>split_owned(stream: UnixStream) -&gt; (OwnedReadHalf, OwnedWriteHalf) {
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>reunite(
...
) -&gt; <span class="prelude-ty">Result</span>&lt;UnixStream, ReuniteError&gt; {
    <span class="kw">if </span>Arc::ptr_eq(<span class="kw-2">&amp;</span>read.inner, <span class="kw-2">&amp;</span>write.inner) {
        ...
        </span><span class="prelude-val">Ok</span>(Arc::try_unwrap(read.inner).expect(<span class="string">"UnixStream: try_unwrap failed in reunite"</span>))
...
<span class="kw">impl </span>OwnedReadHalf {
    <span class="doccomment">/// Attempts to put the two halves of a `UnixStream` back together and
    /// recover the original socket. Succeeds only if the two halves
    /// originated from the same call to [`into_split`].
    ///
    /// [`into_split`]: crate::net::UnixStream::into_split()
    </span><span class="kw">pub fn </span>reunite(<span class="self">self</span>, other: OwnedWriteHalf) -&gt; <span class="prelude-ty">Result</span>&lt;UnixStream, ReuniteError&gt; {
...
<span class="kw">impl </span>OwnedWriteHalf {
    ...
    ///
    /// [`into_split`]: crate::net::UnixStream::into_split()
    </span><span class="kw">pub fn </span>reunite(<span class="self">self</span>, other: OwnedReadHalf) -&gt; <span class="prelude-ty">Result</span>&lt;UnixStream, ReuniteError&gt; {
    ...
    /// `Err(io::ErrorKind::WouldBlock)` is returned.
        ...
        <span class="self">self</span>.inner.try_write_vectored(buf)
    }
...
<span class="kw">impl </span>AsyncWrite <span class="kw">for </span>OwnedWriteHalf {
    ...
    <span class="attr">#[inline]
        ...
        <span class="comment">// flush is a no-op
...
<span class="kw">impl </span>AsRef&lt;UnixStream&gt; <span class="kw">for </span>OwnedReadHalf {
...
<span class="kw">impl </span>AsRef&lt;UnixStream&gt; <span class="kw">for </span>OwnedWriteHalf {
    <span class="kw">fn </span>as_ref(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>UnixStream {
        <span class="kw-2">&amp;</span><span class="self">self</span>.inner
    }
}
