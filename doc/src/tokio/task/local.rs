...
</pre></div><pre class="rust"><code><span class="doccomment">//! Runs `!Send` futures on the current thread.
...
</span><span class="kw">use </span><span class="kw">crate</span>::runtime;
...
<span class="macro">tokio_thread_local!</span>(<span class="kw">static </span>CURRENT: LocalData = <span class="kw">const </span>{ LocalData {
    ctx: RcCell::new(),
    wake_on_schedule: Cell::new(<span class="bool-val">false</span>),
} });
...
<span class="kw">impl </span>LocalData {
    <span class="doccomment">/// Should be called except when we call `LocalSet::enter`.
    /// Especially when we poll a `LocalSet`.
    </span><span class="attr">#[must_use = <span class="string">"dropping this guard will reset the entered state"</span>]
    </span><span class="kw">fn </span>enter(<span class="kw-2">&amp;</span><span class="self">self</span>, ctx: Rc&lt;Context&gt;) -&gt; LocalDataEnterGuard&lt;<span class="lifetime">'_</span>&gt; {
        <span class="kw">let </span>ctx = <span class="self">self</span>.ctx.replace(<span class="prelude-val">Some</span>(ctx));
        <span class="kw">let </span>wake_on_schedule = <span class="self">self</span>.wake_on_schedule.replace(<span class="bool-val">false</span>);
        LocalDataEnterGuard {
            local_data_ref: <span class="self">self</span>,
            ctx,
            wake_on_schedule,
        }
    }
...
<span class="doccomment">/// A guard for `LocalData::enter()`
</span><span class="kw">struct </span>LocalDataEnterGuard&lt;<span class="lifetime">'a</span>&gt; {
    local_data_ref: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>LocalData,
    ctx: <span class="prelude-ty">Option</span>&lt;Rc&lt;Context&gt;&gt;,
    wake_on_schedule: bool,
}

<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>&gt; Drop <span class="kw">for </span>LocalDataEnterGuard&lt;<span class="lifetime">'a</span>&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="self">self</span>.local_data_ref.ctx.set(<span class="self">self</span>.ctx.take());
        <span class="self">self</span>.local_data_ref
            .wake_on_schedule
            .set(<span class="self">self</span>.wake_on_schedule)
    }
}
...
</span><span class="kw">pub struct </span>LocalEnterGuard {
    ctx: <span class="prelude-ty">Option</span>&lt;Rc&lt;Context&gt;&gt;,

    <span class="doccomment">/// Distinguishes whether the context was entered or being polled.
    ...
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        CURRENT.with(
            |LocalData {
                ...
                ctx.set(<span class="self">self</span>.ctx.take());
                wake_on_schedule.set(<span class="self">self</span>.wake_on_schedule);
            },
        );
    }
}
...
<span class="kw">impl </span>LocalSet {
    ...
    /// [`spawn_local`]: fn@crate::task::spawn_local
        ...
        CURRENT.with(
            |LocalData {
                ...
                <span class="kw">let </span>ctx = ctx.replace(<span class="prelude-val">Some</span>(<span class="self">self</span>.context.clone()));
                <span class="kw">let </span>wake_on_schedule = wake_on_schedule.replace(<span class="bool-val">true</span>);
                LocalEnterGuard {
                    ctx,
                    wake_on_schedule,
                }
    ...
    <span class="attr">#[track_caller]
    </span><span class="kw">pub</span>(<span class="kw">in </span><span class="kw">crate</span>::task) <span class="kw">fn </span>spawn_named&lt;F&gt;(
    ...
    /// notified again.
        ...
        <span class="kw">for _ in </span><span class="number">0</span>..MAX_TASKS_PER_TICK {
            ...
            </span><span class="macro">assert!</span>(!<span class="self">self</span>.context.unhandled_panic.get(), <span class="string">"a spawned task panicked and the LocalSet is configured to shutdown on unhandled panic"</span>);
    ...
    <span class="kw">fn </span>with&lt;T&gt;(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw">impl </span>FnOnce() -&gt; T) -&gt; T {
        CURRENT.with(|local_data| {
            <span class="kw">let </span>_guard = local_data.enter(<span class="self">self</span>.context.clone());
            f()
        })
    }
    ...
    /// fails.
        ...
        <span class="kw">let </span>res = CURRENT.try_with(|local_data| {
            <span class="kw">let </span>_guard = local_data.enter(<span class="self">self</span>.context.clone());
            (f.take().unwrap())()
        });
...
<span class="macro">cfg_unstable!</span> {
    <span class="kw">impl </span>LocalSet {
        ...
        <span class="doccomment">/// Returns the [`Id`] of the current `LocalSet` runtime.
        ///
        /// # Examples
        ///
        /// ```rust
        ...
        </span><span class="kw">pub fn </span>id(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; runtime::Id {
            <span class="self">self</span>.context.shared.local_state.owned.id.into()
        }
    }
}
    ...
    <span class="doccomment">/// Schedule the provided task on the scheduler.
        ...
        CURRENT.with(|localdata| {
            <span class="kw">match </span>localdata.ctx.get() {
                <span class="comment">// If the current `LocalSet` is being polled, we don't need to wake it.
                ...
                </span><span class="prelude-val">Some</span>(cx) <span class="kw">if </span>cx.shared.ptr_eq(<span class="self">self</span>) &amp;&amp; !localdata.wake_on_schedule.get() =&gt; <span class="kw">unsafe </span>{
...
<span class="kw">impl </span>LocalState {
    ...
    <span class="attr">#[track_caller]
        ...
        </span><span class="attr">#[cfg(not(any(target_os = <span class="string">"openbsd"</span>, target_os = <span class="string">"freebsd"</span>)))]
