...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::task::JoinHandle;

<span class="kw">use </span>std::future::Future;

<span class="macro">cfg_rt!</span> {
    <span class="doccomment">/// Spawns a new asynchronous task, returning a
    ...
    </span><span class="kw">pub fn </span>spawn&lt;F&gt;(future: F) -&gt; JoinHandle&lt;F::Output&gt;
    <span class="kw">where
        </span>F: Future + Send + <span class="lifetime">'static</span>,
        F::Output: Send + <span class="lifetime">'static</span>,
    {
        <span class="comment">// preventing stack overflows on debug mode, by quickly sending the
        // task to the heap.
        </span><span class="kw">if </span><span class="macro">cfg!</span>(debug_assertions) &amp;&amp; std::mem::size_of::&lt;F&gt;() &gt; <span class="number">2048 </span>{
            spawn_inner(Box::pin(future), <span class="prelude-val">None</span>)
        } <span class="kw">else </span>{
            spawn_inner(future, <span class="prelude-val">None</span>)
        }
    }

    <span class="attr">#[track_caller]
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>spawn_inner&lt;T&gt;(future: T, name: <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>str&gt;) -&gt; JoinHandle&lt;T::Output&gt;
    <span class="kw">where
        </span>T: Future + Send + <span class="lifetime">'static</span>,
        T::Output: Send + <span class="lifetime">'static</span>,
    {
        <span class="kw">use </span><span class="kw">crate</span>::runtime::{context, task};
        ...
        </span><span class="kw">let </span>future = task::trace::Trace::root(future);
        <span class="kw">let </span>id = task::Id::next();
        <span class="kw">let </span>task = <span class="kw">crate</span>::util::trace::task(future, <span class="string">"task"</span>, name, id.as_u64());

        <span class="kw">match </span>context::with_current(|handle| handle.spawn(task, id)) {
            <span class="prelude-val">Ok</span>(join_handle) =&gt; join_handle,
