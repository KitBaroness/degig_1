...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::io::util::fill_buf::{fill_buf, FillBuf};
<span class="kw">use </span><span class="kw">crate</span>::io::util::lines::{lines, Lines};
<span class="kw">use </span><span class="kw">crate</span>::io::util::read_line::{read_line, ReadLine};
<span class="kw">use </span><span class="kw">crate</span>::io::util::read_until::{read_until, ReadUntil};
<span class="kw">use </span><span class="kw">crate</span>::io::util::split::{split, Split};
...
<span class="macro">cfg_io_util!</span> {
    ...
    /// [`AsyncBufRead`]: crate::io::AsyncBufRead
        ...
        <span class="doccomment">/// Reads all bytes into `buf` until the delimiter `byte` or EOF is reached.
        ///
        /// Equivalent to:
        ///
        /// ```ignore
        ...
        {
            read_until(<span class="self">self</span>, byte, buf)
        }

        <span class="doccomment">/// Reads all bytes until a newline (the 0xA byte) is reached, and append
        ...
        {
            read_line(<span class="self">self</span>, buf)
        }

        <span class="doccomment">/// Returns a stream of the contents of this reader split on the byte
        /// `byte`.
        ///
        ...
        /// # }
        /// ```
        </span><span class="kw">fn </span>split(<span class="self">self</span>, byte: u8) -&gt; Split&lt;<span class="self">Self</span>&gt;
        <span class="kw">where
            </span><span class="self">Self</span>: Sized + Unpin,
        {
            split(<span class="self">self</span>, byte)
        }

        <span class="doccomment">/// Returns the contents of the internal buffer, filling it with more
        ...
        {
            fill_buf(<span class="self">self</span>)
        }

        <span class="doccomment">/// Tells this buffer that `amt` bytes have been consumed from the
        ...
        {
            std::pin::Pin::new(<span class="self">self</span>).consume(amt);
        }
        ...
        {
            lines(<span class="self">self</span>)
        }
    }
}
