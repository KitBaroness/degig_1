...
</pre></div><pre class="rust"><code><span class="doccomment">//! In-process memory IO types.
...
<span class="kw">use </span>std::{
    pin::Pin,
    sync::Arc,
    task::{<span class="self">self</span>, Poll, Waker},
};
...
<span class="doccomment">/// A unidirectional IO over a piece of memory.
///
...
/// written to a side before the write returns `Poll::Pending`.
    ...
    <span class="kw">let </span>one = Arc::new(Mutex::new(Pipe::new(max_buf_size)));
    <span class="kw">let </span>two = Arc::new(Mutex::new(Pipe::new(max_buf_size)));
...
<span class="comment">// ===== impl Pipe =====

</span><span class="kw">impl </span>Pipe {
    <span class="kw">fn </span>new(max_buf_size: usize) -&gt; <span class="self">Self </span>{
        Pipe {
            buffer: BytesMut::new(),
...
<span class="kw">impl </span>AsyncRead <span class="kw">for </span>Pipe {
...
<span class="kw">impl </span>AsyncWrite <span class="kw">for </span>Pipe {
    <span class="macro">cfg_coop!</span> {
