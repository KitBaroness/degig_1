...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::io::ReadBuf;
    ...
    </span>V: VecU8,
{
    ...
    <span class="comment">// Returns a boolean telling the caller to try reading into a small local buffer first if true.
    // Doing so would avoid overallocating when vec is filled to capacity and we reached EOF.
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>try_small_read_first(<span class="kw-2">&amp;</span><span class="self">self</span>, num_bytes: usize) -&gt; bool {
        <span class="kw">let </span>vec = <span class="self">self</span>.vec.as_ref();
        vec.capacity() - vec.len() &lt; num_bytes
            &amp;&amp; <span class="self">self</span>.starting_capacity == vec.capacity()
            &amp;&amp; <span class="self">self</span>.starting_capacity &gt;= num_bytes
    }
}
