...
</pre></div><pre class="rust"><code><span class="attr">#[cfg(tokio_unstable)]
</span><span class="kw">use </span><span class="kw">crate</span>::runtime;
<span class="kw">use </span><span class="kw">crate</span>::runtime::{context, scheduler, RuntimeFlavor};
...
<span class="kw">impl </span>Handle {
    ...
    /// [`tokio::time`]: crate::time
        ...
        </span>context::enter_runtime(<span class="kw-2">&amp;</span><span class="self">self</span>.inner, <span class="bool-val">true</span>, |blocking| {
            blocking.block_on(future).expect(<span class="string">"failed to park thread"</span>)
        })
    }
    ...
    /// ```
        ...
        <span class="kw">match </span><span class="self">self</span>.inner {
            ...
            </span>scheduler::Handle::MultiThreadAlt(<span class="kw">_</span>) =&gt; RuntimeFlavor::MultiThreadAlt,
        }
    }

    <span class="macro">cfg_unstable!</span> {
        <span class="doccomment">/// Returns the [`Id`] of the current `Runtime`.
        ///
        /// # Examples
        ///
        /// ```
        ...
        /// [`Id`]: struct@crate::runtime::Id
            ...
            <span class="kw">let </span>owned_id = <span class="kw">match </span><span class="kw-2">&amp;</span><span class="self">self</span>.inner {
                scheduler::Handle::CurrentThread(handle) =&gt; handle.owned_id(),
                <span class="attr">#[cfg(all(feature = <span class="string">"rt-multi-thread"</span>, not(target_os = <span class="string">"wasi"</span>)))]
                </span>scheduler::Handle::MultiThread(handle) =&gt; handle.owned_id(),
                <span class="attr">#[cfg(all(tokio_unstable, feature = <span class="string">"rt-multi-thread"</span>, not(target_os = <span class="string">"wasi"</span>)))]
                </span>scheduler::Handle::MultiThreadAlt(handle) =&gt; handle.owned_id(),
            };
            owned_id.into()
        }
    }
...
<span class="macro">cfg_taskdump!</span> {
    <span class="kw">impl </span>Handle {
        <span class="doccomment">/// Captures a snapshot of the runtime's state.
        ///
        ...
        </span><span class="kw">pub async fn </span>dump(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw">crate</span>::runtime::Dump {
            <span class="kw">match </span><span class="kw-2">&amp;</span><span class="self">self</span>.inner {
                scheduler::Handle::CurrentThread(handle) =&gt; handle.dump(),
                <span class="attr">#[cfg(all(feature = <span class="string">"rt-multi-thread"</span>, not(target_os = <span class="string">"wasi"</span>)))]
                </span>scheduler::Handle::MultiThread(handle) =&gt; {
                    <span class="comment">// perform the trace in a separate thread so that the
                    // trace itself does not appear in the taskdump.
                    </span><span class="kw">let </span>handle = handle.clone();
                    spawn_thread(<span class="kw">async </span>{
                        <span class="kw">let </span>handle = handle;
                        handle.dump().<span class="kw">await
                    </span>}).<span class="kw">await
                </span>},
                <span class="attr">#[cfg(all(tokio_unstable, feature = <span class="string">"rt-multi-thread"</span>, not(target_os = <span class="string">"wasi"</span>)))]
                </span>scheduler::Handle::MultiThreadAlt(<span class="kw">_</span>) =&gt; <span class="macro">panic!</span>(<span class="string">"task dump not implemented for this runtime flavor"</span>),
            }
        }

        <span class="doccomment">/// Produces `true` if the current task is being traced for a dump;
        /// otherwise false. This function is only public for integration
        /// testing purposes. Do not rely on it.
        </span><span class="attr">#[doc(hidden)]
        </span><span class="kw">pub fn </span>is_tracing() -&gt; bool {
            <span class="kw">super</span>::task::trace::Context::is_tracing()
        }
    }

    <span class="macro">cfg_rt_multi_thread!</span> {
        <span class="doccomment">/// Spawn a new thread and asynchronously await on its result.
        </span><span class="kw">async fn </span>spawn_thread&lt;F&gt;(f: F) -&gt; &lt;F <span class="kw">as </span>Future&gt;::Output
        <span class="kw">where
            </span>F: Future + Send + <span class="lifetime">'static</span>,
            &lt;F <span class="kw">as </span>Future&gt;::Output: Send + <span class="lifetime">'static
        </span>{
            <span class="kw">let </span>(tx, rx) = <span class="kw">crate</span>::sync::oneshot::channel();
            <span class="kw">crate</span>::loom::thread::spawn(|| {
                <span class="kw">let </span>rt = <span class="kw">crate</span>::runtime::Builder::new_current_thread().build().unwrap();
                rt.block_on(<span class="kw">async </span>{
                    <span class="kw">let _ </span>= tx.send(f.<span class="kw">await</span>);
                });
            });
...
<span class="kw">impl </span>fmt::Debug <span class="kw">for </span>TryCurrentErrorKind {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="kw">match </span><span class="self">self </span>{
            TryCurrentErrorKind::NoContext =&gt; f.write_str(<span class="string">"NoContext"</span>),
            TryCurrentErrorKind::ThreadLocalDestroyed =&gt; f.write_str(<span class="string">"ThreadLocalDestroyed"</span>),
        }
    }
}
...
<span class="kw">impl </span>fmt::Display <span class="kw">for </span>TryCurrentError {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="kw">use </span>TryCurrentErrorKind <span class="kw">as </span>E;
