...
</pre></div><pre class="rust"><code><span class="doccomment">//! This module has containers for storing the tasks spawned on a scheduler. The
...
<span class="macro">cfg_not_has_atomic_u64!</span> {
    ...
    <span class="kw">fn </span>get_next_id() -&gt; NonZeroU64 {
        <span class="kw">loop </span>{
            <span class="kw">let </span>id = NEXT_OWNED_TASKS_ID.fetch_add(<span class="number">1</span>, Ordering::Relaxed);
            <span class="kw">if let </span><span class="prelude-val">Some</span>(id) = NonZeroU64::new(u64::from(id)) {
                <span class="kw">return </span>id;
            }
        }
    }
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">struct </span>OwnedTasks&lt;S: <span class="lifetime">'static</span>&gt; {
    list: List&lt;S&gt;,
    <span class="kw">pub</span>(<span class="kw">crate</span>) id: NonZeroU64,
    closed: AtomicBool,
}

<span class="kw">type </span>List&lt;S&gt; = sharded_list::ShardedList&lt;Task&lt;S&gt;, &lt;Task&lt;S&gt; <span class="kw">as </span>Link&gt;::Target&gt;;
...
<span class="kw">impl</span>&lt;S: <span class="lifetime">'static</span>&gt; OwnedTasks&lt;S&gt; {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new(num_cores: usize) -&gt; <span class="self">Self </span>{
        <span class="kw">let </span>shard_size = <span class="self">Self</span>::gen_shared_list_size(num_cores);
        ...
        T::Output: Send + <span class="lifetime">'static</span>,
    {
        ...
        <span class="kw">let </span>notified = <span class="kw">unsafe </span>{ <span class="self">self</span>.bind_inner(task, notified) };
        (join, notified)
    }

    <span class="doccomment">/// The part of `bind` that's the same for every type of future.
    </span><span class="kw">unsafe fn </span>bind_inner(<span class="kw-2">&amp;</span><span class="self">self</span>, task: Task&lt;S&gt;, notified: Notified&lt;S&gt;) -&gt; <span class="prelude-ty">Option</span>&lt;Notified&lt;S&gt;&gt;
    <span class="kw">where
        </span>S: Schedule,
    {
        ...
        <span class="kw">let </span>shard = <span class="self">self</span>.list.lock_shard(<span class="kw-2">&amp;</span>task);
        <span class="comment">// Check the closed flag in the lock for ensuring all that tasks
        // will shut down after the OwnedTasks has been closed.
        </span><span class="kw">if </span><span class="self">self</span>.closed.load(Ordering::Acquire) {
            drop(shard);
            task.shutdown();
            <span class="kw">return </span><span class="prelude-val">None</span>;
        }
        shard.push(task);
        <span class="prelude-val">Some</span>(notified)
    }
    ...
    /// a `LocalNotified`, giving the thread permission to poll this task.
        ...
        <span class="macro">debug_assert_eq!</span>(task.header().get_owner_id(), <span class="prelude-val">Some</span>(<span class="self">self</span>.id));
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>close_and_shutdown_all(<span class="kw-2">&amp;</span><span class="self">self</span>, start: usize)
    <span class="kw">where
        </span>S: Schedule,
    {
        <span class="self">self</span>.closed.store(<span class="bool-val">true</span>, Ordering::Release);
        <span class="kw">for </span>i <span class="kw">in </span>start..<span class="self">self</span>.get_shard_size() + start {
            <span class="kw">loop </span>{
                <span class="kw">let </span>task = <span class="self">self</span>.list.pop_back(i);
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>get_shard_size(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.list.shard_size()
    }
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>active_tasks_count(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.list.len()
    }
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>remove(<span class="kw-2">&amp;</span><span class="self">self</span>, task: <span class="kw-2">&amp;</span>Task&lt;S&gt;) -&gt; <span class="prelude-ty">Option</span>&lt;Task&lt;S&gt;&gt; {
        <span class="comment">// If the task's owner ID is `None` then it is not part of any list and
        // doesn't need removing.
        </span><span class="kw">let </span>task_id = task.header().get_owner_id()<span class="question-mark">?</span>;
        ...
        // linked list.
        </span><span class="kw">unsafe </span>{ <span class="self">self</span>.list.remove(task.header_ptr()) }
    }
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>is_empty(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.list.is_empty()
    }

    <span class="doccomment">/// Generates the size of the sharded list based on the number of worker threads.
    ///
    ...
    </span><span class="kw">fn </span>gen_shared_list_size(num_cores: usize) -&gt; usize {
        <span class="kw">const </span>MAX_SHARED_LIST_SIZE: usize = <span class="number">1 </span>&lt;&lt; <span class="number">16</span>;
        usize::min(MAX_SHARED_LIST_SIZE, num_cores.next_power_of_two() * <span class="number">4</span>)
    }
}

<span class="macro">cfg_taskdump!</span> {
    <span class="kw">impl</span>&lt;S: <span class="lifetime">'static</span>&gt; OwnedTasks&lt;S&gt; {
        <span class="doccomment">/// Locks the tasks, and calls `f` on an iterator over them.
        </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>for_each&lt;F&gt;(<span class="kw-2">&amp;</span><span class="self">self</span>, f: F)
        <span class="kw">where
            </span>F: FnMut(<span class="kw-2">&amp;</span>Task&lt;S&gt;),
        {
            <span class="self">self</span>.list.for_each(f);
        }
    }
}
...
<span class="kw">impl</span>&lt;S: <span class="lifetime">'static</span>&gt; LocalOwnedTasks&lt;S&gt; {
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>remove(<span class="kw-2">&amp;</span><span class="self">self</span>, task: <span class="kw-2">&amp;</span>Task&lt;S&gt;) -&gt; <span class="prelude-ty">Option</span>&lt;Task&lt;S&gt;&gt; {
        ...
        <span class="self">self</span>.with_inner(|inner|
            ...
            // other linked list.
            </span><span class="kw">unsafe </span>{ inner.list.remove(task.header_ptr()) })
    }
    ...
    /// it to a `LocalNotified`, giving the thread permission to poll this task.
        ...
        <span class="macro">assert_eq!</span>(task.header().get_owner_id(), <span class="prelude-val">Some</span>(<span class="self">self</span>.id));
