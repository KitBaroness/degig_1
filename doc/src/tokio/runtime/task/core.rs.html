...
</pre></div><pre class="rust"><code><span class="doccomment">//! Core task module.
//!
//! # Safety
//!
...
<span class="doccomment">/// Crate public as this is also needed by the pool.
    ...
    <span class="doccomment">/// This integer contains the id of the `OwnedTasks` or `LocalOwnedTasks`
    ...
    </span><span class="kw">pub</span>(<span class="kw">super</span>) owner_id: UnsafeCell&lt;<span class="prelude-ty">Option</span>&lt;NonZeroU64&gt;&gt;,
    ...
    </span><span class="kw">pub</span>(<span class="kw">super</span>) tracing_id: <span class="prelude-ty">Option</span>&lt;tracing::Id&gt;,
}
...
<span class="doccomment">/// Cold data is stored after the future. Data is considered cold if it is only
/// used during creation or shutdown of the task.
    ...
    <span class="doccomment">/// Pointers for the linked list in the `OwnedTasks` that owns this task.
    </span><span class="kw">pub</span>(<span class="kw">super</span>) owned: linked_list::Pointers&lt;Header&gt;,
...
<span class="macro">generate_addr_of_methods!</span> {
    <span class="kw">impl</span>&lt;&gt; Trailer {
        <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>addr_of_owned(<span class="self">self</span>: NonNull&lt;<span class="self">Self</span>&gt;) -&gt; NonNull&lt;linked_list::Pointers&lt;Header&gt;&gt; {
...
<span class="kw">impl</span>&lt;T: Future, S: Schedule&gt; Cell&lt;T, S&gt; {
    ...
    /// structures.
        ...
        <span class="comment">// Separated into a non-generic function to reduce LLVM codegen
        </span><span class="kw">fn </span>new_header(
            state: State,
            vtable: <span class="kw-2">&amp;</span><span class="lifetime">'static </span>Vtable,
            <span class="attr">#[cfg(all(tokio_unstable, feature = <span class="string">"tracing"</span>))] </span>tracing_id: <span class="prelude-ty">Option</span>&lt;tracing::Id&gt;,
        ) -&gt; Header {
            Header {
                state,
                queue_next: UnsafeCell::new(<span class="prelude-val">None</span>),
                vtable,
                owner_id: UnsafeCell::new(<span class="prelude-val">None</span>),
        ...
        <span class="attr">#[cfg(all(tokio_unstable, feature = <span class="string">"tracing"</span>))]
        </span><span class="kw">let </span>tracing_id = future.id();
        <span class="kw">let </span>vtable = raw::vtable::&lt;T, S&gt;();
        <span class="kw">let </span>result = Box::new(Cell {
            header: new_header(
                state,
                vtable,
                <span class="attr">#[cfg(all(tokio_unstable, feature = <span class="string">"tracing"</span>))]
        ...
        <span class="attr">#[cfg(debug_assertions)]
            ...
            <span class="comment">// Using a separate function for this code avoids instantiating it separately for every `T`.
            </span><span class="kw">unsafe fn </span>check&lt;S&gt;(header: <span class="kw-2">&amp;</span>Header, trailer: <span class="kw-2">&amp;</span>Trailer, scheduler: <span class="kw-2">&amp;</span>S, task_id: <span class="kw-2">&amp;</span>Id) {
                <span class="kw">let </span>trailer_addr = trailer <span class="kw">as </span><span class="kw-2">*const </span>Trailer <span class="kw">as </span>usize;
                <span class="kw">let </span>trailer_ptr = <span class="kw">unsafe </span>{ Header::get_trailer(NonNull::from(header)) };
                <span class="macro">assert_eq!</span>(trailer_addr, trailer_ptr.as_ptr() <span class="kw">as </span>usize);

                <span class="kw">let </span>scheduler_addr = scheduler <span class="kw">as </span><span class="kw-2">*const </span>S <span class="kw">as </span>usize;
                <span class="kw">let </span>scheduler_ptr = <span class="kw">unsafe </span>{ Header::get_scheduler::&lt;S&gt;(NonNull::from(header)) };
                <span class="macro">assert_eq!</span>(scheduler_addr, scheduler_ptr.as_ptr() <span class="kw">as </span>usize);

                <span class="kw">let </span>id_addr = task_id <span class="kw">as </span><span class="kw-2">*const </span>Id <span class="kw">as </span>usize;
                <span class="kw">let </span>id_ptr = <span class="kw">unsafe </span>{ Header::get_id_ptr(NonNull::from(header)) };
                <span class="macro">assert_eq!</span>(id_addr, id_ptr.as_ptr() <span class="kw">as </span>usize);
            }
            <span class="kw">unsafe </span>{
                check(
                    <span class="kw-2">&amp;</span>result.header,
                    <span class="kw-2">&amp;</span>result.trailer,
                    <span class="kw-2">&amp;</span>result.core.scheduler,
                    <span class="kw-2">&amp;</span>result.core.task_id,
                );
            }
        }
...
<span class="doccomment">/// Set and clear the task id in the context when the future is executed or
/// dropped, or when the output produced by the future is dropped.
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">struct </span>TaskIdGuard {
    parent_task_id: <span class="prelude-ty">Option</span>&lt;Id&gt;,
}

<span class="kw">impl </span>TaskIdGuard {
    <span class="kw">fn </span>enter(id: Id) -&gt; <span class="self">Self </span>{
        TaskIdGuard {
            parent_task_id: context::set_current_task_id(<span class="prelude-val">Some</span>(id)),
        }
    }
}

<span class="kw">impl </span>Drop <span class="kw">for </span>TaskIdGuard {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        context::set_current_task_id(<span class="self">self</span>.parent_task_id);
    }
}

<span class="kw">impl</span>&lt;T: Future, S: Schedule&gt; Core&lt;T, S&gt; {
    ...
    /// heap.
        ...
        <span class="kw">let </span>res = {
            <span class="self">self</span>.stage.stage.with_mut(|ptr| {
                ...
                <span class="kw">let </span>_guard = TaskIdGuard::enter(<span class="self">self</span>.task_id);
    ...
    <span class="kw">unsafe fn </span>set_stage(<span class="kw-2">&amp;</span><span class="self">self</span>, stage: Stage&lt;T&gt;) {
        <span class="kw">let </span>_guard = TaskIdGuard::enter(<span class="self">self</span>.task_id);
        <span class="self">self</span>.stage.stage.with_mut(|ptr| <span class="kw-2">*</span>ptr = stage);
    }
}
...
<span class="kw">impl </span>Header {
    ...
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>set_owner_id(<span class="kw-2">&amp;</span><span class="self">self</span>, owner: NonZeroU64) {
        <span class="self">self</span>.owner_id.with_mut(|ptr| <span class="kw-2">*</span>ptr = <span class="prelude-val">Some</span>(owner));
    }

    <span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>get_owner_id(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;NonZeroU64&gt; {
    ...
    <span class="doccomment">/// Gets a pointer to the `Trailer` of the task containing this `Header`.
    ///
    /// # Safety
    ///
    /// The provided raw pointer must point at the header of a task.
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>get_trailer(me: NonNull&lt;Header&gt;) -&gt; NonNull&lt;Trailer&gt; {
        <span class="kw">let </span>offset = me.as_ref().vtable.trailer_offset;
        <span class="kw">let </span>trailer = me.as_ptr().cast::&lt;u8&gt;().add(offset).cast::&lt;Trailer&gt;();
        NonNull::new_unchecked(trailer)
    }

    <span class="doccomment">/// Gets a pointer to the scheduler of the task containing this `Header`.
    ///
    /// # Safety
    ///
    /// The provided raw pointer must point at the header of a task.
    ...
    /// task.
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>get_scheduler&lt;S&gt;(me: NonNull&lt;Header&gt;) -&gt; NonNull&lt;S&gt; {
        <span class="kw">let </span>offset = me.as_ref().vtable.scheduler_offset;
        <span class="kw">let </span>scheduler = me.as_ptr().cast::&lt;u8&gt;().add(offset).cast::&lt;S&gt;();
        NonNull::new_unchecked(scheduler)
    }

    <span class="doccomment">/// Gets a pointer to the id of the task containing this `Header`.
    ///
    /// # Safety
    ///
    /// The provided raw pointer must point at the header of a task.
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>get_id_ptr(me: NonNull&lt;Header&gt;) -&gt; NonNull&lt;Id&gt; {
        <span class="kw">let </span>offset = me.as_ref().vtable.id_offset;
        <span class="kw">let </span>id = me.as_ptr().cast::&lt;u8&gt;().add(offset).cast::&lt;Id&gt;();
        NonNull::new_unchecked(id)
    }

    <span class="doccomment">/// Gets the id of the task containing this `Header`.
    ///
    /// # Safety
    ///
    /// The provided raw pointer must point at the header of a task.
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>get_id(me: NonNull&lt;Header&gt;) -&gt; Id {
        <span class="kw">let </span>ptr = Header::get_id_ptr(me).as_ptr();
        <span class="kw-2">*</span>ptr
    }

    <span class="doccomment">/// Gets the tracing id of the task containing this `Header`.
    ///
    /// # Safety
    ///
    /// The provided raw pointer must point at the header of a task.
    </span><span class="attr">#[cfg(all(tokio_unstable, feature = <span class="string">"tracing"</span>))]
    </span><span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">unsafe fn </span>get_tracing_id(me: <span class="kw-2">&amp;</span>NonNull&lt;Header&gt;) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>tracing::Id&gt; {
