...
</pre></div><pre class="rust"><code><span class="attr">#![cfg_attr(not(feature = <span class="string">"net"</span>), allow(dead_code))]

</span><span class="kw">use </span><span class="kw">crate</span>::io::interest::Interest;
<span class="kw">use </span><span class="kw">crate</span>::runtime::io::{Direction, Handle, ReadyEvent, ScheduledIo};
...
<span class="macro">cfg_io_driver!</span> {
    ...
    /// [`poll_write_ready`]: method@Self::poll_write_ready`
        ...
        <span class="doccomment">/// Handle to the associated runtime.
        ///
        /// TODO: this can probably be moved into `ScheduledIo`.
        </span>handle: scheduler::Handle,

        <span class="doccomment">/// Reference to state stored by the driver.
        </span>shared: Arc&lt;ScheduledIo&gt;,
    }
}
...
<span class="comment">// ===== impl Registration =====
    ...
    <span class="doccomment">/// Registers the I/O resource with the reactor for the provided handle, for
    ...
    /// - `Err` if an error was encountered during registration
        ...
        <span class="kw">let </span>shared = handle.driver().io().add_source(io, interest)<span class="question-mark">?</span>;
    ...
    /// `Err` is returned if an error is encountered.
        ...
        <span class="self">self</span>.handle().deregister_source(<span class="kw-2">&amp;</span><span class="self">self</span>.shared, io)
    }
    ...
    /// received.
        ...
        <span class="kw">if </span>ev.is_shutdown {
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">async fn </span>readiness(<span class="kw-2">&amp;</span><span class="self">self</span>, interest: Interest) -&gt; io::Result&lt;ReadyEvent&gt; {
        <span class="kw">let </span>ev = <span class="self">self</span>.shared.readiness(interest).<span class="kw">await</span>;

        <span class="kw">if </span>ev.is_shutdown {
            <span class="kw">return </span><span class="prelude-val">Err</span>(gone());
        }

        <span class="prelude-val">Ok</span>(ev)
    }

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">async fn </span>async_io&lt;R&gt;(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        interest: Interest,
        <span class="kw-2">mut </span>f: <span class="kw">impl </span>FnMut() -&gt; io::Result&lt;R&gt;,
    ) -&gt; io::Result&lt;R&gt; {
        <span class="kw">loop </span>{
            ...
            <span class="kw">let </span>coop = <span class="kw">crate</span>::future::poll_fn(<span class="kw">crate</span>::runtime::coop::poll_proceed).<span class="kw">await</span>;
    ...
    <span class="kw">fn </span>handle(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>Handle {
        <span class="self">self</span>.handle.driver().io()
    }
}
