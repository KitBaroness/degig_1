...
</pre></div><pre class="rust"><code><span class="comment">// Currently, rust warns when an unsafe fn contains an unsafe {} block. However,
...
<span class="kw">use </span>entry::{EntryList, TimerHandle, TimerShared, MAX_SAFE_MILLIS_DURATION};
...
<span class="doccomment">/// Timer state shared between `Driver`, `Handle`, and `Registration`.
    ...
    </span><span class="attr">#[cfg(feature = <span class="string">"test-util"</span>)]
...
<span class="comment">// ===== impl Driver =====
    ...
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new(park: IoStack, clock: <span class="kw-2">&amp;</span>Clock) -&gt; (Driver, Handle) {
    ...
    <span class="kw">fn </span>park_internal(<span class="kw-2">&amp;mut </span><span class="self">self</span>, rt_handle: <span class="kw-2">&amp;</span>driver::Handle, limit: <span class="prelude-ty">Option</span>&lt;Duration&gt;) {
        ...
        <span class="kw">match </span>next_wake {
            <span class="prelude-val">Some</span>(when) =&gt; {
                <span class="kw">let </span>now = handle.time_source.now(rt_handle.clock());
        ...
        </span>handle.process(rt_handle.clock());
    }
    ...
    <span class="macro">cfg_test_util!</span> {
        <span class="kw">fn </span>park_thread_timeout(<span class="kw-2">&amp;mut </span><span class="self">self</span>, rt_handle: <span class="kw-2">&amp;</span>driver::Handle, duration: Duration) {
            <span class="kw">let </span>handle = rt_handle.time();
            <span class="kw">let </span>clock = rt_handle.clock();
            ...
            <span class="kw">if </span>clock.can_auto_advance() {
                ...
                // advance the clock.
                    ...
                    </span><span class="kw">if let </span><span class="prelude-val">Err</span>(msg) = clock.advance(duration) {
...
<span class="kw">impl </span>Handle {
    ...
    </span><span class="kw">pub</span>(<span class="self">self</span>) <span class="kw">fn </span>process(<span class="kw-2">&amp;</span><span class="self">self</span>, clock: <span class="kw-2">&amp;</span>Clock) {
        <span class="kw">let </span>now = <span class="self">self</span>.time_source().now(clock);
    ...
    <span class="kw">pub</span>(<span class="self">self</span>) <span class="kw">fn </span>process_at_time(<span class="kw-2">&amp;</span><span class="self">self</span>, <span class="kw-2">mut </span>now: u64) {
        ...
        <span class="kw">if </span>now &lt; lock.wheel.elapsed() {
        ...
        <span class="kw">for </span>waker <span class="kw">in </span><span class="kw-2">&amp;mut </span>waker_list[<span class="number">0</span>..waker_idx] {
            waker.take().unwrap().wake();
        }
    }
