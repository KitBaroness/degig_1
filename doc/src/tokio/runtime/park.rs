...
<span class="comment">// Bit of a hack, but it is only for loom
</span><span class="attr">#[cfg(loom)]
</span><span class="macro">tokio_thread_local!</span> {
    <span class="kw">static </span>CURRENT_THREAD_PARK_COUNT: AtomicUsize = AtomicUsize::new(<span class="number">0</span>);
}
...
<span class="comment">// ==== impl ParkThread ====
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>unpark(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; UnparkThread {
        <span class="kw">let </span>inner = <span class="self">self</span>.inner.clone();
        UnparkThread { inner }
    }

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>park(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="attr">#[cfg(loom)]
        </span>CURRENT_THREAD_PARK_COUNT.with(|count| count.fetch_add(<span class="number">1</span>, SeqCst));
        <span class="self">self</span>.inner.park();
    }

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>park_timeout(<span class="kw-2">&amp;mut </span><span class="self">self</span>, duration: Duration) {
        <span class="attr">#[cfg(loom)]
        </span>CURRENT_THREAD_PARK_COUNT.with(|count| count.fetch_add(<span class="number">1</span>, SeqCst));

        <span class="comment">// Wasm doesn't have threads, so just sleep.
        </span><span class="attr">#[cfg(not(target_family = <span class="string">"wasm"</span>))]
        </span><span class="self">self</span>.inner.park_timeout(duration);
        <span class="attr">#[cfg(target_family = <span class="string">"wasm"</span>)]
        </span>std::thread::sleep(duration);
    }
...
<span class="comment">// ==== impl Inner ====
    ...
    </span><span class="kw">fn </span>park_timeout(<span class="kw-2">&amp;</span><span class="self">self</span>, dur: Duration) {
...
</span><span class="kw">impl </span>UnparkThread {
...
<span class="kw">use </span><span class="kw">crate</span>::loom::thread::AccessError;
...
<span class="kw">impl </span>CachedParkThread {
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>waker(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;Waker, AccessError&gt; {
        <span class="self">self</span>.unpark().map(UnparkThread::into_waker)
    }

    <span class="kw">fn </span>unpark(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;UnparkThread, AccessError&gt; {
        <span class="self">self</span>.with_current(ParkThread::unpark)
    }

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>park(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="self">self</span>.with_current(|park_thread| park_thread.inner.park())
            .unwrap();
    }

    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>park_timeout(<span class="kw-2">&amp;mut </span><span class="self">self</span>, duration: Duration) {
        <span class="self">self</span>.with_current(|park_thread| park_thread.inner.park_timeout(duration))
            .unwrap();
    }

    <span class="doccomment">/// Gets a reference to the `ParkThread` handle for this thread.
    </span><span class="kw">fn </span>with_current&lt;F, R&gt;(<span class="kw-2">&amp;</span><span class="self">self</span>, f: F) -&gt; <span class="prelude-ty">Result</span>&lt;R, AccessError&gt;
    <span class="kw">where
    ...
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>block_on&lt;F: Future&gt;(<span class="kw-2">&amp;mut </span><span class="self">self</span>, f: F) -&gt; <span class="prelude-ty">Result</span>&lt;F::Output, AccessError&gt; {
        <span class="kw">use </span>std::task::Context;
        <span class="kw">use </span>std::task::Poll::Ready;

        <span class="kw">let </span>waker = <span class="self">self</span>.waker()<span class="question-mark">?</span>;
        ...
        <span class="kw">loop </span>{
            <span class="kw">if let </span>Ready(v) = <span class="kw">crate</span>::runtime::coop::budget(|| f.as_mut().poll(<span class="kw-2">&amp;mut </span>cx)) {
...
<span class="kw">unsafe fn </span>clone(raw: <span class="kw-2">*const </span>()) -&gt; RawWaker {
    Arc::increment_strong_count(raw <span class="kw">as </span><span class="kw-2">*const </span>Inner);
...
<span class="kw">unsafe fn </span>wake_by_ref(raw: <span class="kw-2">*const </span>()) {
    <span class="kw">let </span>raw = raw <span class="kw">as </span><span class="kw-2">*const </span>Inner;
    (<span class="kw-2">*</span>raw).unpark();
}

<span class="attr">#[cfg(loom)]
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>current_thread_park_count() -&gt; usize {
