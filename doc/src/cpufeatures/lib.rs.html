...
</pre></div><pre class="rust"><code><span class="doccomment">//! This crate provides macros for runtime CPU feature detection. It's intended
...
#[cfg(target_arch = <span class="string">"loongarch64"</span>)]
#[doc(hidden)]
</span><span class="kw">pub mod </span>loongarch64;
...
</span><span class="kw">mod </span>miri;
...
</span><span class="macro">compile_error!</span>(<span class="string">"This crate works only on `aarch64`, `loongarch64`, `x86`, and `x86-64` targets."</span>);

<span class="doccomment">/// Create module with CPU feature detection code.
</span><span class="attr">#[macro_export]
</span><span class="macro">macro_rules!</span> new {
    (<span class="macro-nonterminal">$mod_name</span>:ident, $(<span class="macro-nonterminal">$tf</span>:tt),+ $(,)<span class="question-mark">?</span>) =&gt; {
        <span class="kw">mod </span><span class="macro-nonterminal">$mod_name </span>{
            <span class="kw">use </span>core::sync::atomic::{AtomicU8, Ordering::Relaxed};

            <span class="kw">const </span>UNINIT: u8 = u8::max_value();
            <span class="kw">static </span>STORAGE: AtomicU8 = AtomicU8::new(UNINIT);

            <span class="doccomment">/// Initialization token
            </span><span class="attr">#[derive(Copy, Clone, Debug)]
            </span><span class="kw">pub struct </span>InitToken(());

            <span class="kw">impl </span>InitToken {
                <span class="doccomment">/// Get initialized value
                </span><span class="attr">#[inline(always)]
                </span><span class="kw">pub fn </span>get(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
                    <span class="macro-nonterminal">$</span><span class="macro">crate::__unless_target_features!</span> {
                        $(<span class="macro-nonterminal">$tf</span>),+ =&gt; {
                            STORAGE.load(Relaxed) == <span class="number">1
                        </span>}
                    }
                }
            }

            <span class="doccomment">/// Initialize underlying storage if needed and get
            /// stored value and initialization token.
            </span><span class="attr">#[inline]
            </span><span class="kw">pub fn </span>init_get() -&gt; (InitToken, bool) {
                <span class="kw">let </span>res = <span class="macro-nonterminal">$</span><span class="macro">crate::__unless_target_features!</span> {
                    $(<span class="macro-nonterminal">$tf</span>),+ =&gt; {
                        <span class="comment">// Relaxed ordering is fine, as we only have a single atomic variable.
                        </span><span class="kw">let </span>val = STORAGE.load(Relaxed);

                        <span class="kw">if </span>val == UNINIT {
                            <span class="kw">let </span>res = <span class="macro-nonterminal">$</span><span class="macro">crate::__detect_target_features!</span>($(<span class="macro-nonterminal">$tf</span>),+);
                            STORAGE.store(res <span class="kw">as </span>u8, Relaxed);
                            res
            ...
            </span><span class="kw">pub fn </span>init() -&gt; InitToken {
                init_get().<span class="number">0
            </span>}

            <span class="doccomment">/// Initialize underlying storage if needed and get
            /// stored value.
            </span><span class="attr">#[inline]
            </span><span class="kw">pub fn </span>get() -&gt; bool {
                init_get().<span class="number">1
            </span>}
        }
    };
}
