...
<span class="kw">impl </span>Compress {
    ...
    </span><span class="attr">#[cfg(feature = <span class="string">"any_zlib"</span>)]
    ...
    /// function.
        ...
        write_to_spare_capacity_of_vec(output, |out| {
            ...
            <span class="kw">let </span>ret = <span class="self">self</span>.compress(input, out, flush);
            <span class="kw">let </span>bytes_written = <span class="self">self</span>.total_out() - before;
            (bytes_written <span class="kw">as </span>usize, ret)
        })
    }
}
...
<span class="kw">impl </span>Decompress {
    ...
    /// `DecompressError` to indicate that the stream of input bytes is corrupted.
        ...
        write_to_spare_capacity_of_vec(output, |out| {
            <span class="kw">let </span>before = <span class="self">self</span>.total_out();
            <span class="kw">let </span>ret = <span class="self">self</span>.decompress(input, out, flush);
            <span class="kw">let </span>bytes_written = <span class="self">self</span>.total_out() - before;
            (bytes_written <span class="kw">as </span>usize, ret)
        })
    }
...
<span class="doccomment">/// Allows `writer` to write data into the spare capacity of the `output` vector.
...
</span><span class="kw">fn </span>write_to_spare_capacity_of_vec&lt;T&gt;(
    output: <span class="kw-2">&amp;mut </span>Vec&lt;u8&gt;,
    writer: <span class="kw">impl </span>FnOnce(<span class="kw-2">&amp;mut </span>[u8]) -&gt; (usize, T),
) -&gt; T {
    <span class="kw">let </span>cap = output.capacity();
    <span class="kw">let </span>len = output.len();

    output.resize(output.capacity(), <span class="number">0</span>);
    <span class="kw">let </span>(bytes_written, ret) = writer(<span class="kw-2">&amp;mut </span>output[len..]);

    <span class="kw">let </span>new_len = core::cmp::min(len + bytes_written, cap); <span class="comment">// Sanitizes `bytes_written`.
    </span>output.resize(new_len, <span class="number">0 </span><span class="comment">/* unused */</span>);
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[cfg(feature = <span class="string">"any_zlib"</span>)]
