...
<span class="kw">use </span>std::io::{BufRead, Error, ErrorKind, Read, <span class="prelude-ty">Result</span>, Write};
<span class="kw">use </span>std::time;

<span class="kw">use </span><span class="kw">crate</span>::bufreader::BufReader;
<span class="kw">use crate</span>::{Compression, Crc};
...
<span class="kw">pub static </span>FRESERVED: u8 = <span class="number">1 </span>&lt;&lt; <span class="number">5 </span>| <span class="number">1 </span>&lt;&lt; <span class="number">6 </span>| <span class="number">1 </span>&lt;&lt; <span class="number">7</span>;

<span class="kw">pub mod </span>bufread;
<span class="kw">pub mod </span>read;
<span class="kw">pub mod </span>write;

<span class="comment">// The maximum length of the header filename and comment fields. More than
// enough for these fields in reasonable use, but prevents possible attacks.
</span><span class="kw">const </span>MAX_HEADER_BUF: usize = <span class="number">65535</span>;

<span class="doccomment">/// A structure representing the header of a gzip stream.
///
/// The header can contain metadata about the file that was compressed, if
/// present.
</span><span class="attr">#[derive(PartialEq, Clone, Debug, Default)]
...
</span><span class="kw">pub enum </span>GzHeaderState {
    Start(u8, [u8; <span class="number">10</span>]),
    Xlen(<span class="prelude-ty">Option</span>&lt;Box&lt;Crc&gt;&gt;, u8, [u8; <span class="number">2</span>]),
    Extra(<span class="prelude-ty">Option</span>&lt;Box&lt;Crc&gt;&gt;, u16),
    Filename(<span class="prelude-ty">Option</span>&lt;Box&lt;Crc&gt;&gt;),
    Comment(<span class="prelude-ty">Option</span>&lt;Box&lt;Crc&gt;&gt;),
    Crc(<span class="prelude-ty">Option</span>&lt;Box&lt;Crc&gt;&gt;, u8, [u8; <span class="number">2</span>]),
    Complete,
}

<span class="kw">impl </span>Default <span class="kw">for </span>GzHeaderState {
...
</span><span class="kw">pub struct </span>GzHeaderParser {
    state: GzHeaderState,
    flags: u8,
    header: GzHeader,
}

<span class="kw">impl </span>GzHeaderParser {
    <span class="kw">fn </span>new() -&gt; <span class="self">Self </span>{
        GzHeaderParser {
            state: GzHeaderState::Start(<span class="number">0</span>, [<span class="number">0</span>; <span class="number">10</span>]),
            flags: <span class="number">0</span>,
            header: GzHeader::default(),
        }
    }

    <span class="kw">fn </span>parse&lt;<span class="lifetime">'a</span>, R: Read&gt;(<span class="kw-2">&amp;mut </span><span class="self">self</span>, r: <span class="kw-2">&amp;</span><span class="lifetime">'a </span><span class="kw-2">mut </span>R) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
        <span class="kw">loop </span>{
            <span class="kw">match </span><span class="kw-2">&amp;mut </span><span class="self">self</span>.state {
                GzHeaderState::Start(count, buffer) =&gt; {
                    <span class="kw">while </span>(<span class="kw-2">*</span>count <span class="kw">as </span>usize) &lt; buffer.len() {
                        <span class="kw-2">*</span>count += read_into(r, <span class="kw-2">&amp;mut </span>buffer[<span class="kw-2">*</span>count <span class="kw">as </span>usize..])<span class="question-mark">? </span><span class="kw">as </span>u8;
                    }
                    <span class="comment">// Gzip identification bytes
                    </span><span class="kw">if </span>buffer[<span class="number">0</span>] != <span class="number">0x1f </span>|| buffer[<span class="number">1</span>] != <span class="number">0x8b </span>{
                        <span class="kw">return </span><span class="prelude-val">Err</span>(bad_header());
                    }
                    <span class="comment">// Gzip compression method (8 = deflate)
                    </span><span class="kw">if </span>buffer[<span class="number">2</span>] != <span class="number">8 </span>{
                        <span class="kw">return </span><span class="prelude-val">Err</span>(bad_header());
                    }
                    <span class="self">self</span>.flags = buffer[<span class="number">3</span>];
                    <span class="comment">// RFC1952: "must give an error indication if any reserved bit is non-zero"
                    </span><span class="kw">if </span><span class="self">self</span>.flags &amp; FRESERVED != <span class="number">0 </span>{
                        <span class="kw">return </span><span class="prelude-val">Err</span>(bad_header());
                    }
                    <span class="self">self</span>.header.mtime = ((buffer[<span class="number">4</span>] <span class="kw">as </span>u32) &lt;&lt; <span class="number">0</span>)
                        | ((buffer[<span class="number">5</span>] <span class="kw">as </span>u32) &lt;&lt; <span class="number">8</span>)
                        | ((buffer[<span class="number">6</span>] <span class="kw">as </span>u32) &lt;&lt; <span class="number">16</span>)
                        | ((buffer[<span class="number">7</span>] <span class="kw">as </span>u32) &lt;&lt; <span class="number">24</span>);
                    <span class="kw">let </span>_xfl = buffer[<span class="number">8</span>];
                    <span class="self">self</span>.header.operating_system = buffer[<span class="number">9</span>];
                    <span class="kw">let </span>crc = <span class="kw">if </span><span class="self">self</span>.flags &amp; FHCRC != <span class="number">0 </span>{
                        <span class="kw">let </span><span class="kw-2">mut </span>crc = Box::new(Crc::new());
                        crc.update(buffer);
                        <span class="prelude-val">Some</span>(crc)
                    } <span class="kw">else </span>{
                        <span class="prelude-val">None
                    </span>};
                    <span class="self">self</span>.state = GzHeaderState::Xlen(crc, <span class="number">0</span>, [<span class="number">0</span>; <span class="number">2</span>]);
                }
                GzHeaderState::Xlen(crc, count, buffer) =&gt; {
                    <span class="kw">if </span><span class="self">self</span>.flags &amp; FEXTRA != <span class="number">0 </span>{
                        <span class="kw">while </span>(<span class="kw-2">*</span>count <span class="kw">as </span>usize) &lt; buffer.len() {
                            <span class="kw-2">*</span>count += read_into(r, <span class="kw-2">&amp;mut </span>buffer[<span class="kw-2">*</span>count <span class="kw">as </span>usize..])<span class="question-mark">? </span><span class="kw">as </span>u8;
                        }
                        <span class="kw">if let </span><span class="prelude-val">Some</span>(crc) = crc {
                            crc.update(buffer);
                        }
                        <span class="kw">let </span>xlen = parse_le_u16(<span class="kw-2">&amp;</span>buffer);
                        <span class="self">self</span>.header.extra = <span class="prelude-val">Some</span>(<span class="macro">vec!</span>[<span class="number">0</span>; xlen <span class="kw">as </span>usize]);
                        <span class="self">self</span>.state = GzHeaderState::Extra(crc.take(), <span class="number">0</span>);
                    } <span class="kw">else </span>{
                        <span class="self">self</span>.state = GzHeaderState::Filename(crc.take());
                    }
                }
                GzHeaderState::Extra(crc, count) =&gt; {
                    <span class="macro">debug_assert!</span>(<span class="self">self</span>.header.extra.is_some());
                    <span class="kw">let </span>extra = <span class="self">self</span>.header.extra.as_mut().unwrap();
                    <span class="kw">while </span>(<span class="kw-2">*</span>count <span class="kw">as </span>usize) &lt; extra.len() {
                        <span class="kw-2">*</span>count += read_into(r, <span class="kw-2">&amp;mut </span>extra[<span class="kw-2">*</span>count <span class="kw">as </span>usize..])<span class="question-mark">? </span><span class="kw">as </span>u16;
                    }
                    <span class="kw">if let </span><span class="prelude-val">Some</span>(crc) = crc {
                        crc.update(extra);
                    }
                ...
                GzHeaderState::Filename(crc) =&gt; {
                    <span class="kw">if </span><span class="self">self</span>.flags &amp; FNAME != <span class="number">0 </span>{
                        <span class="kw">let </span>filename = <span class="self">self</span>.header.filename.get_or_insert_with(Vec::new);
                        read_to_nul(r, filename)<span class="question-mark">?</span>;
                        <span class="kw">if let </span><span class="prelude-val">Some</span>(crc) = crc {
                            crc.update(filename);
                            crc.update(<span class="string">b"\0"</span>);
                        }
                    }
                    <span class="self">self</span>.state = GzHeaderState::Comment(crc.take());
                }
                GzHeaderState::Comment(crc) =&gt; {
                    <span class="kw">if </span><span class="self">self</span>.flags &amp; FCOMMENT != <span class="number">0 </span>{
                        <span class="kw">let </span>comment = <span class="self">self</span>.header.comment.get_or_insert_with(Vec::new);
                        read_to_nul(r, comment)<span class="question-mark">?</span>;
                        <span class="kw">if let </span><span class="prelude-val">Some</span>(crc) = crc {
                            crc.update(comment);
                            crc.update(<span class="string">b"\0"</span>);
                        }
                    }
                    <span class="self">self</span>.state = GzHeaderState::Crc(crc.take(), <span class="number">0</span>, [<span class="number">0</span>; <span class="number">2</span>]);
                }
                GzHeaderState::Crc(crc, count, buffer) =&gt; {
                    <span class="kw">if let </span><span class="prelude-val">Some</span>(crc) = crc {
                        <span class="macro">debug_assert!</span>(<span class="self">self</span>.flags &amp; FHCRC != <span class="number">0</span>);
                        <span class="kw">while </span>(<span class="kw-2">*</span>count <span class="kw">as </span>usize) &lt; buffer.len() {
                            <span class="kw-2">*</span>count += read_into(r, <span class="kw-2">&amp;mut </span>buffer[<span class="kw-2">*</span>count <span class="kw">as </span>usize..])<span class="question-mark">? </span><span class="kw">as </span>u8;
                        }
                        <span class="kw">let </span>stored_crc = parse_le_u16(<span class="kw-2">&amp;</span>buffer);
                        <span class="kw">let </span>calced_crc = crc.sum() <span class="kw">as </span>u16;
                        <span class="kw">if </span>stored_crc != calced_crc {
                            <span class="kw">return </span><span class="prelude-val">Err</span>(corrupt());
                        }
                    }
                    <span class="self">self</span>.state = GzHeaderState::Complete;
                }
    ...
    <span class="kw">fn </span>header(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;<span class="kw-2">&amp;</span>GzHeader&gt; {
        <span class="kw">match </span><span class="self">self</span>.state {
            GzHeaderState::Complete =&gt; <span class="prelude-val">Some</span>(<span class="kw-2">&amp;</span><span class="self">self</span>.header),
            <span class="kw">_ </span>=&gt; <span class="prelude-val">None</span>,
        }
    }
}

<span class="kw">impl </span>From&lt;GzHeaderParser&gt; <span class="kw">for </span>GzHeader {
    <span class="kw">fn </span>from(parser: GzHeaderParser) -&gt; <span class="self">Self </span>{
        <span class="macro">debug_assert!</span>(<span class="macro">matches!</span>(parser.state, GzHeaderState::Complete));
        parser.header
    }
}

<span class="comment">// Attempt to fill the `buffer` from `r`. Return the number of bytes read.
...
</span><span class="kw">fn </span>read_into&lt;R: Read&gt;(r: <span class="kw-2">&amp;mut </span>R, buffer: <span class="kw-2">&amp;mut </span>[u8]) -&gt; <span class="prelude-ty">Result</span>&lt;usize&gt; {
    <span class="macro">debug_assert!</span>(!buffer.is_empty());
    <span class="kw">match </span>r.read(buffer) {
        <span class="prelude-val">Ok</span>(<span class="number">0</span>) =&gt; <span class="prelude-val">Err</span>(ErrorKind::UnexpectedEof.into()),
        <span class="prelude-val">Ok</span>(n) =&gt; <span class="prelude-val">Ok</span>(n),
        <span class="prelude-val">Err</span>(<span class="kw-2">ref </span>e) <span class="kw">if </span>e.kind() == ErrorKind::Interrupted =&gt; <span class="prelude-val">Ok</span>(<span class="number">0</span>),
        <span class="prelude-val">Err</span>(e) =&gt; <span class="prelude-val">Err</span>(e),
    }
}

<span class="comment">// Read `r` up to the first nul byte, pushing non-nul bytes to `buffer`.
</span><span class="kw">fn </span>read_to_nul&lt;R: Read&gt;(r: <span class="kw-2">&amp;mut </span>R, buffer: <span class="kw-2">&amp;mut </span>Vec&lt;u8&gt;) -&gt; <span class="prelude-ty">Result</span>&lt;()&gt; {
    <span class="kw">let </span><span class="kw-2">mut </span>bytes = r.bytes();
    <span class="kw">loop </span>{
        <span class="kw">match </span>bytes.next().transpose()<span class="question-mark">? </span>{
            <span class="prelude-val">Some</span>(byte) <span class="kw">if </span>byte == <span class="number">0 </span>=&gt; {
                <span class="kw">return </span><span class="prelude-val">Ok</span>(());
            }
            <span class="prelude-val">Some</span>(<span class="kw">_</span>) <span class="kw">if </span>buffer.len() == MAX_HEADER_BUF =&gt; {
                <span class="kw">return </span><span class="prelude-val">Err</span>(Error::new(
                    ErrorKind::InvalidInput,
                    <span class="string">"gzip header field too long"</span>,
                ));
            }
            ...
            <span class="prelude-val">None </span>=&gt; {
                <span class="kw">return </span><span class="prelude-val">Err</span>(ErrorKind::UnexpectedEof.into());
            }
        }
    }
}

<span class="kw">fn </span>parse_le_u16(buffer: <span class="kw-2">&amp;</span>[u8; <span class="number">2</span>]) -&gt; u16 {
    (buffer[<span class="number">0</span>] <span class="kw">as </span>u16) | ((buffer[<span class="number">1</span>] <span class="kw">as </span>u16) &lt;&lt; <span class="number">8</span>)
}

<span class="kw">fn </span>bad_header() -&gt; Error {
    Error::new(ErrorKind::InvalidInput, <span class="string">"invalid gzip header"</span>)
}

<span class="kw">fn </span>corrupt() -&gt; Error {
    Error::new(
        ErrorKind::InvalidInput,
        <span class="string">"corrupt gzip stream does not have a matching checksum"</span>,
    )
}
...
<span class="attr">#[cfg(test)]
    ...
    <span class="kw">use super</span>::{read, write, GzBuilder, GzHeaderParser};
    <span class="kw">use crate</span>::{Compression, GzHeader};
    ...
    <span class="attr">#[test]
        ...
        <span class="macro">assert_eq!</span>(v, real);
    }
    ...
    <span class="attr">#[test]
        ...
        <span class="macro">assert_eq!</span>(res, v);
    }

    <span class="comment">// A Rust implementation of CRC that closely matches the C code in RFC1952.
    // Only use this to create CRCs for tests.
    </span><span class="kw">struct </span>Rfc1952Crc {
        <span class="comment">/* Table of CRCs of all 8-bit messages. */
        </span>crc_table: [u32; <span class="number">256</span>],
    }

    <span class="kw">impl </span>Rfc1952Crc {
        <span class="kw">fn </span>new() -&gt; <span class="self">Self </span>{
            <span class="kw">let </span><span class="kw-2">mut </span>crc = Rfc1952Crc {
                crc_table: [<span class="number">0</span>; <span class="number">256</span>],
            };
            <span class="comment">/* Make the table for a fast CRC. */
            </span><span class="kw">for </span>n <span class="kw">in </span><span class="number">0usize</span>..<span class="number">256 </span>{
                <span class="kw">let </span><span class="kw-2">mut </span>c = n <span class="kw">as </span>u32;
                <span class="kw">for </span>_k <span class="kw">in </span><span class="number">0</span>..<span class="number">8 </span>{
                    <span class="kw">if </span>c &amp; <span class="number">1 </span>!= <span class="number">0 </span>{
                        c = <span class="number">0xedb88320 </span>^ (c &gt;&gt; <span class="number">1</span>);
                    } <span class="kw">else </span>{
        ...
        */
        </span><span class="kw">fn </span>update_crc(<span class="kw-2">&amp;</span><span class="self">self</span>, crc: u32, buf: <span class="kw-2">&amp;</span>[u8]) -&gt; u32 {
            <span class="kw">let </span><span class="kw-2">mut </span>c = crc ^ <span class="number">0xffffffff</span>;

            <span class="kw">for </span>b <span class="kw">in </span>buf {
                c = <span class="self">self</span>.crc_table[(c <span class="kw">as </span>u8 ^ <span class="kw-2">*</span>b) <span class="kw">as </span>usize] ^ (c &gt;&gt; <span class="number">8</span>);
            }
            c ^ <span class="number">0xffffffff
        </span>}

        <span class="comment">/* Return the CRC of the bytes buf. */
        </span><span class="kw">fn </span>crc(<span class="kw-2">&amp;</span><span class="self">self</span>, buf: <span class="kw-2">&amp;</span>[u8]) -&gt; u32 {
            <span class="self">self</span>.update_crc(<span class="number">0</span>, buf)
        }
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>roundtrip_header() {
        <span class="kw">let </span><span class="kw-2">mut </span>header = GzBuilder::new()
            .mtime(<span class="number">1234</span>)
            .operating_system(<span class="number">57</span>)
            .filename(<span class="string">"filename"</span>)
            .comment(<span class="string">"comment"</span>)
            .into_header(Compression::fast());

        <span class="comment">// Add a CRC to the header
        </span>header[<span class="number">3</span>] = header[<span class="number">3</span>] ^ <span class="kw">super</span>::FHCRC;
        <span class="kw">let </span>rfc1952_crc = Rfc1952Crc::new();
        <span class="kw">let </span>crc32 = rfc1952_crc.crc(<span class="kw-2">&amp;</span>header);
        <span class="kw">let </span>crc16 = crc32 <span class="kw">as </span>u16;
        header.extend(<span class="kw-2">&amp;</span>crc16.to_le_bytes());

        <span class="kw">let </span><span class="kw-2">mut </span>parser = GzHeaderParser::new();
        parser.parse(<span class="kw-2">&amp;mut </span>header.as_slice()).unwrap();
        <span class="kw">let </span>actual = parser.header().unwrap();
        <span class="macro">assert_eq!</span>(
            actual,
            <span class="kw-2">&amp;</span>GzHeader {
                extra: <span class="prelude-val">None</span>,
                filename: <span class="prelude-val">Some</span>(<span class="string">"filename"</span>.as_bytes().to_vec()),
                comment: <span class="prelude-val">Some</span>(<span class="string">"comment"</span>.as_bytes().to_vec()),
                operating_system: <span class="number">57</span>,
                mtime: <span class="number">1234
            </span>}
        )
    }
