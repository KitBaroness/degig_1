...
<span class="kw">use super</span>::{corrupt, GzBuilder, GzHeader, GzHeaderParser};
...
<span class="kw">impl</span>&lt;W: Write&gt; GzDecoder&lt;W&gt; {
    ...
    <span class="doccomment">/// Returns the header associated with this stream.
        ...
        <span class="self">self</span>.header_parser.header()
    }
...
<span class="kw">impl</span>&lt;W: Write&gt; Write <span class="kw">for </span>GzDecoder&lt;W&gt; {
    <span class="kw">fn </span>write(<span class="kw-2">&amp;mut </span><span class="self">self</span>, <span class="kw-2">mut </span>buf: <span class="kw-2">&amp;</span>[u8]) -&gt; io::Result&lt;usize&gt; {
        <span class="kw">let </span>buflen = buf.len();
        <span class="kw">if </span><span class="self">self</span>.header().is_none() {
            <span class="kw">match </span><span class="self">self</span>.header_parser.parse(<span class="kw-2">&amp;mut </span>buf) {
                <span class="prelude-val">Err</span>(err) =&gt; {
                    <span class="kw">if </span>err.kind() == io::ErrorKind::UnexpectedEof {
                        <span class="comment">// all data read but header still not complete
                        </span><span class="prelude-val">Ok</span>(buflen)
                    } <span class="kw">else </span>{
                        <span class="prelude-val">Err</span>(err)
                    }
                }
                <span class="prelude-val">Ok</span>(<span class="kw">_</span>) =&gt; {
                    <span class="macro">debug_assert!</span>(<span class="self">self</span>.header().is_some());
                    <span class="comment">// buf now contains the unread part of the original buf
                    </span><span class="kw">let </span>n = buflen - buf.len();
                    <span class="prelude-val">Ok</span>(n)
                }
            }
        } <span class="kw">else </span>{
...
<span class="doccomment">/// A gzip streaming decoder that decodes a [gzip file] with multiple members.
///
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>decode_writer_partial_header_filename() {
        <span class="kw">let </span>filename = <span class="string">"test.txt"</span>;
        <span class="kw">let </span><span class="kw-2">mut </span>e = GzBuilder::new()
            .filename(filename)
            .read(STR.as_bytes(), Compression::default());
        <span class="kw">let </span><span class="kw-2">mut </span>bytes = Vec::new();
        e.read_to_end(<span class="kw-2">&amp;mut </span>bytes).unwrap();

        <span class="kw">let </span><span class="kw-2">mut </span>writer = Vec::new();
        <span class="kw">let </span><span class="kw-2">mut </span>decoder = GzDecoder::new(writer);
        <span class="macro">assert_eq!</span>(decoder.write(<span class="kw-2">&amp;</span>bytes[..<span class="number">12</span>]).unwrap(), <span class="number">12</span>);
        <span class="kw">let </span>n = decoder.write(<span class="kw-2">&amp;</span>bytes[<span class="number">12</span>..]).unwrap();
        <span class="kw">if </span>n &lt; bytes.len() - <span class="number">12 </span>{
            decoder.write(<span class="kw-2">&amp;</span>bytes[n + <span class="number">12</span>..]).unwrap();
        }
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>decode_writer_partial_header_comment() {
        <span class="kw">let </span>comment = <span class="string">"test comment"</span>;
        <span class="kw">let </span><span class="kw-2">mut </span>e = GzBuilder::new()
            .comment(comment)
            .read(STR.as_bytes(), Compression::default());
        <span class="kw">let </span><span class="kw-2">mut </span>bytes = Vec::new();
    ...
    <span class="comment">// GzDecoder consumes one gzip member and then returns 0 for subsequent writes, allowing any
    // additional data to be consumed by the caller.
        ...
        <span class="kw">let </span><span class="kw-2">mut </span>writer = Vec::new();
        <span class="kw">let </span><span class="kw-2">mut </span>decoder = GzDecoder::new(writer);
        <span class="kw">let </span><span class="kw-2">mut </span>consumed_bytes = <span class="number">0</span>;
        <span class="kw">loop </span>{
            <span class="kw">let </span>n = decoder.write(<span class="kw-2">&amp;</span>compressed[consumed_bytes..]).unwrap();
        ...
        writer = decoder.finish().unwrap();
        <span class="kw">let </span>actual = String::from_utf8(writer).expect(<span class="string">"String parsing error"</span>);
        <span class="macro">assert_eq!</span>(actual, STR);
        <span class="macro">assert_eq!</span>(<span class="kw-2">&amp;</span>compressed[consumed_bytes..], <span class="string">b"x"</span>);
    }
}
