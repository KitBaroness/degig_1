...
</pre></div><pre class="rust"><code><span class="doccomment">//! Bindings for [`SCNetworkConfiguration`].
//!
...
<span class="kw">use </span><span class="kw">crate</span>::preferences::SCPreferences;
...
<span class="kw">impl </span>SCNetworkService {
    <span class="doccomment">/// Returns an array of all network services
    </span><span class="kw">pub fn </span>get_services(prefs: <span class="kw-2">&amp;</span>SCPreferences) -&gt; CFArray&lt;<span class="self">Self</span>&gt; {
        <span class="kw">unsafe </span>{
            <span class="kw">let </span>array_ptr = SCNetworkServiceCopyAll(prefs.to_void());
            <span class="kw">if </span>array_ptr.is_null() {
                <span class="kw">return </span>create_empty_array();
            }
            CFArray::&lt;<span class="self">Self</span>&gt;::wrap_under_create_rule(array_ptr)
        }
    }

    <span class="doccomment">/// Returns true if the network service is currently enabled
    </span><span class="kw">pub fn </span>enabled(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="kw">unsafe </span>{ SCNetworkServiceGetEnabled(<span class="self">self</span>.<span class="number">0</span>) == <span class="number">0 </span>}
    }

    <span class="doccomment">/// Returns the network interface backing this network service, if it has one.
    </span><span class="kw">pub fn </span>network_interface(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;SCNetworkInterface&gt; {
        <span class="kw">unsafe </span>{
            <span class="kw">let </span>ptr = SCNetworkServiceGetInterface(<span class="self">self</span>.<span class="number">0</span>);
            <span class="kw">if </span>ptr.is_null() {
                <span class="prelude-val">None
            </span>} <span class="kw">else </span>{
                <span class="prelude-val">Some</span>(SCNetworkInterface::wrap_under_get_rule(ptr))
            }
        }
    }

    <span class="doccomment">/// Returns the service identifier.
    </span><span class="kw">pub fn </span>id(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Option</span>&lt;CFString&gt; {
        <span class="kw">unsafe </span>{
            <span class="kw">let </span>ptr = SCNetworkServiceGetServiceID(<span class="self">self</span>.<span class="number">0</span>);
            <span class="kw">if </span>ptr.is_null() {
                <span class="prelude-val">None
            </span>} <span class="kw">else </span>{
                <span class="prelude-val">Some</span>(CFString::wrap_under_get_rule(ptr))
...
<span class="macro">core_foundation::declare_TCFType!</span>(
    <span class="doccomment">/// Represents a complete network configuration for a particular host.
    ///
...
<span class="macro">core_foundation::impl_TCFType!</span>(SCNetworkSet, SCNetworkSetRef, SCNetworkSetGetTypeID);

<span class="kw">impl </span>SCNetworkSet {
    <span class="doccomment">/// Constructs a new set of network services from the preferences.
    </span><span class="kw">pub fn </span>new(prefs: <span class="kw-2">&amp;</span>SCPreferences) -&gt; <span class="self">Self </span>{
        <span class="kw">let </span>ptr = <span class="kw">unsafe </span>{ SCNetworkSetCopyCurrent(prefs.to_void()) };
        <span class="kw">unsafe </span>{ SCNetworkSet::wrap_under_create_rule(ptr) }
    }

    <span class="doccomment">/// Returns an list of network service identifiers, ordered by their priority.
    </span><span class="kw">pub fn </span>service_order(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; CFArray&lt;CFString&gt; {
        <span class="kw">unsafe </span>{
            <span class="kw">let </span>array_ptr = SCNetworkSetGetServiceOrder(<span class="self">self</span>.<span class="number">0</span>);
            <span class="kw">if </span>array_ptr.is_null() {
                <span class="kw">return </span>create_empty_array();
            }
...
<span class="kw">fn </span>create_empty_array&lt;T&gt;() -&gt; CFArray&lt;T&gt; {
    <span class="kw">use </span>std::ptr::null;
    <span class="kw">unsafe </span>{
        CFArray::wrap_under_create_rule(core_foundation::array::CFArrayCreate(
            null() <span class="kw">as </span><span class="kw-2">*const </span><span class="kw">_</span>,
            null() <span class="kw">as </span><span class="kw-2">*const </span><span class="kw">_</span>,
...
<span class="attr">#[cfg(test)]
    ...
    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_service_order() {
        <span class="kw">let </span>prefs = SCPreferences::default(<span class="kw-2">&amp;</span>CFString::new(<span class="string">"test"</span>));
        <span class="kw">let </span>services = SCNetworkService::get_services(<span class="kw-2">&amp;</span>prefs);
        <span class="kw">let </span>set = SCNetworkSet::new(<span class="kw-2">&amp;</span>prefs);
        <span class="kw">let </span>service_order = set.service_order();

        <span class="macro">assert!</span>(service_order.iter().all(|service_id| {
            services
                .iter()
                .find(|service| service.id().as_ref() == <span class="prelude-val">Some</span>(<span class="kw-2">&amp;*</span>service_id))
                .is_some()
        }))
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>test_empty_array() {
        <span class="kw">let </span>empty = create_empty_array::&lt;CFString&gt;();
        <span class="kw">let </span>values = empty.get_all_values();
        <span class="macro">assert!</span>(values.is_empty())
    }
}
