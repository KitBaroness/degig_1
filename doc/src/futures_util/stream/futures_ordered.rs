...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::stream::{FuturesUnordered, StreamExt};
<span class="kw">use </span>alloc::collections::binary_heap::{BinaryHeap, PeekMut};
...
<span class="macro">pin_project!</span> {
    <span class="attr">#[must_use = <span class="string">"futures do nothing unless you `.await` or poll them"</span>]
    #[derive(Debug)]
    </span><span class="kw">struct </span>OrderWrapper&lt;T&gt; {
        <span class="attr">#[pin]
        </span>data: T, <span class="comment">// A future or a future's output
        // Use i64 for index since isize may overflow in 32-bit targets.
        </span>index: i64,
    }
}

<span class="kw">impl</span>&lt;T&gt; PartialEq <span class="kw">for </span>OrderWrapper&lt;T&gt; {
    <span class="kw">fn </span>eq(<span class="kw-2">&amp;</span><span class="self">self</span>, other: <span class="kw-2">&amp;</span><span class="self">Self</span>) -&gt; bool {
        <span class="self">self</span>.index == other.index
    }
}

<span class="kw">impl</span>&lt;T&gt; Eq <span class="kw">for </span>OrderWrapper&lt;T&gt; {}

<span class="kw">impl</span>&lt;T&gt; PartialOrd <span class="kw">for </span>OrderWrapper&lt;T&gt; {
...
<span class="kw">impl</span>&lt;T&gt; Ord <span class="kw">for </span>OrderWrapper&lt;T&gt; {
    <span class="kw">fn </span>cmp(<span class="kw-2">&amp;</span><span class="self">self</span>, other: <span class="kw-2">&amp;</span><span class="self">Self</span>) -&gt; Ordering {
        <span class="comment">// BinaryHeap is a max heap, so compare backwards here.
        </span>other.index.cmp(<span class="kw-2">&amp;</span><span class="self">self</span>.index)
    }
}

<span class="kw">impl</span>&lt;T&gt; Future <span class="kw">for </span>OrderWrapper&lt;T&gt;
<span class="kw">where
    </span>T: Future,
{
    <span class="kw">type </span>Output = OrderWrapper&lt;T::Output&gt;;
    ...
    <span class="kw">fn </span>poll(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;<span class="self">Self</span>::Output&gt; {
        ...
        <span class="self">self</span>.project().data.poll(cx).map(|output| OrderWrapper { data: output, index })
    }
}

<span class="doccomment">/// An unbounded queue of futures.
///
...
</span><span class="kw">pub struct </span>FuturesOrdered&lt;T: Future&gt; {
...
<span class="kw">impl</span>&lt;T: Future&gt; Unpin <span class="kw">for </span>FuturesOrdered&lt;T&gt; {}

<span class="kw">impl</span>&lt;Fut: Future&gt; FuturesOrdered&lt;Fut&gt; {
    ...
    /// [`Poll::Ready(None)`](Poll::Ready).
        ...
        <span class="self">Self </span>{
            ...
            next_incoming_index: <span class="number">0</span>,
            next_outgoing_index: <span class="number">0</span>,
        }
    }

    <span class="doccomment">/// Returns the number of futures contained in the queue.
    ///
    ...
    /// which are waiting for earlier futures to complete.
        ...
        <span class="self">self</span>.in_progress_queue.len() + <span class="self">self</span>.queued_outputs.len()
    }

    <span class="doccomment">/// Returns `true` if the queue contains no futures
    </span><span class="kw">pub fn </span>is_empty(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.in_progress_queue.is_empty() &amp;&amp; <span class="self">self</span>.queued_outputs.is_empty()
    }

    <span class="doccomment">/// Push a future into the queue.
    ///
    ...
    </span><span class="attr">#[deprecated(note = <span class="string">"use `push_back` instead"</span>)]
    </span><span class="kw">pub fn </span>push(<span class="kw-2">&amp;mut </span><span class="self">self</span>, future: Fut) {
        <span class="self">self</span>.push_back(future);
    }

    <span class="doccomment">/// Pushes a future to the back of the queue.
    ///
    /// This function submits the given future to the internal set for managing.
    /// This function will not call [`poll`](Future::poll) on the submitted
    /// future. The caller must ensure that [`FuturesOrdered::poll_next`] is
    /// called in order to receive task notifications.
    </span><span class="kw">pub fn </span>push_back(<span class="kw-2">&amp;mut </span><span class="self">self</span>, future: Fut) {
        <span class="kw">let </span>wrapped = OrderWrapper { data: future, index: <span class="self">self</span>.next_incoming_index };
        <span class="self">self</span>.next_incoming_index += <span class="number">1</span>;
        <span class="self">self</span>.in_progress_queue.push(wrapped);
    }

    <span class="doccomment">/// Pushes a future to the front of the queue.
    ///
    /// This function submits the given future to the internal set for managing.
    /// This function will not call [`poll`](Future::poll) on the submitted
    /// future. The caller must ensure that [`FuturesOrdered::poll_next`] is
    ...
    </span><span class="kw">pub fn </span>push_front(<span class="kw-2">&amp;mut </span><span class="self">self</span>, future: Fut) {
        <span class="kw">let </span>wrapped = OrderWrapper { data: future, index: <span class="self">self</span>.next_outgoing_index - <span class="number">1 </span>};
        <span class="self">self</span>.next_outgoing_index -= <span class="number">1</span>;
        <span class="self">self</span>.in_progress_queue.push(wrapped);
    }
}

<span class="kw">impl</span>&lt;Fut: Future&gt; Default <span class="kw">for </span>FuturesOrdered&lt;Fut&gt; {
    <span class="kw">fn </span>default() -&gt; <span class="self">Self </span>{
        <span class="self">Self</span>::new()
    }
}

<span class="kw">impl</span>&lt;Fut: Future&gt; Stream <span class="kw">for </span>FuturesOrdered&lt;Fut&gt; {
    ...
    <span class="kw">fn </span>poll_next(<span class="kw-2">mut </span><span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;<span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt;&gt; {
        ...
        <span class="comment">// Check to see if we've already received the next value
        </span><span class="kw">if let </span><span class="prelude-val">Some</span>(next_output) = this.queued_outputs.peek_mut() {
            <span class="kw">if </span>next_output.index == this.next_outgoing_index {
                this.next_outgoing_index += <span class="number">1</span>;
                <span class="kw">return </span>Poll::Ready(<span class="prelude-val">Some</span>(PeekMut::pop(next_output).data));
            }
        }

        <span class="kw">loop </span>{
            <span class="kw">match </span><span class="macro">ready!</span>(this.in_progress_queue.poll_next_unpin(cx)) {
                <span class="prelude-val">Some</span>(output) =&gt; {
                    <span class="kw">if </span>output.index == this.next_outgoing_index {
                        this.next_outgoing_index += <span class="number">1</span>;
                        <span class="kw">return </span>Poll::Ready(<span class="prelude-val">Some</span>(output.data));
...
<span class="kw">impl</span>&lt;Fut: Future&gt; Debug <span class="kw">for </span>FuturesOrdered&lt;Fut&gt; {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="macro">write!</span>(f, <span class="string">"FuturesOrdered {{ ... }}"</span>)
    }
}

<span class="kw">impl</span>&lt;Fut: Future&gt; FromIterator&lt;Fut&gt; <span class="kw">for </span>FuturesOrdered&lt;Fut&gt; {
    <span class="kw">fn </span>from_iter&lt;T&gt;(iter: T) -&gt; <span class="self">Self
    </span><span class="kw">where
        </span>T: IntoIterator&lt;Item = Fut&gt;,
    {
        <span class="kw">let </span>acc = <span class="self">Self</span>::new();
        iter.into_iter().fold(acc, |<span class="kw-2">mut </span>acc, item| {
            acc.push_back(item);
            acc
        })
    }
...
<span class="kw">impl</span>&lt;Fut: Future&gt; FusedStream <span class="kw">for </span>FuturesOrdered&lt;Fut&gt; {
    <span class="kw">fn </span>is_terminated(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.in_progress_queue.is_terminated() &amp;&amp; <span class="self">self</span>.queued_outputs.is_empty()
    }
}

<span class="kw">impl</span>&lt;Fut: Future&gt; Extend&lt;Fut&gt; <span class="kw">for </span>FuturesOrdered&lt;Fut&gt; {
