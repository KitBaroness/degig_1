...
<span class="macro">pin_project!</span> {
    <span class="doccomment">/// Stream for the [`take`](super::StreamExt::take) method.
    </span><span class="attr">#[derive(Debug)]
    #[must_use = <span class="string">"streams do nothing unless polled"</span>]
    </span><span class="kw">pub struct </span>Take&lt;St&gt; {
        <span class="attr">#[pin]
        </span>stream: St,
        remaining: usize,
    }
    ...
    </span>St: Stream,
{
    ...
    <span class="kw">fn </span>poll_next(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;<span class="prelude-ty">Option</span>&lt;St::Item&gt;&gt; {
        <span class="kw">if </span><span class="self">self</span>.remaining == <span class="number">0 </span>{
            ...
            <span class="kw">let </span>next = <span class="macro">ready!</span>(this.stream.poll_next(cx));
            <span class="kw">if </span>next.is_some() {
                <span class="kw-2">*</span>this.remaining -= <span class="number">1</span>;
            } <span class="kw">else </span>{
    ...
    <span class="kw">fn </span>size_hint(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; (usize, <span class="prelude-ty">Option</span>&lt;usize&gt;) {
        ...
        <span class="kw">let </span>lower = cmp::min(lower, <span class="self">self</span>.remaining);

        <span class="kw">let </span>upper = <span class="kw">match </span>upper {
            <span class="prelude-val">Some</span>(x) <span class="kw">if </span>x &lt; <span class="self">self</span>.remaining =&gt; <span class="prelude-val">Some</span>(x),
            <span class="kw">_ </span>=&gt; <span class="prelude-val">Some</span>(<span class="self">self</span>.remaining),
        };
...
<span class="kw">impl</span>&lt;St&gt; FusedStream <span class="kw">for </span>Take&lt;St&gt;
<span class="kw">where
    </span>St: FusedStream,
{
    <span class="kw">fn </span>is_terminated(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.remaining == <span class="number">0 </span>|| <span class="self">self</span>.stream.is_terminated()
    }
}
...
</span><span class="kw">impl</span>&lt;S, Item&gt; Sink&lt;Item&gt; <span class="kw">for </span>Take&lt;S&gt;
<span class="kw">where
