...
</pre></div><pre class="rust"><code><span class="kw">use </span>futures_core::{ready, Stream};
...
/// [`Semaphore`]: tokio::sync::Semaphore
    ...
    permit_fut: <span class="prelude-ty">Option</span>&lt;(
        u32, <span class="comment">// The number of permits requested.
        </span>ReusableBoxFuture&lt;<span class="lifetime">'static</span>, <span class="prelude-ty">Result</span>&lt;OwnedSemaphorePermit, AcquireError&gt;&gt;,
    )&gt;,
}
...
<span class="kw">impl </span>PollSemaphore {
    ...
    /// scheduled to receive a wakeup.
        ...
        <span class="self">self</span>.poll_acquire_many(cx, <span class="number">1</span>)
    }

    <span class="doccomment">/// Poll to acquire many permits from the semaphore.
    ///
    /// This can return the following values:
    ///
    ///  - `Poll::Pending` if a permit is not currently available.
    ...
    /// the `Waker` from the `Context` passed to the most recent call is
    /// scheduled to receive a wakeup.
    </span><span class="kw">pub fn </span>poll_acquire_many(
        <span class="kw-2">&amp;mut </span><span class="self">self</span>,
        cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;,
        permits: u32,
    ) -&gt; Poll&lt;<span class="prelude-ty">Option</span>&lt;OwnedSemaphorePermit&gt;&gt; {
        <span class="kw">let </span>permit_future = <span class="kw">match </span><span class="self">self</span>.permit_fut.as_mut() {
            <span class="prelude-val">Some</span>((prev_permits, fut)) <span class="kw">if </span><span class="kw-2">*</span>prev_permits == permits =&gt; fut,
            <span class="prelude-val">Some</span>((old_permits, fut_box)) =&gt; {
                <span class="comment">// We're requesting a different number of permits, so replace the future
                // and record the new amount.
                </span><span class="kw">let </span>fut = Arc::clone(<span class="kw-2">&amp;</span><span class="self">self</span>.semaphore).acquire_many_owned(permits);
                fut_box.set(fut);
                <span class="kw-2">*</span>old_permits = permits;
                fut_box
            }
            <span class="prelude-val">None </span>=&gt; {
                <span class="comment">// avoid allocations completely if we can grab a permit immediately
                </span><span class="kw">match </span>Arc::clone(<span class="kw-2">&amp;</span><span class="self">self</span>.semaphore).try_acquire_many_owned(permits) {
                ...
                <span class="kw">let </span>next_fut = Arc::clone(<span class="kw-2">&amp;</span><span class="self">self</span>.semaphore).acquire_many_owned(permits);
        ...
        <span class="comment">// Assume we'll request the same amount of permits in a subsequent call.
        </span><span class="kw">let </span>next_fut = Arc::clone(<span class="kw-2">&amp;</span><span class="self">self</span>.semaphore).acquire_many_owned(permits);
