...
<span class="kw">const </span>DEFAULT_CAPACITY: usize = <span class="number">4096</span>;

<span class="macro">pin_project!</span> {
    <span class="doccomment">/// Convert an [`AsyncRead`] into a [`Stream`] of byte chunks.
    ///
    ...
    </span><span class="kw">pub struct </span>ReaderStream&lt;R&gt; {
        <span class="comment">// Reader itself.
        //
        // This value is `None` if the stream has terminated.
        </span><span class="attr">#[pin]
        </span>reader: <span class="prelude-ty">Option</span>&lt;R&gt;,
        <span class="comment">// Working buffer, used to optimize allocations.
        </span>buf: BytesMut,
        capacity: usize,
    }
}

<span class="kw">impl</span>&lt;R: AsyncRead&gt; ReaderStream&lt;R&gt; {
    <span class="doccomment">/// Convert an [`AsyncRead`] into a [`Stream`] with item type
    ...
    /// [`AsyncRead`]: tokio::io::AsyncRead
    /// [`Stream`]: futures_core::Stream
    </span><span class="kw">pub fn </span>with_capacity(reader: R, capacity: usize) -&gt; <span class="self">Self </span>{
        ReaderStream {
            reader: <span class="prelude-val">Some</span>(reader),
...
<span class="kw">impl</span>&lt;R: AsyncRead&gt; Stream <span class="kw">for </span>ReaderStream&lt;R&gt; {
    <span class="kw">type </span>Item = std::io::Result&lt;Bytes&gt;;
    <span class="kw">fn </span>poll_next(<span class="kw-2">mut </span><span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;<span class="prelude-ty">Option</span>&lt;<span class="self">Self</span>::Item&gt;&gt; {
        ...
        <span class="kw">let </span>reader = <span class="kw">match </span>this.reader.as_pin_mut() {
        ...
        <span class="kw">if </span>this.buf.capacity() == <span class="number">0 </span>{
            this.buf.reserve(<span class="kw-2">*</span>this.capacity);
        }

        <span class="kw">match </span>poll_read_buf(reader, cx, <span class="kw-2">&amp;mut </span>this.buf) {
            Poll::Pending =&gt; Poll::Pending,
            Poll::Ready(<span class="prelude-val">Err</span>(err)) =&gt; {
                <span class="self">self</span>.project().reader.set(<span class="prelude-val">None</span>);
            ...
            Poll::Ready(<span class="prelude-val">Ok</span>(<span class="kw">_</span>)) =&gt; {
                <span class="kw">let </span>chunk = this.buf.split();
                Poll::Ready(<span class="prelude-val">Some</span>(<span class="prelude-val">Ok</span>(chunk.freeze())))
            }
        }
    }
}
