...
<span class="macro">pin_project!</span> {
    <span class="doccomment">/// Convert a [`Sink`] of byte chunks into an [`AsyncWrite`].
    ///
    ...
    </span><span class="kw">pub struct </span>SinkWriter&lt;S&gt; {
        <span class="attr">#[pin]
        </span>inner: S,
    }
}

<span class="kw">impl</span>&lt;S&gt; SinkWriter&lt;S&gt; {
    <span class="doccomment">/// Creates a new [`SinkWriter`].
    </span><span class="kw">pub fn </span>new(sink: S) -&gt; <span class="self">Self </span>{
        <span class="self">Self </span>{ inner: sink }
    }

    <span class="doccomment">/// Gets a reference to the underlying sink.
    ...
    <span class="doccomment">/// Gets a mutable reference to the underlying sink.
    ...
    <span class="doccomment">/// Consumes this [`SinkWriter`], returning the underlying sink.
    </span><span class="kw">pub fn </span>into_inner(<span class="self">self</span>) -&gt; S {
        <span class="self">self</span>.inner
    }
}
<span class="kw">impl</span>&lt;S, E&gt; AsyncWrite <span class="kw">for </span>SinkWriter&lt;S&gt;
<span class="kw">where
    for</span>&lt;<span class="lifetime">'a</span>&gt; S: Sink&lt;<span class="kw-2">&amp;</span><span class="lifetime">'a </span>[u8], Error = E&gt;,
    E: Into&lt;io::Error&gt;,
{
    <span class="kw">fn </span>poll_write(
        ...
        <span class="macro">ready!</span>(this.inner.as_mut().poll_ready(cx).map_err(Into::into))<span class="question-mark">?</span>;
        <span class="kw">match </span>this.inner.as_mut().start_send(buf) {
            <span class="prelude-val">Ok</span>(()) =&gt; Poll::Ready(<span class="prelude-val">Ok</span>(buf.len())),
            <span class="prelude-val">Err</span>(e) =&gt; Poll::Ready(<span class="prelude-val">Err</span>(e.into())),
        }
    }
    ...
    <span class="kw">fn </span>poll_flush(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;<span class="prelude-ty">Result</span>&lt;(), io::Error&gt;&gt; {
        <span class="self">self</span>.project().inner.poll_flush(cx).map_err(Into::into)
    }
    ...
    <span class="kw">fn </span>poll_shutdown(<span class="self">self</span>: Pin&lt;<span class="kw-2">&amp;mut </span><span class="self">Self</span>&gt;, cx: <span class="kw-2">&amp;mut </span>Context&lt;<span class="lifetime">'_</span>&gt;) -&gt; Poll&lt;<span class="prelude-ty">Result</span>&lt;(), io::Error&gt;&gt; {
        <span class="self">self</span>.project().inner.poll_close(cx).map_err(Into::into)
    }
}

<span class="kw">impl</span>&lt;S: Stream&gt; Stream <span class="kw">for </span>SinkWriter&lt;S&gt; {
...
<span class="kw">impl</span>&lt;S: AsyncRead&gt; AsyncRead <span class="kw">for </span>SinkWriter&lt;S&gt; {
