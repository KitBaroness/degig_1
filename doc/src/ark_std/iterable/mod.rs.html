...
</pre></div><pre class="rust"><code><span class="doccomment">//! A base library for interfacing with streams of vectors and matrices.
//!
...
</span><span class="kw">mod </span>rev;
<span class="kw">pub use </span>rev::Reverse;

<span class="doccomment">/// The trait [`Iterable`] represents a streamable object that can produce
...
/// &amp;Iterable)` to perform the addition of two streams.
</span><span class="kw">pub trait </span>Iterable: Send + Sync {
    <span class="doccomment">/// The type of the element being streamed.
    </span><span class="kw">type </span>Item;
    <span class="doccomment">/// The type of the iterator being generated.
    </span><span class="kw">type </span>Iter: Iterator&lt;Item = <span class="self">Self</span>::Item&gt;;

    <span class="doccomment">///  Return the iterator associated to the current instance.
    ///
    ...
    ///  ```
    </span><span class="kw">fn </span>iter(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="self">Self</span>::Iter;

    <span class="doccomment">/// Return a hint on the length of the stream.
    ///
    ...
    <span class="doccomment">/// Return `true` if the stream is empty, else `false`.
    </span><span class="kw">fn </span>is_empty(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.len() == <span class="number">0
    </span>}
}

<span class="kw">impl</span>&lt;I&gt; Iterable <span class="kw">for </span>I
<span class="kw">where
    </span>I: IntoIterator + Copy + Send + Sync,
    I::IntoIter: ExactSizeIterator,
{
    <span class="kw">type </span>Item = &lt;I <span class="kw">as </span>IntoIterator&gt;::Item;
    <span class="kw">type </span>Iter = &lt;I <span class="kw">as </span>IntoIterator&gt;::IntoIter;

    <span class="kw">fn </span>iter(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="self">Self</span>::Iter {
        <span class="self">self</span>.into_iter()
    }

    <span class="kw">fn </span>len(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; usize {
        <span class="self">self</span>.into_iter().len()
    }
}
