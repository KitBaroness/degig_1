<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="generator" content="rustdoc"><meta name="description" content="Source of the Rust file `/Users/jeyakumar/.cargo/registry/src/index.crates.io-6f17d22bba15001f/ed25519-1.5.3/src/lib.rs`."><title>lib.rs - source</title><script>if(window.location.protocol!=="file:")document.head.insertAdjacentHTML("beforeend","SourceSerif4-Regular-46f98efaafac5295.ttf.woff2,FiraSans-Regular-018c141bf0843ffd.woff2,FiraSans-Medium-8f9a781e4970d388.woff2,SourceCodePro-Regular-562dcc5011b6de7d.ttf.woff2,SourceCodePro-Semibold-d899c5a5c4aeb14a.ttf.woff2".split(",").map(f=>`<link rel="preload" as="font" type="font/woff2" crossorigin href="../../static.files/${f}">`).join(""))</script><link rel="stylesheet" href="../../static.files/normalize-76eba96aa4d2e634.css"><link rel="stylesheet" href="../../static.files/rustdoc-081576b923113409.css"><meta name="rustdoc-vars" data-root-path="../../" data-static-root-path="../../static.files/" data-current-crate="ed25519" data-themes="" data-resource-suffix="" data-rustdoc-version="1.79.0 (129f3b996 2024-06-10)" data-channel="1.79.0" data-search-js="search-bf21c90c8c1d92b1.js" data-settings-js="settings-4313503d2e1961c2.js" ><script src="../../static.files/storage-e32f0c247825364d.js"></script><script defer src="../../static.files/src-script-e66d777a5a92e9b2.js"></script><script defer src="../../src-files.js"></script><script defer src="../../static.files/main-20a3ad099b048cf2.js"></script><noscript><link rel="stylesheet" href="../../static.files/noscript-09095024cf37855e.css"></noscript><link rel="alternate icon" type="image/png" href="../../static.files/favicon-32x32-422f7d1d52889060.png"><link rel="icon" type="image/svg+xml" href="../../static.files/favicon-2c020d218678b618.svg"></head><body class="rustdoc src"><!--[if lte IE 11]><div class="warning">This old browser is unsupported and will most likely display funky things.</div><![endif]--><nav class="sidebar"><div class="src-sidebar-title"><h2>Files</h2></div></nav><div class="sidebar-resizer"></div><main><nav class="sub"><form class="search-form"><span></span><div id="sidebar-button" tabindex="-1"><a href="../../ed25519/all.html" title="show sidebar"></a></div><input class="search-input" name="search" aria-label="Run search in the documentation" autocomplete="off" spellcheck="false" placeholder="Type ‘S’ or ‘/’ to search, ‘?’ for more options…" type="search"><div id="help-button" tabindex="-1"><a href="../../help.html" title="help">?</a></div><div id="settings-menu" tabindex="-1"><a href="../../settings.html" title="settings">Settings</a></div></form></nav><section id="main-content" class="content"><div class="example-wrap"><div data-nosnippet><pre class="src-line-numbers"><a href="#1" id="1">1</a>
<a href="#2" id="2">2</a>
<a href="#3" id="3">3</a>
<a href="#4" id="4">4</a>
<a href="#5" id="5">5</a>
...
</span><span class="doccomment">//! # Using Ed25519 generically over algorithm implementations/providers
//!
...
</span><span class="kw">pub mod </span>pkcs8;

<span class="attr">#[cfg(feature = <span class="string">"serde"</span>)]
</span><span class="kw">mod </span>serde;

<span class="kw">pub use </span>signature::{<span class="self">self</span>, Error};

<span class="attr">#[cfg(feature = <span class="string">"pkcs8"</span>)]
</span><span class="kw">pub use </span><span class="kw">crate</span>::pkcs8::KeypairBytes;
...
<span class="doccomment">/// Length of an Ed25519 signature in bytes.
</span><span class="attr">#[deprecated(since = <span class="string">"1.3.0"</span>, note = <span class="string">"use ed25519::Signature::BYTE_SIZE instead"</span>)]
</span><span class="kw">pub const </span>SIGNATURE_LENGTH: usize = Signature::BYTE_SIZE;

<span class="doccomment">/// Ed25519 signature.
</span><span class="attr">#[derive(Copy, Clone, Eq, PartialEq)]
</span><span class="kw">pub struct </span>Signature([u8; Signature::BYTE_SIZE]);

<span class="kw">impl </span>Signature {
    <span class="doccomment">/// Size of an encoded Ed25519 signature in bytes.
    </span><span class="kw">pub const </span>BYTE_SIZE: usize = <span class="number">64</span>;

    <span class="doccomment">/// Parse an Ed25519 signature from a byte slice.
    </span><span class="kw">pub fn </span>from_bytes(bytes: <span class="kw-2">&amp;</span>[u8]) -&gt; signature::Result&lt;<span class="self">Self</span>&gt; {
        <span class="kw">let </span>result = bytes.try_into().map(<span class="self">Self</span>).map_err(|<span class="kw">_</span>| Error::new())<span class="question-mark">?</span>;

        <span class="comment">// Perform a partial reduction check on the signature's `s` scalar.
        ...
        </span><span class="kw">if </span>result.<span class="number">0</span>[Signature::BYTE_SIZE - <span class="number">1</span>] &amp; <span class="number">0b1110_0000 </span>!= <span class="number">0 </span>{
    ...
    <span class="doccomment">/// Return the inner byte array.
    </span><span class="kw">pub fn </span>to_bytes(<span class="self">self</span>) -&gt; [u8; <span class="self">Self</span>::BYTE_SIZE] {
        <span class="self">self</span>.<span class="number">0
    </span>}

    <span class="doccomment">/// Convert this signature into a byte vector.
    </span><span class="attr">#[cfg(feature = <span class="string">"alloc"</span>)]
    ...
    <span class="doccomment">/// DEPRECATED: Create a new signature from a byte array.
    ///
    /// # Panics
    ///
    ...
    </span><span class="attr">#[deprecated(since = <span class="string">"1.3.0"</span>, note = <span class="string">"use ed25519::Signature::from_bytes instead"</span>)]
    </span><span class="kw">pub fn </span>new(bytes: [u8; <span class="self">Self</span>::BYTE_SIZE]) -&gt; <span class="self">Self </span>{
        <span class="self">Self</span>::from_bytes(<span class="kw-2">&amp;</span>bytes[..]).expect(<span class="string">"invalid signature"</span>)
    }
}

<span class="kw">impl </span>signature::Signature <span class="kw">for </span>Signature {
    <span class="kw">fn </span>from_bytes(bytes: <span class="kw-2">&amp;</span>[u8]) -&gt; signature::Result&lt;<span class="self">Self</span>&gt; {
...
<span class="kw">impl </span>From&lt;Signature&gt; <span class="kw">for </span>[u8; Signature::BYTE_SIZE] {
    <span class="kw">fn </span>from(sig: Signature) -&gt; [u8; Signature::BYTE_SIZE] {
        sig.<span class="number">0
    </span>}
}

<span class="kw">impl </span>From&lt;<span class="kw-2">&amp;</span>Signature&gt; <span class="kw">for </span>[u8; Signature::BYTE_SIZE] {
    <span class="kw">fn </span>from(sig: <span class="kw-2">&amp;</span>Signature) -&gt; [u8; Signature::BYTE_SIZE] {
        sig.<span class="number">0
    </span>}
}

<span class="doccomment">/// DEPRECATED: use `TryFrom&lt;&amp;[u8]&gt;` instead.
///
/// # Warning
///
/// This conversion will panic if a signature is invalid.
</span><span class="comment">// TODO(tarcieri): remove this in the next breaking release
</span><span class="kw">impl </span>From&lt;[u8; Signature::BYTE_SIZE]&gt; <span class="kw">for </span>Signature {
    <span class="kw">fn </span>from(bytes: [u8; Signature::BYTE_SIZE]) -&gt; Signature {
        <span class="attr">#[allow(deprecated)]
        </span>Signature::new(bytes)
    }
}

<span class="kw">impl </span>TryFrom&lt;<span class="kw-2">&amp;</span>[u8]&gt; <span class="kw">for </span>Signature {
    <span class="kw">type </span>Error = Error;

    <span class="kw">fn </span>try_from(bytes: <span class="kw-2">&amp;</span>[u8]) -&gt; signature::Result&lt;<span class="self">Self</span>&gt; {
        <span class="self">Self</span>::from_bytes(bytes)
    }
}
...
<span class="kw">impl </span>fmt::Debug <span class="kw">for </span>Signature {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="macro">write!</span>(f, <span class="string">"ed25519::Signature({})"</span>, <span class="self">self</span>)
    }
}
