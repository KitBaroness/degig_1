...
<span class="doccomment">/// Decode the last 1-8 bytes, checking for trailing set bits and padding per the provided
/// parameters.
///
...
/// indicated as already written by `output_index`.
    ...
    <span class="comment">// Decode any leftovers that aren't a complete input block of 8 bytes.
    ...
    <span class="kw">for </span>(i, <span class="kw-2">&amp;</span>b) <span class="kw">in </span>input[start_of_leftovers..].iter().enumerate() {
        <span class="comment">// '=' padding
            ...
            //     may be treated as an error condition.
                ...
                <span class="kw">return </span><span class="prelude-val">Err</span>(DecodeError::InvalidByte(bad_padding_index, b));
            }
        ...
        <span class="kw">let </span>morsel = decode_table[b <span class="kw">as </span>usize];
        <span class="kw">if </span>morsel == INVALID_VALUE {
            <span class="kw">return </span><span class="prelude-val">Err</span>(DecodeError::InvalidByte(start_of_leftovers + i, b));
        }
    ...
    <span class="kw">match </span>padding_mode {
        ...
        DecodePaddingMode::RequireCanonical =&gt; {
            <span class="kw">if </span>(padding_bytes + morsels_in_leftover) % <span class="number">4 </span>!= <span class="number">0 </span>{
                <span class="kw">return </span><span class="prelude-val">Err</span>(DecodeError::InvalidPadding);
            }
        }
        DecodePaddingMode::RequireNone =&gt; {
            <span class="kw">if </span>padding_bytes &gt; <span class="number">0 </span>{
                ...
                // get hit
                </span><span class="kw">return </span><span class="prelude-val">Err</span>(DecodeError::InvalidPadding);
            }
        }
    }
    ...
    // to finish the second original byte.
    ...
    </span><span class="kw">let </span>leftover_bits_ready_to_append = <span class="kw">match </span>morsels_in_leftover {
        ...
        <span class="comment">// can also be detected as case #2 bad padding above
    ...
    <span class="kw">if </span>!decode_allow_trailing_bits &amp;&amp; (leftover_bits &amp; mask) != <span class="number">0 </span>{
    ...
    <span class="comment">// TODO benchmark simply converting to big endian bytes
    </span><span class="kw">let </span><span class="kw-2">mut </span>leftover_bits_appended_to_buf = <span class="number">0</span>;
    ...
    <span class="prelude-val">Ok</span>(DecodeMetadata::new(
        output_index,
        <span class="kw">if </span>padding_bytes &gt; <span class="number">0 </span>{
            <span class="prelude-val">Some</span>(input_index + first_padding_index)
        } <span class="kw">else </span>{
