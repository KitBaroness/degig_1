...
<span class="kw">use </span><span class="kw">crate</span>::anchors::{OwnedTrustAnchor, RootCertStore};
<span class="kw">use </span><span class="kw">crate</span>::client::ServerName;
...
<span class="kw">use </span><span class="kw">crate</span>::key::{Certificate, ParsedCertificate};
...
<span class="kw">use </span><span class="kw">crate</span>::msgs::base::PayloadU16;
<span class="kw">use </span><span class="kw">crate</span>::msgs::codec::{Codec, Reader};
<span class="kw">use </span><span class="kw">crate</span>::msgs::handshake::DistinguishedName;

<span class="kw">use </span>ring::digest::Digest;
...
<span class="comment">// Marker types.  These are used to bind the fact some verification
...
</span><span class="doccomment">/// Zero-sized marker type representing verification of a signature.
...
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">struct </span>FinishedMessageVerified(());

<span class="kw">impl </span>FinishedMessageVerified {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>assertion() -&gt; <span class="self">Self </span>{
        <span class="self">Self</span>(())
    }
}

<span class="doccomment">/// Zero-sized marker type representing verification of a server cert chain.
</span><span class="attr">#[allow(unreachable_pub)]
#[derive(Debug)]
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"dangerous_configuration"</span>)))]
</span><span class="kw">pub struct </span>ServerCertVerified(());

<span class="attr">#[allow(unreachable_pub)]
</span><span class="kw">impl </span>ServerCertVerified {
...
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"dangerous_configuration"</span>)))]
    ...
    <span class="doccomment">/// Verify the end-entity certificate `end_entity` is valid for the
    ...
    /// [Certificate]: https://datatracker.ietf.org/doc/html/rfc8446#section-4.4.2
        ...
        end_entity: <span class="kw-2">&amp;</span>Certificate,
        intermediates: <span class="kw-2">&amp;</span>[Certificate],
        server_name: <span class="kw-2">&amp;</span>ServerName,
        scts: <span class="kw-2">&amp;mut </span><span class="kw">dyn </span>Iterator&lt;Item = <span class="kw-2">&amp;</span>[u8]&gt;,
        ocsp_response: <span class="kw-2">&amp;</span>[u8],
        now: SystemTime,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;ServerCertVerified, Error&gt;;
    ...
    ) -&gt; <span class="prelude-ty">Result</span>&lt;HandshakeSignatureValid, Error&gt; {
    ...
    <span class="doccomment">/// Returns `true` if Rustls should ask the server to send SCTs.
    ///
    /// Signed Certificate Timestamps (SCTs) are used for Certificate
    /// Transparency validation.
    ///
    /// The default implementation of this function returns true.
    </span><span class="kw">fn </span>request_scts(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="bool-val">true
    </span>}
}

<span class="kw">impl </span>fmt::Debug <span class="kw">for dyn </span>ServerCertVerifier {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="macro">write!</span>(f, <span class="string">"dyn ServerCertVerifier"</span>)
    }
}
...
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"dangerous_configuration"</span>)))]
    ...
    <span class="doccomment">/// Return `true` to require a client certificate and `false` to make
    /// client authentication optional.
    /// Defaults to `Some(self.offer_client_auth())`.
    </span><span class="kw">fn </span>client_auth_mandatory(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="self">self</span>.offer_client_auth()
    }

    <span class="doccomment">/// Returns the [Subjects] of the client authentication trust anchors to
    ...
    </span><span class="kw">fn </span>client_auth_root_subjects(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>[DistinguishedName];

    <span class="doccomment">/// Verify the end-entity certificate `end_entity` is valid, acceptable,
    ...
    /// [BadEncoding]: CertificateError#variant.BadEncoding
        ...
        intermediates: <span class="kw-2">&amp;</span>[Certificate],
        now: SystemTime,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;ClientCertVerified, Error&gt;;

    <span class="doccomment">/// Verify a signature allegedly by the given client certificate.
    ///
    /// `message` is not hashed, and needs hashing during the verification.
    /// The signature and algorithm are within `dss`.  `cert` contains the
    /// public key to use.
...
<span class="kw">impl </span>fmt::Debug <span class="kw">for dyn </span>ClientCertVerifier {
    <span class="kw">fn </span>fmt(<span class="kw-2">&amp;</span><span class="self">self</span>, f: <span class="kw-2">&amp;mut </span>fmt::Formatter&lt;<span class="lifetime">'_</span>&gt;) -&gt; fmt::Result {
        <span class="macro">write!</span>(f, <span class="string">"dyn ClientCertVerifier"</span>)
    }
}

<span class="doccomment">/// Verify that the end-entity certificate `end_entity` is a valid server cert
...
#[cfg_attr(not(feature = <span class="string">"dangerous_configuration"</span>), allow(unreachable_pub))]
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"dangerous_configuration"</span>)))]
</span><span class="kw">pub fn </span>verify_server_cert_signed_by_trust_anchor(
    cert: <span class="kw-2">&amp;</span>ParsedCertificate,
    roots: <span class="kw-2">&amp;</span>RootCertStore,
    intermediates: <span class="kw-2">&amp;</span>[Certificate],
    now: SystemTime,
) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt; {
    <span class="kw">let </span>chain = intermediate_chain(intermediates);
    <span class="kw">let </span>trust_roots = trust_roots(roots);
    <span class="kw">let </span>webpki_now = webpki::Time::try_from(now).map_err(|<span class="kw">_</span>| Error::FailedToGetCurrentTime)<span class="question-mark">?</span>;

    cert.<span class="number">0
        </span>.verify_for_usage(
            SUPPORTED_SIG_ALGS,
            <span class="kw-2">&amp;</span>trust_roots,
            <span class="kw-2">&amp;</span>chain,
            webpki_now,
            webpki::KeyUsage::server_auth(),
            <span class="kw-2">&amp;</span>[], <span class="comment">// no CRLs
        </span>)
        .map_err(pki_error)
        .map(|<span class="kw">_</span>| ())
}

<span class="doccomment">/// Verify that the `end_entity` has a name or alternative name matching the `server_name`
...
</span><span class="attr">#[cfg_attr(not(feature = <span class="string">"dangerous_configuration"</span>), allow(unreachable_pub))]
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"dangerous_configuration"</span>)))]
</span><span class="kw">pub fn </span>verify_server_name(cert: <span class="kw-2">&amp;</span>ParsedCertificate, server_name: <span class="kw-2">&amp;</span>ServerName) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt; {
    <span class="kw">match </span>server_name {
        ServerName::DnsName(dns_name) =&gt; {
            <span class="comment">// unlikely error because dns_name::DnsNameRef and webpki::DnsNameRef
            // should have the same encoding rules.
            </span><span class="kw">let </span>dns_name = webpki::DnsNameRef::try_from_ascii_str(dns_name.as_ref())
                .map_err(|<span class="kw">_</span>| Error::InvalidCertificate(CertificateError::BadEncoding))<span class="question-mark">?</span>;
            <span class="kw">let </span>name = webpki::SubjectNameRef::DnsName(dns_name);
            cert.<span class="number">0
                </span>.verify_is_valid_for_subject_name(name)
                .map_err(pki_error)<span class="question-mark">?</span>;
        }
        ServerName::IpAddress(ip_addr) =&gt; {
            <span class="kw">let </span>ip_addr = webpki::IpAddr::from(<span class="kw-2">*</span>ip_addr);
            cert.<span class="number">0
                </span>.verify_is_valid_for_subject_name(webpki::SubjectNameRef::IpAddress(
                    webpki::IpAddrRef::from(<span class="kw-2">&amp;</span>ip_addr),
                ))
                .map_err(pki_error)<span class="question-mark">?</span>;
        }
    }
...
<span class="kw">impl </span>ServerCertVerifier <span class="kw">for </span>WebPkiVerifier {
    ...
    /// - Valid for DNS entry
        ...
        <span class="kw">let </span>cert = ParsedCertificate::try_from(end_entity)<span class="question-mark">?</span>;

        verify_server_cert_signed_by_trust_anchor(<span class="kw-2">&amp;</span>cert, <span class="kw-2">&amp;</span><span class="self">self</span>.roots, intermediates, now)<span class="question-mark">?</span>;

        <span class="kw">if let </span><span class="prelude-val">Some</span>(policy) = <span class="kw-2">&amp;</span><span class="self">self</span>.ct_policy {
            policy.verify(end_entity, now, scts)<span class="question-mark">?</span>;
        }
        ...
        verify_server_name(<span class="kw-2">&amp;</span>cert, server_name)<span class="question-mark">?</span>;
...
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"dangerous_configuration"</span>)))]
    ...
    ct_policy: <span class="prelude-ty">Option</span>&lt;CertificateTransparencyPolicy&gt;,
}

<span class="attr">#[allow(unreachable_pub)]
</span><span class="kw">impl </span>WebPkiVerifier {
    <span class="doccomment">/// Constructs a new `WebPkiVerifier`.
    ///
    ...
    /// &lt;https://github.com/rustls/rustls/issues/479&gt;.
        ...
        roots: <span class="kw">impl </span>Into&lt;Arc&lt;RootCertStore&gt;&gt;,
        ct_policy: <span class="prelude-ty">Option</span>&lt;CertificateTransparencyPolicy&gt;,
    ) -&gt; <span class="self">Self </span>{
        <span class="self">Self </span>{
            roots: roots.into(),
...
<span class="doccomment">/// Policy for enforcing Certificate Transparency.
///
...
#[cfg_attr(docsrs, doc(cfg(feature = <span class="string">"dangerous_configuration"</span>)))]
</span><span class="kw">pub struct </span>CertificateTransparencyPolicy {
    logs: <span class="kw-2">&amp;</span><span class="lifetime">'static </span>[<span class="kw-2">&amp;</span><span class="lifetime">'static </span>sct::Log&lt;<span class="lifetime">'static</span>&gt;],
    validation_deadline: SystemTime,
}

<span class="kw">impl </span>CertificateTransparencyPolicy {
    <span class="doccomment">/// Create a new policy.
    </span><span class="attr">#[allow(unreachable_pub)]
    </span><span class="kw">pub fn </span>new(
        logs: <span class="kw-2">&amp;</span><span class="lifetime">'static </span>[<span class="kw-2">&amp;</span><span class="lifetime">'static </span>sct::Log&lt;<span class="lifetime">'static</span>&gt;],
        validation_deadline: SystemTime,
    ...
    <span class="kw">fn </span>verify(
        ...
        <span class="kw">if </span><span class="self">self</span>.logs.is_empty() {
            <span class="kw">return </span><span class="prelude-val">Ok</span>(());
        } <span class="kw">else if </span><span class="self">self
            </span>.validation_deadline
            .duration_since(now)
            .is_err()
        {
            <span class="macro">warn!</span>(<span class="string">"certificate transparency logs have expired, validation disabled"</span>);
            <span class="kw">return </span><span class="prelude-val">Ok</span>(());
        }

        <span class="kw">let </span>now = unix_time_millis(now)<span class="question-mark">?</span>;
        ...
        <span class="kw">for </span>sct <span class="kw">in </span>scts {
            ...
            </span><span class="kw">match </span>sct::verify_sct(<span class="kw-2">&amp;</span>cert.<span class="number">0</span>, sct, now, <span class="self">self</span>.logs) {
                <span class="prelude-val">Ok</span>(index) =&gt; {
                    <span class="macro">debug!</span>(
                        <span class="string">"Valid SCT signed by {} on {}"</span>,
                        <span class="self">self</span>.logs[index].operated_by, <span class="self">self</span>.logs[index].description
                    );
                    <span class="kw">return </span><span class="prelude-val">Ok</span>(());
                }
                <span class="prelude-val">Err</span>(e) =&gt; {
                    <span class="kw">if </span>e.should_be_fatal() {
                        <span class="kw">return </span><span class="prelude-val">Err</span>(Error::InvalidSct(e));
                    }
        ...
        </span><span class="kw">if let </span><span class="prelude-val">Some</span>(last_sct_error) = last_sct_error {
            <span class="macro">warn!</span>(<span class="string">"No valid SCTs provided"</span>);
            <span class="kw">return </span><span class="prelude-val">Err</span>(Error::InvalidSct(last_sct_error));
        }
...
<span class="kw">fn </span>intermediate_chain(intermediates: <span class="kw-2">&amp;</span>[Certificate]) -&gt; Vec&lt;<span class="kw-2">&amp;</span>[u8]&gt; {
    intermediates
        .iter()
...
<span class="kw">fn </span>trust_roots(roots: <span class="kw-2">&amp;</span>RootCertStore) -&gt; Vec&lt;webpki::TrustAnchor&gt; {
    roots
        .roots
        .iter()
...
<span class="doccomment">/// An unparsed DER encoded Certificate Revocation List (CRL).
</span><span class="kw">pub struct </span>UnparsedCertRevocationList(<span class="kw">pub </span>Vec&lt;u8&gt;);

<span class="kw">impl </span>UnparsedCertRevocationList {
    <span class="doccomment">/// Parse the CRL DER, yielding a [`webpki::CertRevocationList`] or an error if the CRL
    /// is malformed, or uses unsupported features.
    </span><span class="kw">pub fn </span>parse(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="prelude-ty">Result</span>&lt;webpki::OwnedCertRevocationList, CertRevocationListError&gt; {
        webpki::BorrowedCertRevocationList::from_der(<span class="kw-2">&amp;</span><span class="self">self</span>.<span class="number">0</span>)
...
<span class="kw">impl </span>AllowAnyAuthenticatedClient {
    ...
    </span><span class="kw">pub fn </span>new(roots: RootCertStore) -&gt; <span class="self">Self </span>{
        <span class="self">Self </span>{
            subjects: roots
                .roots
                .iter()
    ...
    <span class="doccomment">/// Update the verifier to validate client certificates against the provided DER format
    /// unparsed certificate revocation lists (CRLs).
    </span><span class="kw">pub fn </span>with_crls(
        <span class="self">self</span>,
        crls: <span class="kw">impl </span>IntoIterator&lt;Item = UnparsedCertRevocationList&gt;,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, CertRevocationListError&gt; {
        <span class="prelude-val">Ok</span>(<span class="self">Self </span>{
            crls: crls
                .into_iter()
                .map(|der_crl| der_crl.parse())
                .collect::&lt;<span class="prelude-ty">Result</span>&lt;Vec&lt;<span class="kw">_</span>&gt;, CertRevocationListError&gt;&gt;()<span class="question-mark">?</span>,
            ..<span class="self">self
        </span>})
    }

    <span class="doccomment">/// Wrap this verifier in an [`Arc`] and coerce it to `dyn ClientCertVerifier`
    </span><span class="attr">#[inline(always)]
    </span><span class="kw">pub fn </span>boxed(<span class="self">self</span>) -&gt; Arc&lt;<span class="kw">dyn </span>ClientCertVerifier&gt; {
        <span class="comment">// This function is needed because `ClientCertVerifier` is only reachable if the
        // `dangerous_configuration` feature is enabled, which makes coercing hard to outside users
        </span>Arc::new(<span class="self">self</span>)
    }
}
...
<span class="kw">impl </span>ClientCertVerifier <span class="kw">for </span>AllowAnyAuthenticatedClient {
    ...
    <span class="kw">fn </span>client_auth_root_subjects(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>[DistinguishedName] {
        <span class="kw-2">&amp;</span><span class="self">self</span>.subjects
    }
    ...
    <span class="kw">fn </span>verify_client_cert(
        ...
        <span class="kw">let </span>cert = ParsedCertificate::try_from(end_entity)<span class="question-mark">?</span>;
        <span class="kw">let </span>chain = intermediate_chain(intermediates);
        <span class="kw">let </span>trust_roots = trust_roots(<span class="kw-2">&amp;</span><span class="self">self</span>.roots);
        <span class="kw">let </span>now = webpki::Time::try_from(now).map_err(|<span class="kw">_</span>| Error::FailedToGetCurrentTime)<span class="question-mark">?</span>;

        <span class="attr">#[allow(trivial_casts)] </span><span class="comment">// Cast to &amp;dyn trait is required.
        </span><span class="kw">let </span>crls = <span class="self">self
            </span>.crls
            .iter()
            .map(|crl| crl <span class="kw">as </span><span class="kw-2">&amp;</span><span class="kw">dyn </span>webpki::CertRevocationList)
...
<span class="kw">impl </span>AllowAnyAnonymousOrAuthenticatedClient {
    ...
    /// unparsed certificate revocation lists (CRLs).
        ...
        <span class="prelude-val">Ok</span>(<span class="self">Self </span>{
            inner: <span class="self">self</span>.inner.with_crls(crls)<span class="question-mark">?</span>,
        })
    }
...
<span class="kw">impl </span>ClientCertVerifier <span class="kw">for </span>AllowAnyAnonymousOrAuthenticatedClient {
    ...
    <span class="kw">fn </span>client_auth_mandatory(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        <span class="bool-val">false
    </span>}

    <span class="kw">fn </span>client_auth_root_subjects(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>[DistinguishedName] {
        <span class="self">self</span>.inner.client_auth_root_subjects()
    }
...
<span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>pki_error(error: webpki::Error) -&gt; Error {
    <span class="kw">use </span>webpki::Error::<span class="kw-2">*</span>;
    <span class="kw">match </span>error {
        ...
        <span class="kw">_ </span>=&gt; CertificateError::Other(Arc::new(error)).into(),
    }
}
...
<span class="kw">impl </span>NoClientAuth {
    <span class="doccomment">/// Construct a [`NoClientAuth`], wrap it in an [`Arc`] and coerce it to
    /// `dyn ClientCertVerifier`.
    </span><span class="attr">#[inline(always)]
    </span><span class="kw">pub fn </span>boxed() -&gt; Arc&lt;<span class="kw">dyn </span>ClientCertVerifier&gt; {
        <span class="comment">// This function is needed because `ClientCertVerifier` is only reachable if the
        // `dangerous_configuration` feature is enabled, which makes coercing hard to outside users
        </span>Arc::new(<span class="self">Self</span>)
    }
...
<span class="kw">impl </span>ClientCertVerifier <span class="kw">for </span>NoClientAuth {
    ...
    <span class="kw">fn </span>verify_client_cert(
        <span class="kw-2">&amp;</span><span class="self">self</span>,
        _end_entity: <span class="kw-2">&amp;</span>Certificate,
        _intermediates: <span class="kw-2">&amp;</span>[Certificate],
        _now: SystemTime,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;ClientCertVerified, Error&gt; {
        <span class="macro">unimplemented!</span>();
    }
...
<span class="doccomment">/// This type combines a [`SignatureScheme`] and a signature payload produced with that scheme.
</span><span class="attr">#[derive(Debug, Clone)]
</span><span class="kw">pub struct </span>DigitallySignedStruct {
    <span class="doccomment">/// The [`SignatureScheme`] used to produce the signature.
    </span><span class="kw">pub </span>scheme: SignatureScheme,
    sig: PayloadU16,
}

<span class="kw">impl </span>DigitallySignedStruct {
    <span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>new(scheme: SignatureScheme, sig: Vec&lt;u8&gt;) -&gt; <span class="self">Self </span>{
        <span class="self">Self </span>{
            scheme,
    ...
    <span class="doccomment">/// Get the signature.
    </span><span class="kw">pub fn </span>signature(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>[u8] {
        <span class="kw-2">&amp;</span><span class="self">self</span>.sig.<span class="number">0
    </span>}
}

<span class="kw">impl </span>Codec <span class="kw">for </span>DigitallySignedStruct {
    <span class="kw">fn </span>encode(<span class="kw-2">&amp;</span><span class="self">self</span>, bytes: <span class="kw-2">&amp;mut </span>Vec&lt;u8&gt;) {
        <span class="self">self</span>.scheme.encode(bytes);
        <span class="self">self</span>.sig.encode(bytes);
    }
    ...
    <span class="kw">fn </span>read(r: <span class="kw-2">&amp;mut </span>Reader) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, InvalidMessage&gt; {
        <span class="kw">let </span>scheme = SignatureScheme::read(r)<span class="question-mark">?</span>;
        <span class="kw">let </span>sig = PayloadU16::read(r)<span class="question-mark">?</span>;

        <span class="prelude-val">Ok</span>(<span class="self">Self </span>{ scheme, sig })
    }
}
...
<span class="kw">fn </span>convert_scheme(scheme: SignatureScheme) -&gt; <span class="prelude-ty">Result</span>&lt;SignatureAlgorithms, Error&gt; {
    <span class="kw">match </span>scheme {
        ...
        <span class="kw">_ </span>=&gt; <span class="prelude-val">Err</span>(PeerMisbehaved::SignedHandshakeWithUnadvertisedSigScheme.into()),
    }
}
...
<span class="kw">fn </span>verify_signed_struct(
    ...
    <span class="kw">let </span>cert = webpki::EndEntityCert::try_from(cert.<span class="number">0</span>.as_ref()).map_err(pki_error)<span class="question-mark">?</span>;

    verify_sig_using_any_alg(<span class="kw-2">&amp;</span>cert, possible_algs, message, dss.signature())
        .map_err(pki_error)
...
) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="kw-2">&amp;</span><span class="lifetime">'static </span>webpki::SignatureAlgorithm, Error&gt; {
    <span class="kw">use </span><span class="kw">crate</span>::enums::SignatureScheme::<span class="kw-2">*</span>;
...
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>construct_tls13_client_verify_message(handshake_hash: <span class="kw-2">&amp;</span>Digest) -&gt; Vec&lt;u8&gt; {
    construct_tls13_verify_message(handshake_hash, <span class="string">b"TLS 1.3, client CertificateVerify\x00"</span>)
}

<span class="doccomment">/// Constructs the signature message specified in section 4.4.3 of RFC8446.
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>construct_tls13_server_verify_message(handshake_hash: <span class="kw-2">&amp;</span>Digest) -&gt; Vec&lt;u8&gt; {
    construct_tls13_verify_message(handshake_hash, <span class="string">b"TLS 1.3, server CertificateVerify\x00"</span>)
}

<span class="kw">fn </span>construct_tls13_verify_message(
    handshake_hash: <span class="kw-2">&amp;</span>Digest,
    context_string_with_0: <span class="kw-2">&amp;</span>[u8],
) -&gt; Vec&lt;u8&gt; {
...
<span class="kw">fn </span>unix_time_millis(now: SystemTime) -&gt; <span class="prelude-ty">Result</span>&lt;u64, Error&gt; {
    now.duration_since(std::time::UNIX_EPOCH)
        .map(|dur| dur.as_secs())
        .map_err(|<span class="kw">_</span>| Error::FailedToGetCurrentTime)
        .and_then(|secs| {
...
<span class="attr">#[cfg(test)]
    ...
    </span><span class="kw">fn </span>assertions_are_debug() {
        <span class="macro">assert_eq!</span>(
            <span class="macro">format!</span>(<span class="string">"{:?}"</span>, ClientCertVerified::assertion()),
            <span class="string">"ClientCertVerified(())"
        </span>);
        <span class="macro">assert_eq!</span>(
            <span class="macro">format!</span>(<span class="string">"{:?}"</span>, HandshakeSignatureValid::assertion()),
            <span class="string">"HandshakeSignatureValid(())"
        </span>);
        <span class="macro">assert_eq!</span>(
            <span class="macro">format!</span>(<span class="string">"{:?}"</span>, FinishedMessageVerified::assertion()),
            <span class="string">"FinishedMessageVerified(())"
        </span>);
        <span class="macro">assert_eq!</span>(
            <span class="macro">format!</span>(<span class="string">"{:?}"</span>, ServerCertVerified::assertion()),
            <span class="string">"ServerCertVerified(())"
        </span>);
    }

    <span class="attr">#[test]
    </span><span class="kw">fn </span>pki_crl_errors() {
        <span class="comment">// CRL signature errors should be turned into BadSignature.
        ...
        <span class="comment">// Revoked cert errors should be turned into Revoked.
        </span><span class="macro">assert_eq!</span>(
            pki_error(webpki::Error::CertRevoked),
            Error::InvalidCertificate(CertificateError::Revoked),
        );

        <span class="comment">// Issuer not CRL signer errors should be turned into IssuerInvalidForCrl
        </span><span class="macro">assert_eq!</span>(
