...
<span class="kw">use crate</span>::{key, DistinguishedName};
<span class="kw">use crate</span>::{CertificateError, Error};
...
<span class="kw">impl </span>OwnedTrustAnchor {
    <span class="doccomment">/// Get a `webpki::TrustAnchor` by borrowing the owned elements.
        ...
        webpki::TrustAnchor {
            subject: <span class="kw-2">&amp;</span><span class="self">self</span>.subject_dn.as_ref()[<span class="self">self</span>.subject_dn_header_len..],
    ...
    /// [Name Constraints]: https://datatracker.ietf.org/doc/html/rfc5280#section-4.2.1.10
        ...
        <span class="kw">let </span>(subject_dn, subject_dn_header_len) = {
            <span class="kw">let </span><span class="kw-2">mut </span>subject = subject.into();
            <span class="kw">let </span>before_len = subject.len();
            x509::wrap_in_sequence(<span class="kw-2">&amp;mut </span>subject);
            <span class="kw">let </span>header_len = subject.len().saturating_sub(before_len);
    ...
    <span class="doccomment">/// Return the subject field including its outer SEQUENCE encoding.
    ///
    ...
    /// ```
    </span><span class="kw">pub fn </span>subject(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">&amp;</span>DistinguishedName {
        <span class="kw-2">&amp;</span><span class="self">self</span>.subject_dn
    }
}
...
<span class="kw">impl </span>RootCertStore {
    ...
    </span><span class="kw">pub fn </span>add(<span class="kw-2">&amp;mut </span><span class="self">self</span>, der: <span class="kw-2">&amp;</span>key::Certificate) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt; {
        <span class="self">self</span>.add_internal(<span class="kw-2">&amp;</span>der.<span class="number">0</span>)
    }

    <span class="doccomment">/// Adds all the given TrustAnchors `anchors`.  This does not
    /// fail.
    </span><span class="kw">pub fn </span>add_trust_anchors(<span class="kw-2">&amp;mut </span><span class="self">self</span>, trust_anchors: <span class="kw">impl </span>Iterator&lt;Item = OwnedTrustAnchor&gt;) {
        <span class="self">self</span>.roots.extend(trust_anchors);
    }

    <span class="doccomment">/// Adds all the given TrustAnchors `anchors`.  This does not
    /// fail.
    </span><span class="attr">#[deprecated(since = <span class="string">"0.21.6"</span>, note = <span class="string">"Please use `add_trust_anchors` instead"</span>)]
        ...
        <span class="self">self</span>.add_trust_anchors(trust_anchors);
    }
    ...
    /// Returns the number of certificates added, and the number that were ignored.
    </span><span class="kw">pub fn </span>add_parsable_certificates(<span class="kw-2">&amp;mut </span><span class="self">self</span>, der_certs: <span class="kw-2">&amp;</span>[<span class="kw">impl </span>AsRef&lt;[u8]&gt;]) -&gt; (usize, usize) {
        ...
        <span class="kw">for </span>der_cert <span class="kw">in </span>der_certs {
            ...
            </span><span class="kw">match </span><span class="self">self</span>.add_internal(der_cert.as_ref()) {
                <span class="prelude-val">Ok</span>(<span class="kw">_</span>) =&gt; valid_count += <span class="number">1</span>,
                <span class="prelude-val">Err</span>(err) =&gt; {
                    <span class="macro">trace!</span>(<span class="string">"invalid cert der {:?}"</span>, der_cert.as_ref());
    ...
    <span class="kw">fn </span>add_internal(<span class="kw-2">&amp;mut </span><span class="self">self</span>, der: <span class="kw-2">&amp;</span>[u8]) -&gt; <span class="prelude-ty">Result</span>&lt;(), Error&gt; {
        <span class="kw">let </span>ta = webpki::TrustAnchor::try_from_cert_der(der)
...
<span class="kw">mod </span>tests {
    ...
    </span><span class="kw">fn </span>ownedtrustanchor_subject_is_correctly_encoding_dn() {
        <span class="kw">let </span>subject = <span class="string">b"subject"</span>.to_owned();
        <span class="kw">let </span>ota = <span class="kw">super</span>::OwnedTrustAnchor::from_subject_spki_name_constraints(
            subject,
            <span class="string">b""</span>.to_owned(),
            <span class="prelude-val">None</span>::&lt;Vec&lt;u8&gt;&gt;,
        );
        <span class="kw">let </span>expected_prefix = <span class="macro">vec!</span>[ring::io::der::Tag::Sequence <span class="kw">as </span>u8, subject.len() <span class="kw">as </span>u8];
