...
</pre></div><pre class="rust"><code><span class="kw">use </span><span class="kw">crate</span>::enums::ProtocolVersion;
...
<span class="kw">use </span><span class="kw">crate</span>::msgs::enums::AlertLevel;
...
<span class="kw">impl </span>MessagePayload {
    ...
    <span class="kw">pub fn </span>new(
        ...
        payload: Payload,
    ) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="self">Self</span>, InvalidMessage&gt; {
        ...
        <span class="kw">match </span>typ {
            ContentType::ApplicationData =&gt; <span class="prelude-val">Ok</span>(<span class="self">Self</span>::ApplicationData(payload)),
            ContentType::Alert =&gt; AlertMessagePayload::read(<span class="kw-2">&amp;mut </span>r).map(MessagePayload::Alert),
            ContentType::Handshake =&gt; {
                HandshakeMessagePayload::read_version(<span class="kw-2">&amp;mut </span>r, vers).map(|parsed| <span class="self">Self</span>::Handshake {
                    parsed,
                    encoded: payload,
                })
            }
            ContentType::ChangeCipherSpec =&gt; {
                ChangeCipherSpecPayload::read(<span class="kw-2">&amp;mut </span>r).map(MessagePayload::ChangeCipherSpec)
            }
            <span class="kw">_ </span>=&gt; <span class="prelude-val">Err</span>(InvalidMessage::InvalidContentType),
        }
    }
...
<span class="kw">impl </span>OpaqueMessage {
    ...
    /// become valid if we read more data) and invalid data.
        ...
        <span class="kw">let </span>typ = ContentType::read(r).map_err(|<span class="kw">_</span>| MessageError::TooShortForHeader)<span class="question-mark">?</span>;
        <span class="comment">// Don't accept any new content-types.
            ...
            <span class="kw">return </span><span class="prelude-val">Err</span>(MessageError::InvalidContentType);
        }

        <span class="kw">let </span>version = ProtocolVersion::read(r).map_err(|<span class="kw">_</span>| MessageError::TooShortForHeader)<span class="question-mark">?</span>;
        <span class="comment">// Accept only versions 0x03XX for any XX.
            ...
            ProtocolVersion::Unknown(<span class="kw-2">ref </span>v) <span class="kw">if </span>(v &amp; <span class="number">0xff00</span>) != <span class="number">0x0300 </span>=&gt; {
                <span class="kw">return </span><span class="prelude-val">Err</span>(MessageError::UnknownProtocolVersion);
            }
            <span class="kw">_ </span>=&gt; {}
        };

        <span class="kw">let </span>len = u16::read(r).map_err(|<span class="kw">_</span>| MessageError::TooShortForHeader)<span class="question-mark">?</span>;
        ...
        //              per section 6.2.1 of RFC5246 (TLSv1.2)
            ...
            <span class="kw">return </span><span class="prelude-val">Err</span>(MessageError::InvalidEmptyPayload);
        }

        <span class="comment">// Reject oversize messages
        </span><span class="kw">if </span>len &gt;= <span class="self">Self</span>::MAX_PAYLOAD {
            <span class="kw">return </span><span class="prelude-val">Err</span>(MessageError::MessageTooLarge);
        }

        <span class="kw">let </span><span class="kw-2">mut </span>sub = r
            .sub(len <span class="kw">as </span>usize)
            .map_err(|<span class="kw">_</span>| MessageError::TooShortForLength)<span class="question-mark">?</span>;
...
<span class="kw">impl</span>&lt;<span class="lifetime">'a</span>&gt; BorrowedPlainMessage&lt;<span class="lifetime">'a</span>&gt; {
    <span class="kw">pub fn </span>to_unencrypted_opaque(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; OpaqueMessage {
        OpaqueMessage {
            version: <span class="self">self</span>.version,
            typ: <span class="self">self</span>.typ,
            payload: Payload(<span class="self">self</span>.payload.to_vec()),
        }
    }
}
