...
</pre></div><pre class="rust"><code><span class="kw">use </span>bytes::{Buf, BufMut, Bytes};
...
/// ```
    ...
    deprecated(
        note = <span class="string">"This function has been replaced by a method on the `hyper::body::HttpBody` trait. Use `.collect().await?.to_bytes()` instead."
    </span>)
)]
    ...
    </span>T: HttpBody,
{
    ...
    <span class="comment">// Don't pre-emptively reserve *too* much.
    </span><span class="kw">let </span>rest = (body.size_hint().lower() <span class="kw">as </span>usize).min(<span class="number">1024 </span>* <span class="number">16</span>);
    <span class="kw">let </span>cap = first
        .remaining()
        .saturating_add(second.remaining())
        .saturating_add(rest);
    <span class="comment">// With more than 1 buf, we gotta flatten into a Vec first.
    </span><span class="kw">let </span><span class="kw-2">mut </span>vec = Vec::with_capacity(cap);
    vec.put(first);
    vec.put(second);
