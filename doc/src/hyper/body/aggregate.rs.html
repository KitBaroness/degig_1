...
<span class="doccomment">/// Aggregate the data buffers from a body asynchronously.
///
...
/// `Content-Length` is a possibility, but it is not strictly mandated to be present.
    ...
    deprecated(
        note = <span class="string">"This function has been replaced by a method on the `hyper::body::HttpBody` trait. Use `.collect().await?.aggregate()` instead."
    </span>)
)]
#[cfg_attr(feature = <span class="string">"deprecated"</span>, allow(deprecated))]
</span><span class="kw">pub async fn </span>aggregate&lt;T&gt;(body: T) -&gt; <span class="prelude-ty">Result</span>&lt;<span class="kw">impl </span>Buf, T::Error&gt;
<span class="kw">where
    </span>T: HttpBody,
{
    <span class="kw">let </span><span class="kw-2">mut </span>bufs = BufList::new();
    ...
    <span class="kw">while let </span><span class="prelude-val">Some</span>(buf) = body.data().<span class="kw">await </span>{
        <span class="kw">let </span>buf = buf<span class="question-mark">?</span>;
        <span class="kw">if </span>buf.has_remaining() {
            bufs.push(buf);
        }
    }

    <span class="prelude-val">Ok</span>(bufs)
}
