...
</span><span class="doccomment">//! ed25519 keypairs.
...
<span class="kw">pub use </span>curve25519_dalek::digest::Digest;

<span class="kw">use </span>ed25519::signature::{Signer, Verifier};
...
<span class="kw">impl </span>Keypair {
    ...
    </span><span class="kw">pub fn </span>to_bytes(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; [u8; KEYPAIR_LENGTH] {
        <span class="kw">let </span><span class="kw-2">mut </span>bytes: [u8; KEYPAIR_LENGTH] = [<span class="number">0u8</span>; KEYPAIR_LENGTH];

        bytes[..SECRET_KEY_LENGTH].copy_from_slice(<span class="self">self</span>.secret.as_bytes());
        bytes[SECRET_KEY_LENGTH..].copy_from_slice(<span class="self">self</span>.public.as_bytes());
        bytes
    }

    <span class="doccomment">/// Construct a `Keypair` from the bytes of a `PublicKey` and `SecretKey`.
    ///
    /// # Inputs
    ///
    ...
    </span><span class="kw">pub fn </span>from_bytes&lt;<span class="lifetime">'a</span>&gt;(bytes: <span class="kw-2">&amp;</span><span class="lifetime">'a </span>[u8]) -&gt; <span class="prelude-ty">Result</span>&lt;Keypair, SignatureError&gt; {
        <span class="kw">if </span>bytes.len() != KEYPAIR_LENGTH {
            <span class="kw">return </span><span class="prelude-val">Err</span>(InternalError::BytesLengthError {
                name: <span class="string">"Keypair"</span>,
                length: KEYPAIR_LENGTH,
            }.into());
        }
        <span class="kw">let </span>secret = SecretKey::from_bytes(<span class="kw-2">&amp;</span>bytes[..SECRET_KEY_LENGTH])<span class="question-mark">?</span>;
        <span class="kw">let </span>public = PublicKey::from_bytes(<span class="kw-2">&amp;</span>bytes[SECRET_KEY_LENGTH..])<span class="question-mark">?</span>;

        <span class="prelude-val">Ok</span>(Keypair{ secret: secret, public: public })
    }

    <span class="doccomment">/// Generate an ed25519 keypair.
    ///
    /// # Example
    ///
    /// ```
    ...
    </span><span class="kw">pub fn </span>generate&lt;R&gt;(csprng: <span class="kw-2">&amp;mut </span>R) -&gt; Keypair
    <span class="kw">where
        </span>R: CryptoRng + RngCore,
    {
        <span class="kw">let </span>sk: SecretKey = SecretKey::generate(csprng);
        <span class="kw">let </span>pk: PublicKey = (<span class="kw-2">&amp;</span>sk).into();

        Keypair{ public: pk, secret: sk }
    }

    <span class="doccomment">/// Sign a `prehashed_message` with this `Keypair` using the
    ...
    </span><span class="kw">pub fn </span>sign_prehashed&lt;D&gt;(
    ...
    {
        <span class="kw">let </span>expanded: ExpandedSecretKey = (<span class="kw-2">&amp;</span><span class="self">self</span>.secret).into(); <span class="comment">// xxx thanks i hate this

        </span>expanded.sign_prehashed(prehashed_message, <span class="kw-2">&amp;</span><span class="self">self</span>.public, context).into()
    }
    ...
    {
        <span class="self">self</span>.public.verify(message, signature)
    }
    ...
    {
        <span class="self">self</span>.public.verify_prehashed(prehashed_message, context, signature)
    }
    ...
    {
        <span class="self">self</span>.public.verify_strict(message, signature)
    }
}

<span class="kw">impl </span>Signer&lt;ed25519::Signature&gt; <span class="kw">for </span>Keypair {
    <span class="doccomment">/// Sign a message with this keypair's secret key.
    </span><span class="kw">fn </span>try_sign(<span class="kw-2">&amp;</span><span class="self">self</span>, message: <span class="kw-2">&amp;</span>[u8]) -&gt; <span class="prelude-ty">Result</span>&lt;ed25519::Signature, SignatureError&gt; {
        <span class="kw">let </span>expanded: ExpandedSecretKey = (<span class="kw-2">&amp;</span><span class="self">self</span>.secret).into();
        <span class="prelude-val">Ok</span>(expanded.sign(<span class="kw-2">&amp;</span>message, <span class="kw-2">&amp;</span><span class="self">self</span>.public).into())
    }
}

<span class="kw">impl </span>Verifier&lt;ed25519::Signature&gt; <span class="kw">for </span>Keypair {
    <span class="doccomment">/// Verify a signature on a message with this keypair's public key.
    </span><span class="kw">fn </span>verify(<span class="kw-2">&amp;</span><span class="self">self</span>, message: <span class="kw-2">&amp;</span>[u8], signature: <span class="kw-2">&amp;</span>ed25519::Signature) -&gt; <span class="prelude-ty">Result</span>&lt;(), SignatureError&gt; {
        <span class="self">self</span>.public.verify(message, signature)
    }
}

<span class="attr">#[cfg(feature = <span class="string">"serde"</span>)]
</span><span class="kw">impl </span>Serialize <span class="kw">for </span>Keypair {
...
<span class="attr">#[cfg(feature = <span class="string">"serde"</span>)]
</span><span class="kw">impl</span>&lt;<span class="lifetime">'d</span>&gt; Deserialize&lt;<span class="lifetime">'d</span>&gt; <span class="kw">for </span>Keypair {
