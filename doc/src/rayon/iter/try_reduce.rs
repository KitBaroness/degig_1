...
<span class="kw">pub</span>(<span class="kw">super</span>) <span class="kw">fn </span>try_reduce&lt;PI, R, ID, T&gt;(pi: PI, identity: ID, reduce_op: R) -&gt; T
<span class="kw">where
...
<span class="kw">struct </span>TryReduceConsumer&lt;<span class="lifetime">'r</span>, R, ID&gt; {
    ...
    T: Try,
{
    <span class="kw">fn </span>reduce(<span class="self">self</span>, left: T, right: T) -&gt; T {
        <span class="kw">match </span>(left.branch(), right.branch()) {
            (Continue(left), Continue(right)) =&gt; (<span class="self">self</span>.reduce_op)(left, right),
            (Break(r), <span class="kw">_</span>) | (<span class="kw">_</span>, Break(r)) =&gt; T::from_residual(r),
        }
    }
}

<span class="kw">struct </span>TryReduceFolder&lt;<span class="lifetime">'r</span>, R, T: Try&gt; {
    reduce_op: <span class="kw-2">&amp;</span><span class="lifetime">'r </span>R,
    ...
    T: Try,
{
    ...
    <span class="kw">fn </span>consume(<span class="kw-2">mut </span><span class="self">self</span>, item: T) -&gt; <span class="self">Self </span>{
        ...
        <span class="self">self</span>.control = <span class="kw">match </span>(<span class="self">self</span>.control, item.branch()) {
            (Continue(left), Continue(right)) =&gt; reduce_op(left, right).branch(),
            (control @ Break(<span class="kw">_</span>), <span class="kw">_</span>) | (<span class="kw">_</span>, control @ Break(<span class="kw">_</span>)) =&gt; control,
        };
        <span class="kw">if let </span>Break(<span class="kw">_</span>) = <span class="self">self</span>.control {
