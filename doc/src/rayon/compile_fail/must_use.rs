...
</pre></div><pre class="rust"><code><span class="comment">// Check that we are flagged for ignoring `must_use` parallel adaptors.
// (unfortunately there's no error code for `unused_must_use`)

</span><span class="macro">macro_rules!</span> must_use {
    ($( <span class="macro-nonterminal">$name</span>:ident <span class="attr">#[<span class="macro-nonterminal">$expr</span>:meta] </span>)<span class="kw-2">*</span>) =&gt; {$(
        ...
        /// let _ =
        </span><span class="attr">#[<span class="macro-nonterminal">$expr</span>]
...
<span class="macro">must_use!</span> {
    by_exponential_blocks  <span class="doccomment">/** v.par_iter().by_exponential_blocks(); */
    </span>by_uniform_blocks   <span class="doccomment">/** v.par_iter().by_uniform_blocks(2); */
    </span>step_by             <span class="doccomment">/** v.par_iter().step_by(2); */
    </span>chain               <span class="doccomment">/** v.par_iter().chain(&amp;v); */
    </span>chunks              <span class="doccomment">/** v.par_iter().chunks(2); */
    </span>fold_chunks         <span class="doccomment">/** v.par_iter().fold_chunks(2, || 0, |x, _| x); */
    </span>fold_chunks_with    <span class="doccomment">/** v.par_iter().fold_chunks_with(2, 0, |x, _| x); */
    </span>cloned              <span class="doccomment">/** v.par_iter().cloned(); */
    </span>copied              <span class="doccomment">/** v.par_iter().copied(); */
    ...
    </span>flat_map_iter       <span class="doccomment">/** v.par_iter().flat_map_iter(|x| *x); */
    </span>flatten             <span class="doccomment">/** v.par_iter().flatten(); */
    </span>flatten_iter        <span class="doccomment">/** v.par_iter().flatten_iter(); */
    ...
    </span>panic_fuse          <span class="doccomment">/** v.par_iter().panic_fuse(); */
    </span>positions           <span class="doccomment">/** v.par_iter().positions(|_| true); */
