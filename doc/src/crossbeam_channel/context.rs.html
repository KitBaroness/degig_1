...
</pre></div><pre class="rust"><code><span class="doccomment">//! Thread-local context used in select.

</span><span class="kw">use </span>std::cell::Cell;
<span class="kw">use </span>std::ptr;
<span class="kw">use </span>std::sync::atomic::{AtomicPtr, AtomicUsize, Ordering};
<span class="kw">use </span>std::sync::Arc;
<span class="kw">use </span>std::thread::{<span class="self">self</span>, Thread, ThreadId};
<span class="kw">use </span>std::time::Instant;

<span class="kw">use </span>crossbeam_utils::Backoff;

<span class="kw">use </span><span class="kw">crate</span>::select::Selected;

<span class="doccomment">/// Thread-local context used in select.
...
<span class="kw">impl </span>Context {
    ...
    <span class="doccomment">/// Returns the selected operation.
    </span><span class="attr">#[inline]
    </span><span class="kw">pub fn </span>selected(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; Selected {
        Selected::from(<span class="self">self</span>.inner.select.load(Ordering::Acquire))
    }
    ...
    <span class="doccomment">/// Waits until a packet is provided and returns it.
    </span><span class="attr">#[inline]
    </span><span class="kw">pub fn </span>wait_packet(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; <span class="kw-2">*mut </span>() {
        <span class="kw">let </span>backoff = Backoff::new();
        <span class="kw">loop </span>{
            <span class="kw">let </span>packet = <span class="self">self</span>.inner.packet.load(Ordering::Acquire);
            <span class="kw">if </span>!packet.is_null() {
    ...
    /// If the deadline is reached, `Selected::Aborted` will be selected.
        ...
        <span class="comment">// Spin for a short time, waiting until an operation is selected.
        </span><span class="kw">let </span>backoff = Backoff::new();
        <span class="kw">loop </span>{
            <span class="kw">let </span>sel = Selected::from(<span class="self">self</span>.inner.select.load(Ordering::Acquire));
    ...
    </span><span class="kw">pub fn </span>thread_id(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; ThreadId {
