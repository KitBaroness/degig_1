...
</pre></div><pre class="rust"><code><span class="doccomment">//! Bounded channel based on a preallocated array.
//!
...
<span class="kw">use </span>std::sync::atomic::{<span class="self">self</span>, AtomicUsize, Ordering};
...
<span class="doccomment">/// A slot in a channel.
</span><span class="kw">struct </span>Slot&lt;T&gt; {
    <span class="doccomment">/// The current stamp.
    </span>stamp: AtomicUsize,

    <span class="doccomment">/// The message in this slot.
    </span>msg: UnsafeCell&lt;MaybeUninit&lt;T&gt;&gt;,
}

<span class="doccomment">/// The token type for the array flavor.
</span><span class="attr">#[derive(Debug)]
</span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">struct </span>ArrayToken {
    <span class="doccomment">/// Slot to read from or write to.
    </span>slot: <span class="kw-2">*const </span>u8,

    <span class="doccomment">/// Stamp to store into the slot after reading or writing.
    </span>stamp: usize,
}

<span class="kw">impl </span>Default <span class="kw">for </span>ArrayToken {
    <span class="attr">#[inline]
    </span><span class="kw">fn </span>default() -&gt; <span class="self">Self </span>{
        ArrayToken {
            slot: ptr::null(),
            stamp: <span class="number">0</span>,
        }
    }
}
...
<span class="doccomment">/// Bounded channel based on a preallocated array.
    ...
    <span class="doccomment">/// The channel capacity.
    </span>cap: usize,

    <span class="doccomment">/// A stamp with the value of `{ lap: 1, mark: 0, index: 0 }`.
    </span>one_lap: usize,

    <span class="doccomment">/// If this bit is set in the tail, that means the channel is disconnected.
    </span>mark_bit: usize,

    <span class="doccomment">/// Senders waiting while the channel is full.
    </span>senders: SyncWaker,
...
<span class="kw">impl</span>&lt;T&gt; Channel&lt;T&gt; {
    <span class="doccomment">/// Creates a bounded channel of capacity `cap`.
    </span><span class="kw">pub</span>(<span class="kw">crate</span>) <span class="kw">fn </span>with_capacity(cap: usize) -&gt; <span class="self">Self </span>{
        ...
        <span class="comment">// Allocate a buffer of `cap` slots initialized
        // with stamps.
        </span><span class="kw">let </span>buffer: Box&lt;[Slot&lt;T&gt;]&gt; = (<span class="number">0</span>..cap)
            .map(|i| {
    ...
    <span class="doccomment">/// Attempts to reserve a slot for sending a message.
        ...
        <span class="kw">loop </span>{
            <span class="comment">// Check if the channel is disconnected.
                ...
                token.array.stamp = <span class="number">0</span>;
            ...
            </span><span class="macro">debug_assert!</span>(index &lt; <span class="self">self</span>.buffer.len());
            <span class="kw">let </span>slot = <span class="kw">unsafe </span>{ <span class="self">self</span>.buffer.get_unchecked(index) };
            ...
            <span class="comment">// If the tail and the stamp match, we may attempt to push.
                ...
                <span class="kw">let </span>new_tail = <span class="kw">if </span>index + <span class="number">1 </span>&lt; <span class="self">self</span>.cap {
                ...
                <span class="comment">// Try moving the tail.
                    ...
                    <span class="prelude-val">Ok</span>(<span class="kw">_</span>) =&gt; {
                        <span class="comment">// Prepare the token for the follow-up call to `write`.
                        </span>token.array.slot = slot <span class="kw">as </span><span class="kw-2">*const </span>Slot&lt;T&gt; <span class="kw">as </span><span class="kw-2">*const </span>u8;
                        token.array.stamp = tail + <span class="number">1</span>;
                        <span class="kw">return </span><span class="bool-val">true</span>;
                    }
                ...
                <span class="comment">// If the head lags one lap behind the tail as well...
                    ...
                    <span class="comment">// ...then the channel is full.
                ...
                <span class="comment">// Snooze because we need to wait for the stamp to get updated.
                </span>backoff.snooze();
                tail = <span class="self">self</span>.tail.load(Ordering::Relaxed);
            }
        }
    ...
    <span class="doccomment">/// Writes a message into the channel.
        ...
        </span><span class="kw">if </span>token.array.slot.is_null() {
            <span class="kw">return </span><span class="prelude-val">Err</span>(msg);
        }

        <span class="kw">let </span>slot: <span class="kw-2">&amp;</span>Slot&lt;T&gt; = <span class="kw-2">&amp;*</span>token.array.slot.cast::&lt;Slot&lt;T&gt;&gt;();

        <span class="comment">// Write the message into the slot and update the stamp.
        </span>slot.msg.get().write(MaybeUninit::new(msg));
    ...
    <span class="doccomment">/// Attempts to reserve a slot for receiving a message.
        ...
        <span class="kw">loop </span>{
            ...
            <span class="comment">// If the the stamp is ahead of the head by 1, we may attempt to pop.
                ...
                <span class="kw">let </span>new = <span class="kw">if </span>index + <span class="number">1 </span>&lt; <span class="self">self</span>.cap {
                    <span class="comment">// Same lap, incremented index.
                    // Set to `{ lap: lap, mark: 0, index: index + 1 }`.
                ...
                <span class="comment">// Try moving the head.
                    ...
                    <span class="prelude-val">Ok</span>(<span class="kw">_</span>) =&gt; {
                        <span class="comment">// Prepare the token for the follow-up call to `read`.
                        </span>token.array.slot = slot <span class="kw">as </span><span class="kw-2">*const </span>Slot&lt;T&gt; <span class="kw">as </span><span class="kw-2">*const </span>u8;
                        token.array.stamp = head.wrapping_add(<span class="self">self</span>.one_lap);
                        <span class="kw">return </span><span class="bool-val">true</span>;
                    }
                ...
                <span class="comment">// If the tail equals the head, that means the channel is empty.
                    ...
                    <span class="comment">// If the channel is disconnected...
                        ...
                        </span>token.array.slot = ptr::null();
                        token.array.stamp = <span class="number">0</span>;
                        <span class="kw">return </span><span class="bool-val">true</span>;
                    } <span class="kw">else </span>{
    ...
    <span class="doccomment">/// Reads a message from the channel.
        ...
        <span class="kw">if </span>token.array.slot.is_null() {
        ...
        <span class="comment">// Read the message from the slot and update the stamp.
        </span><span class="kw">let </span>msg = slot.msg.get().read().assume_init();
        slot.stamp.store(token.array.stamp, Ordering::Release);

        <span class="comment">// Wake a sleeping sender.
        </span><span class="self">self</span>.senders.notify();
    ...
    <span class="doccomment">/// Attempts to send a message into the channel.
        ...
        <span class="kw">if </span><span class="self">self</span>.start_send(token) {
            <span class="kw">unsafe </span>{ <span class="self">self</span>.write(token, msg).map_err(TrySendError::Disconnected) }
        } <span class="kw">else </span>{
    ...
    <span class="doccomment">/// Sends a message into the channel.
        ...
        <span class="kw">let </span>token = <span class="kw-2">&amp;mut </span>Token::default();
        <span class="kw">loop </span>{
            <span class="comment">// Try sending a message several times.
            </span><span class="kw">let </span>backoff = Backoff::new();
            <span class="kw">loop </span>{
                <span class="kw">if </span><span class="self">self</span>.start_send(token) {
                    <span class="kw">let </span>res = <span class="kw">unsafe </span>{ <span class="self">self</span>.write(token, msg) };
                    <span class="kw">return </span>res.map_err(SendTimeoutError::Disconnected);
                }
            ...
            <span class="kw">if let </span><span class="prelude-val">Some</span>(d) = deadline {
                <span class="kw">if </span>Instant::now() &gt;= d {
                    <span class="kw">return </span><span class="prelude-val">Err</span>(SendTimeoutError::Timeout(msg));
                }
            }
            ...
            Context::with(|cx| {
                ...
                <span class="self">self</span>.senders.register(oper, cx);

                <span class="comment">// Has the channel become ready just now?
                </span><span class="kw">if </span>!<span class="self">self</span>.is_full() || <span class="self">self</span>.is_disconnected() {
                ...
                <span class="kw">match </span>sel {
                    ...
                    Selected::Aborted | Selected::Disconnected =&gt; {
                        <span class="self">self</span>.senders.unregister(oper).unwrap();
                    }
    ...
    <span class="doccomment">/// Receives a message from the channel.
        ...
        <span class="kw">loop </span>{
            ...
            <span class="kw">loop </span>{
                <span class="kw">if </span><span class="self">self</span>.start_recv(token) {
                    <span class="kw">let </span>res = <span class="kw">unsafe </span>{ <span class="self">self</span>.read(token) };
                    <span class="kw">return </span>res.map_err(|<span class="kw">_</span>| RecvTimeoutError::Disconnected);
                }
        ...
        <span class="kw">loop </span>{
            ...
            <span class="comment">// If the tail didn't change, we've got consistent values to work with.
                ...
                <span class="kw">return if </span>hix &lt; tix {
                    ...
                    <span class="self">self</span>.cap - hix + tix
    ...
    <span class="doccomment">/// Returns the capacity of the channel.
        ...
        <span class="prelude-val">Some</span>(<span class="self">self</span>.cap)
    }
    ...
    /// Returns `true` if this call disconnected the channel.
        ...
        <span class="kw">if </span>tail &amp; <span class="self">self</span>.mark_bit == <span class="number">0 </span>{
            <span class="self">self</span>.senders.disconnect();
...
<span class="kw">impl</span>&lt;T&gt; Drop <span class="kw">for </span>Channel&lt;T&gt; {
    <span class="kw">fn </span>drop(<span class="kw-2">&amp;mut </span><span class="self">self</span>) {
        <span class="kw">if </span>mem::needs_drop::&lt;T&gt;() {
            ...
            </span><span class="kw">let </span>head = <span class="kw-2">*</span><span class="self">self</span>.head.get_mut();
            <span class="kw">let </span>tail = <span class="kw-2">*</span><span class="self">self</span>.tail.get_mut();
            ...
            <span class="comment">// Loop over all slots that hold a message and drop them.
            </span><span class="kw">for </span>i <span class="kw">in </span><span class="number">0</span>..len {
                <span class="comment">// Compute the index of the next slot holding a message.
                </span><span class="kw">let </span>index = <span class="kw">if </span>hix + i &lt; <span class="self">self</span>.cap {
                    hix + i
                } <span class="kw">else </span>{
                    hix + i - <span class="self">self</span>.cap
                };

                <span class="kw">unsafe </span>{
                    <span class="macro">debug_assert!</span>(index &lt; <span class="self">self</span>.buffer.len());
                    <span class="kw">let </span>slot = <span class="self">self</span>.buffer.get_unchecked_mut(index);
...
<span class="kw">impl</span>&lt;T&gt; SelectHandle <span class="kw">for </span>Sender&lt;<span class="lifetime">'_</span>, T&gt; {
    ...
    <span class="kw">fn </span>register(<span class="kw-2">&amp;</span><span class="self">self</span>, oper: Operation, cx: <span class="kw-2">&amp;</span>Context) -&gt; bool {
        <span class="self">self</span>.<span class="number">0</span>.senders.register(oper, cx);
        <span class="self">self</span>.is_ready()
    }

    <span class="kw">fn </span>unregister(<span class="kw-2">&amp;</span><span class="self">self</span>, oper: Operation) {
        <span class="self">self</span>.<span class="number">0</span>.senders.unregister(oper);
    }
    ...
    <span class="kw">fn </span>is_ready(<span class="kw-2">&amp;</span><span class="self">self</span>) -&gt; bool {
        !<span class="self">self</span>.<span class="number">0</span>.is_full() || <span class="self">self</span>.<span class="number">0</span>.is_disconnected()
    }

    <span class="kw">fn </span>watch(<span class="kw-2">&amp;</span><span class="self">self</span>, oper: Operation, cx: <span class="kw-2">&amp;</span>Context) -&gt; bool {
        <span class="self">self</span>.<span class="number">0</span>.senders.watch(oper, cx);
        <span class="self">self</span>.is_ready()
    }

    <span class="kw">fn </span>unwatch(<span class="kw-2">&amp;</span><span class="self">self</span>, oper: Operation) {
        <span class="self">self</span>.<span class="number">0</span>.senders.unwatch(oper);
    }
}
