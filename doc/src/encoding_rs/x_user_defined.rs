...
<span class="macro">cfg_if!</span> {
    <span class="kw">if </span><span class="attr">#[cfg(feature = <span class="string">"simd-accel"</span>)] </span>{
        ...
        <span class="kw">use </span>core::simd::cmp::SimdPartialOrd;
        ...
        <span class="attr">#[inline(always)]
            ...
            unpacked + unpacked.simd_gt(highest_ascii).select(u16x8::splat(<span class="number">0xF700</span>), u16x8::splat(<span class="number">0</span>))        }
...
<span class="kw">impl </span>UserDefinedDecoder {
    ...
    <span class="attr">#[cfg(feature = <span class="string">"simd-accel"</span>)]
        ...
        <span class="comment">// Safety: This is `for i in 0..length / 16`
        </span><span class="kw">for </span>i <span class="kw">in </span><span class="number">0</span>..simd_iterations {
            <span class="comment">// Safety: This is in bounds: length is the minumum valid length for both src/dst
            // and i ranges to length/16, so multiplying by 16 will always be `&lt; length` and can do
            // a 16 byte read
            </span><span class="kw">let </span>input = <span class="kw">unsafe </span>{ load16_unaligned(src_ptr.add(i * <span class="number">16</span>)) };
            <span class="kw">let </span>(first, second) = simd_unpack(input);
            <span class="kw">unsafe </span>{
                <span class="comment">// Safety: same as above, but this is two consecutive 8-byte reads
                </span>store8_unaligned(dst_ptr.add(i * <span class="number">16</span>), shift_upper(first));
